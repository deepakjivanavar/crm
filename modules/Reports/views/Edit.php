...
Class Reports_Edit_View extends nectarcrm_Edit_View {
	...
	public function preProcess(nectarcrm_Request $request) {
		...
		$moduleName = $request->getModule();
		$reportModuleModel = nectarcrm_Module_Model::getInstance($moduleName);
	...
	function step1(nectarcrm_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		$record = $request->get('record');
	...
	function step2(nectarcrm_request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		$record = $request->get('record');
				...
				foreach($blockFields as $key => $value){
					if(isset($value)){
						foreach($value as $key1 => $value1){
							if($key1 == 'nectarcrm_troubletickets:update_log:HelpDesk_Update_History:update_log:V'){
	...
	function step3(nectarcrm_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		$record = $request->get('record');
		...
		$recordStructureInstance = nectarcrm_RecordStructure_Model::getInstanceFromRecordModel($reportModel);
