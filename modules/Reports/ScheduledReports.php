...
class VTScheduledReport extends Reports {
	...
	public function getReportScheduleInfo() {
		...
		if(!empty($this->id)) {
			...
			if($cachedInfo == false) {
				$result = $adb->pquery('SELECT * FROM nectarcrm_scheduled_reports WHERE reportid=?', array($this->id));
	...
	public function sendEmail() {
		...
		foreach($recipientEmails as $name => $email) {
			$nectarcrmMailer->AddAddress($email, $name);
		}
		...
		$nectarcrmMailer->Body    = $contents;
		$nectarcrmMailer->ContentType = "text/html";
		...
		foreach($attachments as $attachmentName => $path) {
			$nectarcrmMailer->AddAttachment($path, $attachmentName);
		}

		$nectarcrmMailer->Send(true);
	...
	public function updateNextTriggerTime() {
		...
		$adb->pquery('UPDATE nectarcrm_scheduled_reports SET next_trigger_time=? WHERE reportid=?', array($nextTriggerTime, $this->id));
	}
	...
	public static function getScheduledReports($adb, $user) {
		...
		$currentTime = date('Y-m-d H:i:s');
		$result = $adb->pquery("SELECT * FROM nectarcrm_scheduled_reports
