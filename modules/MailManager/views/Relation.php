...
class MailManager_Relation_View extends MailManager_Abstract_View {
	...
	public function process(nectarcrm_Request $request) {
		...
		if ('find' == $this->getOperationArg($request)) {
			...
			if (!class_exists($quickCreateviewClassName)) {
				$quickCreateviewClassName = 'nectarcrm_QuickCreateAjax_View';
			}
			...
			$linkedto = MailManager_Relate_Action::getSalesEntityInfo($parent);
			$recordModel = nectarcrm_Record_Model::getCleanInstance($linkModule);
			...
			switch ($linkModule) {
				case 'Calendar' :   $activityType = $recordModel->get('activitytype');
									...
									$startDateTime = nectarcrm_Datetime_UIType::getDBDateTimeValue($request->get('date_start')." ".$startTime);
									...
									if ($activityType != 'Task') {
										$endTime = nectarcrm_Time_UIType::getTimeValueWithSeconds($request->get('time_end'));
	...
	public function setParentForHelpDesk($parent, $from) {
		...
		if(empty($parent)) {
			if(!empty($from)) {
				$parentInfo = MailManager::lookupMailInnectarcrm($from[0], $currentUserModel);
	...
	public function lookupRelModuleRecords($wsId) {
		...
		$wsObject = nectarcrmWebserviceObject::fromId($db, $wsId);
		$entityName = $wsObject->getEntityName();

		foreach ($relModules as $relModule) {
			$relation = nectarcrm_Relation_Model::getInstanceByModuleName($entityName, $relModule);
			if(!$relation) {
				continue;
			}
