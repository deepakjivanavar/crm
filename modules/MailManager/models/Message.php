...
class MailManager_Message_Model extends nectarcrm_MailRecord  {
	...
	public static function pruneOlderInDB($waybacktime) {
		...
		$db->pquery("DELETE FROM nectarcrm_mailmanager_mailrecord
		WHERE userid=? AND lastsavedtime < ?", array($currentUserModel->getId(), $waybacktime));
		$db->pquery("DELETE FROM nectarcrm_mailmanager_mailattachments
		WHERE userid=? AND lastsavedtime < ?", array($currentUserModel->getId(), $waybacktime));
	}
	...
	public static function removeSavedAttachmentFiles($waybacktime) {
		...
		$mailManagerAttachments = $db->pquery("SELECT attachid, aname, path FROM nectarcrm_mailmanager_mailattachments
			WHERE userid=? AND lastsavedtime < ?", array($currentUserModel->getId(), $waybacktime));

		for($i=0; $i<$db->num_rows($mailManagerAttachments); $i++) {
			$atResultRow = $db->raw_query_result_rowdata($mailManagerAttachments, $i);

			$db->pquery("UPDATE nectarcrm_crmentity set deleted = 1 WHERE crmid = ?", array($atResultRow['attachid']));
	...
	public function readFromDB($uid, $folder = false) {
		$db = PearDatabase::getInstance();
		$currentUserModel = Users_Record_Model::getCurrentUserModel();
		$query = "SELECT * FROM nectarcrm_mailmanager_mailrecord WHERE userid=? AND muid=?";
	...
	protected function loadAttachmentsFromDB($withContent, $aName=false, $aId=false) {
		...
		if (empty($this->_attachments)) {
			...
			$atResult = $db->pquery("SELECT {$filteredColumns} FROM nectarcrm_mailmanager_mailattachments
	...
	protected function saveToDB($uid, $folder = '') {
		...
		$db->pquery("INSERT INTO nectarcrm_mailmanager_mailrecord (userid, muid, mfrom, mto, mcc, mbcc,
		...
		if (!empty($this->_attachments)) {
			foreach($this->_attachments as $index => $aValue) {
				...
				if(is_array($attachInfo) && !empty($attachInfo)) {
					$db->pquery("INSERT INTO nectarcrm_mailmanager_mailattachments
	...
	public function __SaveAttachmentFile($filename, $filecontent) {
		...
		$attachid = $db->getUniqueId('nectarcrm_crmentity');
		...
		$db->pquery("INSERT INTO nectarcrm_crmentity(crmid, smcreatorid, smownerid,
		...
		$db->pquery("INSERT INTO nectarcrm_attachments SET attachmentsid=?, name=?, description=?, type=?, path=?",
	...
	public function date($format = false) {
		...
		if ($date) {
			if ($format) {
				$dateTimeFormat = nectarcrm_Util_Helper::convertDateTimeIntoUsersDisplayFormat(date('Y-m-d H:i:s', strtotime($date)));
				...
				return nectarcrm_Util_Helper::convertDateTimeIntoUsersDisplayFormat(date('Y-m-d H:i:s', $date));
			}
		}
		return '';
	}
