...
class MailManager {
	...
	static function updateMailAssociation($mailuid, $emailid, $crmid) {
		global $adb;
		$adb->pquery("INSERT INTO nectarcrm_mailmanager_mailrel (mailuid, emailid, crmid) VALUES (?,?,?)", array($mailuid, $emailid, $crmid));
	}

	static function lookupMailInnectarcrm($searchTerm, $user) {
		...
		foreach($referenceModules as $referenceModule) {
			...
			if(!empty($searchFieldList) && !empty($referenceModuleEmailFields)) {
				...
				if($referenceModule == 'Users' && !is_admin($user)){
					...
					global $adb;
					$where .= " AND nectarcrm_users.status='Active'";
					$query = "select $searchFieldListString,id from nectarcrm_users $where";
	...
	static function lookupMailAssociation($mailuid) {
		...
		$result = $adb->pquery(
			"SELECT nectarcrm_mailmanager_mailrel.* FROM nectarcrm_mailmanager_mailrel INNER JOIN
			nectarcrm_crmentity ON nectarcrm_crmentity.crmid=nectarcrm_mailmanager_mailrel.crmid AND nectarcrm_crmentity.deleted=0
			AND nectarcrm_mailmanager_mailrel.mailuid=? LIMIT 1", array(decode_html($mailuid)));
