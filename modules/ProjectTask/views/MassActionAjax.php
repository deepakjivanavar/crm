...
class ProjectTask_MassActionAjax_View extends Project_MassActionAjax_View {
	...
	protected function getEmailFieldsInfo(nectarcrm_Request $request) {
		...
		$moduleModel = nectarcrm_Module_Model::getInstance('Project');
		...
		foreach ($referenceFields as $referenceField) {
			...
			$refModule = nectarcrm_Module_Model::getInstance($refModuleName);
			if ($refModule) {
		...
		if (!empty($referenceFieldValues)) {
			foreach ($referenceFieldValues as $recordId => $refRecordIds) {
				foreach ($refRecordIds as $refRecordId) {
					$refModuleName = nectarcrm_Functions::getCRMRecordType($refRecordId);
					if (empty($refModuleName) || $refModuleName == 'Users')
						continue;
	...
	protected function getProjectIds($taskIds = array()) {
		...
		if (!empty($taskIds)) {
			$db = PearDatabase::getInstance();
			$sql = 'SELECT projectid FROM nectarcrm_projecttask INNER JOIN nectarcrm_crmentity  ON nectarcrm_projecttask.projecttaskid = nectarcrm_crmentity.crmid
						WHERE nectarcrm_crmentity.deleted=0 AND nectarcrm_crmentity.crmid IN ('.generateQuestionMarks($taskIds).')';
	...
	protected function isPreferencesNeedToBeUpdated(nectarcrm_Request $request) {
		...
		if (!$parentStatus) {
			$recipientPrefModel = nectarcrm_RecipientPreference_Model::getInstance($request->getModule());
			if (!$recipientPrefModel)
				return $parentStatus;
			...
			foreach ($refFields as $refField) {
				if ($refField && $refField->isViewable()) {
					...
					foreach ($referenceList as $moduleName) {
						if ($moduleName !== 'Users') {
							$refModuleModel = nectarcrm_Module_Model::getInstance($moduleName);
							if (!$prefs[$refModuleModel->getId()])
								continue;
							$moduleEmailPrefs = $prefs[$refModuleModel->getId()];
							foreach ($moduleEmailPrefs as $fieldId) {
								$field = nectarcrm_Field_Model::getInstance($fieldId, $refModuleModel);
								if ($field) {
