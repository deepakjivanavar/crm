...
class Webforms {
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			// Mark the module as Standard module
			$this->updateSettings();
			$adb->pquery('UPDATE nectarcrm_tab SET customized=0 WHERE name=?', array($this->LBL_WEBFORMS));
		} else if($eventType == 'module.disabled') {
		// TODO Handle actions when this module is disabled.
			global $log,$adb;
			$adb->pquery('UPDATE nectarcrm_settings_field SET active= 1  WHERE  name= ?',array($this->LBL_WEBFORMS));
		} else if($eventType == 'module.enabled') {
		// TODO Handle actions when this module is enabled.
			global $log,$adb;
			$adb->pquery('UPDATE nectarcrm_settings_field SET active= 0  WHERE  name= ?',array($this->LBL_WEBFORMS));
	...
	function updateSettings(){
		...
		$result=$adb->pquery('SELECT 1 FROM nectarcrm_settings_field WHERE name=?',array($this->LBL_WEBFORMS));
	...
	static function checkAdminAccess($user) {
		...
		<tbody><tr>
		<td rowspan='2' width='11%'><img src= " . nectarcrm_imageurl('denied.gif', $theme) . " ></td>
