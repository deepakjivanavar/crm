...
class RecycleBin_Module_Model extends nectarcrm_Module_Model {
	...
	public function getListViewMassActions() {
		...
		foreach($massActionLinks as $massActionLink) {
			$links[] = nectarcrm_Link_Model::getInstanceFromValues($massActionLink);
		}
	...
	public function emptyRecycleBin(){
		...
		$db->pquery('DELETE nectarcrm_modtracker_basic.* FROM nectarcrm_modtracker_basic INNER JOIN nectarcrm_crmentity on 
					nectarcrm_crmentity.crmid = nectarcrm_modtracker_basic.crmid AND nectarcrm_crmentity.deleted = 1', array());

		$getIdsQuery='SELECT crmid from nectarcrm_crmentity WHERE deleted=?';
		...
		$db->query('DELETE FROM nectarcrm_crmentity WHERE deleted = 1');
		$db->query('DELETE FROM nectarcrm_relatedlists_rb');
	...
	public function deleteRecords($recordIds){
		...
		$query = 'DELETE FROM nectarcrm_relatedlists_rb WHERE entityid in('.generateQuestionMarks($recordIds).')';
		$db->pquery($query, array($recordIds));

		// TODO - Remove records from module tables and other related stores.
		$query = 'DELETE FROM nectarcrm_modtracker_basic WHERE crmid in(' . generateQuestionMarks($recordIds) . ')';
		$db->pquery($query, array($recordIds));
		// Delete entries of attachments from nectarcrm_attachments and nectarcrm_seattachmentsrel
		$this->deleteFiles($recordIds);

		//Delete the records in nectarcrm crmentity and relatedlists.
		$query = 'DELETE FROM nectarcrm_crmentity WHERE deleted = ? and crmid in('.generateQuestionMarks($recordIds).')';
	...
	public function deleteFiles($recordIds){
		global $db;
		$db=PearDatabase::getInstance();
		$getAttachmentsIdQuery='SELECT * FROM nectarcrm_seattachmentsrel WHERE crmid in('.generateQuestionMarks($recordIds).')';
		...
		if(!empty($attachmentsIds)){
			$deleteRelQuery='DELETE FROM nectarcrm_seattachmentsrel WHERE crmid in('.generateQuestionMarks($recordIds).')';
			$db->pquery($deleteRelQuery,array($recordIds));
			$attachmentsLocation=array();
			$getPathQuery='SELECT * FROM nectarcrm_attachments WHERE attachmentsid in ('.generateQuestionMarks($attachmentsIds).')';
			...
			$deleteAttachmentQuery='DELETE FROM nectarcrm_attachments WHERE attachmentsid in ('.generateQuestionMarks($attachmentsIds).')';
	...
	public function getDeletedRecordsTotalCount() {  
		$db = PearDatabase::getInstance();  
		$totalCount = $db->pquery('select count(*) as count from nectarcrm_crmentity where deleted=1',array());  
	...
	public function isRecordsDeleted() {
		$db = PearDatabase::getInstance(); 
		$totalCount = $db->pquery('SELECT 1 FROM nectarcrm_crmentity WHERE deleted=1 LIMIT 1',array());
