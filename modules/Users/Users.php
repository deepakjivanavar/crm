...
/*********************************************************************************
 * $Header: /advent/projects/wesat/nectarcrm_crm/sugarcrm/modules/Users/Users.php,v 1.10 2005/04/19 14:40:48 ray Exp $
...
class Users extends CRMEntity {
	...
	var $tab_name = Array('nectarcrm_users','nectarcrm_attachments','nectarcrm_user2role','nectarcrm_asteriskextensions');
	var $tab_name_index = Array('nectarcrm_users'=>'id','nectarcrm_attachments'=>'attachmentsid','nectarcrm_user2role'=>'userid','nectarcrm_asteriskextensions'=>'userid');

	var $table_name = "nectarcrm_users";
	var $table_index= 'id';
	...
	var $search_fields = Array(
			'Name'=>Array('nectarcrm_users'=>'last_name'),
			'Email'=>Array('nectarcrm_users'=>'email1'),
			'Email2'=>Array('nectarcrm_users'=>'email2')
	);
	...
	var $list_fields = Array(
			'First Name'=>Array('nectarcrm_users'=>'first_name'),
			'Last Name'=>Array('nectarcrm_users'=>'last_name'),
			'Role Name'=>Array('nectarcrm_user2role'=>'roleid'),
			'User Name'=>Array('nectarcrm_users'=>'user_name'),
			'Status'=>Array('nectarcrm_users'=>'status'),
			'Email'=>Array('nectarcrm_users'=>'email1'),
			'Email2'=>Array('nectarcrm_users'=>'email2'),
			'Admin'=>Array('nectarcrm_users'=>'is_admin'),
			'Phone'=>Array('nectarcrm_users'=>'phone_work')
	);
	...
	function encrypt_password($user_password, $crypt_type='') {
		...
		// Fix for: http://trac.nectarcrm.com/cgi-bin/trac.cgi/ticket/4923
		if($crypt_type == '') {
	...
	function get_user_crypt_type() {
		...
		global $adb;
		$table_cols = $adb->getColumnNames("nectarcrm_users");
	...
	function retrieve_user_id($user_name) {
		global $adb;
		$query = "SELECT id from nectarcrm_users where user_name=? AND deleted=0";
	...
	function verify_data() {
		...
		$query = "SELECT user_name from nectarcrm_users where user_name=? AND id<>? AND deleted=0";
		$result =$this->db->pquery($query, array($usr_name, $this->id), true, "Error selecting possible duplicate users: ");
		$dup_users = $this->db->fetchByAssoc($result);

		$query = "SELECT user_name from nectarcrm_users where is_admin = 'on' AND deleted=0";
		$result =$this->db->pquery($query, array(), true, "Error selecting possible duplicate nectarcrm_users: ");
	...
	function fill_in_additional_detail_fields() {
		$query = "SELECT u1.first_name, u1.last_name from nectarcrm_users u1, nectarcrm_users u2 where u1.id = u2.reports_to_id AND u2.id = ? and u1.deleted=0";
		$result =$this->db->pquery($query, array($this->id), true, "Error filling in additional detail nectarcrm_fields") ;
	...
	function retrieveCurrentUserInfoFromFile($userid) {
		$acl = nectarcrm_AccessControl::loadUserPrivileges($userid);
	...
	function saveentity($module, $fileid='') {
		...
		foreach($this->tab_name as $table_name) {
			if($table_name == 'nectarcrm_attachments') {
	...
	function createAccessKey() {
		global $adb;
		$updateQuery = "update nectarcrm_users set accesskey=? where id=?";
	...
	function insertIntoEntityTable($table_name, $module, $fileid='') {
		...
		if($insertion_mode == 'edit') {
			...
			$sql = "select * from nectarcrm_field where tabid=? and tablename=? and displaytype in (1,3,5) and nectarcrm_field.presence in (0,2)";
			$params = array($tabid, $table_name);
		}
		else {
			$column = $this->tab_name_index[$table_name];
			if($column == 'id' && $table_name == 'nectarcrm_users') {
				$currentuser_id = $this->db->getUniqueID("nectarcrm_users");
				$this->id = $currentuser_id;
			}
			$qparams = array($this->id);
			$tabid= getTabid($module);
			$sql = "select * from nectarcrm_field where tabid=? and tablename=? and displaytype in (1,3,4,5) and nectarcrm_field.presence in (0,2)";
			$params = array($tabid, $table_name);
		...
		for($i=0; $i<$noofrows; $i++) {
			...
			if($uitype == 31) {
				$themeList = array_keys(nectarcrm_Util_Helper::getAllSkins());
				...
				if($current_user->id == $this->id) {
					$_SESSION['nectarcrm_authenticated_user_theme'] = $fldvalue;
				}
			} elseif($uitype == 32) {
				$languageList = nectarcrm_Language::getAll();
		...
		if($insertion_mode == 'edit') {
			...
			// Set the crypt_type being used, to override the DB default constraint as it is not in nectarcrm_field
			if($table_name == 'nectarcrm_users' && strpos('crypt_type', $column) === false) {
				$column .= ', crypt_type';
				$qparams[]= $crypt_type;
			}
			// END

			if($table_name == 'nectarcrm_users' && strpos('user_hash', $column) === false) {
				$column .= ', user_hash';
	...
	function retrieve_entity_info($record, $module, $allowDeleted = false) {
		...
		$sql1 =  "select columnname, tablename, fieldname from nectarcrm_field where tabid=? and nectarcrm_field.presence in (0,2)";
		...
		$currency_query = "select * from nectarcrm_currency_info where id=? and currency_status='Active' and deleted=0";
		$currency_result = $adb->pquery($currency_query, array($this->column_fields["currency_id"]));
		if($adb->num_rows($currency_result) == 0) {
			$currency_query = "select * from nectarcrm_currency_info where id =1";
	...
	function uploadAndSaveFile($id,$module,$file_details,$attachmentType='Attachment') {
		...
		$current_id = $this->db->getUniqueID("nectarcrm_crmentity");
		...
		if($save_file == 'true') {
			...
			$params1 = array($current_id, $current_user->id, $ownerid, $module." Image", $this->column_fields['description'], $this->db->formatString("nectarcrm_crmentity","createdtime",$date_var), $this->db->formatDate($date_var, true));
			...
			if($id != '') {
				$delquery = 'delete from nectarcrm_salesmanattachmentsrel where smid = ?';
				$this->db->pquery($delquery, array($id));
			}

			$sql3='insert into nectarcrm_salesmanattachmentsrel values(?,?)';
			$this->db->pquery($sql3, array($id, $current_id));

			//we should update the imagename in the users table
			$this->db->pquery("update nectarcrm_users set imagename=? where id=?", array($filename, $id));
		}
		else {
	...
	function save($module_name, $fileid='') {
		...
		$query_prev_interval = $adb->pquery("SELECT reminder_interval from nectarcrm_users where id=?",
				array($this->id));
		...
		nectarcrm_Util_Helper::setCalendarDefaultActivityTypesForUser($this->id);

		require_once('modules/Users/CreateUserPrivilegeFile.php');
		createUserPrivilegesfile($this->id);
		createUserSharingPrivilegesfile($this->id);
		nectarcrm_AccessControl::clearUserPrivileges($this->id);
	}
	...
	function getHomeStuffOrder($id) {
		...
		$homeorder=Array();
		if($id != '') {
			$qry=" select distinct(nectarcrm_homedefault.hometype) from nectarcrm_homedefault inner join nectarcrm_homestuff  on nectarcrm_homestuff.stuffid=nectarcrm_homedefault.stuffid where nectarcrm_homestuff.visible=0 and nectarcrm_homestuff.userid=?";
	...
	function insertUserdetails($inVal) {
		...
		$s3=$adb->getUniqueID("nectarcrm_homestuff");
		...
		$s6=$adb->getUniqueID("nectarcrm_homestuff");
		...
		$s7=$adb->getUniqueID("nectarcrm_homestuff");
		...
		$s8=$adb->getUniqueID("nectarcrm_homestuff");
		...
		$s9=$adb->getUniqueID("nectarcrm_homestuff");
		...
		$s10=$adb->getUniqueID("nectarcrm_homestuff");
		...
		$s11=$adb->getUniqueID("nectarcrm_homestuff");
		...
		$s12=$adb->getUniqueID("nectarcrm_homestuff");
		...
		$s13=$adb->getUniqueID("nectarcrm_homestuff");
		...
		$s14=$adb->getUniqueID("nectarcrm_homestuff");
		...
		// Non-Default Home Page widget (no entry is requried in nectarcrm_homedefault below)
		$tc = $adb->getUniqueID("nectarcrm_homestuff");
		$visibility=0;
		$sql="insert into nectarcrm_homestuff values($tc, 15, 'Tag Cloud', $uid, $visibility, 'Tag Cloud')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s1.",'ALVT',5,'Accounts')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s2.",'HDB',5,'Dashboard')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s3.",'PLVT',5,'Potentials')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s4.",'QLTQ',5,'Quotes')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s5.",'CVLVT',5,'NULL')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s6.",'HLT',5,'HelpDesk')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s7.",'UA',5,'Calendar')";
		$adb->pquery($sql,array());

		$sql="insert into nectarcrm_homedefault values(".$s8.",'GRT',5,'NULL')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s9.",'OLTSO',5,'SalesOrder')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s10.",'ILTI',5,'Invoice')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s11.",'MNL',5,'Leads')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s12.",'OLTPO',5,'PurchaseOrder')";
		$adb->pquery($sql, array());

		$sql="insert into nectarcrm_homedefault values(".$s13.",'PA',5,'Calendar')";
		$adb->pquery($sql,array());

		$sql="insert into nectarcrm_homedefault values(".$s14.",'LTFAQ',5,'Faq')";
		$adb->pquery($sql, array());
		 ...
		 if($this->mode == 'edit')
		 {
			 for($i = 0;$i < count($this->homeorder_array);$i++)
			 {
				 if($_REQUEST[$this->homeorder_array[$i]] != '')
				 {
					...
					$qry=" update nectarcrm_homestuff,nectarcrm_homedefault set nectarcrm_homestuff.visible=0 where nectarcrm_homestuff.stuffid=nectarcrm_homedefault.stuffid and nectarcrm_homestuff.userid=".$id." and nectarcrm_homedefault.hometype='".$this->homeorder_array[$i]."'";//To show the default Homestuff on the the Home Page
					$result=$adb->pquery($qry, array());
				}
				 else
				 {
					$qry="update nectarcrm_homestuff,nectarcrm_homedefault set nectarcrm_homestuff.visible=1 where nectarcrm_homestuff.stuffid=nectarcrm_homedefault.stuffid and nectarcrm_homestuff.userid=".$id." and nectarcrm_homedefault.hometype='".$this->homeorder_array[$i]."'";//To hide the default Homestuff on the the Home Page
					$result=$adb->pquery($qry, array());
				}
			}
			if($save_array !="")
	...
	function resetReminderInterval($prev_reminder_interval) {
		...
		if($prev_reminder_interval != $this->column_fields['reminder_interval'] ) {
			...
			$adb->pquery("UPDATE nectarcrm_users SET reminder_next_time=?, date_entered=? WHERE id=?",array($set_reminder_next, $this->column_fields['date_entered'], $this->id));
		}
	}
	...
	function deleteImage() {
		$sql1 = 'SELECT attachmentsid FROM nectarcrm_salesmanattachmentsrel WHERE smid = ?';
		$res1 = $this->db->pquery($sql1, array($this->id));
		if ($this->db->num_rows($res1) > 0) {
			$attachmentId = $this->db->query_result($res1, 0, 'attachmentsid');

			$sql2 = "DELETE FROM nectarcrm_crmentity WHERE crmid=? AND setype='Users Attachments'";
			$this->db->pquery($sql2, array($attachmentId));

			$sql3 = 'DELETE FROM nectarcrm_salesmanattachmentsrel WHERE smid=? AND attachmentsid=?';
			$this->db->pquery($sql3, array($this->id, $attachmentId));

			$sql2 = "UPDATE nectarcrm_users SET imagename='' WHERE id=?";
			$this->db->pquery($sql2, array($this->id));

			$sql4 = 'DELETE FROM nectarcrm_attachments WHERE attachmentsid=?';
	...
	function transformOwnerShipAndDelete($userId,$transformToUserId){
		...
		//delete from user nectarcrm_table;
		$sql = "UPDATE nectarcrm_users SET status=?,deleted=? where id=?";
		$adb->pquery($sql, array('Inactive',true,$userId));

		$em->triggerEvent("nectarcrm.user.afterdelete", $entityData);
	...
	function mark_deleted($id) {
		...
		$date_var = date('Y-m-d H:i:s');
		$query = "UPDATE nectarcrm_users set status=?,date_modified=?,modified_user_id=? where id=?";
	...
	public static function getActiveAdminId() {
		...
		$sql = "SELECT id FROM nectarcrm_users WHERE is_admin = 'on' AND status = 'Active' AND is_owner = 1 limit 1";
	...
	*/
	public function setUserPreferences($requestArray) {
		...
		if (!empty($updateData)) {
			$updateQuery = 'UPDATE nectarcrm_users SET '. ( implode('=?,', array_keys($updateData)). '=?') . ' WHERE id = ?';
	...
	public function uploadOrgLogo($requestArray, $fileArray) {
		...
		if ($file_size != 0 && in_array($file_type_val, $validFileFormats)) {
			...
			//Updating Database
			$sql = 'UPDATE nectarcrm_organizationdetails SET logoname = ? WHERE organization_id = ?';
	...
	*/
	public function updateBaseCurrency($requestArray) {
		...
		if (isset ($requestArray['currency_name'])) {
			...
			$result = $adb->pquery('SELECT currency_code, currency_symbol FROM nectarcrm_currencies WHERE currency_name = ?', array($currency_name));
			...
			//Updating Database
			$query = 'UPDATE nectarcrm_currency_info SET currency_name = ?, currency_code = ?, currency_symbol = ? WHERE id = ?';
   ...
   public function getAccountOwnerId(){
	   global $adb;
		$query = 'SELECT id FROM nectarcrm_users WHERE is_owner = ?';
   ...
   public function transferAccountOwnerShipAndSave(){
		...
		$adb->pquery('UPDATE nectarcrm_users SET is_owner=? WHERE id = ?', array(1, $current_user->id));
   ...
   function transferOwnership($recordId) {
		if ($recordId) {
		   ...
		   $recordModel = Users_Record_Model::getInstanceById($recordId, 'Users');
			...
			$db->pquery("UPDATE nectarcrm_users SET is_owner = ? WHERE id = ?", array('', $activeAdminId));
			$db->pquery("UPDATE nectarcrm_users SET is_owner = ? WHERE id = ?", array(1, $recordId));
			require_once('modules/Users/CreateUserPrivilegeFile.php');
   ...
   //END
	...
	function getImportableFields() {
		...
		$moduleModel = nectarcrm_Module_Model::getInstance('Users');
	...
	function createRecords($obj) {
		...
		$fieldInstances = nectarcrm_Field_Model::getAllForModule($moduleModel);
		$allRoles = Settings_Roles_Record_Model::getAll();
		$allCurrency = Settings_Currency_Record_Model::getAll();
		$allLanguages = nectarcrm_Language_Handler::getAllLanguages();
		...
		if(!empty($createdRecords)) {
			$recordModels = nectarcrm_Record_Model::getInstancesFromIds($createdRecords, $moduleName);
			$entityInfos = array();
		...
		if($obj->entitydata) {
			...
			$entity->triggerEvent('nectarcrm.batchevent.save', $obj->entitydata);
		}
		$obj->entitydata = null;
		$result = null;
		return true;
...
class Users_CRMSetup {
	...
	public static function isFirstUser($user) {
		...
		$isFirstUser = false;
		if (is_admin($user)) {
			$query = 'SELECT COUNT(*) AS count FROM nectarcrm_crmsetup';
	...
	public static function insertEntryIntoCRMSetup($id) {
		...
		$insertQuery = 'INSERT INTO nectarcrm_crmsetup (userid, setup_status) VALUES (?, ?)';
	...
	public static function getUserSetupStatus($id) {
		...
		$query = 'SELECT 1 FROM nectarcrm_crmsetup WHERE userid = ? AND setup_status = ?';
	...
	public static function getPackagesList() {
		...
		global $adb;
		$result = $adb->pquery('SELECT parent, name, tablabel FROM nectarcrm_tab', array());
