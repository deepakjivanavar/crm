...
class Users_ListView_Model extends nectarcrm_ListView_Model {
	...
	public function getListViewLinks($linkParams) {
        ...
        if(count($settingLinks) > 0) {
            foreach($settingLinks as $settingLink) {
                $links['LISTVIEWSETTING'][] = nectarcrm_Link_Model::getInstanceFromValues($settingLink);
            }
        }
    ...
    public function getQuery() {
            ...
            if(!empty($searchKey)) {
                $listQueryComponents = explode(" WHERE nectarcrm_users.status='Active' AND", $listQuery);
                $listQuery = implode(' WHERE ', $listQueryComponents);
            }
            $listQuery .= " AND (nectarcrm_users.user_name != 'admin' OR nectarcrm_users.is_owner = 1)";
            return $listQuery;
    }

	/**
	 * Function to get the list view entries
	 * @param nectarcrm_Paging_Model $pagingModel, $status (Active or Inactive User). Default false
