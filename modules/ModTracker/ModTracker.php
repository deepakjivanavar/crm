...
class ModTracker {
	...
	*/	
	function vtlib_handler($moduleName, $eventType) {
		...
		$modtrackerModule = nectarcrm_Module::getInstance($currentModule);
    ...
    */
    function getModTrackerEnabledModules() {
        global $adb;
        $moduleResult = $adb->pquery('SELECT * FROM nectarcrm_modtracker_tabs', array());
	...
	static function disableTrackingForModule($tabid){
		...
		if(!self::isModulePresent($tabid)){
				$adb->pquery("INSERT INTO nectarcrm_modtracker_tabs VALUES(?,?)",array($tabid,0));
				self::updateCache($tabid,0);
		} else{
				$adb->pquery("UPDATE nectarcrm_modtracker_tabs SET visible = 0 WHERE tabid = ?", array($tabid));
				self::updateCache($tabid,0);
		}
		if(self::isModtrackerLinkPresent($tabid)) {
			$moduleInstance=nectarcrm_Module::getInstance($tabid);
	...
	static function enableTrackingForModule($tabid){
		...
		if(!self::isModulePresent($tabid)){
			// Shouldn't assign any thing to variable if we are not going use that
			$adb->pquery("INSERT INTO nectarcrm_modtracker_tabs VALUES(?,?)",array($tabid,1));
			self::updateCache($tabid,1);
		} else{
			// Shouldn't assign any thing to variable if we are not going use that
			$adb->pquery("UPDATE nectarcrm_modtracker_tabs SET visible = 1 WHERE tabid = ?", array($tabid));
			self::updateCache($tabid,1);
		}
		if(!self::isModTrackerLinkPresent($tabid)){
			$moduleInstance=nectarcrm_Module::getInstance($tabid);
	...
	static function isTrackingEnabledForModule($modulename){
		...
		if(!self::getVisibilityForModule($tabid) || self::getVisibilityForModule($tabid) !== 0) {
			$query = $adb->pquery("SELECT * FROM nectarcrm_modtracker_tabs WHERE nectarcrm_modtracker_tabs.visible = 1
								   AND nectarcrm_modtracker_tabs.tabid=?", array($tabid));
	...
	static function isModulePresent($tabid){
		...
		if(!self::checkModuleInModTrackerCache($tabid)){
			$query=$adb->pquery("SELECT * FROM nectarcrm_modtracker_tabs WHERE tabid = ?",array($tabid));
			$rows = $adb->num_rows($query);
			if($rows){
	...
	static function isModtrackerLinkPresent($tabid){
		global $adb;
		$query1=$adb->pquery("SELECT * FROM nectarcrm_links WHERE linktype='DETAILVIEWBASIC' AND
    ...
    function getChangedRecords($uniqueId, $mtime, $limit = 100) {
        ...
        $query = "SELECT id, module, modifiedtime, nectarcrm_crmentity.crmid, smownerid, nectarcrm_modtracker_basic.status
                ...
                INNER JOIN nectarcrm_crmentity ON nectarcrm_modtracker_basic.crmid = nectarcrm_crmentity.crmid
                    AND nectarcrm_modtracker_basic.changedon = nectarcrm_crmentity.modifiedtime
        ...
        $moreQuery = "SELECT * FROM nectarcrm_modtracker_basic WHERE id > ? AND changedon >= ? AND module
    ...
    static function getRecordFieldChanges($crmid, $time, $decodeHTML = false) {
        ...
        $fieldResult = $adb->pquery('SELECT * FROM nectarcrm_modtracker_detail
    ...
    static function trackRelation($sourceModule, $sourceId, $targetModule, $targetId, $type) {
        ...
        $id = $adb->getUniqueId('nectarcrm_modtracker_basic');
        $adb->pquery('INSERT INTO nectarcrm_modtracker_basic(id, crmid, module, whodid, changedon, status) VALUES(?,?,?,?,?,?)',
                array($id , $sourceId, $sourceModule, $current_user->id, $currentTime, $type));

        $adb->pquery('INSERT INTO nectarcrm_modtracker_relations(id, targetmodule, targetid, changedon)
