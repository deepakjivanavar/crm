...
class Products_RelationListView_Model extends nectarcrm_RelationListView_Model {
	...
	public function getHeaders() {
		...
		if($this->getRelationModel()->getRelationModuleModel()->getName() == 'PriceBooks') {
			...
			$unitPriceField = new nectarcrm_Field_Model();
			...
			$field = new nectarcrm_Field_Model();
	...
	public function getRelationQuery() {
		...
		if ($parentModuleName === $relatedModuleName && $this->tab_label === 'Product Bundles' && $quantityField->isActiveField()) {//Products && Child Products
			...
			$query = $queryComponents[0]. ', nectarcrm_seproductsrel.quantity AS qty_per_unit ';
			for($i=1; $i<$count; $i++) {
