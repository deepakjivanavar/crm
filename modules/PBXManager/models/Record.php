...
class PBXManager_Record_Model extends nectarcrm_Record_Model{
    
    const moduletableName = 'nectarcrm_pbxmanager';
    const lookuptableName = 'nectarcrm_pbxmanager_phonelookup';
    const entitytableName = 'nectarcrm_crmentity';
    ...
    public function saveRecordWithArrray($params){
        $moduleModel = nectarcrm_Module_Model::getInstance('PBXManager');
        $recordModel = nectarcrm_Record_Model::getCleanInstance('PBXManager');
    ...
    public static function getUserNumbers(){
        $numbers = null;
        $db = PearDatabase::getInstance();
        $query = 'SELECT id, phone_crm_extension FROM nectarcrm_users';
        $result = $db->pquery($query, array());
