 ...
 */
class Settings_Groups_Member_Model extends nectarcrm_Base_Model {
    ...
    public static $groupTables = array(self::MEMBER_TYPE_USERS => array('table'=>'nectarcrm_users2group','index'=>'groupid','refIndex'=>'userid'),
                                self::MEMBER_TYPE_GROUPS => array('table'=>'nectarcrm_group2grouprel','index'=>'groupid','refIndex'=>'containsgroupid'),
                                self::MEMBER_TYPE_ROLES => array('table'=>'nectarcrm_group2role' ,'index'=>'groupid','refIndex'=>'roleid'), 
                                self::MEMBER_TYPE_ROLE_AND_SUBORDINATES => array('table'=>'nectarcrm_group2rs' ,'index'=>'groupid','refIndex'=>'roleandsubid'));
    
    public static $cvTables = array(self::MEMBER_TYPE_USERS => array('table'=>'nectarcrm_cv2users','index'=>'cvid','refIndex'=>'userid'),
                                self::MEMBER_TYPE_GROUPS => array('table'=>'nectarcrm_cv2group','index'=>'cvid','refIndex'=>'groupid'),
                                self::MEMBER_TYPE_ROLES => array('table'=>'nectarcrm_cv2role' ,'index'=>'cvid','refIndex'=>'roleid'), 
                                self::MEMBER_TYPE_ROLE_AND_SUBORDINATES => array('table'=>'nectarcrm_cv2rs' ,'index'=>'cvid','refIndex'=>'rsid'));
	 public static $reportTables = array(self::MEMBER_TYPE_USERS => array('table'=>'nectarcrm_report_shareusers','index'=>'reportid','refIndex'=>'userid'),
                                self::MEMBER_TYPE_GROUPS => array('table'=>'nectarcrm_report_sharegroups','index'=>'reportid','refIndex'=>'groupid'),
                                self::MEMBER_TYPE_ROLES => array('table'=>'nectarcrm_report_sharerole' ,'index'=>'reportid','refIndex'=>'roleid'), 
                                self::MEMBER_TYPE_ROLE_AND_SUBORDINATES => array('table'=>'nectarcrm_report_sharers' ,'index'=>'reportid','refIndex'=>'rsid'));
	...
	public static function getAllByTypeForGroup($groupModel, $type, $mode = self::GROUP_MODE) {
		...
		if($type == self::MEMBER_TYPE_USERS) {
			...
			$sql = "SELECT nectarcrm_users.id, nectarcrm_users.last_name, nectarcrm_users.first_name FROM nectarcrm_users
							INNER JOIN $tableName ON $tableName.$refIndex = nectarcrm_users.id
		...
		if($type == self::MEMBER_TYPE_GROUPS) {
			...
			$sql = "SELECT nectarcrm_groups.groupid, nectarcrm_groups.groupname FROM nectarcrm_groups
							INNER JOIN $tableName ON $tableName.$refIndex = nectarcrm_groups.groupid
							WHERE $tableName.$tableIndex = ?";
			$params = array($groupModel->getId());
			$result = $db->pquery($sql, $params);
		...
		if($type == self::MEMBER_TYPE_ROLES) {
			...
			$sql = "SELECT nectarcrm_role.roleid, nectarcrm_role.rolename FROM nectarcrm_role
							INNER JOIN $tableName ON $tableName.$refIndex = nectarcrm_role.roleid
							WHERE $tableName.$tableIndex = ?";
			$params = array($groupModel->getId());
			$result = $db->pquery($sql, $params);
