 ...
 */
class Settings_Groups_Record_Model extends Settings_nectarcrm_Record_Model {
	...
	public function save() {
		...
		if (empty($groupId)) {
			$mode = '';
			$groupId = $db->getUniqueId('nectarcrm_users');
			$this->setId($groupId);
		}

		if ($mode == 'edit') {
			$sql = 'UPDATE nectarcrm_groups SET groupname=?, description=? WHERE groupid=?';
			$params = array($this->getName(), $this->getDescription(), $groupId);
		} else {
			$sql = 'INSERT INTO nectarcrm_groups(groupid, groupname, description) VALUES (?,?,?)';
		...
		if (is_array($members)) {
			$db->pquery('DELETE FROM nectarcrm_users2group WHERE groupid=?', array($groupId));
			$db->pquery('DELETE FROM nectarcrm_group2grouprel WHERE groupid=?', array($groupId));
			$db->pquery('DELETE FROM nectarcrm_group2role WHERE groupid=?', array($groupId));
			$db->pquery('DELETE FROM nectarcrm_group2rs WHERE groupid=?', array($groupId));
			...
			for ($i = 0; $i < $noOfMembers; ++$i) {
				...
				if ($idComponents && count($idComponents) == 2) {
					...
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_USERS) {
						$db->pquery('INSERT INTO nectarcrm_users2group(userid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_GROUPS) {
						$db->pquery('INSERT INTO nectarcrm_group2grouprel(containsgroupid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLES) {
						$db->pquery('INSERT INTO nectarcrm_group2role(roleid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLE_AND_SUBORDINATES) {
						$db->pquery('INSERT INTO nectarcrm_group2rs(roleandsubid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
				}
			}
		}
	...
	protected function transferOwnership($transferToGroup) {
		...
		$query = 'UPDATE nectarcrm_crmentity SET smownerid=? WHERE smownerid=?';
		$params = array($transferGroupId, $groupId);
		$db->pquery($query, $params);

		if (nectarcrm_Utils::CheckTable('nectarcrm_customerportal_prefs')) {
			$query = 'UPDATE nectarcrm_customerportal_prefs SET prefvalue = ? WHERE prefkey = ? AND prefvalue = ?';
	...
	public function delete($transferToGroup) {
		...
		$em->triggerEvent("nectarcrm.entity.beforegroupdelete", $entityData);
		...
		$db->pquery('DELETE FROM nectarcrm_group2rs WHERE groupid=?', array($groupId));
		$db->pquery('DELETE FROM nectarcrm_users2group WHERE groupid=?', array($groupId));
		$db->pquery("DELETE FROM nectarcrm_reportsharing WHERE shareid=? AND setype='groups'", array($groupId));

		$db->pquery('DELETE FROM nectarcrm_groups WHERE groupid=?', array($groupId));
	}
	...
	public static function getAll() {
		...
		$sql = 'SELECT * FROM nectarcrm_groups';
		$params = array();
	...
	public static function getInstance($value) {
		...
		if (nectarcrm_Utils::isNumber($value)) {
			$sql = 'SELECT * FROM nectarcrm_groups WHERE groupid = ?';
		} else {
			$sql = 'SELECT * FROM nectarcrm_groups WHERE groupname = ?';
		}
		$params = array($value);
		$result = $db->pquery($sql, $params);
