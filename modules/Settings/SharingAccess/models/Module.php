 ...
 */
class Settings_SharingAccess_Module_Model extends nectarcrm_Module_Model {
	...
	public function save() {
		...
		$sql = 'UPDATE nectarcrm_def_org_share SET permission = ? WHERE tabid = ?';
	...
	public static function getInstance($value) {
		...
		if(nectarcrm_Utils::isNumber($value)) {
			$query = 'SELECT * FROM nectarcrm_def_org_share INNER JOIN nectarcrm_tab ON nectarcrm_tab.tabid = nectarcrm_def_org_share.tabid WHERE nectarcrm_tab.tabid=?';
		} else {
			$query = 'SELECT * FROM nectarcrm_def_org_share INNER JOIN nectarcrm_tab ON nectarcrm_tab.tabid = nectarcrm_def_org_share.tabid WHERE name=?';
		}
	...
	public static function getAll($editable=false) {
		...
		$query = 'SELECT * FROM nectarcrm_def_org_share INNER JOIN nectarcrm_tab ON nectarcrm_tab.tabid = nectarcrm_def_org_share.tabid WHERE nectarcrm_tab.presence IN (0,2)';
		$params = array();
		if($editable) {
	...
	public static function recalculateSharingRules() {
		...
		$result = $db->pquery('SELECT id FROM nectarcrm_users WHERE deleted = ?', array(0));
