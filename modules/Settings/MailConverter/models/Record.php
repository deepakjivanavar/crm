...
class Settings_MailConverter_Record_Model extends Settings_nectarcrm_Record_Model {
	...
	public function hasRules() {
		...
		$result = $db->pquery('SELECT 1 FROM nectarcrm_mailscanner_rules WHERE scannerid = ?', array($this->getId()));
	...
	public function getRecordLinks() {
		...
		$links = array();
		if ($this->hasRules()) {
			$links[] = nectarcrm_Link_Model::getInstanceFromValues(array(
					'linktype' => 'LISTVIEW',
	...
	public function delete() {
        ...
        $scanner = new nectarcrm_MailScannerInfo(trim($this->getName()));
		$scanner->delete();
	}
	...
	public function save() {
        vimport('~~modules/Settings/MailConverter/handlers/MailScannerInfo.php');
		$scannerLatestInfo = new nectarcrm_MailScannerInfo(false, false);
		...
		$isConnected = true;
		$scannerOldInfo = new nectarcrm_MailScannerInfo($this->get('scannerOldName'));

		if(!$scannerOldInfo->compare($scannerLatestInfo)) {
            vimport('~~modules/Settings/MailConverter/handlers/MailBox.php');
			$mailBox = new nectarcrm_MailBox($scannerLatestInfo);
	...
	public function scanNow() {
		...
		if ($isValid) {
			...
			$scannerInfo = new nectarcrm_MailScannerInfo($this->getName());
			/** Start the scanning. */
	...
	public function getUpdatedFoldersList() {
        vimport('~~modules/Settings/MailConverter/handlers/MailBox.php');
		$scannerInfo = new nectarcrm_MailScannerInfo($this->getName());
		$mailBox = new nectarcrm_MailBox($scannerInfo);
	...
	public function saveFolders() {
		...
		$updateQuery = "UPDATE nectarcrm_mailscanner_folders SET enabled = CASE folderid ";
	...
	public function updateSequence($sequencesList) {
		...
		$updateQuery = "UPDATE nectarcrm_mailscanner_rules SET sequence = CASE";
	...
	public static function getCleanInstance() {
		...
		return $recordModel->setModule(Settings_nectarcrm_Module_Model::getInstance('Settings:MailConverter'));
	}
	...
	public static function getInstanceById($recordId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM nectarcrm_mailscanner WHERE scannerid = ?', array($recordId));
		if ($db->num_rows($result)) {
	...
	public static function getAll() {
		$db = PearDatabase::getInstance();
		$moduleModel = Settings_nectarcrm_Module_Model::getInstance('Settings:MailConverter');

		$result = $db->pquery('SELECT * FROM nectarcrm_mailscanner', array());
	...
	public static function getCount() {
		...
		$result = $db->pquery('SELECT 1 FROM nectarcrm_mailscanner', array());
		$numOfRows = $db->num_rows($result);
		return $numOfRows;
	}
