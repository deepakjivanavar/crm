...
class Settings_Currency_Record_Model extends Settings_nectarcrm_Record_Model{
    ...
    public function isDisabledRestricted() {
        ...
        $query = 'SELECT 1 FROM nectarcrm_users WHERE currency_id = ?';
    ...
    public static function getInstance($id) {
        $db = PearDatabase::getInstance();
        if(nectarcrm_Utils::isNumber($id)){
    ...
    public static function getAllNonMapped($includedIds = array()) {
        ...
        $query = 'SELECT nectarcrm_currencies.* FROM nectarcrm_currencies 
                    LEFT JOIN nectarcrm_currency_info ON nectarcrm_currency_info.currency_name = nectarcrm_currencies.currency_name
                    WHERE nectarcrm_currency_info.currency_name IS NULL or nectarcrm_currency_info.deleted=1';
        $params = array();
        if(!empty($includedIds)) {
            $params = $includedIds;
            $query .= ' OR nectarcrm_currency_info.id IN('.generateQuestionMarks($includedIds).')';
        }
		$query .= ' ORDER BY nectarcrm_currencies.currency_name';
