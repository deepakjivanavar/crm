...
class Settings_nectarcrm_OutgoingServer_Model extends Settings_nectarcrm_Systems_Model {
    ...
    public function getBody() {
        ...
        return 'Dear '.$currentUser->get('user_name').', <br><br><b> This is a test mail sent to confirm if a mail is 
                ...
                <br><br>Thanks  and  Regards,<br> Team nectarcrm <br><br>';
    }
    
	public function loadDefaultValues() {
        $defaultOutgoingServerDetails = nectarcrmConfig::getOD('DEFAULT_OUTGOING_SERVER_DETAILS');
        if (empty($defaultOutgoingServerDetails)) {
            $db = PearDatabase::getInstance();
            $db->pquery('DELETE FROM nectarcrm_systems WHERE server_type = ?', array('email'));
            return;
        }
	...
	public function getMenuItem() {
		$menuItem = Settings_nectarcrm_MenuItem_Model::getInstance('LBL_MAIL_SERVER_SETTINGS');
		return $menuItem;
	}
    
	public function getEditViewUrl() {
		$menuItem = $this->getMenuItem();
		return '?module=nectarcrm&parent=Settings&view=OutgoingServerEdit&block='.$menuItem->get('blockid').'&fieldid='.$menuItem->get('fieldid');
	}
	
	public function getDetailViewUrl() {
		$menuItem = $this->getMenuItem();
		return '?module=nectarcrm&parent=Settings&view=OutgoingServerDetail&block='.$menuItem->get('blockid').'&fieldid='.$menuItem->get('fieldid');
	}
