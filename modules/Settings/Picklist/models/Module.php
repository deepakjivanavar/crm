...
class Settings_Picklist_Module_Model extends nectarcrm_Module_Model {

	public function getPickListTableName($fieldName) {
		return 'nectarcrm_'.$fieldName;
	}
	...
	public function addPickListValues($fieldModel, $newValue, $rolesSelected = array(), $color = '') {
		...
		$id = $db->getUniqueID("nectarcrm_$pickListFieldName");
		vimport('~~/include/ComboUtil.php');
		$picklist_valueid = getUniquePicklistID();
		$tableName = 'nectarcrm_'.$pickListFieldName;
		...
		if($fieldModel->isRoleBased() && !empty($rolesSelected)) {
			...
			for($j=0;$j<count($rolesSelected);$j++){
				...
				nectarcrm_Cache::delete('PicklistRoleBasedValues',$pickListFieldName.$roleid);
				$sql ="SELECT max(sortid)+1 as sortid
					   FROM nectarcrm_role2picklist left join nectarcrm_$pickListFieldName
						   on nectarcrm_$pickListFieldName.picklist_valueid=nectarcrm_role2picklist.picklistvalueid
					   WHERE roleid=? and picklistid=?";
				$sortid = $db->query_result($db->pquery($sql, array($roleid, $picklistid)),0,'sortid');

				$sql = "insert into nectarcrm_role2picklist values(?,?,?,?)";
				$db->pquery($sql, array($roleid, $picklist_valueid, $picklistid, $sortid));
			}

		}
		// we should clear cache to update with latest values
		nectarcrm_Cache::flushPicklistCache($pickListFieldName);
	...
	public function renamePickListValues($pickListFieldName, $oldValue, $newValue, $moduleName, $id, $rolesList = false, $color = '') {
		...
		$query = 'SELECT tablename, fieldid, columnname FROM nectarcrm_field WHERE fieldname=? and presence IN (0,2)';
		...
		$primaryKey = nectarcrm_Util_Helper::getPickListId($pickListFieldName);
		if(!empty($color)) {
		...
		$query = "UPDATE nectarcrm_field SET defaultvalue=? WHERE defaultvalue=? AND columnname=?";
		$db->pquery($query, array($newValue, $oldValue, $columnName));

		vimport('~~/include/utils/CommonUtils.php');

		$query = "UPDATE nectarcrm_picklist_dependency SET sourcevalue=? WHERE sourcevalue=? AND sourcefield=?";
		...
		$fieldModel = nectarcrm_Field_Model::getInstance($pickListFieldName, $moduleInstance);

		nectarcrm_Cache::flushPicklistCache($pickListFieldName, $rolesList);
		...
		$em->triggerEvent('nectarcrm.picklist.afterrename', $data);
	...
	public function remove($pickListFieldName , $valueToDeleteId, $replaceValueId , $moduleName) {
		...
		foreach ($pickListValues as $key => $value) {
			$encodedValueToDelete[$key]  = nectarcrm_Util_Helper::toSafeHTML($value);
		}
		...
		if($fieldModel->isRoleBased()) {
			...
			$query = 'SELECT DISTINCT picklist_valueid,roleid FROM '.$this->getPickListTableName($pickListFieldName).
					 ' AS picklisttable LEFT JOIN nectarcrm_role2picklist AS roletable ON roletable.picklistvalueid = picklisttable.picklist_valueid
			...
			$query = 'DELETE FROM nectarcrm_role2picklist WHERE picklistvalueid IN ('.generateQuestionMarks($picklistValueIdToDelete).')';
		...
		$query = 'DELETE FROM nectarcrm_picklist_dependency WHERE sourcevalue IN ('. generateQuestionMarks($pickListValues) .')'.
				' AND sourcefield=?';
		$params = array();
		...
		$query='SELECT tablename,columnname FROM nectarcrm_field WHERE fieldname=? AND presence in (0,2)';
		$result = $db->pquery($query, array($pickListFieldName));
		...
		$query = 'UPDATE nectarcrm_field SET defaultvalue=? WHERE defaultvalue IN ('. generateQuestionMarks($pickListValues) .') AND columnname=?';
		$params = array($replaceValue);
		array_push($params, $pickListValues);
		...
		$data['module'] = $moduleName;
		$em->triggerEvent('nectarcrm.picklist.afterdelete', $data);
	...
	public function enableOrDisableValuesForRole($picklistFieldName, $valuesToEnables, $valuesToDisable, $roleIdList) {
		...
		foreach($roleIdList as $roleId) {
			...
			foreach($valuesToEnables  as $picklistValue) {
				...
				if(empty($valueDetail)){
					 $valueDetail = $pickListValueDetails[nectarcrm_Util_Helper::toSafeHTML($picklistValue)];
				}
		...
		$query = 'INSERT IGNORE INTO nectarcrm_role2picklist (roleid,picklistvalueid,picklistid) VALUES '.implode(',',$insertValueList);
		$result = $db->pquery($query,array());

		$deleteQuery = 'DELETE FROM nectarcrm_role2picklist WHERE '.implode(' OR ',$deleteValueList);
	...
	public static function getPicklistSupportedModules() {
		...
		$query = "SELECT distinct nectarcrm_tab.tablabel, nectarcrm_tab.name as tabname
				  FROM nectarcrm_tab
						inner join nectarcrm_field on nectarcrm_tab.tabid=nectarcrm_field.tabid
				  WHERE uitype IN (15,33,16,114) and nectarcrm_field.tabid NOT IN (". implode(',', $unsupportedModuleIds) .")  and nectarcrm_tab.presence != 1 and nectarcrm_field.presence in (0,2)
				  ORDER BY nectarcrm_tab.tabid ASC";
		$result = $db->pquery($query, array());
	...
	public function handleLabels($moduleName, $newValues, $oldValues, $mode) {
		...
		$allLang = nectarcrm_Language_Handler::getAllLanguages();
	...
	public static function getPicklistColor($pickListFieldName, $pickListId) {
		$db = PearDatabase::getInstance();
		$primaryKey = nectarcrm_Util_Helper::getPickListId($pickListFieldName);
		$colums = $db->getColumnNames("nectarcrm_$pickListFieldName");
		if(in_array('color',$colums)) {
			$query = 'SELECT color FROM nectarcrm_'.$pickListFieldName . ' WHERE '.$primaryKey.' = ?';
	...
	public static function getPicklistColorMap($fieldName, $key = false) {
		...
		$colums = $db->getColumnNames("nectarcrm_$fieldName");
		if(in_array('color',$colums)) {
			$query = 'SELECT '.$primaryKey.',color,'.$fieldName.' FROM nectarcrm_'.$fieldName;
