...
class Settings_MenuEditor_Module_Model extends Settings_nectarcrm_Module_Model {
	...
	public function saveMenuStruncture() {
		...
		$updateQuery = "UPDATE nectarcrm_tab SET tabsequence = CASE tabid ";
	...
	public static function getHiddenModulesForApp($appName) {
		$db = PearDatabase::getInstance();
		$modules = array();
		$result = $db->pquery('SELECT tabid FROM nectarcrm_app2tab WHERE appname = ? AND visible = ?', array($appName, 0));
	...
	public static function getAllVisibleModules() {
		...
		$presence = array('0', '2');
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM nectarcrm_app2tab WHERE visible = ? ORDER BY appname,sequence', array(1));
		$count = $db->num_rows($result);
	...
	public static function addModuleToApp($moduleName, $parent) {
		...
		$oldToNewAppMapping = nectarcrm_MenuStructure_Model::getOldToNewAppMapping();
		if (!empty($oldToNewAppMapping[$parent])) {
			$parent = $oldToNewAppMapping[$parent];
		}

		$ignoredModules = nectarcrm_MenuStructure_Model::getIgnoredModules();
		if (!in_array($moduleName, $ignoredModules)) {
			$moduleModel = nectarcrm_Module_Model::getInstance($moduleName);
			$result = $db->pquery('SELECT * FROM nectarcrm_app2tab WHERE tabid = ? AND appname = ?', array($moduleModel->getId(), $parent));

			$sequence = self::getMaxSequenceForApp($parent) + 1;
			if ($db->num_rows($result) == 0) {
				$db->pquery('INSERT INTO nectarcrm_app2tab(tabid,appname,sequence) VALUES(?,?,?)', array($moduleModel->getId(), $parent, $sequence));
			}
		}
	}
	...
	public static function updateModuleApp($moduleName, $parent, $oldParent = false) {
		...
		$query = "UPDATE nectarcrm_app2tab SET appname=? WHERE tabid=?";
	...
	public static function getMaxSequenceForApp($appName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT MAX(sequence) AS maxsequence FROM nectarcrm_app2tab WHERE appname=?', array($appName));
		$sequence = 0;
		if ($db->num_rows($result) > 0) {
