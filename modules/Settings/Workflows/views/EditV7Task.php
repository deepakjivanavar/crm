...
class Settings_Workflows_EditV7Task_View extends Settings_nectarcrm_Index_View {
	...
	public function process(nectarcrm_Request $request) {
		...
		if ($taskType === 'VTCreateEntityTask') {
			if ($taskObject->entity_type && getTabid($taskObject->entity_type)) {
				$relationModuleModel = nectarcrm_Module_Model::getInstance($taskObject->entity_type);
		...
		if($moduleModel->getName() == 'Invoice' || $moduleModel->getName() == 'Quotes') {
			$metaVariables['Portal Pdf Url'] = '(general : (__nectarcrmMeta__) portalpdfurl)';
		}
		...
		if(!empty ($repeat_date)){
			$repeat_date = nectarcrm_Date_UIType::getDisplayDateValue($repeat_date);
		}
		...
		if($moduleModel->getField('assigned_user_id')) {
			$emailFieldoptions .= '<option value=",$(general : (__nectarcrmMeta__) reports_to_id)"> '.
		...
		$fromEmailFieldOptions .= '<option value="$(general : (__nectarcrmMeta__) supportName)<$(general : (__nectarcrmMeta__) supportEmailId)>"
		...
		$assignedToValues[vtranslate('LBL_USERS', 'nectarcrm')] = $userList;
		$assignedToValues[vtranslate('LBL_GROUPS', 'nectarcrm')] = $groupList;
