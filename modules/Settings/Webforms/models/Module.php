...
class Settings_Webforms_Module_Model extends Settings_nectarcrm_Module_Model {

	var $baseTable = 'nectarcrm_webforms';
	var $baseIndex = 'id';
	...
	public function deleteRecord($recordModel) {
		...
		$db->pquery("DELETE from nectarcrm_webforms_field WHERE webformid = ?", array($recordId));
		$db->pquery("DELETE from nectarcrm_webforms WHERE id = ?", array($recordId));
		return true;
	}
