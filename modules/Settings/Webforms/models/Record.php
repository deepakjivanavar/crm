...
class Settings_Webforms_Record_Model extends Settings_nectarcrm_Record_Model {
	...
	public function getRecordLinks() {
		...
		$recordLinks = array(
				...
				array(
						...
						'linkurl' => "javascript:Settings_nectarcrm_List_Js.triggerDelete(event,'".$this->getDeleteUrl()."');",
	...
	public function getDetailViewLinks() {
		...
		foreach ($detailViewLinks as $detailViewLink) {
			$linkModelList['DETAILVIEWBASIC'][] = nectarcrm_Link_Model::getInstanceFromValues($detailViewLink);
		}
	...
	public function getSelectedFieldsList($mode='') {
		if (!$this->selectedFields) {
			...
			if ($targetModule) {
				...
				$targetModuleModel = nectarcrm_Module_Model::getInstance($targetModule);
				$allFields = $targetModuleModel->getFields();

				$result = $db->pquery("SELECT * FROM nectarcrm_webforms_field WHERE webformid = ? ORDER BY sequence", array($this->getId()));
	...
	public function save() {
		...
		if ($mode) {
			$updateQuery = "UPDATE nectarcrm_webforms SET description = ?, returnurl = ?, ownerid = ?, enabled = ?, captcha = ? , roundrobin = ?, roundrobin_userid = ?, roundrobin_logic = ? ,targetmodule = ? WHERE id = ?";
			...
			$insertQuery = "INSERT INTO nectarcrm_webforms(name, targetmodule, publicid, enabled, description, ownerid, returnurl, captcha, roundrobin, roundrobin_userid, roundrobin_logic) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		$db->pquery("DELETE FROM nectarcrm_webforms_field WHERE webformid = ?", array($this->getId()));

		//Saving data of source module fields info for this webform
		$selectedFieldsData = $this->get('selectedFieldsData');
		$sourceModuleModel = nectarcrm_Module_Model::getInstance($this->get('targetmodule'));

		$fieldInsertQuery = "INSERT INTO nectarcrm_webforms_field(webformid, fieldname, neutralizedfield, defaultvalue, required, sequence, hidden) VALUES(?, ?, ?, ?, ?, ?, ?)";
		foreach ($selectedFieldsData as $fieldName => $fieldDetails) {
			...
			$fieldModel = nectarcrm_Field_Model::getInstance($fieldName, $sourceModuleModel);
			...
			if ($dataType === 'date') {
				$fieldDefaultValue = nectarcrm_Date_UIType::getDBInsertedValue($fieldDefaultValue);
			}

			if ($dataType === 'time') {
				$fieldDefaultValue = nectarcrm_Time_UIType::getTimeValueWithSeconds($fieldDefaultValue);
			}

			if ($dataType === 'reference') {
				$referenceModule = $fieldDetails['referenceModule'];
				$referenceObject = nectarcrmWebserviceObject::fromName($db,$referenceModule);
		...
		$db->pquery('DELETE FROM nectarcrm_webform_file_fields WHERE webformid = ?', array($this->getId()));
		$fileFieldQuery = 'INSERT INTO nectarcrm_webform_file_fields (webformid, fieldname, fieldlabel, required) VALUES (?, ?, ?, ?)';
		$i = 1;
	...
	function getFileFields() {
		...
		if (!$fileFields) {
			...
			$result = $db->pquery('SELECT * FROM nectarcrm_webform_file_fields WHERE webformid = ?', array($this->getId()));
	...
	public function checkDuplicate() {
		...
		$query = "SELECT 1 FROM nectarcrm_webforms WHERE name = ?";
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		...
		$result = $db->pquery("SELECT * FROM nectarcrm_webforms WHERE id = ?", array($recordId));
		if ($db->num_rows($result)) {
	 ...
	 * @return <Settings_nectarcrm_Module_Model>
	 */
	static public function getCleanInstance($moduleName) {
		$recordModel = new self();
		$moduleModel = Settings_nectarcrm_Module_Model::getInstance($moduleName);
