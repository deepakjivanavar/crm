...
class Settings_LayoutEditor_Module_Model extends nectarcrm_Module_Model {
	...
	public function addField($fieldType, $blockId, $params) {
		...
		$max_fieldid = $db->getUniqueID("nectarcrm_field");
		...
		if (isset($focus->customFieldTable)) {
			...
			$tableName= 'nectarcrm_'.strtolower($moduleName).'cf';
		}
		...
		if(strtolower($fieldType) == 'date') {
			$dateInstance = new nectarcrm_Date_UIType();
			$defaultValue = $dateInstance->getDBInsertedValue($defaultValue);
		} else if (strtolower($fieldType) == 'time') {
			$defaultValue = nectarcrm_Time_UIType::getTimeValueWithSeconds($defaultValue);
		...
		$blockModel = nectarcrm_Block_Model::getInstance($blockId, $this);
		$blockModel->addField($fieldModel);

		// If the column failed to create then do not add entry to nectarcrm_field table
	...
	public function getTypeDetailsForAddField($fieldType,$params) {
		switch ($fieldType) {
				...
				Case 'Decimal' :
							   ...
							   // Fix for http://trac.nectarcrm.com/cgi-bin/trac.cgi/ticket/6363
							   $uichekdata='NN~O';
							   break;
			   Case 'Percent' :
							   $uitype = 9;
	...
	public function checkFieldExists($fieldLabel) {
		...
		$query = 'SELECT 1 FROM nectarcrm_field WHERE tabid IN ('.  generateQuestionMarks($tabId).') AND fieldlabel=?';
	...
	public static function getEntityModulesList() {
		...
		$query = 'SELECT name FROM nectarcrm_tab WHERE
	...
	public function getRelations() {
		if($this->relations === null) {
			$this->relations = nectarcrm_Relation_Model::getAllRelations($this, false);
		}
	...
	public function getRelationTypeFromRelationField($fieldModel) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT 1 FROM nectarcrm_relatedlists WHERE relationfieldid=?',array($fieldModel->getId()));
	...
	public function updateDuplicateHandling($rule, $fieldIdsList = array(), $syncActionId = 1) {
		...
		if (count($fieldIdsList) < 4) {//Maximum 3 fields are allowed
			$query = 'UPDATE nectarcrm_field SET isunique = CASE WHEN fieldid IN ('.  generateQuestionMarks($fieldIdsList).') THEN 1 ELSE 0 END WHERE tabid=?';
		...
		//Rule
		$db->pquery('UPDATE nectarcrm_tab SET allowduplicates=?, sync_action_for_duplicates=? WHERE tabid=?', array($rule, $syncActionId, $tabId));
		nectarcrm_Cache::flushModuleCache($this->getName());
		return true;
	}
