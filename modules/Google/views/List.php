...
class Google_List_View extends nectarcrm_PopupAjax_View {
	...
	function renderWidgetUI(nectarcrm_Request $request) {
	...
	function renderSyncUI(nectarcrm_Request $request) {
		$sourceModule = $request->get('sourcemodule');
		$viewer = $this->getViewer($request);
		$viewer->assign('SCRIPTS',$this->getHeaderScripts($request));
		$oauth2 = new Google_Oauth2_Connector($sourceModule);
	...
	public function getSyncRecordsCount($syncRecords) {
		$countRecords = array(	'nectarcrm'	=> array('update' => 0, 'create' => 0, 'delete' => 0),
