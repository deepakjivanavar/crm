...
class CustomerPortal_DownloadFile extends CustomerPortal_API_Abstract {
	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($current_user) {
			...
			$id = $idComponents[1];
			if ($module == 'Documents') {
				$query = "SELECT filetype FROM nectarcrm_notes INNER JOIN nectarcrm_crmentity ON nectarcrm_notes.notesid= nectarcrm_crmentity.crmid 
                          WHERE notesid =? AND nectarcrm_crmentity.deleted=?";
				...
				$modCommentsRecordModel = nectarcrm_Record_Model::getInstanceById($id, $module);
