 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/nectarcrm_crm/sugarcrm/modules/Emails/Emails.php,v 1.41 2005/04/28 08:11:21 rank Exp $
...
class Emails extends CRMEntity {
	...
	// Stored nectarcrm_fields
	// added to check email save from plugin or not
	var $plugin_save = false;
	var $rel_users_table = "nectarcrm_salesmanactivityrel";
	var $rel_contacts_table = "nectarcrm_cntactivityrel";
	var $rel_serel_table = "nectarcrm_seactivityrel";
	var $tab_name = Array('nectarcrm_crmentity', 'nectarcrm_activity', 'nectarcrm_emaildetails');
	var $tab_name_index = Array('nectarcrm_crmentity' => 'crmid', 'nectarcrm_activity' => 'activityid',
		'nectarcrm_seactivityrel' => 'activityid', 'nectarcrm_cntactivityrel' => 'activityid', 'nectarcrm_email_track' => 'mailid', 'nectarcrm_emaildetails' => 'emailid');
	...
	function save_module($module) {
		...
		//modified by Richie as raju's implementation broke the feature for addition of webmail to nectarcrm_crmentity.need to be more careful in future while integrating code
		if ($_REQUEST['module'] == "Emails" && $_REQUEST['smodule'] != 'webmails' && (!$this->plugin_save)) {
			...
			if ($_REQUEST['module'] != 'Emails' && $_REQUEST['module'] != 'Webmails') {
				if (!$parentid) {
					$parentid = $adb->getUniqueID('nectarcrm_seactivityrel');
				}
				$mysql = 'insert into nectarcrm_seactivityrel values(?,?)';
				...
				for ($i = 0; $i < (count($myids) - 1); $i++) {
					...
					$mycrmid = $realid[0];
					//added to handle the relationship of emails with nectarcrm_users
					if ($realid[1] == -1) {
						$del_q = 'delete from nectarcrm_salesmanactivityrel where smid=? and activityid=?';
						$adb->pquery($del_q, array($mycrmid, $actid));
						$mysql = 'insert into nectarcrm_salesmanactivityrel values(?,?)';
					} else {
						$del_q = 'delete from nectarcrm_seactivityrel where crmid=? and activityid=?';
						$adb->pquery($del_q, array($mycrmid, $actid));
						$mysql = 'insert into nectarcrm_seactivityrel values(?,?)';
					}
			...
			if (isset($this->column_fields['parent_id']) && $this->column_fields['parent_id'] != '') {
				$adb->pquery("DELETE FROM nectarcrm_seactivityrel WHERE crmid = ? AND activityid = ? ",
						array($this->column_fields['parent_id'], $this->id));
				//$this->insertIntoEntityTable('nectarcrm_seactivityrel', $module);
				$sql = 'insert into nectarcrm_seactivityrel values(?,?)';
	...
	function insertIntoAttachment($id, $module) {
		...
		if ($module == 'Emails' && isset($_REQUEST['att_id_list']) && $_REQUEST['att_id_list'] != '') {
			...
			if ($id_cnt != 0) {
				for ($i = 0; $i < $id_cnt; $i++) {
					$sql_rel = 'insert into nectarcrm_seattachmentsrel values(?,?)';
	...
	function saveForwardAttachments($id, $module, $file_details) {
		...
		if ($_REQUEST['mode'] == 'edit') {
			if ($id != '' && $_REQUEST['fileid'] != '') {
				$delquery = 'delete from nectarcrm_seattachmentsrel where crmid = ? and attachmentsid = ?';
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select nectarcrm_contactdetails.accountid, nectarcrm_contactdetails.contactid, nectarcrm_contactdetails.firstname,nectarcrm_contactdetails.lastname, nectarcrm_contactdetails.department, nectarcrm_contactdetails.title, nectarcrm_contactdetails.email, nectarcrm_contactdetails.phone, nectarcrm_contactdetails.emailoptout, nectarcrm_crmentity.crmid, nectarcrm_crmentity.smownerid, nectarcrm_crmentity.modifiedtime from nectarcrm_contactdetails inner join nectarcrm_cntactivityrel on nectarcrm_cntactivityrel.contactid=nectarcrm_contactdetails.contactid inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid = nectarcrm_contactdetails.contactid left join nectarcrm_groups on nectarcrm_groups.groupid=nectarcrm_crmentity.smownerid where nectarcrm_cntactivityrel.activityid=' . $adb->quote($id) . ' and nectarcrm_crmentity.deleted=0';
	 ...
	 * Portions created by nectarcrmCRM are Copyright (C) nectarcrmCRM.
	 * All Rights Reserved..
	...
	/** Returns a list of the associated nectarcrm_users
	 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc..
	 * All Rights Reserved..
	 * Contributor(s): ______________________________________..
	 */
	function get_users($id) {
		...
		$query = 'SELECT nectarcrm_users.id, nectarcrm_users.first_name,nectarcrm_users.last_name, nectarcrm_users.user_name, nectarcrm_users.email1, nectarcrm_users.email2, nectarcrm_users.secondaryemail , nectarcrm_users.phone_home, nectarcrm_users.phone_work, nectarcrm_users.phone_mobile, nectarcrm_users.phone_other, nectarcrm_users.phone_fax from nectarcrm_users inner join nectarcrm_salesmanactivityrel on nectarcrm_salesmanactivityrel.smid=nectarcrm_users.id and nectarcrm_salesmanactivityrel.activityid=?';
	...
	function create_export_query(&$order_by, &$where) {
		...
		$query = "SELECT $fields_list FROM nectarcrm_activity
			...
			LEFT JOIN nectarcrm_seattachmentsrel
				ON nectarcrm_activity.activityid=nectarcrm_seattachmentsrel.crmid
		...
		$query .= "WHERE nectarcrm_activity.activitytype='Emails' AND nectarcrm_crmentity.deleted=0 ";
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		$sql = 'DELETE FROM nectarcrm_seactivityrel WHERE activityid=? AND crmid = ?';
		$this->db->pquery($sql, array($id, $return_id));

		parent::unlinkRelationship($id, $return_module, $return_id);

		$this->db->pquery('UPDATE nectarcrm_crmentity SET modifiedtime = ? WHERE crmid = ?', array(date('y-m-d H:i:d'), $id));
	}
	...
	*/
	function setRelationTables($secmodule) {
		$rel_tables = array (
				...
				"Vendors" => array("nectarcrm_seactivityrel" => array("activityid", "crmid"), "nectarcrm_activity" => "activityid"),
				"Contacts" => array("nectarcrm_seactivityrel" => array("activityid", "crmid"), "nectarcrm_activity" => "activityid"),
	...
	*/
	function generateReportsSecQuery($module, $secmodule, $queryPlanner){
		...
		$matrix->setDependency("nectarcrm_crmentityEmails",array("nectarcrm_groupsEmails","nectarcrm_usersEmails","nectarcrm_lastModifiedByEmails"));

		if (!$queryPlanner->requireTable('nectarcrm_activity', $matrix)) {
			return '';
		}

		$matrix->setDependency("nectarcrm_activity",array("nectarcrm_crmentityEmails","nectarcrm_email_track"));

		$query = $this->getRelationQuery($module, $secmodule, "nectarcrm_activity","activityid", $queryPlanner);
		if ($queryPlanner->requireTable("nectarcrm_crmentityEmails")){
			$query .= " LEFT JOIN nectarcrm_crmentity AS nectarcrm_crmentityEmails ON nectarcrm_crmentityEmails.crmid=nectarcrm_activityEmails.activityid and nectarcrm_crmentityEmails.deleted = 0";
		}
		if ($queryPlanner->requireTable("nectarcrm_groupsEmails")){
			$query .= " LEFT JOIN nectarcrm_groups AS nectarcrm_groupsEmails ON nectarcrm_groupsEmails.groupid = nectarcrm_crmentityEmails.smownerid";
		}
		if ($queryPlanner->requireTable("nectarcrm_usersEmails")){
			$query .= " LEFT JOIN nectarcrm_users AS nectarcrm_usersEmails ON nectarcrm_usersEmails.id = nectarcrm_crmentityEmails.smownerid";
		}
		if ($queryPlanner->requireTable("nectarcrm_lastModifiedByEmails")){
			$query .= " LEFT JOIN nectarcrm_users AS nectarcrm_lastModifiedByEmails ON nectarcrm_lastModifiedByEmails.id = nectarcrm_crmentityEmails.modifiedby and nectarcrm_seactivityreltmpEmails.activityid = nectarcrm_activityEmails.activityid";
		}
		if ($queryPlanner->requireTable("nectarcrm_createdbyEmails")){
			$query .= " left join nectarcrm_users as nectarcrm_createdbyEmails on nectarcrm_createdbyEmails.id = nectarcrm_crmentityEmails.smcreatorid and nectarcrm_seactivityreltmpEmails.activityid = nectarcrm_activityEmails.activityid";
		}
		if ($queryPlanner->requireTable("nectarcrm_email_track")){
			$query .= " LEFT JOIN nectarcrm_email_track ON nectarcrm_email_track.mailid = nectarcrm_activityEmails.activityid and nectarcrm_email_track.crmid = ".$focus->table_name.".".$focus->table_index;
		}
	...
	/*
	 * Function to store the email access count value of emails in 'nectarcrm_email_track' table
	 * @param - $mailid
	 */
	function setEmailAccessCountValue($mailid) {
		global $adb;
		$successIds = array();
		$result = $adb->pquery('SELECT idlists FROM nectarcrm_emaildetails WHERE emailid=?', array($mailid));
		...
		for ($i=0; $i<count($successIds); $i++) {
			$adb->pquery("INSERT INTO nectarcrm_email_track(crmid, mailid,  access_count) VALUES(?,?,?)", array($successIds[$i], $mailid, 0));
		}
	}
...
function pdfAttach($obj, $module, $file_name, $id, $filePath = 'storage/') {
	...
	if ($status) {
		$query1 = "insert into nectarcrm_crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) values(?,?,?,?,?,?,?)";
		...
		$query2 = "insert into nectarcrm_attachments(attachmentsid, name, description, type, path) values(?,?,?,?,?)";
		$params2 = array($current_id, $file_name, $obj->column_fields['description'], 'pdf', $upload_file_path);
		$result = $adb->pquery($query2, $params2);

		$query3 = 'insert into nectarcrm_seattachmentsrel values(?,?)';
