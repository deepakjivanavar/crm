...
class Install_Index_view extends nectarcrm_View_Controller {
	...
	public function preProcess(nectarcrm_Request $request, $display = true) {
		...
		if(is_file($configFileName) && filesize($configFileName) > 0) {
			...
			$defaultModuleInstance = nectarcrm_Module_Model::getInstance($defaultModule);
	...
	public function Step1(nectarcrm_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
	...
	public function Step4(nectarcrm_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
	...
	public function Step5(nectarcrm_Request $request) {
	...
	public function Step6(nectarcrm_Request $request) {
		$moduleName = $request->getModule();
		$viewer = $this->getViewer($request);
	...
	public function Step7(nectarcrm_Request $request) {
		$moduleName = $request->getModule();
		$webuiInstance = new nectarcrm_WebUI();
		...
		if(!$isInstalled){
			...
			$viewer->assign('CURRENT_VERSION', $_SESSION['nectarcrm_version']);
	...
	public function getHeaderScripts(nectarcrm_Request $request) {
		...
		$jsFileNames = array("modules.nectarcrm.resources.List",
