...
class Mobile_WS_RelatedRecordsWithGrouping extends Mobile_WS_QueryWithGrouping {
	...
	function process(Mobile_API_Request $request) {
		...
		if ($functionHandler) {
			...
			$querySEtype = "nectarcrm_crmentity.setype as setype";
			if ($relatedmodule == 'Calendar') {
				$querySEtype = "nectarcrm_activity.activitytype as setype";
			}
			
			$query = sprintf("SELECT nectarcrm_crmentity.crmid, $querySEtype %s", substr($query, stripos($query, 'FROM')));
