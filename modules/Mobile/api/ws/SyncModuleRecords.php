		...
		if (!empty($lastSyncTime) && $currentPage === 0) {
			...
			if ($hasAssignedToField) {
				if ($isPrivateMode) {
					...
					$andsmowneridequal = " AND nectarcrm_crmentity.smownerid=?";
				} else {
			...
			if ($module == 'Calendar') {
				$queryDeleted = $adb->pquery("SELECT activityid as crmid, activitytype as setype FROM nectarcrm_activity
					INNER JOIN nectarcrm_crmentity ON nectarcrm_activity.activityid=nectarcrm_crmentity.crmid
					AND nectarcrm_crmentity.deleted=1 AND nectarcrm_crmentity.setype=? AND nectarcrm_crmentity.modifiedtime > ?
					LEFT JOIN nectarcrm_users ON nectarcrm_users.id = nectarcrm_crmentity.smownerid
					LEFT JOIN nectarcrm_groups ON nectarcrm_groups.groupid = nectarcrm_crmentity.smownerid WHERE 1=1	$andsmowneridequal ",
				$queryDeletedParameters);
			} else if ($module == 'Leads') {
				$queryDeleted = $adb->pquery("SELECT crmid, modifiedtime, setype FROM nectarcrm_crmentity
				...
				WHERE (nectarcrm_crmentity.deleted=1 OR (nectarcrm_crmentity.deleted=0 AND nectarcrm_leaddetails.converted=1)) AND nectarcrm_crmentity.setype=? AND nectarcrm_crmentity.modifiedtime > ? $andsmowneridequal", $queryDeletedParameters);
			} else {
				$queryDeleted = $adb->pquery("SELECT crmid, modifiedtime, setype FROM nectarcrm_crmentity
				LEFT JOIN nectarcrm_users ON nectarcrm_users.id = nectarcrm_crmentity.smownerid
				LEFT JOIN nectarcrm_groups ON nectarcrm_groups.groupid = nectarcrm_crmentity.smownerid
				WHERE nectarcrm_crmentity.deleted=1 AND nectarcrm_crmentity.setype=? AND nectarcrm_crmentity.modifiedtime > ? $andsmowneridequal", $queryDeletedParameters);
			}
