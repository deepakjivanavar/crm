...
class Mobile_ICAL {
	...
	function generate() {
		...
		$properties = array();
		$properties['prodid'] = '-//nectarcrm/Mobile/NONSGML 1.0//EN';
		...
		$query = "SELECT " . implode(',', $fieldnames) . " FROM nectarcrm_activity
			INNER JOIN nectarcrm_crmentity ON
			(nectarcrm_activity.activityid=nectarcrm_crmentity.crmid 	AND nectarcrm_crmentity.deleted = 0 AND nectarcrm_crmentity.smownerid = ?)
			LEFT JOIN nectarcrm_activity_reminder ON nectarcrm_activity_reminder.activity_id=nectarcrm_activity.activityid
