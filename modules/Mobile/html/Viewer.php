...
class Mobile_HTML_Viewer extends nectarcrm_Viewer{
	...
	function viewController() {
		if(!$this->_smarty) {
			$this->_smarty = nectarcrm_Viewer::getInstance();
		}
		...
		$this->_smarty->assign("TEMPLATE_WEBPATH", "layouts/".nectarcrm_Viewer::getLayoutName()."/modules/Mobile/".Mobile::config('Default.Layout'));
	...
	function process($module, $view) {
        ...
        if (!$smarty->templateExists($templatePath)) {
			$templatePath = $templateBasePath . "/nectarcrm/{$view}.tpl";
			if (!$smarty->templateExists($templatePath)) {
				$templatePath = null;
			}
		}
		...
		$baseTemplatePath = $templateBasePath. "/nectarcrm/js/{$view}.js";
		if(file_exists($baseTemplatePath)) {
			$scripts[] = "/nectarcrm/js/{$view}.js";
			$controller = 'nectarcrm'.$view.'Controller';
		}
...
function mobile_templatepath($template, $module) {
    $smarty = nectarcrm_Viewer::getInstance();
    ...
    if (!$smarty->templateExists($templatePath)) {
        $templatePath = $templateBasePath . "/nectarcrm/{$template}";
    }
    if (!$templatePath) {
