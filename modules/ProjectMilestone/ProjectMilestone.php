...
class ProjectMilestone extends CRMEntity {
	...
	var $table_name = 'nectarcrm_projectmilestone';
	 ...
	 */
	var $customFieldTable = Array('nectarcrm_projectmilestonecf', 'projectmilestoneid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('nectarcrm_crmentity', 'nectarcrm_projectmilestone', 'nectarcrm_projectmilestonecf');
	...
	var $tab_name_index = Array(
		...
		'nectarcrm_projectmilestone'   => 'projectmilestoneid',
		'nectarcrm_projectmilestonecf' => 'projectmilestoneid');
	...
	function vtlib_handler($modulename, $event_type) {
		...
		if($event_type == 'module.postinstall') {
			...
			$projectMilestoneResult = $adb->pquery('SELECT tabid FROM nectarcrm_tab WHERE name=?', array('ProjectMilestone'));
			...
			if(getTabid('CustomerPortal')) {
				$checkAlreadyExists = $adb->pquery('SELECT 1 FROM nectarcrm_customerportal_tabs WHERE tabid=?', array($projectmilestoneTabid));
				if($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
					...
					$adb->query("INSERT INTO nectarcrm_customerportal_tabs(tabid,visible,sequence) VALUES ($projectmilestoneTabid,1,$nextSequence)");
					$adb->query("INSERT INTO nectarcrm_customerportal_prefs(tabid,prefkey,prefvalue) VALUES ($projectmilestoneTabid,'showrelatedinfo',1)");
				}
			}
			...
			if (!($adb->num_rows($result))) {
				...
				$adb->pquery("INSERT INTO nectarcrm_modentity_num values(?,?,?,?,?,?)", array($adb->getUniqueId("nectarcrm_modentity_num"), $modulename, 'PM', 1, 1, 1));
			}
	...
	function generateReportsSecQuery($module, $secmodule, $queryPlanner) {
		...
		$matrix->setDependency('nectarcrm_crmentityProjectMilestone', array('nectarcrm_groupsProjectMilestone', 'nectarcrm_usersProjectMilestone', 'nectarcrm_lastModifiedByProjectMilestone'));

		if (!$queryPlanner->requireTable('nectarcrm_projectmilestone', $matrix)) {
			return '';
		}
		$matrix->setDependency('nectarcrm_projectmilestone', array('nectarcrm_crmentityProjectMilestone'));

		$query .= $this->getRelationQuery($module,$secmodule,"nectarcrm_projectmilestone","projectmilestoneid", $queryPlanner);

		if ($queryPlanner->requireTable('nectarcrm_crmentityProjectMilestone', $matrix)) {
			$query .= " LEFT JOIN nectarcrm_crmentity AS nectarcrm_crmentityProjectMilestone ON nectarcrm_crmentityProjectMilestone.crmid=nectarcrm_projectmilestone.projectmilestoneid and nectarcrm_crmentityProjectMilestone.deleted=0";
		}
		if ($queryPlanner->requireTable('nectarcrm_projectmilestonecf')) {
			$query .= " LEFT JOIN nectarcrm_projectmilestonecf ON nectarcrm_projectmilestone.projectmilestoneid = nectarcrm_projectmilestonecf.projectmilestoneid";
		}
		if ($queryPlanner->requireTable('nectarcrm_groupsProjectMilestone')) {
			$query .= "	left join nectarcrm_groups AS nectarcrm_groupsProjectMilestone ON nectarcrm_groupsProjectMilestone.groupid = nectarcrm_crmentityProjectMilestone.smownerid";
		}
		if ($queryPlanner->requireTable('nectarcrm_usersProjectMilestone')) {
			$query .= " LEFT JOIN nectarcrm_users AS nectarcrm_usersProjectMilestone ON nectarcrm_usersProjectMilestone.id = nectarcrm_crmentityProjectMilestone.smownerid";
		}
		if ($queryPlanner->requireTable('nectarcrm_lastModifiedByProjectMilestone')) {
			$query .= " LEFT JOIN nectarcrm_users AS nectarcrm_lastModifiedByProjectMilestone ON nectarcrm_lastModifiedByProjectMilestone.id = nectarcrm_crmentityProjectMilestone.modifiedby ";
		}
		if ($queryPlanner->requireTable("nectarcrm_createdbyProjectMilestone")){
			$query .= " LEFT JOIN nectarcrm_users AS nectarcrm_createdbyProjectMilestone ON nectarcrm_createdbyProjectMilestone.id = nectarcrm_crmentityProjectMilestone.smcreatorid ";
		}
