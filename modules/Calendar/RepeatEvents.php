...
class Calendar_RepeatEvents {
	...
	static function repeat($focus, $recurObj) {
		...
		if($focus->column_fields['mode'] == 'edit') {
			$childRecords = array();
			$result = $adb->pquery("SELECT * FROM nectarcrm_activity_recurring_info WHERE activityid=?", array($parentId));
			$noofrows = $adb->num_rows($result);
			$parentRecurringId = $parentId;
			if($noofrows <= 0) {
				$queryResult = $adb->pquery("SELECT * FROM nectarcrm_activity_recurring_info WHERE recurrenceid=?", array($parentId));
				if($adb->num_rows($queryResult) > 0) {
					$parentRecurringId = $adb->query_result($queryResult, 0,"activityid");
					$result = $adb->pquery("SELECT * FROM nectarcrm_activity_recurring_info WHERE activityid=?", array($parentRecurringId));
					$noofrows = $adb->num_rows($result);
					
					if($focus->column_fields['recurringEditMode'] == 'all') {
						$parentModel = nectarcrm_Record_Model::getInstanceById($parentId);
						$parentResult = $adb->pquery("SELECT 1 FROM nectarcrm_activity_recurring_info WHERE recurrenceid=?", array($parentRecurringId));
						if($adb->num_rows($parentResult) >= 1) {
							$parentModel = nectarcrm_Record_Model::getInstanceById($parentRecurringId);
						} else {
							...
							$parentModel = nectarcrm_Record_Model::getInstanceById($recurringRecords[0]);
						}
				...
				if(!empty($recordId) && !empty($startDate)) {
					...
					if($focus->column_fields['recurringEditMode'] == 'future' && $recordModel->get('date_start') >= $eventStartDate) {
						...
						if(self::$recurringTypeChanged) {
							$adb->pquery("INSERT INTO nectarcrm_activity_recurring_info VALUES (?,?)", array($parentId, $recordId));
						}
			...
			if(self::$recurringDataChanged && !empty($deletingRecords)) {
				foreach($deletingRecords as $record) {
					...
					$adb->pquery("DELETE FROM nectarcrm_activity_recurring_info WHERE activityid=? AND recurrenceid=?", array($parentId, $record));
					$recordModel->delete();
				}
			}
		} else {
	...
	static function createRecurringEvents($focus, $recurObj, $recurringDates, $parentId = false) {
		...
		foreach ($recurringDates as $index => $startDate) {
			...
			$adb->pquery("INSERT INTO nectarcrm_activity_recurring_info VALUES (?,?)", array($parentId, $record));
	...
	static function repeatFromRequest($focus, $recurObjDb = false) {
		...
		if($focus->column_fields['recurringtype'] != '' && $focus->column_fields['recurringtype'] != '--None--' && $focus->column_field['recurringEditMode'] != 'current') {
			...
			if(empty($recurObjDb) && self::$recurringDataChanged) {
				$adb->pquery("INSERT INTO nectarcrm_activity_recurring_info VALUES (?,?)", array($originalRecordId, $originalRecordId));
			}
	...
	static function deleteRepeatEvents($parentId) {
		$adb = PearDatabase::getInstance();
		$recordModel = nectarcrm_Record_Model::getCleanInstance('Events');
