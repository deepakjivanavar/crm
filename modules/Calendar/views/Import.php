...
class Calendar_Import_View extends nectarcrm_Import_View {
	 ...
	 */
	public function importResult(nectarcrm_Request $request) {
		...
		if (Import_Utils_Helper::validateFileUpload($request)) {
			...
			$calendarModel = nectarcrm_Module_Model::getInstance($moduleName);
	 ...
	 * @param nectarcrm_Request $request
	 */
	public function undoIcalImport(nectarcrm_Request $request) {
		$currentUserModel = Users_Record_Model::getCurrentUserModel();
		$moduleName = $request->getModule();
