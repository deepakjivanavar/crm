 ...
 */
class Calendar_Module_Model extends nectarcrm_Module_Model {
	...
	/**
	 * Function to get a nectarcrm Record Model instance from an array of key-value mapping
	...
	public function getSideBarLinks($linkParams) {
		...
		$links = nectarcrm_Link_Model::getAllByType($this->getId(), $linkTypes, $linkParams);
	...
	public function getExportQuery() {
		...
		$query = "SELECT nectarcrm_activity.*, nectarcrm_crmentity.description, nectarcrm_activity_reminder.reminder_time FROM nectarcrm_activity
					INNER JOIN nectarcrm_crmentity ON nectarcrm_activity.activityid = nectarcrm_crmentity.crmid
					LEFT JOIN nectarcrm_activity_reminder ON nectarcrm_activity_reminder.activity_id = nectarcrm_activity.activityid AND nectarcrm_activity_reminder.recurringid = 0
					WHERE nectarcrm_crmentity.deleted = 0 AND nectarcrm_crmentity.smownerid IN ($value) AND nectarcrm_activity.activitytype NOT IN ('Emails')";
		return $query;
	}
	...
	*/
	public static function getCaledarSharedUsers($id){
		...
		$query = "SELECT nectarcrm_users.user_name, nectarcrm_sharedcalendar.* FROM nectarcrm_sharedcalendar
				LEFT JOIN nectarcrm_users ON nectarcrm_sharedcalendar.sharedid=nectarcrm_users.id WHERE userid=?";
	...
	public static function getSharedUsersOfCurrentUser($id){
		...
		if($currentUser->isAdminUser()) {
			$query = "SELECT first_name,last_name, id AS userid
					FROM nectarcrm_users WHERE status='Active' AND (user_name != 'admin' OR is_owner = 1) AND id <> ?";
			$result = $db->pquery($query, array($id));
		} else {
			$query = "SELECT nectarcrm_users.first_name,nectarcrm_users.last_name, nectarcrm_users.id AS userid
				FROM nectarcrm_sharedcalendar RIGHT JOIN nectarcrm_users ON nectarcrm_sharedcalendar.userid=nectarcrm_users.id AND nectarcrm_users.status= 'Active'
				WHERE nectarcrm_sharedcalendar.sharedid=? OR (nectarcrm_users.status='Active' AND nectarcrm_users.calendarsharedtype='public' AND nectarcrm_users.id <> ?)";
	...
	*/
	public static function getSharedUsersInfoOfCurrentUser($id){
		...
		$query = "SELECT shareduserid,color,visible FROM nectarcrm_shareduserinfo where userid = ?";
		$result = $db->pquery($query, array($id));
		$rows = $db->num_rows($result);
	...
	*/
	public static function getCalendarViewTypes($id){
		...
		$query = "SELECT * FROM nectarcrm_calendar_user_activitytypes 
			INNER JOIN nectarcrm_calendar_default_activitytypes on nectarcrm_calendar_default_activitytypes.id=nectarcrm_calendar_user_activitytypes.defaultid 
			WHERE nectarcrm_calendar_user_activitytypes.userid=?";
		$result = $db->pquery($query, array($id));
		$rows = $db->num_rows($result);
		...
		for($i=0; $i<$rows; $i++){
			...
			$moduleInstance = nectarcrm_Module_Model::getInstance($activityTypes['module']);
			...
			foreach ($fieldNamesList as $fieldName) {
				$fieldInstance = nectarcrm_Field_Model::getInstance($fieldName, $moduleInstance);
				if ($fieldInstance) {
	...
	public static function getDateFieldModulesList() {
		...
		$query = 'SELECT DISTINCT(name) AS modulename FROM nectarcrm_tab 
				  LEFT JOIN nectarcrm_field ON nectarcrm_field.tabid = nectarcrm_tab.tabid
				  WHERE nectarcrm_field.typeofdata LIKE ?';
		...
		$moduleInstance = nectarcrm_Module_Model::getInstance('Events');
		...
		$calendarInstance = nectarcrm_Module_Model::getInstance('Calendar');
		foreach ($calendarFieldsList as $fieldName) {
			$fieldModel = nectarcrm_Field_Model::getInstance($fieldName, $calendarInstance);
			if ($fieldModel) {
	...
	function getVisibleCalendarViewTypes($userId) {
		...
		$query = "SELECT * FROM nectarcrm_calendar_user_activitytypes 
			INNER JOIN nectarcrm_calendar_default_activitytypes on nectarcrm_calendar_default_activitytypes.id=nectarcrm_calendar_user_activitytypes.defaultid 
			WHERE nectarcrm_calendar_user_activitytypes.userid=? AND nectarcrm_calendar_user_activitytypes.visible=?";
	 ...
	 * @return <boolean>
	 */
	public function checkDuplicateView(nectarcrm_Request $request) {
		$currentUser = Users_Record_Model::getCurrentUserModel();
		...
		$queryResult = $db->pquery('SELECT id FROM nectarcrm_calendar_default_activitytypes WHERE module=? AND fieldname=? AND conditions=?', array($viewmodule, $viewfieldname,$conditions));
		if($db->num_rows($queryResult) > 0) {
			$defaultId = $db->query_result($queryResult, 0, 'id');

			$query = $db->pquery('SELECT 1 FROM nectarcrm_calendar_user_activitytypes WHERE defaultid=? AND userid=? AND visible=?', array($defaultId, $userId, '1'));
	 ...
	 */
	public function deleteCalendarView(nectarcrm_Request $request) {
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$userId = $currentUser->getId();
		$viewmodule = $request->get('viewmodule');
		$fieldName = $request->get('viewfieldname');
		...
		$db = PearDatabase::getInstance();
		$defaultIdQueryResult = $db->pquery('SELECT id FROM nectarcrm_calendar_default_activitytypes WHERE module=? AND fieldname=? AND isdefault=? AND conditions=?', array($viewmodule, $viewfieldname, 0, $viewconditions));
		if($db->num_rows($defaultIdQueryResult) > 0) {
			$defaultId = $db->query_result($defaultIdQueryResult, 0, 'id');
			$db->pquery('DELETE FROM nectarcrm_calendar_user_activitytypes WHERE defaultid=? AND nectarcrm_calendar_user_activitytypes.userid=?', array($defaultId, $userId));

			$queryResult = $db->pquery('SELECT 1 FROM nectarcrm_calendar_user_activitytypes WHERE defaultid=?', array($defaultId));
			if($db->num_rows($queryResult) <= 0) {
				$db->pquery('DELETE FROM nectarcrm_calendar_default_activitytypes WHERE module=? AND fieldname=? AND isdefault=? AND id=? AND conditions=?', array($viewmodule, $viewfieldname, 0, $defaultId, $viewconditions));
			}
		} else {
			$db->pquery('UPDATE nectarcrm_calendar_user_activitytypes 
						INNER JOIN nectarcrm_calendar_default_activitytypes ON nectarcrm_calendar_default_activitytypes.id = nectarcrm_calendar_user_activitytypes.defaultid
						SET nectarcrm_calendar_user_activitytypes.visible=? WHERE nectarcrm_calendar_user_activitytypes.userid=? AND nectarcrm_calendar_default_activitytypes.module=? AND nectarcrm_calendar_default_activitytypes.fieldname=? AND 
						nectarcrm_calendar_default_activitytypes.conditions=?', 
	 ...
	 * @return <string>
	 */
	public function addCalendarView(nectarcrm_Request $request) {
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$userId = $currentUser->getId();
		$viewmodule = $request->get('viewmodule');
		$fieldName = $request->get('viewfieldname');
		...
		$db = PearDatabase::getInstance();
		$queryResult = $db->pquery('SELECT id,isdefault FROM nectarcrm_calendar_default_activitytypes WHERE module=? AND fieldname=? AND conditions=?', array($viewmodule, $viewfieldname, $viewconditions));
		$type = '';
		if($db->num_rows($queryResult) > 0) {
			$defaultId = $db->query_result($queryResult, 0, 'id');
			...
			$query = $db->pquery('SELECT 1 FROM nectarcrm_calendar_user_activitytypes WHERE defaultid=? AND userid=?', array($defaultId, $userId));
			if($db->num_rows($query) > 0) {
				$db->pquery('UPDATE nectarcrm_calendar_user_activitytypes 
							INNER JOIN nectarcrm_calendar_default_activitytypes ON nectarcrm_calendar_default_activitytypes.id = nectarcrm_calendar_user_activitytypes.defaultid
							SET nectarcrm_calendar_user_activitytypes.color=?, nectarcrm_calendar_user_activitytypes.visible=? 
							WHERE nectarcrm_calendar_user_activitytypes.userid=? AND nectarcrm_calendar_default_activitytypes.module=? AND nectarcrm_calendar_default_activitytypes.fieldname=? 
							AND nectarcrm_calendar_default_activitytypes.conditions=?',
								array($viewcolor, '1', $userId, $viewmodule, $viewfieldname, $viewconditions));
			} else {
				$db->pquery('INSERT INTO nectarcrm_calendar_user_activitytypes (id, defaultid, userid, color) VALUES (?,?,?,?)', array($db->getUniqueID('nectarcrm_calendar_user_activitytypes'), $defaultId, $userId, $viewcolor));
			}
		} else {
			$defaultId = $db->getUniqueID('nectarcrm_calendar_default_activitytypes');
			$db->pquery('INSERT INTO nectarcrm_calendar_default_activitytypes (id, module, fieldname, defaultcolor, isdefault, conditions) VALUES (?,?,?,?,?,?)', array($defaultId, $viewmodule, $viewfieldname, $viewcolor, '0', $viewconditions));
	...
	public function deleteSharedUsers($currentUserId){
		$db = PearDatabase::getInstance();
		$delquery = "DELETE FROM nectarcrm_sharedcalendar WHERE userid=?";
	...
	public function getSharedType($currentUserId){
		...
		$query = "SELECT calendarsharedtype FROM nectarcrm_users WHERE id=?";
	...
	public function getRecentRecords($limit=10) {
		...
		if($nonAdminQuery){
			$query .= " INNER JOIN nectarcrm_activity ON nectarcrm_crmentity.crmid = nectarcrm_activity.activityid ".$nonAdminQuery;
		}
			...
			foreach($quickCreateFields as $fieldName => $fieldModel){
				...
				switch($fieldType){
					case "reference":	if(!empty($value)){
											$value = array("id"=>$value,"display_value"=>nectarcrm_Functions::getCRMRecordLabel($value),"module"=>nectarcrm_Functions::getCRMRecordType($value));

										}
										break;
					case "datetime":	$value = nectarcrm_Date_UIType::getDisplayDateValue($value);
										break;
				}
	...
	public static function getCalendarReminder() {
		...
		if($activityReminder != '' ) {
			...
			$reminderActivitiesResult = "SELECT reminderid, recordid FROM nectarcrm_activity_reminder_popup
								INNER JOIN nectarcrm_activity on nectarcrm_activity.activityid = nectarcrm_activity_reminder_popup.recordid
								INNER JOIN nectarcrm_crmentity ON nectarcrm_activity_reminder_popup.recordid = nectarcrm_crmentity.crmid
								WHERE nectarcrm_activity_reminder_popup.status = 0
								AND nectarcrm_crmentity.smownerid = ? AND nectarcrm_crmentity.deleted = 0
								AND ((DATE_FORMAT(nectarcrm_activity_reminder_popup.date_start,'%Y-%m-%d') <= ?)
								AND (TIME_FORMAT(nectarcrm_activity_reminder_popup.time_start,'%H:%i') <= ?))
								AND nectarcrm_activity.eventstatus <> 'Held' AND (nectarcrm_activity.status <> 'Completed' OR nectarcrm_activity.status IS NULL) LIMIT 20";
			...
			for($i=0; $i<$rows; $i++) {
				...
				$recordModels[] = nectarcrm_Record_Model::getInstanceById($recordId, 'Calendar');
			}
		}
		return $recordModels;
	}
	 ...
	 * @return <Array of nectarcrm_Field_Model> - list of field models
	 */
	...
	public function getSettingLinks() {
		...
		if($currentUserModel->isAdminUser()) {
			$settingLinks[] = array(
					...
					'linkicon' => nectarcrm_Theme::getImagePath('LayoutEditor.gif')
			);
	...
	*/
	...
	public function getModuleIcon($activityType = null) {
		...
		$moduleIcon = "<i class='vicon-$activityType' title='$title' ></i>";
		//$moduleIcon = "<span class='material-symbols-outlined'>task_alt</span>";
