 ...
 */
class Calendar_Field_Model extends nectarcrm_Field_Model {
	...
	*/
		...
		if ($recordInstance) {
			if ($this->getName() == 'date_start') {
				...
				if($currentUser->get('hour_format') == '12')
					$startTime = nectarcrm_Time_UIType::getTimeValueInAMorPM($startTime);
    ...
    public static function getReccurencePicklistValues() {
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$fieldModel = nectarcrm_Field_Model::getInstance('recurringtype', nectarcrm_Module_Model::getInstance('Events'));
		if($fieldModel->isRoleBased() && !$currentUser->isAdminUser()) {
			$userModel = Users_Record_Model::getCurrentUserModel();
			$picklistValues = nectarcrm_Util_Helper::getRoleBasedPicklistValues('recurringtype', $userModel->get('roleid'));
		}else{
			$picklistValues = nectarcrm_Util_Helper::getPickListValues('recurringtype');
		}
