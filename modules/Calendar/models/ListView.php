 ...
 */
class Calendar_ListView_Model extends nectarcrm_ListView_Model {
	...
	public function getAdvancedLinks(){
		...
		if($exportPermission) {
			$advancedLinks[] = array(
					...
					'linkurl' => 'javascript:nectarcrm_List_Js.triggerExportAction("'.$moduleModel->getExportUrl().'")',
					'linkicon' => ''
				);
		}
			...
			if(count($matches) > 0) {
				...
				if(!$fieldInstance) {
					if($moduleName == 'Calendar') {
						...
						$fieldInstance = nectarcrm_Field_Model::getInstance($fieldName,$eventmodule);
					}
				}
