...
class Appointment
{
	...
	function readAppointment($userid, &$from_datetime, &$to_datetime, $view)
	{
		...
		$and = "AND (
					(
						...
						AND nectarcrm_recurringevents.activityid is NULL
					)
				OR (
						(CAST(CONCAT(nectarcrm_recurringevents.recurringdate,' ',time_start) AS DATETIME) >= ?
							AND CAST(CONCAT(nectarcrm_recurringevents.recurringdate,' ',time_start) AS DATETIME) <= ?)
						OR	(CAST(CONCAT(due_date,' ',time_end) AS DATETIME) >= ? AND CAST(CONCAT(due_date,' ',time_end) AS DATETIME) <= ?)
						OR	(CAST(CONCAT(nectarcrm_recurringevents.recurringdate,' ',time_start) AS DATETIME) <= ?
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
        	...
        	$q= "select nectarcrm_activity.*, nectarcrm_crmentity.*,
					...
					FROM nectarcrm_activity
						...
						left join nectarcrm_recurringevents on nectarcrm_activity.activityid=nectarcrm_recurringevents.activityid
					...
					WHERE nectarcrm_crmentity.deleted = 0 and nectarcrm_activity.activitytype not in ('Emails','Task') $and ";
        ...
        $q .= " AND nectarcrm_recurringevents.activityid is NULL ";
        $q .= " group by nectarcrm_activity.activityid ORDER by nectarcrm_activity.date_start,nectarcrm_activity.time_start";
		...
		$q = "SELECT nectarcrm_activity.*, nectarcrm_crmentity.*, case when (nectarcrm_users.user_name not like '') then $userNameSql else nectarcrm_groups.groupname end as user_name , nectarcrm_recurringevents.recurringid, nectarcrm_recurringevents.recurringdate as date_start ,nectarcrm_recurringevents.recurringtype,nectarcrm_groups.groupname from nectarcrm_activity inner join nectarcrm_crmentity on nectarcrm_activity.activityid = nectarcrm_crmentity.crmid inner join nectarcrm_recurringevents on nectarcrm_activity.activityid=nectarcrm_recurringevents.activityid left join nectarcrm_groups on nectarcrm_groups.groupid = nectarcrm_crmentity.smownerid LEFT JOIN nectarcrm_users ON nectarcrm_users.id = nectarcrm_crmentity.smownerid";
		$q .= getNonAdminAccessControlQuery('Calendar',$current_user);
        $q.=" where nectarcrm_crmentity.deleted = 0 and nectarcrm_activity.activitytype not in ('Emails','Task') AND (cast(concat(recurringdate, ' ', time_start) as datetime) between ? and ?) ";
        ...
        $q .= " ORDER by nectarcrm_recurringevents.recurringid";
		$r = $adb->pquery($q, $params);
        $n = $adb->getRowCount($r);
        $a = 0;
		while ( $a < $n )
	...
	/** To read and set the events value in Appointment Obj
          * @param $act_array -- The nectarcrm_activity array :: Type Array
          * @param $view -- The calendar view :: Type String
         */
	function readResult($act_array, $view)
	{
		...
		if(!is_admin($current_user))
		{
			if($act_array["smownerid"]!=0 && $act_array["smownerid"] != $current_user->id && $act_array["visibility"] == "Public"){
				$que = "select * from nectarcrm_sharedcalendar where sharedid=? and userid=?";
...
/** To two array values
  * @param $a -- The nectarcrm_activity array :: Type Array
  * @param $b -- The nectarcrm_activity array :: Type Array
		...
		if($is_Admin == 'off' && $fldname != '')
			{
				...
				$sql="select * from nectarcrm_$fldname where $fldname=?";
				...
				if ($picklistvalueid != null) {
					$pick_query="select * from nectarcrm_role2picklist where picklistvalueid=$picklistvalueid and roleid in (". generateQuestionMarks($roleids) .")";
