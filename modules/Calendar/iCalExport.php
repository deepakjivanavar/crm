...
//$ical_query = "SELECT * FROM nectarcrm_activity WHERE (( STATUS != 'Completed' AND STATUS != 'Deferred' ) OR STATUS IS NULL)
//               AND (( eventstatus != 'Held' AND eventstatus != 'Not Held' ) OR eventstatus IS NULL )";

global $current_user,$adb,$default_timezone;
$filename = $_REQUEST['filename'];
$ical_query = "select nectarcrm_activity.*,nectarcrm_crmentity.description,nectarcrm_activity_reminder.reminder_time from nectarcrm_activity inner join nectarcrm_crmentity on nectarcrm_activity.activityid = nectarcrm_crmentity.crmid " .
	...
	" where nectarcrm_crmentity.deleted = 0 and nectarcrm_crmentity.smownerid = " . $current_user->id . 
