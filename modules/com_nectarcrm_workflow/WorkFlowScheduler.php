...
require_once ('modules/com_nectarcrm_workflow/WorkflowScheduler.inc');
				...
				do {
					...
					for ($j = 0; $j < $noOfRecords; ++$j) {
						...
						if($moduleName == 'Events') {
							nectarcrm_Functions::setEventsContactIdToRequest($recordId);
						}
						foreach ($tasks as $task) {
							if ($task->active) {
								$delay = 0;
        ...
        if ($noOfConditions > 0) {
            ...
            foreach ($conditions as $index => $condition) {
                ...
                if($fieldname == 'assigned_user_id') {
                    $userName = nectarcrm_Functions::getUserRecordLabel($value);
                    if(empty($userName)) {
                        $userName = nectarcrm_Functions::getGroupRecordLabel($value);
                    }
                    $value = $userName;
                }
				if($referenceField) {
