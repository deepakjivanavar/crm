...
class Documents_DetailView_Model extends nectarcrm_DetailView_Model {
        ...
        if($recordModel->get('filestatus') && $recordModel->get('filename')) {
            ...
            if($previewAvailable){
                $filePreviewLinkModel = array(
                            ...
                            'linkicon' => '',
                            'linkurl' => 'nectarcrm_Header_Js.previewFile(event,'.$recordModel->getId().')',
                            'filelocationtype' => $recordModel->get('filelocationtype'),
                            'filename' => $recordModel->get('filename'),
                    );
                $linkModelList['DETAILVIEWBASIC'][] = nectarcrm_Link_Model::getInstanceFromValues($filePreviewLinkModel);
            }
		}
