...
class Documents_Module_Model extends nectarcrm_Module_Model {
	...
	public static function getAllFolders() {
		...
		$result = $db->pquery('SELECT * FROM nectarcrm_attachmentsfolder ORDER BY sequence', array());
	...
	public function getQueryByModuleField($sourceModule, $field, $record, $listQuery) {
		if($sourceModule === 'Emails' && $field === 'composeEmail') {
			$condition = ' (( nectarcrm_notes.filelocationtype LIKE "%I%")) AND nectarcrm_notes.filename != "" AND nectarcrm_notes.filestatus = 1';
		} else {
			$condition = " nectarcrm_notes.notesid NOT IN (SELECT notesid FROM nectarcrm_senotesrel WHERE crmid = '$record') AND nectarcrm_notes.filestatus = 1";
		}
		...
		if($this->isEntityModule()) {
            ...
            if($createPermission) {
                $basicListViewLinks[] = array(
					...
					'linkurl' => 'javascript:nectarcrm_Header_Js.getQuickCreateFormForModule("index.php?module=Documents&view=EditAjax&type=I","Documents")',
					'linkicon' => ''
				);
                $basicListViewLinks[] = array(
					'linktype' => 'LISTVIEW',
					'linklabel' => 'LBL_EXTERNAL_DOCUMENT_TYPE',
					'linkurl' => 'javascript:nectarcrm_Header_Js.getQuickCreateFormForModule("index.php?module=Documents&view=EditAjax&type=E")',
					'linkicon' => ''
				);
                $basicListViewLinks[] = array(
					'linktype' => 'LISTVIEW',
					'linklabel' => 'LBL_WEBDOCUMENT_TYPE',
					'linkurl' => 'javascript:nectarcrm_Header_Js.getQuickCreateFormForModule("index.php?module=Documents&view=EditAjax&type=W")',
					'linkicon' => ''
				);
            }
