...
function getUserFldArray($fld_module,$roleid){
	...
	$query="select nectarcrm_field.fieldlabel,nectarcrm_field.columnname,nectarcrm_field.fieldname, nectarcrm_field.uitype" .
			" FROM nectarcrm_field inner join nectarcrm_picklist on nectarcrm_field.fieldname = nectarcrm_picklist.name" .
			" where (displaytype=1 and nectarcrm_field.tabid=? and nectarcrm_field.uitype in ('15','55','33','16') " .
			" or (nectarcrm_field.tabid=? and fieldname='salutationtype' and fieldname !='vendortype')) " .
...
function getPickListModules(){
	...
	$query = 'select distinct nectarcrm_field.fieldname,nectarcrm_field.tabid,nectarcrm_tab.tablabel, nectarcrm_tab.name as tabname,uitype from nectarcrm_field inner join nectarcrm_tab on nectarcrm_tab.tabid=nectarcrm_field.tabid where uitype IN (15,33) and nectarcrm_field.tabid != 29 and nectarcrm_tab.presence != 1 and nectarcrm_field.presence in (0,2) order by nectarcrm_field.tabid ASC';
	// END
...
function getrole2picklist(){
	global $adb;
	$query = "select rolename,roleid from nectarcrm_role where roleid not in('H1') order by roleid";
	$result = $adb->pquery($query, array());
	while($row = $adb->fetch_array($result)){
...
function getAllPickListValues($fieldName,$lang = Array() ){
	global $adb;
	if(nectarcrm_Cache::get('AllPicklistValues',$fieldName)){
		return nectarcrm_Cache::get('AllPicklistValues',$fieldName);
	}
	...
	if($fieldName == 'group_id'){
		...
		$sql = 'SELECT * FROM nectarcrm_'.$adb->sql_escape_string($fieldName);
	...
	nectarcrm_Cache::set('AllPicklistValues', $fieldName, $arr);
	return $arr;
}
...
function getEditablePicklistValues($fieldName, $lang= array(), $adb){
	...
	$sql="select $fieldName from nectarcrm_$fieldName where presence=1 and $fieldName <> '--None--'";
	$res = $adb->query($sql);
...
function getNonEditablePicklistValues($fieldName, $lang=array(), $adb){
	$values = array();
	$fieldName = $adb->sql_escape_string($fieldName);
	$sql = "select $fieldName from nectarcrm_$fieldName where presence=0";
	$result = $adb->query($sql);
	$count = $adb->num_rows($result);
	for($i=0;$i<$count;$i++){
...
function getAssignedPicklistValues($tableName, $roleid, $adb, $lang=array()){
	...
	if($cache->hasAssignedPicklistValues($tableName,$roleid)) {
		...
		$sql = "SELECT distinct ".$adb->sql_escape_string($tableName)." FROM ". $adb->sql_escape_string("nectarcrm_$tableName")
				. " inner join nectarcrm_role2picklist on ".$adb->sql_escape_string("nectarcrm_$tableName").".picklist_valueid=nectarcrm_role2picklist.picklistvalueid"
