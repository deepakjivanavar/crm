...
function Contacts_sendCustomerPortalLoginDetails($entityData){
	...
	if ($isPortalEnabled) {
		...
		$sql = "SELECT id, user_name, user_password, isactive FROM nectarcrm_portalinfo WHERE id=?";
		...
		if ($adb->num_rows($result)) {
			...
			if($email == $dbusername && $isactive == 1 && !$entityData->isNew()){
				...
				$sql = "UPDATE nectarcrm_portalinfo SET user_name=?, isactive=? WHERE id=?";
		...
		$sendEmail = false;
		if ($insert) {
			$sql = "INSERT INTO nectarcrm_portalinfo(id,user_name,user_password,cryptmode,type,isactive) VALUES(?,?,?,?,?,?)";
		...
		$sql = "UPDATE nectarcrm_portalinfo SET user_name=?,isactive=0 WHERE id=?";
