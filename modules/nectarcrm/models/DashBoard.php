...
class nectarcrm_DashBoard_Model extends nectarcrm_Base_Model {
	 ...
	 * @return <Array of nectarcrm_Widget_Model>
	 */
	public function getSelectableDashboard() {
		...
		$sql = 'SELECT * FROM nectarcrm_links WHERE linktype = ? AND tabid IN ('. generateQuestionMarks($moduleTabIdList) .') AND linkid NOT IN (SELECT linkid FROM nectarcrm_module_dashboard_widgets WHERE userid = ? and dashboardtabid=? )';
		...
		$sql .= ' UNION SELECT * FROM nectarcrm_links WHERE linklabel in (?,?)';
		$params[] = 'Mini List';
		$params[] = 'Notebook';
		...
		for($i=0; $i<$db->num_rows($result); $i++) {
			...
			if($this->checkModulePermission($row)) {
				$widgets[] = nectarcrm_Widget_Model::getInstanceFromValues($row);
			}
		}
	...
	public function getDashboards($moduleDashboard) {
		...
		$sql = "SELECT nectarcrm_links.*, nectarcrm_module_dashboard_widgets.userid, nectarcrm_module_dashboard_widgets.filterid, nectarcrm_module_dashboard_widgets.data, nectarcrm_module_dashboard_widgets.id as widgetid, nectarcrm_module_dashboard_widgets.position as position, nectarcrm_module_dashboard_widgets.size as size, nectarcrm_links.linkid as id FROM nectarcrm_links ".
				" INNER JOIN nectarcrm_module_dashboard_widgets ON nectarcrm_links.linkid=nectarcrm_module_dashboard_widgets.linkid".
				" WHERE nectarcrm_module_dashboard_widgets.userid = ? AND linktype = ? AND tabid = ?";
		...
		$sql = "SELECT reportid FROM nectarcrm_module_dashboard_widgets WHERE userid = ? AND linkid= ? AND reportid IS NOT NULL";
		...
		for($i=0, $len=$db->num_rows($result); $i<$len; $i++) {
			...
			if($moduleDashboard == 'Home' || $moduleDashboard == $chartReportModel->getPrimaryModule()){
				...
				if($tabId && $currentUserPrivilegeModel->hasModulePermission($tabId)) {
					$widgets[] = nectarcrm_Widget_Model::getInstanceWithReportId($row['reportid'], $currentUser->getId());
				}
			}
		}
		return $widgets;
	...
	public function getActiveTabs() {
		...
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$query = "SELECT id,tabname,sequence,isdefault,appname,modulename FROM nectarcrm_dashboard_tabs WHERE userid=? ORDER BY sequence ASC ";
	 ...
	 * Purpose : If user added a widget in nectarcrm6 then we need add that widget for first tab
	 * @param type $userId
	 * @return type
	 */
	public function getUserDefaultTab($userId){
		$db = PearDatabase::getInstance();
		$query = "SELECT id,tabname,sequence,isdefault FROM nectarcrm_dashboard_tabs WHERE userid=? AND isdefault =?";
	...
	public function addTab($tabName){
		...
		$result = $db->pquery("SELECT MAX(sequence)+1 AS sequence FROM nectarcrm_dashboard_tabs",array());
		$sequence = $db->query_result($result, 0,'sequence');

		$query = "INSERT INTO nectarcrm_dashboard_tabs(tabname, userid,sequence) VALUES(?,?,?)";
	...
	public function deleteTab($tabId) {
		$db = PearDatabase::getInstance();
		$query = "DELETE FROM nectarcrm_dashboard_tabs WHERE id=?";
	...
	public function renameTab($tabId, $tabName) {
		 $db = PearDatabase::getInstance();
		$query = "UPDATE nectarcrm_dashboard_tabs SET tabname=? WHERE id=?";
	...
	public function checkTabExist($tabName) {
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$query = "SELECT * FROM nectarcrm_dashboard_tabs WHERE tabname=? and userid=?";
	...
	public function checkTabsLimitExceeded() {
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$query = "SELECT count(*) AS count FROM nectarcrm_dashboard_tabs WHERE userid=?";
		$result = $db->pquery($query, array($currentUser->getId()));
	...
	public function updateTabSequence($sequence){
		...
		$query = "UPDATE nectarcrm_dashboard_tabs SET sequence = ? WHERE id=?";
	...
	public function getTabInfo($tabId) {
		...
		$query = "SELECT * FROM nectarcrm_dashboard_tabs WHERE id=? ";
		$params = array($tabId);
	 ...
	 * @return <nectarcrm_DashBoard_Model>
	 */
	public static function getInstance($moduleName) {
		$modelClassName = nectarcrm_Loader::getComponentClassName('Model', 'DashBoard', $moduleName);
	...
	public function checkModulePermission($resultData) {
		...
		return ($currentUserPrivilegeModel->hasModulePermission(getTabid($module)) && !nectarcrm_Runtime::isRestricted('modules', $module));
	}
