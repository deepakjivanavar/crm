...
class nectarcrm_Tag_Model extends nectarcrm_Base_Model {
	...
	public function create(){
		...
		$checkRes = $db->pquery("SELECT id FROM nectarcrm_freetags WHERE tag=?",array($this->getName()));
		if($db->num_rows($checkRes) > 0) {
			$id = $db->query_result($checkRes, 0, 'id');
		}else{
			$id = $db->getUniqueId('nectarcrm_freetags');
			$db->pquery("INSERT INTO nectarcrm_freetags values(?,?,?,?,?)", array($id, $tagName, $tagName, $visibility, $currentUser->getId()));
		}
		$this->set('id', $id);
		return $id;
	}
	...
	public function update() {
		$db = PearDatabase::getInstance();
		$query = "UPDATE nectarcrm_freetags SET tag=?, raw_tag=?, visibility=? WHERE id=?";
	...
	public function delete() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM nectarcrm_freetagged_objects WHERE tag_id = ? AND object_id = ?',
				array($this->get('tag_id'), $this->get('record')));
	}

	public function remove() {
		$db = PearDatabase::getInstance();
		$query = "DELETE FROM nectarcrm_freetags WHERE id=?";
	...
	public static function getAllUserTags($userId) {
		...
		$result = $db->pquery("SELECT * FROM nectarcrm_freetags WHERE owner=? OR visibility=?", array($userId, self::PUBLIC_TYPE));
	...
	public static function getAllAccessible($userId, $module = "", $recordId = null, $mode="") {
		...
		$query .= " FROM nectarcrm_freetags 
					INNER JOIN nectarcrm_freetagged_objects ON nectarcrm_freetags.id = nectarcrm_freetagged_objects.tag_id 
					WHERE (nectarcrm_freetagged_objects.tagger_id = ? OR nectarcrm_freetags.visibility='public') ";
		$params = array($userId);
		if(!empty($module)) {
			$query .=  ' AND nectarcrm_freetagged_objects.module=?';
			array_push($params, $module);
		}

		if(!empty($recordId)) {
			$query .= ' AND nectarcrm_freetagged_objects.object_id=?';
			array_push($params, $recordId);
		}
//        if($mode != "count"){
//            $query .= ' GROUP BY nectarcrm_freetags.id';
//        }
	...
	public static function getTaggedRecords($tagId) {
		...
		if(!empty($tagId)) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT nectarcrm_crmentity.* FROM nectarcrm_freetags 
				INNER JOIN nectarcrm_freetagged_objects ON nectarcrm_freetags.id = nectarcrm_freetagged_objects.tag_id 
				INNER JOIN nectarcrm_crmentity ON nectarcrm_freetagged_objects.object_id=nectarcrm_crmentity.crmid 
					AND nectarcrm_crmentity.deleted=0 
				WHERE tag_id = ?", array($tagId));
			$rows = $db->num_rows($result);
			for($i=0; $i<$rows; $i++) {
				$row = $db->query_result_rowdata($result, $i);
				$recordModel = nectarcrm_Record_Model::getCleanInstance($row['setype']);
	...
	public static function saveForRecord($recordId , $tagList , $userId='', $module='') {
		...
		if(empty($module)) {
			$module = nectarcrm_Functions::getCRMRecordType($recordId);
		}
		...
		foreach ($tagList as $tagId) {
			$saveQuery = "INSERT INTO nectarcrm_freetagged_objects VALUES(?,?,?,?,?) ON DUPLICATE KEY UPDATE tag_id=?";
	...
	public static function getInstanceById($tagId) {
		...
		$query = "SELECT * FROM nectarcrm_freetags WHERE id=?";
	...
	public static function getInstanceByName($name, $userId, $excludedTagId = false) {
		$db = PearDatabase::getInstance();
		$query = "SELECT * FROM nectarcrm_freetags WHERE (tag=? OR raw_tag=?) AND (owner=? OR visibility=?)";
	...
	public static function checkIfOtherUsersUsedTag($tagId, $userIdToExclude) {
		$db = PearDatabase::getInstance();
		$checkQuery = "SELECT 1 FROM nectarcrm_freetagged_objects 
						INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid=nectarcrm_freetagged_objects.object_id
						WHERE tag_id=? and tagger_id !=? AND nectarcrm_crmentity.deleted=0";
