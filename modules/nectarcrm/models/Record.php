...
/**
 * nectarcrm Entity Record Model Class
 */
class nectarcrm_Record_Model extends nectarcrm_Base_Model {
	...
	public function getName() {
		...
		return nectarcrm_Util_Helper::toSafeHTML(decode_html($displayName));
	}
	 ...
	 * @return nectarcrm_Record_Model instance
	 */
	...
	public function getDisplayValue($fieldName,$recordId = false) {
		...
		if($fieldName == "time_start" && $this->getModule()->getName() == "Emails"){
			...
			$value = nectarcrm_Time_UIType::getDisplayValue($dateTime->getDisplayTime());
	...
	/**
	 * Function returns the nectarcrm_Field_Model
	...
	/**
	 * Static Function to get the instance of a clean nectarcrm Record Model for the given module name
	 * @param <String> $moduleName
	 * @return nectarcrm_Record_Model or Module Specific Record Model instance
	 */
	...
	/**
	 * Static Function to get the instance of the nectarcrm Record Model given the recordid and the module name
	...
	public function deleteImage($imageId) {
		...
		$checkResult = $db->pquery('SELECT crmid FROM nectarcrm_seattachmentsrel WHERE attachmentsid = ?', array($imageId));
		$crmId = intval($db->query_result($checkResult, 0, 'crmid'));
		if (intval($this->getId()) === $crmId) {
			$db->pquery('DELETE FROM nectarcrm_seattachmentsrel WHERE crmid = ? AND attachmentsid = ?', array($crmId,$imageId));
	...
	public function getDescriptionValue() {
		...
		if(empty($description)) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT description FROM nectarcrm_crmentity WHERE crmid = ?", array($this->getId()));
	...
	public static function getInstancesFromIds($recordIds, $moduleName) {
		...
		if($moduleName == 'Users') {
			$query .= ' AND nectarcrm_users.id IN (' . generateQuestionMarks($recordIds) . ')';
		} else{
			$query .= ' AND nectarcrm_crmentity.crmid IN (' . generateQuestionMarks($recordIds) . ')';
		}
	...
	public function getFileDetails($attachmentId = false) {
		$db = PearDatabase::getInstance();
		$fileDetails = array();
		$query = "SELECT * FROM nectarcrm_attachments
				INNER JOIN nectarcrm_seattachmentsrel ON nectarcrm_seattachmentsrel.attachmentsid = nectarcrm_attachments.attachmentsid
				WHERE crmid = ? ";
		$params = array($this->get('id'));
		if($attachmentId) {
			$query .= 'AND nectarcrm_attachments.attachmentsid = ?';
			$params[] = $attachmentId;
		}
		$result = $db->pquery($query, $params);
	...
	public function getTitle($fieldInstance) {
		...
		if ($fieldInstance) {
			...
			if (in_array($dataType, array('reference', 'multireference'))) {
				$recordName = nectarcrm_Util_Helper::getRecordName($rawValue);
				if ($recordName) {
					return $recordName;
				} else {
					return '';
	...
	function getRollupCommentsForModule($startIndex = 0, $pageLimit = 10) {
		...
		if ($relatedModuleRecordIds) {
			...
			$listView = nectarcrm_ListView_Model::getInstance('ModComments');
			...
			$query .= " AND nectarcrm_modcomments.related_to IN (" . generateQuestionMarks($relatedModuleRecordIds)
					. ") AND nectarcrm_modcomments.parent_comments=0 ORDER BY nectarcrm_crmentity.createdtime DESC LIMIT "
	...
	function getCommentEnabledRelatedEntityIds($modulename, $recordId) {
		...
		foreach ($referenceFieldsModels as $referenceFieldsModel) {
			...
			if (!empty($relModuleFieldValue) && !in_array($relmoduleFieldname, $restrictedFieldnames) && isRecordExists($relModuleFieldValue)) {
				$relModuleRecordModel = nectarcrm_Record_Model::getInstanceById($relModuleFieldValue);
				$relmodule = $relModuleRecordModel->getModuleName();

				$relatedmoduleModel = nectarcrm_Module_Model::getInstance($relmodule);
		...
		$relatedModuleModels = nectarcrm_Relation_Model::getAllRelations($moduleModel, false);
		...
		$indirectrelatedModuleRecordIds = $moduleModel->getRelatedModuleRecordIds(new nectarcrm_Request($commentEnabledModules), array($recordId), true);
