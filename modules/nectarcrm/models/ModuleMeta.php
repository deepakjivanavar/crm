...
class nectarcrm_ModuleMeta_Model extends nectarcrm_Base_Model {
	...
	/**
	 * creates an instance of nectarcrm_ModuleMeta_Model
	 * @param <String> $name - module name
	 * @param <Object> $user - Users Object
	 * @return nectarcrm_ModuleMeta_Model
	 */
	public static function getInstance($name, $user) {
		$moduleMetaClassName = nectarcrm_Loader::getComponentClassName('Model', 'ModuleMeta', $name);
	 ...
	 * @return <Array of nectarcrm_Field>
	 */
		...
		if(method_exists($focus, 'getImportableFields')) {
			...
			foreach($moduleFields as $fieldName => $fieldInstance) {
				if(($this->isEditableField($fieldInstance)
							&& ($fieldInstance->getTableName() != 'nectarcrm_crmentity' || $fieldInstance->getColumnName() != 'modifiedby')
