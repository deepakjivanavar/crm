...
class nectarcrm_Import_View extends nectarcrm_Index_View {
	...
	//nectarcrm7
	function importLandingPage(nectarcrm_Request $request) {
	...
	function importBasicStep(nectarcrm_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		...
		$viewer->assign('IMPORT_UPLOAD_SIZE_MB', nectarcrm_Util_Helper::getMaxUploadSize());
		$viewer->assign('IMPORT_UPLOAD_SIZE', nectarcrm_Util_Helper::getMaxUploadSizeInBytes());

		if(in_array($moduleName, nectarcrm_Functions::getLineItemFieldModules())){
	...
	function uploadAndParse(nectarcrm_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		$duplicateHandlingNotSupportedModules = $this->getUnsupportedDuplicateHandlingModules();
		if(in_array($moduleName, $duplicateHandlingNotSupportedModules)){
	...
	function initializeMappingParameters(nectarcrm_Request $request) {
			...
			if(!$autoMerge) {
				...
				if (is_array($merge_fields)) {
					foreach ($merge_fields as $value) {
						$fieldInstance = nectarcrm_Field_Model::getInstance($value, $moduleModel);
	...
	function import(nectarcrm_Request $request) {
		$user = Users_Record_Model::getCurrentUserModel();
		Import_Main_View::import($request, $user);
	}

	function undoImport(nectarcrm_Request $request) {
		...
		$entity->triggerEvent('nectarcrm.batchevent.delete',$entityData);
		$NECTARCRM_BULK_SAVE_MODE = $previousBulkSaveMode;
		$viewer->assign('FOR_MODULE', $moduleName);
		$viewer->assign('MODULE', 'Import');
	...
	function lastImportedRecords(nectarcrm_Request $request) {
		$importList = new Import_List_View();
		$importList->process($request);
	}

	function deleteMap(nectarcrm_Request $request) {
		Import_Main_View::deleteMap($request);
	}

	//TODO need to move it to an action
	function clearCorruptedData(nectarcrm_Request $request) {
		$user = Users_Record_Model::getCurrentUserModel();
		Import_Utils_Helper::clearUserImportInfo($user);
		$this->importBasicStep($request);
	}

	function cancelImport(nectarcrm_Request $request) {
	...
	function checkImportStatus(nectarcrm_Request $request) {
		$moduleName = $request->getModule();
		$user = Users_Record_Model::getCurrentUserModel();
		$mode = $request->getMode();
	...
	public function updateSavedMapping(nectarcrm_Request $request) {
