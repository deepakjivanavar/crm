...
class nectarcrm_ComposeEmail_View extends nectarcrm_Footer_View {
	...
	public function composeMailData($request){
		...
		$viewer->assign('MAX_UPLOAD_SIZE', nectarcrm_Util_Helper::getMaxUploadSizeInBytes());
		...
		if ($documentIds) {
			...
			foreach ($documentIds as $documentId) {
				$documentRecordModel = nectarcrm_Record_Model::getInstanceById($documentId, $sourceModule);
		...
		if ($sourceRecordId) {
			$sourceRecordModel = nectarcrm_Record_Model::getInstanceById($sourceRecordId);
		...
		if (!empty($selectedFields)) {
			if($request->get('emailSource') == 'ListView') {
				...
				foreach($selectIds as $recordId) {
					$recordModel = nectarcrm_Record_Model::getInstanceById($recordId, $sourceModule);
					if($recordModel) {
						...
						foreach($selectedFields as $selectedFieldJson) {
							...
							if(!empty($selectedFieldInfo['basefield'])) {
								...
								if(!empty($refFieldValue)) {
									try {
										$refRecordModel = nectarcrm_Record_Model::getInstanceById($refFieldValue, $refModule);
			...
			foreach($selectIds as $id) { 
				  if ($id) { 
						...
						if (count($parentIdComponents) > 1) { 
								...
								$recordModel = nectarcrm_Record_Model::getInstanceById($id, $fieldModule); 
						} else { 
								$recordModel = nectarcrm_Record_Model::getInstanceById($id); 
						} 
						if($selectedFields){ 
		...
		$documentsModel = nectarcrm_Module_Model::getInstance('Documents');
		$documentsURL = $documentsModel->getInternalDocumentsURL();

		$emailTemplateModuleModel = nectarcrm_Module_Model::getInstance('EmailTemplates');
		...
		$viewer->assign('MODULE_IS_ACTIVE', $userPrevilegesModel->hasModulePermission(nectarcrm_Module_Model::getInstance('EmailTemplates')->getId()));
		//
	...
	function getHeaderScripts(nectarcrm_Request $request) {
		...
		$jsHeaderScriptNames = array(
			'layouts.vlayout.modules.nectarcrm.resources.Header',
			'layouts.vlayout.modules.nectarcrm.resources.BasicSearch'
		);
...
//var_dump($this->record);
		...
		$recordModel = $this->record->getRecord();
//var_dump(nectarcrm_DetailView_Model::getInstance($moduleName, $recordId));
		$viewer = $this->getViewer($request);
	...
	function emailEdit($request){
		...
		foreach($parentIds as $index=>$parentFieldId) {
			...
			if ($parentIdComponents[1] === '-1') {
				...
				$parentRecordModel = nectarcrm_Record_Model::getInstanceById($parentId);
			}
			...
			foreach ($referenceFields as $fieldName) {
				...
				$refFieldModlule = nectarcrm_Functions::getCRMRecordType($refRecordId);
				if(!$refFieldModlule) continue;
				if(!isRecordExists($refRecordId)) {
					continue;
				}
				$refRecordModel = nectarcrm_Record_Model::getInstanceById($refRecordId);
