...
class nectarcrm_ListAjax_View extends nectarcrm_List_View {
	...
	public function ShowListColumnsEdit(nectarcrm_Request $request){
		...
		$moduleModel = nectarcrm_Module_Model::getInstance($request->get('source_module'));
		$recordStructureModel = nectarcrm_RecordStructure_Model::getInstanceForModule($moduleModel, nectarcrm_RecordStructure_Model::RECORD_STRUCTURE_MODE_FILTER);
	...
	public function searchAll(nectarcrm_Request $request) {
		$moduleName = $request->getModule();
		...
		$searchableModules = nectarcrm_Module_Model::getSearchableModules();
		$matchingRecords = array();
		foreach ($searchableModules as $searchModule => $searchModuleModel) {
			$searchedRecords = nectarcrm_Record_Model::getSearchResult($searchValue, $searchModule);
		...
		foreach ($matchingRecords as $module => $recordModelsList) {
			...
			$listViewModel = nectarcrm_ListView_Model::getInstance($module, $cvId);
			...
			foreach ($recordModelsList as $recordId) {
				$recordModel = nectarcrm_Record_Model::getInstanceById($recordId, $listViewModel->getModule());
	...
	public function showSearchResultsWithValue(nectarcrm_Request $request) {
		$moduleName = $request->getModule();
