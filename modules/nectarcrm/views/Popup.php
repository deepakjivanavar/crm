...
class nectarcrm_Popup_View extends nectarcrm_Footer_View {
	 ...
	 * @return <String>
	 */
	function getModule(nectarcrm_request $request) {
		$moduleName = $request->getModule();
		return $moduleName;
	}
	...
	public function initializeListViewContents(nectarcrm_Request $request, nectarcrm_Viewer $viewer) {
		...
		$isRecordExists = nectarcrm_Util_Helper::checkRecordExistance($relatedParentId);
		...
		if(!empty($relatedParentModule) && !empty($relatedParentId)) {
			$parentRecordModel = nectarcrm_Record_Model::getInstanceById($relatedParentId, $relatedParentModule);
			$listViewModel = nectarcrm_RelationListView_Model::getInstance($parentRecordModel, $moduleName, $label,$relationId);
			$searchModuleModel = $listViewModel->getRelatedModuleModel();
		}else{
			$listViewModel = nectarcrm_ListView_Model::getInstanceForPopup($moduleName);
	...
	function getListViewCount(nectarcrm_Request $request){
		...
		if(!empty($relatedParentModule) && !empty($relatedParentId)) {
			$parentRecordModel = nectarcrm_Record_Model::getInstanceById($relatedParentId, $relatedParentModule);
			$listViewModel = nectarcrm_RelationListView_Model::getInstance($parentRecordModel, $moduleName, $label);
		}else{
			$listViewModel = nectarcrm_ListView_Model::getInstanceForPopup($moduleName);
		}
