...
class ModCommentsCore extends CRMEntity {
	...
	var $table_name = 'nectarcrm_modcomments';
	var $table_index= 'modcommentsid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('nectarcrm_modcommentscf', 'modcommentsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('nectarcrm_crmentity', 'nectarcrm_modcomments', 'nectarcrm_modcommentscf');
	...
	var $tab_name_index = Array(
		...
		'nectarcrm_modcomments' => 'modcommentsid',
		'nectarcrm_modcommentscf'=>'modcommentsid');
	...
	function generateReportsSecQuery($module,$secmodule,$queryplanner){
		...
		$matrix->setDependency('nectarcrm_crmentityModComments',array('nectarcrm_groupsModComments','nectarcrm_usersModComments', 'nectarcrm_contactdetailsRelModComments', 'nectarcrm_modcommentsRelModComments'));

		if (!$queryplanner->requireTable("nectarcrm_modcomments",$matrix)){
			return '';
		}
		$matrix->setDependency('nectarcrm_modcomments', array('nectarcrm_crmentityModComments'));

		$query = $this->getRelationQuery($module,$secmodule,"nectarcrm_modcomments","modcommentsid", $queryplanner);

		if ($queryplanner->requireTable("nectarcrm_crmentityModComments",$matrix)){
			$query .= " left join nectarcrm_crmentity as nectarcrm_crmentityModComments on nectarcrm_crmentityModComments.crmid=nectarcrm_modcomments.modcommentsid and nectarcrm_crmentityModComments.deleted=0";
		}
		if ($queryplanner->requireTable("nectarcrm_groupsModComments")){
			$query .= " left join nectarcrm_groups nectarcrm_groupsModComments on nectarcrm_groupsModComments.groupid = nectarcrm_crmentityModComments.smownerid";
		}
		if ($queryplanner->requireTable("nectarcrm_usersModComments")){
			$query .= " left join nectarcrm_users as nectarcrm_usersModComments on nectarcrm_usersModComments.id = nectarcrm_crmentityModComments.smownerid";
		}
		if ($queryplanner->requireTable("nectarcrm_contactdetailsRelModComments")){
			$query .= " left join nectarcrm_contactdetails as nectarcrm_contactdetailsRelModComments on nectarcrm_contactdetailsRelModComments.contactid = nectarcrm_crmentityModComments.crmid";
		}
		if ($queryplanner->requireTable("nectarcrm_modcommentsRelModComments")){
			$query .= " left join nectarcrm_modcomments as nectarcrm_modcommentsRelModComments on nectarcrm_modcommentsRelModComments.modcommentsid = nectarcrm_crmentityModComments.crmid";
		}
