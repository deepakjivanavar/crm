...
class ModComments_Module_Model extends nectarcrm_Module_Model{
	...
	static function storeRollupSettingsForUser($currentUserModel, $request) {
		...
		if (!$rollupid) {
			...
			$query = "INSERT INTO nectarcrm_rollupcomments_settings(userid, tabid, rollup_status)"
			...
			$query = "UPDATE nectarcrm_rollupcomments_settings set rollup_status=?"
	...
	static function getRollupSettingsForUser($currentUserModel, $modulename) {
		...
		$query = 'SELECT rollupid, rollup_status FROM nectarcrm_rollupcomments_settings WHERE userid=? AND tabid=?';
