 ...
 */
class ModComments_Record_Model extends nectarcrm_Record_Model {
	...
	public function getImagePath() {
		...
		if($commentor) {
			...
			if (!empty($customer)) {
				$recordModel = nectarcrm_Record_Model::getInstanceById($customer);
	...
	public static function getInstanceById($record) {
		...
		$result = $db->pquery('SELECT nectarcrm_modcomments.*, nectarcrm_crmentity.smownerid,
					nectarcrm_crmentity.createdtime, nectarcrm_crmentity.modifiedtime FROM nectarcrm_modcomments
					INNER JOIN nectarcrm_crmentity ON nectarcrm_modcomments.modcommentsid = nectarcrm_crmentity.crmid
	...
	public function getParentRecordModel() {
		...
		return nectarcrm_Record_Model::getInstanceById($parentRecordId);
	...
	public function getCommentedByModel() {
		...
		if(!empty($customer)) {
			try {
				return nectarcrm_Record_Model::getInstanceById($customer, 'Contacts');
			} catch(Exception $e) {
				return false;
			}
		} else {
			...
			if($commentedBy) {
				$commentedByModel = nectarcrm_Record_Model::getInstanceById($commentedBy, 'Users');
				if(empty($commentedByModel->entity->column_fields['user_name'])) {
				$activeAdmin = Users::getActiveAdminUser();
				$commentedByModel = nectarcrm_Record_Model::getInstanceById($activeAdmin->id, 'Users');
				}
	 ...
	 * @param <nectarcrm_Paging_Model> - paging model
	...
	public static function getRecentComments($parentRecordId, $pagingModel = false){
		...
		$query = $query ." AND related_to = ? ORDER BY nectarcrm_crmentity.createdtime DESC";
	...
	public static function getAllParentComments($parentId) {
			...
			if($query) {
				$query .= " AND related_to = ? AND parent_comments = ? ORDER BY nectarcrm_crmentity.createdtime DESC";
	...
	public function getChildCommentsCount() {
		...
		$query = 'SELECT 1 FROM nectarcrm_modcomments WHERE parent_comments = ? AND related_to = ?';
	...
	public function getCommentedByName() {
		...
		if($customer && !empty($customer)) {
			$label = nectarcrm_Util_Helper::getRecordName($customer);
			if(!$label)
				$label = $customerEmail;
			return $label;
	...
	static function getAllComments($parentId) {
		...
		$query = $focus->get_comments();
		if($query) {
			$query .= " AND related_to = ? ORDER BY nectarcrm_crmentity.createdtime DESC";
			$result = $db->pquery($query, array($parentId));
			$count = $db->num_rows($result);
			for($i = 0; $i < $count; $i++) {
				$rowData = $db->query_result_rowdata($result, $i);
	...
	function uploadAndSaveFile($emailId,$attachmentId) {
		$db = PearDatabase::getInstance();
		$db->pquery('INSERT INTO nectarcrm_seattachmentsrel(crmid,attachmentsid) VALUES(?,?)',array($emailId,$attachmentId));
	}
	...
	public function getParsedContent(){
		...
		$htmlParser = new nectarcrm_MailParser($this->getName());
