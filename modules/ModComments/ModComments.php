...
class ModComments extends ModCommentsCore {
	...
	static function transferRecords($currentParentId, $targetParentId) {
		global $adb;
		$adb->pquery("UPDATE nectarcrm_modcomments SET related_to=? WHERE related_to=?", array($targetParentId, $currentParentId));
	}
	...
	static function addWidgetTo($moduleNames, $widgetType='DETAILVIEWWIDGET', $widgetName='DetailViewBlockCommentWidget') {
		...
		$modCommentsModule = nectarcrm_Module::getInstance('ModComments');
		
		$commentWidgetModules = array();
		foreach($moduleNames as $moduleName) {
			$module = nectarcrm_Module::getInstance($moduleName);
			if($module) {
		...
		if (count($commentWidgetModules) > 0) {
			...
			$modCommentsRelatedToField = nectarcrm_Field::getInstance('related_to', $modCommentsModule);
	...
	public static function copyCommentsToRelatedRecord($currentParentId, $targetParentId) {
		...
		$relatedIdMap = array();
		$result = $db->pquery("SELECT *FROM nectarcrm_modcomments WHERE related_to=?", array($currentParentId));
		...
		for($i=0;$i<$count;$i++) {
			...
			$crmEntityResult = $db->pquery('SELECT *FROM nectarcrm_crmentity where crmid = ?', array($commentId));
			...
			$commentCrmId = $db->getUniqueID('nectarcrm_crmentity');
			...
			$db->pquery('INSERT INTO nectarcrm_crmentity values('. generateQuestionMarks($crmentityParams) .')', $crmentityParams);
			...
			$db->pquery('INSERT INTO nectarcrm_modcomments values('. generateQuestionMarks($modcommentsParams) .')', $modcommentsParams);
