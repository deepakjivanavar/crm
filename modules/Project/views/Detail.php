...
class Project_Detail_View extends nectarcrm_Detail_View {
	...
	function showRelatedRecords(nectarcrm_Request $request) {
		...
		$moduleName = $request->getModule();
		$relatedModuleInstance = nectarcrm_Module_Model::getInstance($relatedModuleName);
		...
		if($relatedModuleName == 'ProjectTask') {
			$fieldModel = nectarcrm_Field_Model::getInstance('projecttaskstatus', $relatedModuleInstance);
			if($fieldModel && $fieldModel->isViewableInDetailView()) {
				$header['projecttaskstatus'] = $relatedModuleModel->getField('projecttaskstatus');
			}
			$fieldModel = nectarcrm_Field_Model::getInstance('projecttaskprogress', $relatedModuleInstance);
			if($fieldModel && $fieldModel->isViewableInDetailView()) {
	 ...
	 * @param nectarcrm_Request $request
	 */
	public function showChart(nectarcrm_Request $request) {
		$parentId = $request->get('record');
		$projectTasks = array();
		$moduleName = $request->getModule();
		$currentUserModel = Users_Record_Model::getCurrentUserModel();
		$parentRecordModel = nectarcrm_Record_Model::getInstanceById($parentId, $moduleName);
		$projectTaskModel = nectarcrm_Module_Model::getInstance('ProjectTask');
		...
		$viewer->assign('TASK_STATUS', nectarcrm_Util_Helper::getRoleBasedPicklistValues('projecttaskstatus', $currentUserModel->get('roleid')));
		...
		$viewer->assign('STATUS_FIELD_MODEL', nectarcrm_Field_Model::getInstance('projecttaskstatus', $projectTaskModel));
