...
class Rss_Record_Model extends nectarcrm_Record_Model {
	...
	public function save($url) {
        ...
        $title = $this->getName();
        $id = $db->getUniqueID("nectarcrm_rss");
        ...
        $sql = "INSERT INTO nectarcrm_rss (rssid,rssurl,rsstitle) values (?,?,?)";
    ...
    public function delete() {
        ...
        $sql = 'DELETE FROM nectarcrm_rss where rssid = ?';
    ...
    public function makeDefault() {
        ...
        $sql = 'UPDATE nectarcrm_rss set starred = 0';
        $db->pquery($sql,array());
        
        $sql = 'UPDATE nectarcrm_rss set starred = 1 where rssid = ?';
        $db->pquery($sql,array($recordId));
    }
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM nectarcrm_rss WHERE rssid = ?', array($recordId));
    ...
    public function getDefaultRss() {
        ...
        $result = $db->pquery('SELECT rssid FROM nectarcrm_rss where starred = 1', array());
        $recordId = $db->query_result($result,'0','rssid');
        if($recordId) {
            $this->setId($recordId);
        } else {
            $result = $db->pquery('SELECT rssid FROM nectarcrm_rss', array());
            $recordId = $db->query_result($result,'0','rssid');
            $this->setId($recordId);
        }
    }
