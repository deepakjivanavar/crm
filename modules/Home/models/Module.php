...
class Home_Module_Model extends nectarcrm_Module_Model {
	...
	public function getComments($pagingModel, $user, $dateFilter='') {
		...
		$sql = 'SELECT nectarcrm_modcomments.*,nectarcrm_crmentity.setype AS setype,nectarcrm_crmentity.createdtime AS createdtime, nectarcrm_crmentity.smownerid AS smownerid,
				crmentity2.crmid AS parentId, crmentity2.setype AS parentModule FROM nectarcrm_modcomments
		...
		$sql .= ' ORDER BY nectarcrm_crmentity.crmid DESC LIMIT ?, ?';
	...
	function getCalendarActivities($mode, $pagingModel, $user) {
		...
		$query = "SELECT nectarcrm_crmentity.crmid, nectarcrm_crmentity.smownerid, nectarcrm_crmentity.setype, nectarcrm_activity.* FROM nectarcrm_activity
