...
class Migration_Index_View extends nectarcrm_View_Controller {
	...
	protected function step1(nectarcrm_Request $request) {
	...
	protected function step2(nectarcrm_Request $request){
		$moduleName = $request->getModule();
		$viewer = $this->getViewer($request);
	...
	public function applyDBChanges(){
		...
		//update nectarcrm version in db
		$migrationModuleModel->updatenectarcrmVersion();
	...
	public static function insertAdvFilter($queryid, $filters) {
		if ($queryid != "") {
			...
			self::ExecuteQuery('INSERT INTO nectarcrm_relcriteria_grouping VALUES(?,?,?,?)', array(1, $queryid, '', $conditionExpression));
		}
	}
