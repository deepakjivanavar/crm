...
include_once 'modules/com_nectarcrm_workflow/VTTaskManager.inc';
include_once 'include/utils/utils.php';

if(defined('NECTARCRM_UPGRADE')) {
        //Collating all module package updates here
	updateVtlibModule('Import', 'packages/nectarcrm/mandatory/Import.zip');
        updateVtlibModule('PBXManager', 'packages/nectarcrm/mandatory/PBXManager.zip');
	...
	updateVtlibModule('WSAPP', 'packages/nectarcrm/mandatory/WSAPP.zip');
        updateVtlibModule('Arabic_ar_ae', 'packages/nectarcrm/optional/Arabic_ar_ae.zip');
        updateVtlibModule('Assets', 'packages/nectarcrm/optional/Assets.zip');
        updateVtlibModule('EmailTemplates', 'packages/nectarcrm/optional/EmailTemplates.zip');
	...
	updateVtlibModule('SMSNotifier', "packages/nectarcrm/optional/SMSNotifier.zip");
        updateVtlibModule("Sweden_sv_se","packages/nectarcrm/optional/Sweden_sv_se.zip");
	updateVtlibModule("Webforms","packages/nectarcrm/optional/Webforms.zip");
        installVtlibModule('ExtensionStore', 'packages/nectarcrm/marketplace/ExtensionStore.zip');
}
...
$unWanted=array(
 ...
 "modules/com_nectarcrm_workflow/resources/functional.js",
 "modules/com_nectarcrm_workflow/resources/parallelexecuter.js",
 "modules/com_nectarcrm_workflow/resources/editworkflowscript.js",
 "modules/com_nectarcrm_workflow/resources/createtodotaskscript.js",
 "modules/com_nectarcrm_workflow/resources/fieldexpressionpopup.js",
 "modules/com_nectarcrm_workflow/resources/workflowlistscript.js",
 "modules/com_nectarcrm_workflow/resources/fieldvalidator.js",
 "modules/com_nectarcrm_workflow/resources/updatefieldstaskscript.js",
 "modules/com_nectarcrm_workflow/resources/jquery-1.2.6.js",
 "modules/com_nectarcrm_workflow/resources/json2.js",
 "modules/com_nectarcrm_workflow/resources/nectarcrmwebservices.js",
 "modules/com_nectarcrm_workflow/resources/jquery.timepicker.js",
 "modules/com_nectarcrm_workflow/resources/createentitytaskscript.js",
 "modules/com_nectarcrm_workflow/resources/edittaskscript.js",
 "modules/com_nectarcrm_workflow/resources/createeventtaskscript.js",
 "modules/com_nectarcrm_workflow/resources/emailtaskscript.js",
 "modules/FieldFormulas/editexpressionscript.js",              
 "modules/FieldFormulas/jquery-1.2.6.js",
 "modules/FieldFormulas/json2.js",
 "modules/FieldFormulas/nectarcrmwebservices.js",
...
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_nectarcrm_workflows ADD schtypeid INT(10)", array());
}
$result = $adb->pquery("show columns from com_nectarcrm_workflows like ?", array('schtime'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_nectarcrm_workflows ADD schtime TIME", array());
}
$result = $adb->pquery("show columns from com_nectarcrm_workflows like ?", array('schdayofmonth'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_nectarcrm_workflows ADD schdayofmonth VARCHAR(100)", array());
}
$result = $adb->pquery("show columns from com_nectarcrm_workflows like ?", array('schdayofweek'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_nectarcrm_workflows ADD schdayofweek VARCHAR(100)", array());
}
$result = $adb->pquery("show columns from com_nectarcrm_workflows like ?", array('schannualdates'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_nectarcrm_workflows ADD schannualdates VARCHAR(100)", array());
}
$result = $adb->pquery("show columns from com_nectarcrm_workflows like ?", array('nexttrigger_time'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_nectarcrm_workflows ADD nexttrigger_time DATETIME", array());
}
...
//73 starts
$query = 'SELECT 1 FROM nectarcrm_currencies WHERE currency_name=?';
$result = $adb->pquery($query, array('Sudanese Pound'));
if($adb->num_rows($result) <= 0){
    //Inserting Currency Sudanese Pound to nectarcrm_currencies
    Migration_Index_View::ExecuteQuery('INSERT INTO nectarcrm_currencies (currencyid,currency_name,currency_code,currency_symbol) VALUES ('.$adb->getUniqueID("nectarcrm_currencies").',"Sudanese Pound","SDG","£")',array());
    nectarcrm_Utils::AddColumn('nectarcrm_mailmanager_mailattachments', 'cid', 'VARCHAR(100)');
}
//73 ends
...
$adb->query("CREATE TABLE IF NOT EXISTS nectarcrm_feedback (userid INT(19), dontshow VARCHAR(19) default false);");

//75 ends

//76 starts
$moduleInstance = nectarcrm_Module::getInstance('Calendar');
$fieldInstance = nectarcrm_Field::getInstance('activitytype',$moduleInstance);

$fieldInstance->setPicklistValues(array('Mobile Call'));
//76 ends

//77 starts
$sql = "ALTER TABLE nectarcrm_products MODIFY productname VARCHAR( 100 )";
Migration_Index_View::ExecuteQuery($sql,array());
echo "<br>Updated to varchar(100) for productname";

$result = $adb->pquery('SELECT 1 FROM nectarcrm_currencies WHERE currency_name = ?', array('CFA Franc BCEAO'));
    if(!$adb->num_rows($result)) {
        Migration_Index_View::ExecuteQuery('INSERT INTO nectarcrm_currencies (currencyid, currency_name, currency_code, currency_symbol) VALUES(?, ?, ?, ?)',
            array($adb->getUniqueID('nectarcrm_currencies'), 'CFA Franc BCEAO', 'XOF', 'CFA'));
    }
$result = $adb->pquery('SELECT 1 FROM nectarcrm_currencies WHERE currency_name = ?', array('CFA Franc BEAC'));
    if(!$adb->num_rows($result)) {
        Migration_Index_View::ExecuteQuery('INSERT INTO nectarcrm_currencies (currencyid, currency_name, currency_code, currency_symbol) VALUES(?, ?, ?, ?)',
            array($adb->getUniqueID('nectarcrm_currencies'), 'CFA Franc BEAC', 'XAF', 'CFA'));
    }    
echo "<br>Added CFA Franc BCEAO and CFA Franc BEAC currencies";

$sql = "ALTER TABLE nectarcrm_loginhistory MODIFY user_name VARCHAR( 255 )";
Migration_Index_View::ExecuteQuery($sql,array());

$sql = "UPDATE nectarcrm_activitytype SET presence = '0' WHERE activitytype ='Mobile Call'";
Migration_Index_View::ExecuteQuery($sql,array());
...
//79 starts
Migration_Index_View::ExecuteQuery("CREATE TABLE IF NOT EXISTS nectarcrm_shareduserinfo
						(userid INT(19) NOT NULL default 0, shareduserid INT(19) NOT NULL default 0,
						color VARCHAR(50), visible INT(19) default 1);", array());

Migration_Index_View::ExecuteQuery('ALTER TABLE nectarcrm_mailscanner_rules ADD assigned_to INT(10), ADD cc VARCHAR(255), ADD bcc VARCHAR(255)', array());
$assignedToId = Users::getActiveAdminId();
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_mailscanner_rules SET assigned_to=?", array($assignedToId));
echo "<br> Adding assigned to, cc, bcc fields for mail scanner rules";


//Schema changes for nectarcrm_troubletickets hours & days column
Migration_Index_View::ExecuteQuery('ALTER TABLE nectarcrm_troubletickets MODIFY hours decimal(25,8)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE nectarcrm_troubletickets MODIFY days decimal(25,8)', array());

Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET defaultvalue=? WHERE tablename=? and fieldname=?", array('1', 'nectarcrm_pricebook', 'active'));
echo "<br> updated default value for pricebooks active";

$relationId = $adb->getUniqueID('nectarcrm_relatedlists');
$contactTabId = getTabid('Contacts');
$vendorTabId = getTabId('Vendors');
$actions = 'SELECT';

$query = 'SELECT max(sequence) as maxsequence FROM nectarcrm_relatedlists where tabid = ?';
$result = $adb->pquery($query, array($contactTabId));
$sequence = $adb->query_result($result, 0 ,'maxsequence');

$query = 'INSERT INTO nectarcrm_relatedlists VALUES(?,?,?,?,?,?,?,?,?,?,?)';
...
//Schema changes for nectarcrm_troubletickets hours & days column
Migration_Index_View::ExecuteQuery('UPDATE nectarcrm_field set typeofdata=? WHERE fieldname IN(?,?) AND tablename = ?', array('N~O', 'hours', 'days', 'nectarcrm_troubletickets'));

//79 ends

//80 starts
//Added recurring enddate column for events,to nectarcrm_recurringevents table
Migration_Index_View::ExecuteQuery('ALTER TABLE nectarcrm_recurringevents ADD COLUMN recurringenddate date', array());
echo "added field recurring enddate to nectarcrm_recurringevents to save untill date of repeat events";
...
//82 starts
Migration_Index_View::ExecuteQuery("ALTER TABLE nectarcrm_mailscanner CHANGE timezone time_zone VARCHAR(10)", array());
...
//84 starts
$query = "ALTER table nectarcrm_relcriteria modify comparator varchar(20)";
Migration_Index_View::ExecuteQuery($query, array());

//To copy imagename saved in nectarcrm_attachments for products and contacts into respectively base table
//to support filters on imagename field
$productIdSql = 'SELECT productid,name FROM nectarcrm_seattachmentsrel INNER JOIN nectarcrm_attachments ON
                                        nectarcrm_seattachmentsrel.attachmentsid = nectarcrm_attachments.attachmentsid INNER JOIN nectarcrm_products ON
                                        nectarcrm_products.productid = nectarcrm_seattachmentsrel.crmid';
...
foreach ($productImageMap as $productId => $imageNames) {
        ...
        Migration_Index_View::ExecuteQuery('UPDATE nectarcrm_products SET imagename = ? WHERE productid = ?',array($implodedNames,$productId));
}
echo 'updating image information for products table is completed';

$ContactIdSql = 'SELECT contactid,name FROM nectarcrm_seattachmentsrel INNER JOIN nectarcrm_attachments ON
                                        nectarcrm_seattachmentsrel.attachmentsid = nectarcrm_attachments.attachmentsid INNER JOIN nectarcrm_contactdetails ON
                                        nectarcrm_contactdetails.contactid = nectarcrm_seattachmentsrel.crmid';
...
for ($i = 0; $i < $numOfRows; $i++) {
        ...
        Migration_Index_View::ExecuteQuery('UPDATE nectarcrm_contactdetails SET imagename = ? WHERE contactid = ?',array($imageName,$contactId));
}
...
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_relatedlists SET actions=? WHERE label=? and tabid=? ",array('ADD,SELECT', 'PriceBooks', $productsTabId));
echo '<br>Updated PriceBooks related list actions for products and services';

$adb->pquery("CREATE TABLE IF NOT EXISTS nectarcrm_schedulereports(
            reportid INT(10),
            scheduleid INT(3),
            recipients TEXT,
            schdate VARCHAR(20),
...
nectarcrm_Cron::register('ScheduleReports', 'cron/modules/Reports/ScheduleReports.service', 900);

Migration_Index_View::ExecuteQuery('UPDATE nectarcrm_cron_task set description = ?  where name = "ScheduleReports" ', array("Recommended frequency for ScheduleReports is 15 mins"));
Migration_Index_View::ExecuteQuery('UPDATE nectarcrm_cron_task set module = ? where name = "ScheduleReports" ', array("Reports"));
...
$module = nectarcrm_Module::getInstance('Users');
  if ($module) {
      $blockInstance = nectarcrm_Block::getInstance('LBL_CALENDAR_SETTINGS', $module);
      if ($blockInstance) {
          $desField = nectarcrm_Field::getInstance('defaulteventstatus', $module);
          if(!$desField) {
          ...
          $datField = nectarcrm_Field::getInstance('defaultactivitytype', $module);
          if(!$datField) {
          $fieldInstance1 = new nectarcrm_Field();
...
//85 starts
Migration_Index_View::ExecuteQuery('ALTER TABLE nectarcrm_account ALTER isconvertedfromlead SET DEFAULT ?', array('0'));
Migration_Index_View::ExecuteQuery('ALTER TABLE nectarcrm_contactdetails ALTER isconvertedfromlead SET DEFAULT ?', array('0'));
Migration_Index_View::ExecuteQuery('ALTER TABLE nectarcrm_potential ALTER isconvertedfromlead SET DEFAULT ?', array('0'));
Migration_Index_View::ExecuteQuery('Update nectarcrm_account SET isconvertedfromlead = ? where isconvertedfromlead is NULL',array('0'));
Migration_Index_View::ExecuteQuery('Update nectarcrm_contactdetails SET isconvertedfromlead = ? where isconvertedfromlead is NULL',array('0'));
Migration_Index_View::ExecuteQuery('Update nectarcrm_potential SET isconvertedfromlead = ? where isconvertedfromlead is NULL',array('0'));
...
//87 starts
$result = $adb->pquery('SELECT 1 FROM nectarcrm_currencies WHERE currency_name = ?', array('Haiti, Gourde'));
if(!$adb->num_rows($result)) {
                    Migration_Index_View::ExecuteQuery('INSERT INTO nectarcrm_currencies (currencyid, currency_name, currency_code, currency_symbol) VALUES(?, ?, ?, ?)',
                    array($adb->getUniqueID('nectarcrm_currencies'), 'Haiti, Gourde', 'HTG', 'G'));
}
//87 ends   

//88 starts
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_currencies SET currency_symbol=? WHERE currency_code=?", array('₹','INR'));
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_currency_info SET currency_symbol=? WHERE currency_code=?", array('₹','INR'));

Migration_Index_View::ExecuteQuery('UPDATE nectarcrm_projecttaskstatus set presence = 0 where projecttaskstatus in (?,?,?,?,?)',
...
$updateQuery = "UPDATE nectarcrm_field SET sequence = CASE fieldname ";
...
$query = "SELECT tabid FROM nectarcrm_def_org_share";
...
if(!$exist){
        $ruleid = $adb->getUniqueID('nectarcrm_def_org_share');
        $shareaccessquery = "INSERT INTO nectarcrm_def_org_share VALUES(?,?,?,?)";
...
//92 starts
$result = $adb->pquery('SELECT max(templateid) AS maxtemplateid FROM nectarcrm_emailtemplates', array());
Migration_Index_View::ExecuteQuery('UPDATE nectarcrm_emailtemplates_seq SET id = ?', array(1 + ((int)$adb->query_result($result, 0, 'maxtemplateid'))));

 $result = $adb->pquery("SELECT 1 FROM nectarcrm_eventhandlers WHERE event_name=? AND handler_class=?",
                                    array('nectarcrm.entity.aftersave','nectarcrm_RecordLabelUpdater_Handler'));
if($adb->num_rows($result) <= 0) {
    $lastMaxCRMId = 0;
    do {
        $rs = $adb->pquery("SELECT crmid,setype FROM nectarcrm_crmentity WHERE crmid > ? LIMIT 500", array($lastMaxCRMId));
        if (!$adb->num_rows($rs)) {
            break;
        }
    ...
    nectarcrm_Event::register($homeModule, 'nectarcrm.entity.aftersave', 'nectarcrm_RecordLabelUpdater_Handler', 'modules/nectarcrm/handlers/RecordLabelUpdater.php');
...
$updateQuery = "update nectarcrm_campaignrelstatus set campaignrelstatus=? where campaignrelstatus=?";
...
//94 starts
$result = $adb->pquery('SELECT 1 FROM nectarcrm_currencies WHERE currency_name = ?', array('Libya, Dinar'));
if(!$adb->num_rows($result)) {
        Migration_Index_View::ExecuteQuery('INSERT INTO nectarcrm_currencies (currencyid, currency_name, currency_code, currency_symbol) VALUES(?, ?, ?, ?)',
        array($adb->getUniqueID('nectarcrm_currencies'), 'Libya, Dinar', 'LYD', 'LYD'));
}

//Start: Customer - Feature #17656 Allow users to add/remove date format with the date fields in workflow send mail task.
$fieldResult = $adb->pquery('SELECT fieldname, name, typeofdata FROM nectarcrm_field
                                INNER JOIN nectarcrm_tab ON nectarcrm_tab.tabid = nectarcrm_field.tabid WHERE typeofdata LIKE ?', array('D%'));
...
$taskIdsList = array();
$result = $adb->pquery('SELECT task_id, module_name FROM com_nectarcrm_workflowtasks
                        INNER JOIN com_nectarcrm_workflows ON com_nectarcrm_workflows.workflow_id = com_nectarcrm_workflowtasks.workflow_id
...
$timeZone = '($(general : (__nectarcrmMeta__) usertimezone))';
foreach ($taskIdsList as $taskId => $taskModuleName) {
        ...
        $properties = get_object_vars($task);
        foreach ($properties as $propertyName => $propertyValue) {
                $propertyValue = str_replace('$(general : (__nectarcrmMeta__) date)', "(general : (__nectarcrmMeta__) date) $dateFormat", $propertyValue);
...
// To update nectarcrm_modcomments table for permormance issue
$datatypeQuery = "ALTER TABLE nectarcrm_modcomments MODIFY COLUMN related_to int(19)";
...
echo '</br>';
$indexQuery = "ALTER TABLE nectarcrm_modcomments ADD INDEX relatedto_idx (related_to)";
...
$maxActionIdResult = $adb->pquery('SELECT MAX(actionid) AS maxid FROM nectarcrm_actionmapping', array());
$maxActionId = $adb->query_result($maxActionIdResult, 0, 'maxid');
Migration_Index_View::ExecuteQuery('INSERT INTO nectarcrm_actionmapping(actionid, actionname, securitycheck) VALUES(?,?,?)', array($maxActionId+1 ,'Print', '0'));
echo "<br> added print to nectarcrm_actionnmapping";
$module = nectarcrm_Module_Model::getInstance('Reports');
...
//95 starts
Migration_Index_View::ExecuteQuery('ALTER TABLE nectarcrm_webforms MODIFY COLUMN description TEXT',array());
require_once 'vtlib/nectarcrm/Module.php';
$module = nectarcrm_Module::getInstance('Users');
if ($module) {
    $blockInstance = nectarcrm_Block::getInstance('LBL_CALENDAR_SETTINGS', $module);
    if ($blockInstance) {
        $hideCompletedField = nectarcrm_Field::getInstance('hidecompletedevents', $module);
...
$entityModulesModels = nectarcrm_Module_Model::getEntityModules();
$modules = array();
...
foreach($modules as $module){
    ...
    if($moduleInstance){
        $result = Migration_Index_View::ExecuteQuery("select blocklabel from nectarcrm_blocks where tabid=? and sequence = ?", array($moduleInstance->id, 1));
...
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET presence=0 WHERE fieldname='unit_price' and columnname='unit_price'", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE nectarcrm_portal ADD createdtime datetime", array());

$adb->query("CREATE TABLE IF NOT EXISTS nectarcrm_calendar_default_activitytypes (id INT(19), module VARCHAR(50), fieldname VARCHAR(50), defaultcolor VARCHAR(50));");

$result = Migration_Index_View::ExecuteQuery('SELECT * FROM nectarcrm_calendar_default_activitytypes', array());
if ($adb->num_rows($result) <= 0) {
        ...
        foreach($calendarViewTypes as $module=>$viewInfo) {
                foreach($viewInfo as $fieldname=>$color) {
                        Migration_Index_View::ExecuteQuery('INSERT INTO nectarcrm_calendar_default_activitytypes (id, module, fieldname, defaultcolor) VALUES (?,?,?,?)', array($adb->getUniqueID('nectarcrm_calendar_default_activitytypes'), $module, $fieldname, $color));
                }
        }
        echo '<br>Default Calendar view types added to the table.<br>';
}
$adb->query("CREATE TABLE IF NOT EXISTS nectarcrm_calendar_user_activitytypes (id INT(19), defaultid INT(19), userid INT(19), color VARCHAR(50), visible INT(19) default 1);");

$result = Migration_Index_View::ExecuteQuery('SELECT * FROM nectarcrm_calendar_user_activitytypes', array());
if ($adb->num_rows($result) <= 0) {
    $queryResult = Migration_Index_View::ExecuteQuery('SELECT id, defaultcolor FROM nectarcrm_calendar_default_activitytypes', array());
    ...
    foreach($allUsers as $userId=>$userModel) {
            foreach($activityIds as $activityId=>$color) {
                   Migration_Index_View::ExecuteQuery('INSERT INTO nectarcrm_calendar_user_activitytypes (id, defaultid, userid, color) VALUES (?,?,?,?)', array($adb->getUniqueID('nectarcrm_calendar_user_activitytypes'), $activityId, $userId, $color));
            }
    }
    echo '<br>Default Calendar view types added to the table for all existing users';
}

Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET quickcreate = ? WHERE tabid = 8 AND (fieldname = ? OR fieldname = ?);", array(0,"filename","filelocationtype"));
    ...
    if($entityModulesModels){
        foreach($entityModulesModels as $moduleInstance){
            if($moduleInstance){
                ...
                $fieldInstance = nectarcrm_Field::getInstance($fieldNameToDelete,$moduleInstance);
                if($fieldInstance){
...
//97 starts
    ...
    Migration_Index_View::ExecuteQuery('DELETE FROM nectarcrm_eventhandlers WHERE handler_path IN ('.generateQuestionMarks($handlers) .')', $handlers);

    //delete modtracker detail view links
    Migration_Index_View::ExecuteQuery('DELETE FROM nectarcrm_links WHERE linktype = ? AND handler_class = ? AND linkurl like "javascript:ModTrackerCommon.showhistory%"',
                    array('DETAILVIEWBASIC', 'ModTracker'));

    //Added New field in mailmanager
    Migration_Index_View::ExecuteQuery('ALTER TABLE nectarcrm_mail_accounts ADD COLUMN sent_folder VARCHAR(50)', array());
...
if(!defined('INSTALLATION_MODE')) {
    $moduleInstance = nectarcrm_Module_Model::getInstance('PBXManager');
    if(!$moduleInstance){ 
       echo '<br>Installing PBX Manager starts<br>'; 
       installVtlibModule('PBXManager', 'packages/nectarcrm/mandatory/PBXManager.zip'); 
    }else{ 
        $result = $adb->pquery('SELECT server, port FROM nectarcrm_asterisk', array());
        ...
        foreach ($modules as $module) {
            ...
            //Data migrate from old columns to new columns in nectarcrm_pbxmanager 
            $query = 'SELECT * FROM nectarcrm_pbxmanager';
            $result = $adb->pquery($query, array());
            $params = array();
            ...
            for ($i = 0; $i < $rowCount; $i++) {
                ...
                $userIdQuery = $adb->pquery('SELECT userid FROM nectarcrm_asteriskextensions WHERE asterisk_extension = ?', array($callto));
                ...
                //Update query 
                $adb->pquery('UPDATE nectarcrm_pbxmanager SET customer = ? AND user = ? AND totalduration = ? AND callstatus = ? WHERE pbxmanagerid = ?', array($customer, $user, $timeofcall, $callstatus, $pbxmanagerid));
            }
            ...
            $extensionResult = $adb->pquery('SELECT userid, asterisk_extension FROM nectarcrm_asteriskextensions', array());
            for ($i = 0; $i < $adb->num_rows($extensionResult); $i++) {
                ...
                $adb->pquery('UPDATE nectarcrm_users SET phone_crm_extension = ? WHERE id = ?', array($extensionNumber, $userId));
            }
            ...
            nectarcrm_Link::addLink(0, $headerScriptLinkType, $incominglinkLabel, 'modules/PBXManager/resources/PBXManagerJS.js', '', '', $handlerInfo);
            ...
            //Existing Asterisk extension block removed from nectarcrm_users if exist 
            $moduleInstance = nectarcrm_Module_Model::getInstance('Users');
...
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET presence=? WHERE tabid=? AND fieldname=?;", array(1, $tabId, "callfrom"));
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET presence=? WHERE tabid=? AND fieldname=?;", array(1, $tabId, "callto"));
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET presence=? WHERE tabid=? AND fieldname=?;", array(1, $tabId, "timeofcall"));
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET presence=? WHERE tabid=? AND fieldname=?;", array(1, $tabId, "status"));
echo '<br>Hiding previous PBXManager fields done.<br>'; 
//PBXManager porting ends.

//Making document module fields masseditable
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"notes_title")); 
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"assigned_user_id")); 
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"notecontent")); 
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"fileversion")); 
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"filestatus")); 
Migration_Index_View::ExecuteQuery("UPDATE nectarcrm_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"folderid")); 

//Add Vat ID to Company Details 
nectarcrm_Utils::AddColumn('nectarcrm_organizationdetails', 'vatid', 'VARCHAR(100)');

//Add Column trial for nectarcrm_tab table if not exists
$result = $adb->pquery("SHOW COLUMNS FROM nectarcrm_tab LIKE ?", array('trial'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE nectarcrm_tab ADD trial INT(1) NOT NULL DEFAULT 0",array());
}

##--http://trac.nectarcrm.com/cgi-bin/trac.cgi/ticket/7635--##
...
$relatedToQuery = "SELECT fieldid FROM nectarcrm_field WHERE tabid=? AND fieldname=?";
$calendarInstance = nectarcrm_Module::getInstance('Calendar');
$tabId = $calendarInstance->getId();
$result = $adb->pquery($relatedToQuery, array($tabId, 'parent_id'));
$fieldId = $adb->query_result($result,0, 'fieldid');
$insertQuery = "INSERT INTO nectarcrm_fieldmodulerel (fieldid,module,relmodule,status,sequence) VALUES(?,?,?,?,?)";
$relModule = 'Calendar';
foreach ($moduleArray as $module) {
    $adb->pquery($insertQuery, array($fieldId, $module, $relModule, NULL, NULL));
}
//For contacts the fieldname is contact_id
$contactsRelatedToQuery = "SELECT fieldid FROM nectarcrm_field WHERE tabid=? AND fieldname=?";
...
$insertContactsQuery = "INSERT INTO nectarcrm_fieldmodulerel (fieldid,module,relmodule,status,sequence) VALUES(?,?,?,?,?)";
$module = 'Contacts';
...
//Adding is_owner to existing nectarcrm users

$usersModuleInstance = nectarcrm_Module::getInstance('Users');
$usersBlockInstance = nectarcrm_Block::getInstance('LBL_USERLOGIN_ROLE', $usersModuleInstance);

$usersFieldInstance = nectarcrm_Field::getInstance('is_owner', $usersModuleInstance);
...
$adb = PearDatabase::getInstance();
$idResult = $adb->pquery('SELECT id FROM nectarcrm_users WHERE is_admin = ? AND status=?', array('on', 'Active'));
if ($adb->num_rows($idResult) > 0) {
    for($i = 0;$i<=$adb->num_rows($idResult);$i++) {
        $userid = $adb->query_result($idResult, $i, 'id');
        $adb->pquery('UPDATE nectarcrm_users SET is_owner=? WHERE id=?', array(1, $userid));
        echo '<br>Account Owner Informnation saved in nectarcrm';
...
Migration_Index_View::ExecuteQuery("CREATE TABLE IF NOT EXISTS nectarcrm_reporttype(
                        reportid INT(10),
                        data text,
						PRIMARY KEY (`reportid`),
						CONSTRAINT `fk_1_nectarcrm_reporttype` FOREIGN KEY (`reportid`) REFERENCES `nectarcrm_report` (`reportid`) ON DELETE CASCADE)
                        ENGINE=InnoDB DEFAULT CHARSET=utf8;", array()); 

//Configuration Editor fix
$sql = "UPDATE nectarcrm_settings_field SET name = ? WHERE name = ?";
