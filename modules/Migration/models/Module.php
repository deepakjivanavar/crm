...
class Migration_Module_Model extends nectarcrm_Module_Model {
	
	public function getDBVersion(){
		$db = PearDatabase::getInstance();
		
		$result = $db->pquery('SELECT current_version FROM nectarcrm_version', array());
	...
	public function getLatestSourceVersion(){
		return vglobal('nectarcrm_current_version');
	}
	
	/**
	 * Function to update the latest nectarcrm version in db
	 * @return type
	 */
	public function updatenectarcrmVersion(){
		$db = PearDatabase::getInstance();
		$db->pquery('UPDATE nectarcrm_version SET current_version=?,old_version=?', array($this->getLatestSourceVersion(), $this->getDBVersion()));
		return true;
	}
