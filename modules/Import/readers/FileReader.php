...
class Import_FileReader_Reader {
	...
	public function  __construct($request, $user) {
		...
		$this->user = $user;
		$this->moduleModel = nectarcrm_Module_Model::getInstance($this->request->get('module'));
	}
	 ...
	 * @param $fieldObject <nectarcrm_Field_Model>
	...
	public function getModuleFieldDBColumnType() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT tablename FROM nectarcrm_field WHERE tabid=? GROUP BY tablename', array($this->moduleModel->getId()));
		$tables = array();
