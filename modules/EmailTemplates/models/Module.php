...
class EmailTemplates_Module_Model extends nectarcrm_Module_Model {
	...
	*/
	public function saveRecord(EmailTemplates_Record_Model $recordModel) {
		...
		if(empty($templateid)){
			$templateid = $db->getUniqueID('nectarcrm_emailtemplates');
			$sql = "INSERT INTO nectarcrm_emailtemplates(templatename, subject, description, module, body, deleted, systemtemplate, templateid) VALUES (?,?,?,?,?,?,?,?)";
		}else{
			if($systemtemplate) {
				$sql = "UPDATE nectarcrm_emailtemplates SET templatename=?, description=?, module=?, body=?, deleted=?, systemtemplate=? WHERE templateid = ?";
			} else {
				$sql = "UPDATE nectarcrm_emailtemplates SET templatename=?, subject=?, description=?, module=?, body=?, deleted=?, systemtemplate=? WHERE templateid = ?";
			}
		}
	...
	public function deleteRecord(EmailTemplates_Record_Model $recordModel) {
		...
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM nectarcrm_emailtemplates WHERE templateid = ? AND systemtemplate = ? ', array($recordId, '0'));
	}
	...
	public function deleteAllRecords() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM nectarcrm_emailtemplates WHERE systemtemplate = ?', array('0'));
	}
	...
	* Function retrives all company details merge tags and add to field array 
	...
	function getCompanyMergeTagsInfo(){ 
		...
		$qualifiedModule = "Settings:nectarcrm"; 
	...
	public function getRecordIds($skipRecords){
		...
		$query = 'SELECT templateid FROM nectarcrm_emailtemplates WHERE templateid NOT IN ('.generateQuestionMarks($skipRecords).')';
