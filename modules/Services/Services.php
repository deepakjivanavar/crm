...
class Services extends CRMEntity {
	...
	var $table_name = 'nectarcrm_service';
	 ...
	 */
	var $customFieldTable = Array('nectarcrm_servicecf', 'serviceid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('nectarcrm_crmentity','nectarcrm_service','nectarcrm_servicecf');
	...
	var $tab_name_index = Array(
		...
		'nectarcrm_service'=>'serviceid',
		'nectarcrm_servicecf'=>'serviceid',
		'nectarcrm_producttaxrel'=>'productid');
	...
	function save_module($module)
	{
		...
		{
			$this->insertTaxInformation('nectarcrm_producttaxrel', 'Services');

			if ($_REQUEST['action'] != 'MassEditSave' ) {
				$this->insertPriceInformation('nectarcrm_productcurrencyrel', 'Services');
			}
		}
	...
	/**	function to save the service tax information in nectarcrm_servicetaxrel table
	 *	@param string $tablename - nectarcrm_tablename to save the service tax relationship (servicetaxrel)
	...
	*/
	...
	/**	function to save the service price information in nectarcrm_servicecurrencyrel table
	 *	@param string $tablename - nectarcrm_tablename to save the service currency relationship (servicecurrencyrel)
	 *	@param string $module	 - current module name
	 *	$return void
	*/
		...
		for($i=0;$i<count($currency_details);$i++)
		{
			...
			if($_REQUEST[$cur_checkname] == 'on' || $_REQUEST[$cur_checkname] == 1 || $isQuickCreate)
			{
				...
				if ($_REQUEST['base_currency'] == $cur_valuename) {
					$currencySet = 1;
					$adb->pquery("update nectarcrm_service set currency_id=?, unit_price=? where serviceid=?", array($curid, $actualPrice, $this->id));
				}
			}
			if (!$currencySet) {
				$curid = fetchCurrency($current_user->id);
				$adb->pquery("update nectarcrm_service set currency_id=? where serviceid=?", array($curid, $this->id));
			}
		}
	...
	function updateUnitPrice() {
		$prod_res = $this->db->pquery("select unit_price, currency_id from nectarcrm_service where serviceid=?", array($this->id));
	...
	function getListQuery($module, $where='') {
		...
		$query .= " LEFT JOIN nectarcrm_groups
	...
	function getDuplicatesQuery($module,$table_cols,$field_values,$ui_type_arr,$select_cols='') {
		...
		$from_clause .=	" LEFT JOIN nectarcrm_groups ON nectarcrm_groups.groupid = nectarcrm_crmentity.smownerid
	...
	function get_purchase_orders($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT nectarcrm_crmentity.*,
			...
			INNER JOIN nectarcrm_service
				ON nectarcrm_service.serviceid = nectarcrm_inventoryproductrel.productid
			LEFT JOIN nectarcrm_groups
				ON nectarcrm_groups.groupid = nectarcrm_crmentity.smownerid
			...
			LEFT JOIN nectarcrm_users
				ON nectarcrm_users.id = nectarcrm_crmentity.smownerid
			WHERE nectarcrm_crmentity.deleted = 0
			AND nectarcrm_service.serviceid = ".$id;
	...
	function get_salesorder($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT nectarcrm_crmentity.*,
			...
			nectarcrm_service.servicename AS servicename,
			nectarcrm_account.accountname,
			case when (nectarcrm_users.user_name not like '') then $userNameSql
				else nectarcrm_groups.groupname end as user_name
	...
	function getPriceBookRelatedServices($query,$focus,$returnset='')
	{
		...
		if(PerformancePrefs::getBoolean('LISTVIEW_COMPUTE_PAGE_COUNT', false) === true ||
			...
			$noofrows = $adb->query_result($adb->query(nectarcrm_Functions::mkCountQuery($query)),0,'count');
		}else{
			$noofrows = null;
		}
		$module = 'PriceBooks';
		...
		for($i=0; $i<$numRows; $i++) {
			...
			if(isPermitted("PriceBooks","EditView","") == 'yes' && isPermitted('Services', 'EditView', $entity_id) == 'yes') {
				...
				$action .= '<img src="'. nectarcrm_imageurl('blank.gif', $theme).'" border="0" />';
			}
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Quotes"=>"nectarcrm_inventoryproductrel","PurchaseOrder"=>"nectarcrm_inventoryproductrel","SalesOrder"=>"nectarcrm_inventoryproductrel",
				"Invoice"=>"nectarcrm_inventoryproductrel","PriceBooks"=>"nectarcrm_pricebookproductrel","Documents"=>"nectarcrm_senotesrel");

		$tbl_field_arr = Array("nectarcrm_inventoryproductrel"=>"id","nectarcrm_pricebookproductrel"=>"pricebookid","nectarcrm_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("nectarcrm_inventoryproductrel"=>"productid","nectarcrm_pricebookproductrel"=>"productid","nectarcrm_senotesrel"=>"crmid");
	...
	function generateReportsQuery($module,$queryPlanner){
		global $current_user;
			...
			$matrix->setDependency('nectarcrm_seproductsrel',array('nectarcrm_crmentityRelServices','nectarcrm_accountRelServices','nectarcrm_leaddetailsRelServices','nectarcrm_servicecf','nectarcrm_potentialRelServices'));
			$query = "from nectarcrm_service
				inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_service.serviceid";
			if ($queryPlanner->requireTable("nectarcrm_servicecf")){
				$query .= " left join nectarcrm_servicecf on nectarcrm_service.serviceid = nectarcrm_servicecf.serviceid";
			}
			if ($queryPlanner->requireTable("nectarcrm_usersServices")){
				$query .= " left join nectarcrm_users as nectarcrm_usersServices on nectarcrm_usersServices.id = nectarcrm_crmentity.smownerid";
			}
			if ($queryPlanner->requireTable("nectarcrm_groupsServices")){
				$query .= " left join nectarcrm_groups as nectarcrm_groupsServices on nectarcrm_groupsServices.groupid = nectarcrm_crmentity.smownerid";
			}
			if ($queryPlanner->requireTable("nectarcrm_seproductsrel")){
				$query .= " left join nectarcrm_seproductsrel on nectarcrm_seproductsrel.productid= nectarcrm_service.serviceid";
			}
			if ($queryPlanner->requireTable("nectarcrm_crmentityRelServices")){
				$query .= " left join nectarcrm_crmentity as nectarcrm_crmentityRelServices on nectarcrm_crmentityRelServices.crmid = nectarcrm_seproductsrel.crmid and nectarcrm_crmentityRelServices.deleted = 0";
			}
			if ($queryPlanner->requireTable("nectarcrm_accountRelServices")){
				$query .= " left join nectarcrm_account as nectarcrm_accountRelServices on nectarcrm_accountRelServices.accountid=nectarcrm_seproductsrel.crmid";
			}
			if ($queryPlanner->requireTable("nectarcrm_leaddetailsRelServices")){
				$query .= " left join nectarcrm_leaddetails as nectarcrm_leaddetailsRelServices on nectarcrm_leaddetailsRelServices.leadid = nectarcrm_seproductsrel.crmid";
			}
			if ($queryPlanner->requireTable("nectarcrm_potentialRelServices")){
				$query .= " left join nectarcrm_potential as nectarcrm_potentialRelServices on nectarcrm_potentialRelServices.potentialid = nectarcrm_seproductsrel.crmid";
			}
			if ($queryPlanner->requireTable("nectarcrm_lastModifiedByServices")){
				$query .= " left join nectarcrm_users as nectarcrm_lastModifiedByServices on nectarcrm_lastModifiedByServices.id = nectarcrm_crmentity.modifiedby";
			}
			if($queryPlanner->requireTable("nectarcrm_createdbyServices")){
			...
			if ($queryPlanner->requireTable("innerService")){
				$query .= " LEFT JOIN (
					SELECT nectarcrm_service.serviceid,
							(CASE WHEN (nectarcrm_service.currency_id = 1 ) THEN nectarcrm_service.unit_price
								ELSE (nectarcrm_service.unit_price / nectarcrm_currency_info.conversion_rate) END
							) AS actual_unit_price
					FROM nectarcrm_service
					LEFT JOIN nectarcrm_currency_info ON nectarcrm_service.currency_id = nectarcrm_currency_info.id
					LEFT JOIN nectarcrm_productcurrencyrel ON nectarcrm_service.serviceid = nectarcrm_productcurrencyrel.productid
					AND nectarcrm_productcurrencyrel.currencyid = ". $current_user->currency_id . "
				) AS innerService ON innerService.serviceid = nectarcrm_service.serviceid";
			}
			return $query;
	}
	...
	function generateReportsSecQuery($module,$secmodule, $queryPlanner) {
		global $current_user;
		$matrix = $queryPlanner->newDependencyMatrix();
		$matrix->setDependency('nectarcrm_crmentityServices',array('nectarcrm_usersServices','nectarcrm_groupsServices','nectarcrm_lastModifiedByServices'));
		if (!$queryPlanner->requireTable("nectarcrm_service",$matrix)){
			return '';
		}
		$matrix->setDependency('nectarcrm_service',array('actual_unit_price','nectarcrm_currency_info','nectarcrm_productcurrencyrel','nectarcrm_servicecf','nectarcrm_crmentityServices'));

		$query = $this->getRelationQuery($module,$secmodule,"nectarcrm_service","serviceid", $queryPlanner);
		if ($queryPlanner->requireTable("innerService")){
			$query .= " LEFT JOIN (
			SELECT nectarcrm_service.serviceid,
			(CASE WHEN (nectarcrm_service.currency_id = " . $current_user->currency_id . " ) THEN nectarcrm_service.unit_price
			WHEN (nectarcrm_productcurrencyrel.actual_price IS NOT NULL) THEN nectarcrm_productcurrencyrel.actual_price
			ELSE (nectarcrm_service.unit_price / nectarcrm_currency_info.conversion_rate) * ". $current_user->conv_rate . " END
			) AS actual_unit_price FROM nectarcrm_service
			LEFT JOIN nectarcrm_currency_info ON nectarcrm_service.currency_id = nectarcrm_currency_info.id
			LEFT JOIN nectarcrm_productcurrencyrel ON nectarcrm_service.serviceid = nectarcrm_productcurrencyrel.productid
			AND nectarcrm_productcurrencyrel.currencyid = ". $current_user->currency_id . ")
			AS innerService ON innerService.serviceid = nectarcrm_service.serviceid";
		}
		if ($queryPlanner->requireTable("nectarcrm_crmentityServices",$matrix)){
			$query .= " left join nectarcrm_crmentity as nectarcrm_crmentityServices on nectarcrm_crmentityServices.crmid=nectarcrm_service.serviceid and nectarcrm_crmentityServices.deleted=0";
		}
		if ($queryPlanner->requireTable("nectarcrm_servicecf")){
			$query .= " left join nectarcrm_servicecf on nectarcrm_service.serviceid = nectarcrm_servicecf.serviceid";
		}
		if ($queryPlanner->requireTable("nectarcrm_usersServices")){
			$query .= " left join nectarcrm_users as nectarcrm_usersServices on nectarcrm_usersServices.id = nectarcrm_crmentityServices.smownerid";
		}
		if ($queryPlanner->requireTable("nectarcrm_groupsServices")){
			$query .= " left join nectarcrm_groups as nectarcrm_groupsServices on nectarcrm_groupsServices.groupid = nectarcrm_crmentityServices.smownerid";
		}
		if ($queryPlanner->requireTable("nectarcrm_lastModifiedByServices")){
			$query .= " left join nectarcrm_users as nectarcrm_lastModifiedByServices on nectarcrm_lastModifiedByServices.id = nectarcrm_crmentityServices.modifiedby ";
		}
		if ($queryPlanner->requireTable("nectarcrm_createdbyServices")){
			$query .= " left join nectarcrm_users as nectarcrm_createdbyServices on nectarcrm_createdbyServices.id = nectarcrm_crmentityServices.smcreatorid ";
		}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Quotes" => array("nectarcrm_inventoryproductrel"=>array("productid","id"),"nectarcrm_service"=>"serviceid"),
			"PurchaseOrder" => array("nectarcrm_inventoryproductrel"=>array("productid","id"),"nectarcrm_service"=>"serviceid"),
			"SalesOrder" => array("nectarcrm_inventoryproductrel"=>array("productid","id"),"nectarcrm_service"=>"serviceid"),
			"Invoice" => array("nectarcrm_inventoryproductrel"=>array("productid","id"),"nectarcrm_service"=>"serviceid"),
			"PriceBooks" => array("nectarcrm_pricebookproductrel"=>array("productid","pricebookid"),"nectarcrm_service"=>"serviceid"),
			"Documents" => array("nectarcrm_senotesrel"=>array("crmid","notesid"),"nectarcrm_service"=>"serviceid"),
		);
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			$ttModuleInstance = nectarcrm_Module::getInstance('HelpDesk');
			$ttModuleInstance->setRelatedList($moduleInstance,'Services',array('select'));

			$leadModuleInstance = nectarcrm_Module::getInstance('Leads');
			...
			$potModuleInstance = nectarcrm_Module::getInstance('Potentials');
			$potModuleInstance->setRelatedList($moduleInstance,'Services',array('select'));

			$pbModuleInstance = nectarcrm_Module::getInstance('PriceBooks');
			$pbModuleInstance->setRelatedList($moduleInstance,'Services',array('select'),'get_pricebook_services');

			// Initialize module sequence for the module
			$adb->pquery("INSERT into nectarcrm_modentity_num values(?,?,?,?,?,?)",array($adb->getUniqueId("nectarcrm_modentity_num"),$moduleName,'SER',1,1,1));
		...
		// TODO Handle actions after this module is updated.
			...
			$ServicesModule  = nectarcrm_Module::getInstance('Services');
			nectarcrm_Access::setDefaultSharing($ServicesModule);
		}
	}
	...
	*/
	function get_services($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT nectarcrm_service.serviceid, nectarcrm_service.servicename,
			nectarcrm_service.service_no, nectarcrm_service.commissionrate,
			nectarcrm_service.service_usageunit, nectarcrm_service.unit_price,
			nectarcrm_crmentity.crmid, nectarcrm_crmentity.smownerid
			FROM nectarcrm_service
			INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_service.serviceid
			INNER JOIN nectarcrm_servicecf
				ON nectarcrm_service.serviceid = nectarcrm_servicecf.serviceid
			LEFT JOIN nectarcrm_crmentityrel ON nectarcrm_crmentityrel.relcrmid = nectarcrm_service.serviceid AND nectarcrm_crmentityrel.module='Services'
			LEFT JOIN nectarcrm_users
				ON nectarcrm_users.id=nectarcrm_crmentity.smownerid
			LEFT JOIN nectarcrm_groups
				ON nectarcrm_groups.groupid = nectarcrm_crmentity.smownerid
			WHERE nectarcrm_crmentity.deleted = 0 AND nectarcrm_crmentityrel.crmid = $id ";
