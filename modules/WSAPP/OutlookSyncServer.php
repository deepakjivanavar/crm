...
class OutlookSyncServer extends SyncServer{
    ...
    function getDestinationHandleDetails(){
        return array('handlerclass' => 'OutlooknectarcrmCRMHandler',
            'handlerpath' => 'modules/WSAPP/Handlers/OutlooknectarcrmCRMHandler.php');
	}
     ...
     function put($key, $element, $user) {
        ...
        foreach ($records as $record) {
            ...
            if (!empty($record['crmid'])) {
                ...
                $lookupResult = $db->pquery("SELECT crmid,modifiedtime FROM nectarcrm_crmentity WHERE crmid=?", array($crmid[1]));
            ...
            // End
                ...
                $lookupResult = $db->pquery("SELECT serverid,clientmodifiedtime FROM nectarcrm_wsapp_recordmapping WHERE appid=? AND clientid=?", array($appid, $clientRecordId));
                if ($db->num_rows($lookupResult))
