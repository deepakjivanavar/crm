...
class nectarcrmCRMHandler extends SyncHandler {
		...
		foreach ($referenceFieldDetails as $referenceFieldName => $referenceModuleDetails) {
			...
			foreach ($records as $recordDetails) {
				...
				if (!empty($referenceWsId)) {
					...
					$webserviceObject = nectarcrmWebserviceObject::fromId($db, $referenceIdComp[0]);
	...
	/**
	 * Function to fillMandatory fields in nectarcrm with given values
	...
	public function fillMandatoryFields($recordLists, $user) {
		...
		foreach ($recordLists as $index => $record) {
			...
			if(!isset($record['source'])){
				$record['source'] = nectarcrm_Cache::get('WSAPP', 'appName');
			}
	...
	public function triggerSyncActionForDuplicate($recordData, $duplicateRecordIds) {
		...
		if ($moduleModel && $moduleModel->isSyncable) {
			$webSeviceModuleModel = nectarcrmWebserviceObject::fromName($db, $moduleName);
			...
			try {
				$nectarcrmRecordData = vtws_retrieve($recordId, $user);
			} catch (Exception $e) {
		 		return $result;
			}
			...
			switch ($moduleModel->syncActionForDuplicate) {
				case 1	:	//Prefer latest record
							$finalRecordData = $nectarcrmRecordData;
							if ($recordData['modifiedtime'] > $nectarcrmRecordData['modifiedtime']) {
							...
							break;
//				case 3	:	//Prefer nectarcrm Record
//							$result = $nectarcrmRecordData;
//							break;
