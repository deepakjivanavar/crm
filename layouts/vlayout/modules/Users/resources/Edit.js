...
nectarcrm_Edit_Js("Users_Edit_Js",{},{
	...
	registerRecordPreSaveEvent : function(form){
		...
		form.on(nectarcrm_Edit_Js.recordPreSave, function(e, data) {
			...
			if(groupingSeparatorValue == decimalSeparatorValue){
				nectarcrm_Helper_Js.showPnotify(app.vtranslate('JS_DECIMAL_SEPARATOR_AND_GROUPING_SEPARATOR_CANT_BE_SAME'));
				e.preventDefault();
			}
			...
			if(record == ''){
				if(newPassword != confirmPassword){
					nectarcrm_Helper_Js.showPnotify(app.vtranslate('JS_REENTER_PASSWORDS'));
					e.preventDefault();
				}
					...
					thisInstance.checkDuplicateUser(userName).then(
						function(data){
							if(data.result) {
								...
								nectarcrm_Helper_Js.showPnotify(app.vtranslate('JS_USER_EXISTS'));
							}
						}, 
						function (data, error){
							thisInstance.duplicateCheckCache[userName] = data.result;
