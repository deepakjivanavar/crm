...
nectarcrm_Detail_Js("Users_Detail_Js",{
		...
		if(new_password.val() == confirm_password.val()){
			...
			AppConnector.request(params).then(
				function(data) {
					if(data.success){
						app.hideModalWindow();
						nectarcrm_Helper_Js.showPnotify(app.vtranslate(data.result.message));
					}else{
	...
	deleteUser: function (form){
		...
		AppConnector.request(params).then(
			function(data) {
				if(data.success){
					app.hideModalWindow();
					nectarcrm_Helper_Js.showPnotify(app.vtranslate(data.result.status.message));
	...
	triggerChangeAccessKey: function (url) {
		...
		nectarcrm_Helper_Js.showConfirmationBox({'title': title,'message': message}).then(function (data) {
	...
	registerEvents : function() {
        this._super();
		var form = this.getForm();
		this.usersEditInstance = nectarcrm_Edit_Js.getInstance();
