...
var nectarcrm_Index_Js = {
	...
	registerWidgetsEvents : function() {
		...
		widgets.on({
				shown: function(e) {
					...
					nectarcrm_Index_Js.loadWidgets(widgetContainer);
	...
	loadWidgets : function(widgetContainer, open) {
		...
		AppConnector.request(listViewWidgetParams).then(
			...
			if(typeof open == 'undefined') open = true;
				...
				jQuery('.bodyContents').trigger('nectarcrm.Widget.Load.'+label,jQuery(widgetContainer));
			}
		);
	},
	...
	showComposeEmailPopup : function(params, cb){
		...
		nectarcrm_Helper_Js.checkServerConfig(currentModule).then(function(data){
			if(data == true){
	...
	registerActivityReminder : function() {
		...
		if(activityReminder != '') {
			...
			if((currentTime + activityReminder) > nextActivityReminderCheck) {
				...
				setTimeout('nectarcrm_Index_Js.requestReminder()', activityReminder);
	...
	showReminderPopup : function(record) {
		...
		var notify = nectarcrm_Helper_Js.showPnotify(params);
	...
	registerResizeEvent: function(){
		...
		$(window).bind('resizeEnd', function() {
			nectarcrm_Index_Js.adjustTopMenuBarItems();
		});
	},
	/**
	...
	registerTooltipEvents: function() {
		...
		function registerToolTipDestroy() {
			jQuery('button[name="vtTooltipClose"]').on('click', function(e){
				...
				lastPopover.popover('hide');
				// Fix suggested http://code.nectarcrm.com/nectarcrm/nectarcrmcrm/issues/43
	...
	registerEvents : function(){
		nectarcrm_Index_Js.registerWidgetsEvents();
		nectarcrm_Index_Js.loadWidgetsOnLoad();
		nectarcrm_Index_Js.registerActivityReminder();
		nectarcrm_Index_Js.adjustTopMenuBarItems();
		nectarcrm_Index_Js.registerPostAjaxEvents();
		nectarcrm_Index_Js.changeSkin();
		nectarcrm_Index_Js.registerShowHideLeftPanelEvent();
		nectarcrm_Index_Js.registerResizeEvent();
	},

	registerPostAjaxEvents: function() {
		nectarcrm_Index_Js.registerTooltipEvents();
	}
}
...
jQuery(document).ready(function() {
	nectarcrm_Index_Js.registerEvents();
