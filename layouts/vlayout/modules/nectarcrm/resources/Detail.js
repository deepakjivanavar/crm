...
jQuery.Class("nectarcrm_Detail_Js",{
	...
	triggerSendEmail : function(detailActionUrl, module){
        nectarcrm_Helper_Js.checkServerConfig(module).then(function(data){
			if(data == true){
                var currentInstance = nectarcrm_Detail_Js.getInstance();
                ...
                nectarcrm_Index_Js.showComposeEmailPopup(params);
			} else {
    ...
    triggerDetailViewAction : function(detailActionUrl, callBackFunction){
		var detailInstance = nectarcrm_Detail_Js.getInstance();
    ...
    triggerSendSms : function(detailActionUrl, module) {
        nectarcrm_Helper_Js.checkServerConfig(module).then(function(data){
			if(data == true){
                nectarcrm_Detail_Js.triggerDetailViewAction(detailActionUrl);
			} else {
    ...
    deleteRecord : function(deleteRecordActionUrl) {
		...
		nectarcrm_Helper_Js.showConfirmationBox({'message' : message}).then(function(data) {
				AppConnector.request(deleteRecordActionUrl+'&ajaxDelete=true').then(
				function(data){
					if(data.success == true){
						window.location.href = data.result;
					}else{
						nectarcrm_Helper_Js.showPnotify(data.error.message);
					}
				});
			},
			function(error, err){
	...
	fieldUpdatedEvent : 'nectarcrm.Field.Updated',
	...
	fieldPreSave : 'nectarcrm.Field.PreSave',
	...
	loadRelatedList : function(pageNumber){
		var relatedListInstance = new nectarcrm_RelatedList_Js(this.getRecordId(), app.getModuleName(), this.getSelectedTab(), this.getRelatedModuleName());
	...
	registerEventForRelatedListPagination : function(){
		...
		detailContentsHolder.on('click','#relatedListNextPageButton',function(e){
			...
			var relatedController = new nectarcrm_RelatedList_Js(thisInstance.getRecordId(), app.getModuleName(), selectedTabElement, relatedModuleName);
	...
	registerEventForRelatedList : function(){
		...
		detailContentsHolder.on('click', 'a.relationDelete', function(e){
			e.stopImmediatePropagation();
			var element = jQuery(e.currentTarget);
			var instance = nectarcrm_Detail_Js.getInstance();
			var key = instance.getDeleteMessageKey();
			var message = app.vtranslate(key);
			nectarcrm_Helper_Js.showConfirmationBox({'message' : message}).then(
				function(e) {
	...
	ajaxEditHandling : function(currentTdElement) {
			...
			var saveHandler = function(e) {
                ...
                var fieldInfo = nectarcrm_Field_Js.getInstance(fieldElement.data('fieldinfo'));
                ...
                if((""+previousValue) == (""+ajaxEditNewValue)) { // Normalize(99!="099") Fix http://code.nectarcrm.com/nectarcrm/nectarcrmcrm/issues/16 
	...
	registerEventForActivityWidget : function(){
		...
		jQuery('.createActivity').on('click', function(e){
			...
			if(quickCreateNode.length <= 0) {
				nectarcrm_Helper_Js.showPnotify(app.vtranslate('JS_NO_CREATE_OR_NOT_QUICK_CREATE_ENABLED'))
			}
			...
			var callbackFunction = function() {
				...
				if(previousValue == ajaxEditNewValue) {
					...
					currentTarget.show();
                                        ...
                                        //If validation fails  
                                        if(errorExists) {  
                                                nectarcrm_Helper_Js.addClickOutSideEvent(currentDiv, callbackFunction);   
                                                                return;   
                                        }
	...
	addRelationBetweenRecords : function(relatedModule, relatedModuleRecordId){
		...
		var thisInstance = this;
		var selectedTabElement = thisInstance.getSelectedTab();
		var relatedController = new nectarcrm_RelatedList_Js(thisInstance.getRecordId(), app.getModuleName(), selectedTabElement, relatedModule);
		...
		detailContentsHolder.on('click','[name="emailsEditView"]',function(e){
			...
			nectarcrm_Helper_Js.checkServerConfig(module).then(function(data){
				if(data == true){
					...
					nectarcrm_Helper_Js.showPnotify(app.vtranslate('JS_EMAIL_SERVER_CONFIGURATION'));
				}
			})
		})
	},
	...
	registerPostTagCloudWidgetLoad : function() {
		var thisInstance = this;
		app.getContentsContainer().on('nectarcrm.Widget.Load.LBL_TAG_CLOUD',function(e,data){
		...
		jQuery('.related', detailContainer).on('click', 'li', function(e, urlAttributes){
			...
			thisInstance.loadContents(url,urlAttributes).then(
				function(data){
					...
					nectarcrm_Helper_Js.showHorizontalTopScrollBar();
					element.progressIndicator({'mode': 'hide'});
					if(typeof callBack == 'function'){
						callBack(data);
					}
	...
	registerEvents : function(){
		...
		detailContentsHolder.on('click', '.relatedPopup', function(e){
			var editViewObj = new nectarcrm_Edit_Js();
