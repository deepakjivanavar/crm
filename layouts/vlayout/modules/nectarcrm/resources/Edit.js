...
jQuery.Class("nectarcrm_Edit_Js",{

	//Event that will triggered when reference field is selected
	referenceSelectionEvent : 'nectarcrm.Reference.Selection',

	//Event that will triggered when reference field is selected
	referenceDeSelectionEvent : 'nectarcrm.Reference.DeSelection',

	//Event that will triggered before saving the record
	recordPreSave : 'nectarcrm.Record.PreSave',

	refrenceMultiSelectionEvent : 'nectarcrm.MultiReference.Selection',

	preReferencePopUpOpenEvent : 'nectarcrm.Referece.Popup.Pre',

	editInstance : false,

    postReferenceSelectionEvent: 'nectarcrm.PostReference.Selection',
	...
	getInstanceByModuleName : function(moduleName){
		...
		if(parentModule == 'Settings'){
			...
			var fallbackClassName = parentModule+"_nectarcrm_Edit_Js";
			if(typeof window[fallbackClassName] == 'undefined') {
				fallbackClassName = "nectarcrm_Edit_Js";
			}
		} else {
			moduleClassName = moduleName+"_Edit_Js";
			fallbackClassName = "nectarcrm_Edit_Js";
	...
	getInstance: function(){
		if(nectarcrm_Edit_Js.editInstance == false){
			var instance = nectarcrm_Edit_Js.getInstanceByModuleName();
			nectarcrm_Edit_Js.editInstance = instance;
			return instance;
		}
		return nectarcrm_Edit_Js.editInstance;
	}
	...
	registerAutoCompleteFields : function(container) {
		...
		container.find('input.autoComplete').autocomplete({
			...
			'select' : function(event, ui ){
                ...
                fieldElement.trigger(nectarcrm_Edit_Js.postReferenceSelectionEvent,{'data':selectedItemData});
			},
	...
	registerEventForImageDelete : function(){
		...
		formElement.find('.imageDelete').on('click',function(e){
			...
			if(exisitingImages.length < 1 && mandatoryStatus){
				...
				imageUploadElement.attr('data-validation-engine','validate[required,funcCall[nectarcrm_Base_Validator_Js.invokeValidation]]');
		...
		editViewForm.submit(function(e){
			...
			if(typeof editViewForm.data('submit') != "undefined") {
				...
				if(editViewForm.validationEngine('validate')) {
					...
					var recordPreSaveEvent = jQuery.Event(nectarcrm_Edit_Js.recordPreSave);
	...
	registerRecordAccessCheckEvent : function(form) {
		...
		form.on(nectarcrm_Edit_Js.recordPreSave, function(e, data) {
