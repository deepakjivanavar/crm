...
var nectarcrm_CustomView_Js = {
	...
	loadFilterView : function(url) {
		...
		AppConnector.request(url).then(
			function(data){
				...
				nectarcrm_CustomView_Js.registerEvents();
				nectarcrm_CustomView_Js.advanceFilterInstance = nectarcrm_AdvanceFilter_Js.getInstance(jQuery('.filterContainer',contents));
			},
			function(error,err){
	...
	getContentsContainer : function() {
		if(nectarcrm_CustomView_Js.contentsCotainer == false) {
			nectarcrm_CustomView_Js.contentsCotainer = jQuery('div.contentsDiv');
		}
		return nectarcrm_CustomView_Js.contentsCotainer;
	},

	getColumnListSelect2Element : function() {
		return nectarcrm_CustomView_Js.columnListSelect2Element;
	},
	...
	getColumnSelectElement : function() {
		if(nectarcrm_CustomView_Js.columnSelectElement == false) {
			nectarcrm_CustomView_Js.columnSelectElement = jQuery('#viewColumnsSelect');
		}
		return nectarcrm_CustomView_Js.columnSelectElement;
	},
	...
	getSelectedColumnsList : function() {
		if(nectarcrm_CustomView_Js.selectedColumnsList == false) {
			nectarcrm_CustomView_Js.selectedColumnsList = jQuery('#selectedColumnsList');
		}
		return nectarcrm_CustomView_Js.selectedColumnsList;
	},
	...
	makeColumnListSortable : function() {
		var select2Element = nectarcrm_CustomView_Js.getColumnListSelect2Element();
		...
		chozenChoiceElement.sortable({
                ...
                start: function() { nectarcrm_CustomView_Js.getSelectedColumnsList().select2("onSortStart"); },
                update: function() { nectarcrm_CustomView_Js.getSelectedColumnsList().select2("onSortEnd"); }
            });
	},
	...
	getSelectedColumns : function() {
		var columnListSelectElement = nectarcrm_CustomView_Js.getColumnSelectElement();
		var select2Element = nectarcrm_CustomView_Js.getColumnListSelect2Element();
	...
	arrangeSelectChoicesInOrder : function() {
		var contentsContainer = nectarcrm_CustomView_Js.getContentsContainer();
		var chosenElement = nectarcrm_CustomView_Js.getColumnListSelect2Element();
		var choicesContainer = chosenElement.find('ul.select2-choices');
		var choicesList = choicesContainer.find('li.select2-search-choice');
		var coulmnListSelectElement = nectarcrm_CustomView_Js.getColumnSelectElement();
	...
	saveAndViewFilter : function(){
		nectarcrm_CustomView_Js.saveFilter().then(
			function(response){
				if (response.success) {
					...
					nectarcrm_Helper_Js.showPnotify(params);
				}
			},
			function(error) {
	...
	registerSelect2ElementForColumnsSelection : function() {
		var selectElement = nectarcrm_CustomView_Js.getColumnSelectElement();
		app.changeSelectElementView(selectElement, 'select2', {maximumSelectionSize: 12,dropdownCss : {'z-index' : 0}});
	},

	registerEvents: function(){
		nectarcrm_CustomView_Js.registerSelect2ElementForColumnsSelection();
		var contentsContainer = nectarcrm_CustomView_Js.getContentsContainer();
		jQuery('.stndrdFilterDateSelect').datepicker();
		jQuery('.chzn-select').chosen();

		var select2Element = app.getSelect2ElementFromSelect(nectarcrm_CustomView_Js.getColumnSelectElement());
		nectarcrm_CustomView_Js.columnListSelect2Element = select2Element;

		//To arrange the chosen choices in the order that is selected
		nectarcrm_CustomView_Js.arrangeSelectChoicesInOrder();

		jQuery("#standardDateFilter").change(function(){
			nectarcrm_CustomView_Js.loadDateFilterValues();
		});

		nectarcrm_CustomView_Js.makeColumnListSortable();

		jQuery("#CustomView").submit(function(e) {
			var selectElement = nectarcrm_CustomView_Js.getColumnSelectElement();
			var select2Element = app.getSelect2ElementFromSelect(selectElement);
			var result = nectarcrm_MultiSelect_Validator_Js.invokeValidation(selectElement);
			if(result != true){
			...
			if(result == true){
				...
				var advfilterlist = nectarcrm_CustomView_Js.advanceFilterInstance.getValues();
				jQuery('#advfilterlist').val(JSON.stringify(advfilterlist));
				jQuery('input[name="columnslist"]', contentsContainer).val(JSON.stringify(nectarcrm_CustomView_Js.getSelectedColumns()));
				nectarcrm_CustomView_Js.saveAndViewFilter();
				return false;
			} else {
