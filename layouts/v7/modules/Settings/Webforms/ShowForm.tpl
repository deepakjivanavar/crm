		...
		<div class="modal-body">
			...
			<code>
				<pre>
					...
					<form id="__nectarcrmWebForm" name="{$RECORD_MODEL->getName()}" action="{$ACTION_PATH}" method="post" accept-charset="utf-8" enctype="multipart/form-data">
										...
										{else if $DATA_TYPE eq 'multipicklist'}
											...
											<select name="{$PICKLIST_NAME}[]" data-label="{$PICKLIST_DATA_LABEL}" {if $FIELD_MODEL->get('required') eq 1} required{/if} multiple style="width: 60%;" {if $FIELD_MODEL->get('hidden') eq 1} hidden{/if}>
												{foreach item=PICKLIST_VALUE from=$PICKLIST_VALUES}
													<option value="{$PICKLIST_VALUE}" {if in_array(nectarcrm_Util_Helper::toSafeHTML($PICKLIST_VALUE), $FIELD_VALUE_LIST)} selected {/if}>{vtranslate($PICKLIST_VALUE, $MODULE)}</option>
												{/foreach}
											</select>
