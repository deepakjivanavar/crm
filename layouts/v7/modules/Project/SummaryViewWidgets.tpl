...
{strip}
								...
								<div class="dropdown">
									...
									<ul class="dropdown-menu">
										...
										<li id="nectarcrmAction">
											<a href="javascript:Documents_Index_Js.uploadTo('nectarcrm',{$PARENT_ID},'{$MODULE_NAME}')">
												<img style="  margin-top: -3px;margin-right: 4%;" title="nectarcrm" alt="nectarcrm" src="layouts/v7/skins//images/nectarcrm.png">
												{vtranslate('LBL_TO_SERVICE', 'Documents', {vtranslate('LBL_NECTARCRM', 'Documents')})}
											</a>
										</li>
	...
	<div class="right-block col-lg-4 col-md-4 col-sm-4">
				...
				<div class="widgetContainer_troubleTickets" data-url="{$HELPDESK_WIDGET_MODEL->getUrl()}" data-name="{$HELPDESK_WIDGET_MODEL->getLabel()}">
					...
					<div class="clearfix">
						<div class="widget_filter clearfix">
							<div class="pull-left">
								{assign var=RELATED_MODULE_MODEL value=nectarcrm_Module_Model::getInstance('HelpDesk')}
								...
								{assign var=FIELD_INFO value=nectarcrm_Util_Helper::toSafeHTML(Zend_Json::encode($FIELD_INFO))}
								{assign var="SPECIAL_VALIDATOR" value=$FIELD_MODEL->getValidator()}
								<select class="select2" name="{$FIELD_MODEL->get('name')}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[nectarcrm_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} >
		...
		{if $TASKS_WIDGET_MODEL}
			{assign var=RELATED_MODULE_MODEL value=nectarcrm_Module_Model::getInstance('ProjectTask')}
						...
						<div class="widget_filter clearfix">
							{if $PROGRESS_FIELD_MODEL->isViewableInDetailView()}
								<div class="pull-left marginRight15">
									...
									<select class="select2" name="{$PROGRESS_FIELD_MODEL->get('name')}" data-validation-engine="validate[{if $PROGRESS_FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[nectarcrm_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} >
