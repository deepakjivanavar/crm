...
* All Rights Reserved.
************************************************************************************}
{* modules/nectarcrm/views/List.php *}
...
<div class="col-sm-12 col-xs-12 ">
	{if $MODULE neq 'EmailTemplates' && $SEARCH_MODE_RESULTS neq true}
		...
		<div class="essentials-toggle" title="{vtranslate('LBL_LEFT_PANEL_SHOW_HIDE', 'nectarcrm')}">
	...
	<input type="hidden" name="currentSearchParams" value="{nectarcrm_Util_Helper::toSafeHTML(Zend_JSON::encode($SEARCH_DETAILS))}" id="currentSearchParams" />
	<input type="hidden" name="currentTagParams" value="{nectarcrm_Util_Helper::toSafeHTML(Zend_JSON::encode($TAG_DETAILS))}" id="currentTagParams" />
	...
	{if !empty($PICKIST_DEPENDENCY_DATASOURCE)}
		<input type="hidden" name="picklistDependency" value='{nectarcrm_Util_Helper::toSafeHTML($PICKIST_DEPENDENCY_DATASOURCE)}' />
	{/if}
								...
								<div class="listColumnFilter {if $CURRENT_CV_MODEL and !($CURRENT_CV_MODEL->isCvEditable())}disabled{/if}"  
									 ...
									 {else}
										 ...
										 {elseif !$CURRENT_CV_MODEL->isMine()}
											 ...
											 {if !nectarcrm_Functions::isUserExist($CURRENT_CV_USER_ID)}
									...
									{else}
										...
										{else if $LISTVIEW_HEADER->getFieldDataType() eq 'picklist'}
											...
											{else}
												{if $LISTVIEW_HEADER->getName() eq 'taskstatus'}
													{assign var="EVENT_STATUS_FIELD_MODEL" value=nectarcrm_Field_Model::getInstance('eventstatus', nectarcrm_Module_Model::getInstance('Events'))}
												...
												{/if}
											{/if}
											<span {if !empty($LISTVIEW_ENTRY_VALUE)} class="picklist-color picklist-{$PICKLIST_FIELD_ID}-{nectarcrm_Util_Helper::convertSpaceToHyphen($LISTVIEW_ENTRY_RAWVALUE)}" {/if}> {$LISTVIEW_ENTRY_VALUE} </span>
										{else if $LISTVIEW_HEADER->getFieldDataType() eq 'multipicklist'}
											...
											{foreach item=MULTI_PICKLIST_VALUE key=MULTI_PICKLIST_INDEX from=$MULTI_PICKLIST_VALUES}
												<span {if !empty($LISTVIEW_ENTRY_VALUE)} class="picklist-color picklist-{$LISTVIEW_HEADER->getId()}-{nectarcrm_Util_Helper::convertSpaceToHyphen(trim($ALL_MULTI_PICKLIST_VALUES[trim($MULTI_PICKLIST_VALUE)]))}"{/if} > 
							...
							<div class="emptyRecordsContent">
								...
								{if $IS_CREATE_PERMITTED}
									...
									{if Users_Privileges_Model::isPermitted($MODULE, 'Import') && $LIST_VIEW_MODEL->isImportEnabled()}
										...
										<a style="color:blue" href="#" onclick="return nectarcrm_Import_Js.triggerImportAction()">{vtranslate('LBL_IMPORT', $MODULE)}</a>
