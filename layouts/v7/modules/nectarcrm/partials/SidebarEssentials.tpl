...
<div class="sidebar-menu">
            ...
            <div class="menu-scroller scrollContainer" style="position:relative; top:0; left:0;">
				<div class="list-menu-content">
                            ...
                            <div class="list-group" id="{if $GROUP_LABEL eq 'Mine'}myList{else}sharedList{/if}">   
								...
								<div class='clearfix'> 
									{if $smarty.foreach.customView.iteration - 10 - $count} 
										<a class="toggleFilterSize" data-more-text=" {$smarty.foreach.customView.iteration - 10 - $count} {vtranslate('LBL_MORE',nectarcrm)|@strtolower}" data-less-text="Show less">
											{if $smarty.foreach.customView.iteration gt 10} 
												{$smarty.foreach.customView.iteration - 10 - $count} {vtranslate('LBL_MORE',nectarcrm)|@strtolower} 
											{/if} 
										</a>{/if} 
									</div>
                             </div>
					{/foreach}
							...
							<input type="hidden" id='allFilterNames'  value='{nectarcrm_Util_Helper::toSafeHTML(Zend_JSON::encode($CUSTOM_VIEWS_NAMES))}'/>
    ...
    {assign var=EXTENSION_LINKS value=nectarcrm_Extension_View::getExtensionLinks($MODULE)}
    {if !empty($EXTENSION_LINKS)}
        <div class="module-filters module-extensions">
            <div class="sidebar-container lists-menu-container">
             {* Edited by Mahesh hide google tab
               <h5 class="sidebar-header">{vtranslate('LBL_EXTENSIONS',$MODULE)}</h5>
**End by Mahesh hide google and etesion tab**
                *}
                <hr>
                <div class="menu-scroller scrollContainer" style="position:relative; top:0; left:0;">
                    <div class="list-menu-content">
        ...
        <div class="sidebar-container lists-menu-container">
            ...
            <div class="menu-scroller scrollContainer" style="position:relative; top:0; left:0;">
                <div class="list-menu-content">
                    ...
                    data-list-tag-count="{nectarcrm_Tag_Model::NUM_OF_TAGS_LIST}">
                        {foreach item=TAG_MODEL from=$TAGS name=tagCounter}
                            ...
                            {if $smarty.foreach.tagCounter.iteration gt nectarcrm_Tag_Model::NUM_OF_TAGS_LIST}
                                {break}
                            {/if}
                            {include file="Tag.tpl"|vtemplate_path:$MODULE NO_DELETE=true ACTIVE= $CURRENT_TAG eq $TAG_ID}
                        {/foreach}
                        <div> 
                            <a class="moreTags {if (count($TAGS) - nectarcrm_Tag_Model::NUM_OF_TAGS_LIST) le 0} hide {/if}">
                                <span class="moreTagCount">{count($TAGS) - nectarcrm_Tag_Model::NUM_OF_TAGS_LIST}</span>
                                &nbsp;{vtranslate('LBL_MORE',$MODULE)|strtolower}
                            </a>
                            <div class="moreListTags hide">
                        {foreach item=TAG_MODEL from=$TAGS name=tagCounter}
                            {if $smarty.foreach.tagCounter.iteration le nectarcrm_Tag_Model::NUM_OF_TAGS_LIST}
                                {continue}
                            {/if}
                            {include file="Tag.tpl"|vtemplate_path:$MODULE NO_DELETE=true ACTIVE= $CURRENT_TAG eq $TAG_ID}
                        {/foreach}
                ...
                <div id="dummyTagElement" class="hide">
                    {assign var=TAG_MODEL value=nectarcrm_Tag_Model::getCleanInstance()}
                    ...
                    <div class="editTagContainer hide">
                        ...
                        <div class="editTagContents">
                            ...
                            <div>
                                <div class="checkbox">
                                    <label>
                                        <input type="hidden" name="visibility" value="{nectarcrm_Tag_Model::PRIVATE_TYPE}"/>
                                        <input type="checkbox" name="visibility" value="{nectarcrm_Tag_Model::PUBLIC_TYPE}" />
