...
<style type="text/css">
    ...
    {if ($MODULE_NAME eq 'Calendar' && $MODULE_MODEL) OR ($RELATED_MODULE_NAME  eq 'Calendar' && $RELATED_MODULE) OR ($SOURCE_MODULE eq 'Calendar')}
		{assign var=STATUS_FIELD value=nectarcrm_Field_Model::getInstance('eventstatus', nectarcrm_Module_Model::getInstance('Events'))}
        ...
        {if $STATUS_FIELD}
            ...
            {foreach item=PICKLIST_COLOR key=PICKLIST_VALUE from=$PICKLIST_COLOR_MAP}
                ...
                {assign var=CONVERTED_PICKLIST_VALUE value=nectarcrm_Util_Helper::convertSpaceToHyphen($PICKLIST_VALUE)}
                    .picklist-{$STATUS_FIELD->getId()}-{nectarcrm_Util_Helper::escapeCssSpecialCharacters($CONVERTED_PICKLIST_VALUE)} {
