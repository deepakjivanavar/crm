		...
		{foreach key=$index item=HISTORY from=$HISTORIES}
			...
			{if $MODELNAME == 'ModTracker_Record_Model'}
				...
				{if $PROCEED}
					...
					{if $TIME}<p class="pull-right muted" style="padding-right:10px;"><small title="{nectarcrm_Util_Helper::formatDateTimeIntoDayString("$TIME")}">{nectarcrm_Util_Helper::formatDateDiffInStrings("$TIME")}</small></p>{/if}
				</div>
			{/if}
			{else if $MODELNAME == 'ModComments_Record_Model'}
				<div class="row">
					...
					<p class="pull-right muted" style="padding-right:10px;"><small title="{nectarcrm_Util_Helper::formatDateTimeIntoDayString("$COMMENT_TIME")}">{nectarcrm_Util_Helper::formatDateDiffInStrings("$COMMENT_TIME")}</small></p>
				</div>
			{/if}
		{/foreach}
