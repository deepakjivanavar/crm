...
* All Rights Reserved.
************************************************************************************}
{* modules/nectarcrm/views/MassActionAjax.php *}
                                ...
                                {if $RECORDS_COUNT > 1}
                                    ...
                                    {foreach item=EMAIL_FIELD key=EMAIL_FIELD_NAME from=$EMAIL_FIELDS}
                                        <label class="checkbox" style="padding-left: 7%;">
                                            <input type="checkbox" class="emailField" name="selectedFields[{$count}]" data-moduleName="{$EMAIL_FIELD->getModule()->getName()}" value='{nectarcrm_Functions::jsonEncode(['field' => $EMAIL_FIELD_NAME, 'field_id' => $EMAIL_FIELD->getId(), 'module_id' => $EMAIL_FIELD->getModule()->getId()])}' {if $EMAIL_FIELD->get('isPreferred')}checked="true"{/if}/>
                                    ...
                                    {foreach item=RECORD_INFO key=EMAIL_MODULE from=$EMAIL_FIELDS_INFO}
                                        <div class="control-group">
                                            {foreach item=EMAIL_FIELDS key=RECORD_ID from=$RECORD_INFO}
                                                {assign var=RECORD_LABEL value={decode_html(textlength_check(nectarcrm_Util_Helper::getRecordName($RECORD_ID)))}}
                                                ...
                                                {foreach item=EMAIL_FIELD  key=EMAIL_VALUE from=$EMAIL_FIELDS}
                                                    <label class="checkbox" style="{if $RECORDS_COUNT > 1}padding-left: 7%;{/if} font-weight:normal">
                                                        <input type="checkbox" class="emailField" name="selectedFields[{$count}]" data-moduleName="{$EMAIL_MODULE}" value='{nectarcrm_Functions::jsonEncode(['record'=>$RECORD_ID,'field_value'=>$EMAIL_VALUE,'record_label'=>$RECORD_LABEL,'field_id'=> $EMAIL_FIELD->getId(),'module_id'=>$EMAIL_FIELD->getModule()->getId()])}' {if $EMAIL_FIELD->get('isPreferred')}checked="true"{/if}/>
