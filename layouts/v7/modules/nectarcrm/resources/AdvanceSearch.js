...
nectarcrm_BasicSearch_Js("nectarcrm_AdvanceSearch_Js",{
	...
	getAdvanceSearch : function() {
		...
		//Exists in the cache
		if(searchModule in nectarcrm_AdvanceSearch_Js.cache) {
			aDeferred.resolve(nectarcrm_AdvanceSearch_Js.cache[searchModule]);
			return aDeferred.promise();
		}
        
        //if you are in settings then module should be nectarcrm
        if(app.getParentModuleName().length > 0) {
            moduleName = 'nectarcrm';
        }
		...
		app.request.post({data:searchableModulesParams}).then(
			function(err, data){
				...
				//add to cache
				nectarcrm_AdvanceSearch_Js.cache[searchModule] = data;
		...
		this.getAdvanceSearch().then(
			function(data){
                thisInstance.showAdvanceSearch(data).then(function(){
                    ...
                    thisInstance.advanceFilter = new nectarcrm_SearchAdvanceFilter_Js(jQuery('.filterContainer'));
    ...
    initiateListInstance : function(container)   {
        var listInstance = new nectarcrm_AdvanceSearchList_Js();
