...
jQuery.Class("nectarcrm_RelatedList_Js",{
	loaded : true,
	...
	getInstance: function(parentId, parentModule, selectedRelatedTabElement, relatedModuleName) {
		...
		var fallbackClassName = nectarcrm_RelatedList_Js;
	...
	updateRelatedRecordsCount: function (relationId) {
		...
		var detailInstance = new nectarcrm_Detail_Js();
    ...
    showSelectRelationPopup : function(){
        ...
        var popupjs = new nectarcrm_Popup_Js();
	...
	registerScrollForRollupComments : function() {
        jQuery(document).scroll(function() {
            if ($(window).scrollTop() + $(window).height() >= $(document).height() - 30
                ...
                if(nectarcrm_RelatedList_Js.loaded && jQuery('#rollupcomments').attr('hascomments') == 1) {
                    nectarcrm_RelatedList_Js.loaded = false;
					app.helper.showProgress();
                    ...
                    var url = 'index.php?module=nectarcrm&view=ModCommentsDetailAjax&parent='+
                    ...
                    app.request.get(params).then(function(err, data){
						nectarcrm_RelatedList_Js.loaded = true;
						app.helper.hideProgress();
						if(data) {
	...
	initializePaginationEvents : function() {
		var thisInstance = this;
		var paginationObj = new nectarcrm_Pagination_Js;
...
jQuery(document).ready(function(){
	var recordId = app.getRecordId();
	var moduleName = app.getModuleName();
        var detailViewInstance = nectarcrm_Detail_Js.getInstance();
        var selectedTabElement = detailViewInstance.getSelectedTab();
        var relatedModuleName = detailViewInstance.getRelatedModuleName();
            var instance = nectarcrm_RelatedList_Js.getInstance(recordId, moduleName, selectedTabElement, relatedModuleName);
