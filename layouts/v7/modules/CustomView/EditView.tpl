			...
			<div class="modal-content" style="height:100%">
				...
				<div class="modal-body" style="height:100%">
					<div class="customview-content row" style="height:90%">
						...
						{/if}
						<input type="hidden" name="date_filters" data-value='{nectarcrm_Util_Helper::toSafeHTML(ZEND_JSON::encode($DATE_FILTERS))}' />
						<div class="form-group">
							<label>{vtranslate('LBL_VIEW_NAME',$MODULE)}&nbsp;<span class="redColor">*</span> </label>
							<div class="row">
								<div class="col-lg-5 col-md-5 col-sm-5">
									<input class="form-control" type="text" data-record-id="{$RECORD_ID}" id="viewname" name="viewname" value="{$CUSTOMVIEW_MODEL->get('viewname')}" data-rule-required="true" data-rule-maxsize="100" data-rule-check-filter-duplicate='{nectarcrm_Util_Helper::toSafeHTML(Zend_JSON::encode($CUSTOM_VIEWS_LIST))}'>
								</div>
								<div class="col-lg-5 col-md-5 col-sm-5">
						...
						<div class="form-group">
							...
							<div class="columnsSelectDiv clearfix">
										...
										<optgroup label='{vtranslate($BLOCK_LABEL, $SOURCE_MODULE)}'>
											{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS}
												...
												<option value="{$FIELD_MODEL->getCustomViewColumnName()}" data-field-name="{$FIELD_NAME}"
													...
													{/if}
													>{nectarcrm_Util_Helper::toSafeHTML(vtranslate($FIELD_MODEL->get('label'), $SOURCE_MODULE))}
								...
								<input type="hidden" name="columnslist" value='{nectarcrm_Functions::jsonEncode($SELECTED_FIELDS)}' />
								<input id="mandatoryFieldsList" type="hidden" value='{nectarcrm_Util_Helper::toSafeHTML(ZEND_JSON::encode($MANDATORY_FIELDS))}' />
							</div>
