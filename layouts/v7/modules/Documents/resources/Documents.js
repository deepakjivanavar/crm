...
nectarcrm.Class('Documents_Index_Js', {
	...
	reloadListView : function() {
		...
		var list = nectarcrm_List_Js.getInstance();
	...
	reloadRelatedListView : function() {
		...
		var relatedList = nectarcrm_RelatedList_Js.getInstance(parentId, parentModule, selectedRelatedTabElement, relatedModuleName);
	...
	uploadFileTonectarcrm : function(container) {
		var self = this;
		var file = this.getFile();
		if(!file) {
	...
	postQuickCreateSave: function (data) {
		...
		var container = nectarcrmInstance.getParentElement(jQuery('[name="'+Documents_Index_Js.referenceFieldName+'"]'));
		var module = nectarcrmInstance.getReferencedModuleName(container);
		var params = {};
		params.name = data._recordLabel;
		params.id = data._recordId;
		params.module = module;
		nectarcrmInstance.setReferenceFieldValue(container, params);

		var tdElement = nectarcrmInstance.getParentElement(container.find('[value="' + module + '"]'));
		var sourceField = tdElement.find('input[class="sourceField"]').attr('name');
		var fieldElement = tdElement.find('input[name="' + sourceField + '"]');
		nectarcrmInstance.autoFillElement = fieldElement;
		var parentModule = jQuery('.editViewContents [name=module]').val();
		if (parentModule != "Events") {
			nectarcrmInstance.postRefrenceSearch(params, container);
		}
	...
	registerUploadDocumentEvents : function(container) {
		...
		container.find('form').vtValidate({
			'submitHandler' : function() {
				self.uploadFileTonectarcrm(container);
				return false;
			}
		});
	...
	showUploadTonectarcrmModal : function(parentId,relatedModule) {
		var self = this;
		var url = 'index.php?module=Documents&view=QuickCreateAjax&service=nectarcrm&operation=UploadTonectarcrm&type=I';
	...
	uploadTo : function(service,parentId,relatedModule) {
		this.setFile(false);
		this.showUploadTonectarcrmModal(parentId,relatedModule);
	},
	...
	applyEditor : function(element) {
		var cke = new nectarcrm_CkEditor_Js();
	...
	registerQuickCreateEvents : function(container) {
		var nectarcrmInstance = nectarcrm_Index_Js.getInstance();
		nectarcrmInstance.registerReferenceCreate(container);
		nectarcrmInstance.registerPostReferenceEvent(container);
		nectarcrmInstance.referenceModulePopupRegisterEvent(container);
		nectarcrmInstance.registerClearReferenceSelectionEvent(container);
		nectarcrmInstance.registerAutoCompleteFields(container);
		app.helper.registerModalDismissWithoutSubmit(container.find('form'));
		var moduleInstance = nectarcrm_Edit_Js.getInstanceByModuleName('Documents');
		...
		app.event.on('post.documents.save', function(event, data){
			...
			if(relatedTabs.length > 0){
				...
				var relatedModuleName = jQuery('.relatedModuleName').val();
				var relatedInstance = new nectarcrm_RelatedList_Js(app.getRecordId(), app.getModuleName(), tabElement, relatedModuleName);
