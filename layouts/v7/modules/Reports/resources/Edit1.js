		...
		jQuery('#annualDatePicker').datepick({autoSize: true, multiSelect:100,monthsToShow: [1,2],
				...
				onSelect : function(dates) {
					...
					for(index in dates) {
						...
						html += '<option selected value='+thisInstance.DateToYMD(date)+'>'+app.getDateInnectarcrmFormat(dateFormat,date)+'</option>';
					}
	...
	updateAnnualDates : function(annualDatesEle) {
		...
		if(annualDatesJSON) {
			...
			for(i in annualDates) {
				...
				hiddenDates += '<option selected value='+annualDates[i]+'>'+app.getDateInnectarcrmFormat(dateFormat,dateInstance)+'</option>';
			}
