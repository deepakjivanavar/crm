...
if (!$errorMessage) {
	...
	if (version_compare($nectarcrm_current_version, '7.0.0') < 0) {
		...
		$nectarcrmStandardModules = array('Accounts', 'Assets', 'Calendar', 'Campaigns', 'Contacts', 'CustomerPortal', 
		...
		$skipCheckForModules = array_merge($skipCheckForModules, $nectarcrmStandardModules);

		$nonPortedExtns = array();
		$moduleModelsList = array();
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT name FROM nectarcrm_tab WHERE isentitytype != ? AND presence != ? AND trim(name) NOT IN ('.generateQuestionMarks($skipCheckForModules).')', array(1, 1, $skipCheckForModules));
		...
		if ($moduleModelsList) {
			while($row = $db->fetch_row($result)) {
				...
				if ($moduleName) {
					...
					if ($moduleModel) {
						$nectarcrmVersion = $moduleModel->get('nectarcrmVersion');
						$nectarcrmMaxVersion = $moduleModel->get('nectarcrmMaxVersion');
						if (($nectarcrmVersion && strpos($nectarcrmVersion, '7.') === false)
								&& ($nectarcrmMaxVersion && strpos($nectarcrmMaxVersion, '7.') === false)) {
			...
			if ($nonPortedExtns) {
				$portingMessage = 'Following custom modules are not compatible with nectarcrm 7. Please disable these modules to proceed.';
...
<html>
	<head>
		<title>nectarcrm CRM Setup</title>
	...
	<body style="font-size: 14px !important;">
		<div class="container-fluid page-container">
			<div class="row">
				<div class="col-lg-6">
					<div class="logo">
						<img src="resources/images/vt1.png" alt="nectarcrm Logo"/>
					</div>
				</div>
				<div class="col-lg-6">
			...
			<div class="row main-container">
				<div class="col-lg-12 inner-container">
					<div class="row">
						...
						<div class="col-lg-2">
							<a href="https://wiki.nectarcrm.com/nectarcrm7/" target="_blank" class="pull-right">
					...
					<div class="row">
						<div class="col-lg-4 welcome-image">
							<img src="resources/images/migration_screen.png" alt="nectarcrm Logo" style="width: 100%; margin-left: 15px;"/>
						</div>
						<?php
							$currentVersion = explode('.', $nectarcrm_current_version);
							 if ($portingMessage) { ?>
							...
							<div class="button-container col-lg-12">
								<div class="pull-right">
									<form action="../index.php?module=Migration&action=DisableModules&mode=fromMig" method="POST">
										<input type="hidden" name="modulesList" <?php echo 'value="'.nectarcrm_Util_Helper::toSafeHTML(Zend_JSON::encode($nonPortedExtns)).'"'; ?> />
						...
						<?php } else if($currentVersion[0] >= 6 && $currentVersion[1] >= 0) { ?>
							<div class="col-lg-8" style="padding-left: 30px;">
								<h3> Welcome to nectarcrm Migration</h3>
								<?php if(isset($errorMessage)) {
									echo '<span><font color="red"><b>'.filter_var($errorMessage, FILTER_SANITIZE_STRING).'</b></font></span><br><br>';
								} ?>
								<p>We have detected that you have <strong>nectarcrm <?php echo $nectarcrm_current_version ?></strong> installation.<br><br></p>
								<p>
									<strong>Warning: </strong>
									Please note that it is not possible to revert back to <?php echo $nectarcrm_current_version ?>&nbsp;after the upgrade to nectarcrm 7 <br>
									So, it is important to take a backup of the <?php echo $nectarcrm_current_version ?> installation, including the source files and database.
								</p><br>
								<form action="../index.php?module=Migration&action=Extract&mode=fromMig" method="POST">
									<div><input type="checkbox" id="checkBox1" name="checkBox1"/><div class="chkbox"></div> I have taken the backup of database <a href="http://community.nectarcrm.com/help/nectarcrmcrm/administrators/backup.html" target="_blank" >(how to?)</a></div><br>
									<div><input type="checkbox" id="checkBox2" name="checkBox2"/><div class="chkbox"></div> I have taken the backup of source folder <a href="http://community.nectarcrm.com/help/nectarcrmcrm/administrators/backup.html" target="_blank" >(how to?)</a></div><br>
									<br>
									<div>
										<span id="error"></span>
						...
						<?php } else if($currentVersion[0] < 6) { ?>
							...
							<div class="col-lg-7">
								<h3><font color="red">WARNING : Cannot continue with Migration</font></h3><br>
								<p>We detected that this installation is running <strong>nectarcrm CRM</strong>
										<?php
											if($nectarcrm_current_version < 6 ) {
												echo '<b>'.$nectarcrm_current_version.'</b>';
											}
										?>.
