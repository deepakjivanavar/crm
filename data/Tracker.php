 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/nectarcrm_crm/sugarcrm/data/Tracker.php,v 1.15 2005/04/28 05:44:22 samk Exp $
...
class Tracker {
    var $log;
    var $db;
    var $table_name = "nectarcrm_tracker";

    // Tracker nectarcrm_table
    ...
    /**
     * Add this new item to the nectarcrm_tracker nectarcrm_table.  If there are too many items (global config for now)
...
//No genius required. Just add an if case and change the query so that it puts the tracker entry whenever you touch on the DetailView of the required entity
	 ...
	 #if condition added to skip nectarcrm_faq in last viewed history
     ...
     * return - return the array of result set rows from the query.  All of the nectarcrm_table nectarcrm_fields are included
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.
 * All Rights Reserved.
 * Contributor(s): ______________________________________..
     */
...
//        $query = "SELECT * from $this->table_name WHERE user_id='$user_id' ORDER BY id DESC";
	$query = "SELECT * from $this->table_name inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_tracker.item_id WHERE user_id=? and nectarcrm_crmentity.deleted=0 ORDER BY id DESC";
