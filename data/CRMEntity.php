 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/nectarcrm_crm/nectarcrmcrm/data/CRMEntity.php,v 1.16 2005/04/29 04:21:31 mickie Exp $
...
class CRMEntity {
	...
	static function isBulkSaveMode() {
		global $NECTARCRM_BULK_SAVE_MODE;
		if (isset($NECTARCRM_BULK_SAVE_MODE) && $NECTARCRM_BULK_SAVE_MODE) {
			return true;
		}
		return false;
	}
	...
	static function isTimeStampNoChangeMode(){
		global $NECTARCRM_TIMESTAMP_NO_CHANGE_MODE;
		if (isset($NECTARCRM_TIMESTAMP_NO_CHANGE_MODE) && $NECTARCRM_TIMESTAMP_NO_CHANGE_MODE) {
			return true;
		}
		return false;
	}
	...
	function initialize() {
		...
		$userSpecificTable = nectarcrm_Functions::getUserSpecificTableName($moduleName);
	...
	function saveentity($module, $fileid = '') {
		...
		if(count($_FILES)) {
			$_FILES = nectarcrm_Util_Helper::transformUploadedFiles($_FILES, true);
		}

		$this->db->startTransaction();
		foreach ($this->tab_name as $table_name) {
			if ($table_name == "nectarcrm_crmentity") {
	...
	function uploadAndSaveFile($id, $module, $file_details, $attachmentType='Attachment') {
		...
		$current_id = $adb->getUniqueID("nectarcrm_crmentity");
		...
		if ($save_file == 'true' && $upload_status == 'true') {
			if($attachmentType != 'Image' && $this->mode == 'edit') {
				...
				$res = $adb->pquery('SELECT nectarcrm_seattachmentsrel.attachmentsid FROM nectarcrm_seattachmentsrel 
									INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_seattachmentsrel.attachmentsid AND nectarcrm_crmentity.setype = ? 
									WHERE nectarcrm_seattachmentsrel.crmid = ?',array($module.' Attachment',$id));
				...
				if(count($oldAttachmentIds)) {
					$adb->pquery('DELETE FROM nectarcrm_seattachmentsrel WHERE attachmentsid IN ('.generateQuestionMarks($oldAttachmentIds).')',$oldAttachmentIds);
					...
					//$adb->pquery('DELETE FROM nectarcrm_attachments WHERE attachmentsid IN ('.generateQuestionMarks($oldAttachmentIds).')',$oldAttachmentIds);
					//$adb->pquery('DELETE FROM nectarcrm_crmentity WHERE crmid IN ('.generateQuestionMarks($oldAttachmentIds).')',$oldAttachmentIds);
				}
			}
			//Add entry to crmentity
			$sql1 = "INSERT INTO nectarcrm_crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) VALUES (?, ?, ?, ?, ?, ?, ?)";
			...
			$sql2 = "INSERT INTO nectarcrm_attachments(attachmentsid, name, description, type, path) values(?, ?, ?, ?, ?)";
			$params2 = array($current_id, $filename, $this->column_fields['description'], $filetype, $upload_file_path);
			$adb->pquery($sql2, $params2);
			//Add relation
			$sql3 = 'INSERT INTO nectarcrm_seattachmentsrel VALUES(?,?)';
	...
	/** Function to insert values in the nectarcrm_crmentity for the specified module
	...
	function insertIntoCrmEntity($module, $fileid = '') {
		...
		$log->debug('default_charset : '.$default_charset);
			...
			$acl = nectarcrm_AccessControl::loadUserPrivileges($current_user->id);
			if ($acl->is_admin == true || $acl->profileGlobalPermission[1] == 0 || $acl->profileGlobalPermission[2] == 0 || $this->isWorkFlowFieldUpdate) {
				$sql = "update nectarcrm_crmentity set smownerid=?, smgroupid=?,modifiedby=?,description=?, modifiedtime=? where crmid=?";
				$params = array($ownerid, $groupid, $current_user->id, $description_val, $adb->formatDate($date_var, true), $this->id);
			} else {
				$profileList = getCurrentUserProfileList();
				$perm_qry = "SELECT columnname FROM nectarcrm_field INNER JOIN nectarcrm_profile2field ON nectarcrm_profile2field.fieldid = nectarcrm_field.fieldid INNER JOIN nectarcrm_def_org_field ON nectarcrm_def_org_field.fieldid = nectarcrm_field.fieldid WHERE nectarcrm_field.tabid = ? AND nectarcrm_profile2field.visible = 0 AND nectarcrm_profile2field.readonly = 0 AND nectarcrm_profile2field.profileid IN (" . generateQuestionMarks($profileList) . ") AND nectarcrm_def_org_field.visible = 0 and nectarcrm_field.tablename='nectarcrm_crmentity' and nectarcrm_field.displaytype in (1,3) and nectarcrm_field.presence in (0,2);";
				...
				if (is_array($columname) && in_array("description", $columname)) {
					...
					$sql = "update nectarcrm_crmentity set smownerid=?, smgroupid=?,modifiedby=?, modifiedtime=? where crmid=?";
			...
			$description_val = from_html($this->column_fields['description'], ($insertion_mode == 'edit') ? true : false);
			$sql = "insert into nectarcrm_crmentity (crmid,smcreatorid,smownerid,smgroupid,setype,description,modifiedby,createdtime,modifiedtime,source) values(?,?,?,?,?,?,?,?,?,?)";
	...
	function insertIntoEntityTable($table_name, $module, $fileid = '') {
		...
		$log->info("function insertIntoEntityTable " . $module . ' nectarcrm_table name ' . $table_name);
		global $adb;
		$insertion_mode = $this->mode;

		//Checkin whether an entry is already is present in the nectarcrm_table to update
		if ($insertion_mode == 'edit') {
			...
			if (nectarcrm_Functions::isUserSpecificFieldTable($table_name, $module)) {
		...
		if ($insertion_mode == 'edit') {
			...
			if ($acl->is_admin == true || $acl->profileGlobalPermission[1] == 0 || $acl->profileGlobalPermission[2] == 0 || $this->isWorkFlowFieldUpdate) {
				$sql = "select fieldname,columnname,uitype,generatedtype,
										typeofdata from nectarcrm_field where tabid in (" . generateQuestionMarks($tabid) . ") and tablename=? and displaytype in (1,3,6) and presence in (0,2) group by columnname";
				$params = array($tabid, $table_name);
			} else {
				$profileList = getCurrentUserProfileList();

				if (count($profileList) > 0) {
					$sql = "SELECT nectarcrm_field.fieldname,nectarcrm_field.columnname,nectarcrm_field.uitype,nectarcrm_field.generatedtype,nectarcrm_field.typeofdata FROM nectarcrm_field
						INNER JOIN nectarcrm_profile2field
						ON nectarcrm_profile2field.fieldid = nectarcrm_field.fieldid
						INNER JOIN nectarcrm_def_org_field
						ON nectarcrm_def_org_field.fieldid = nectarcrm_field.fieldid
						WHERE nectarcrm_field.tabid = ?
						AND nectarcrm_profile2field.visible = 0 AND nectarcrm_profile2field.readonly = 0
						AND nectarcrm_profile2field.profileid IN (" . generateQuestionMarks($profileList) . ")
						AND nectarcrm_def_org_field.visible = 0 and nectarcrm_field.tablename=? and nectarcrm_field.displaytype in (1,3,6) and nectarcrm_field.presence in (0,2) group by columnname";
			...
			if ($table_index_column == 'id' && $table_name == 'nectarcrm_users') {
				$currentuser_id = $adb->getUniqueID("nectarcrm_users");
			...
			$sql = "select fieldname,columnname,uitype,generatedtype,typeofdata from nectarcrm_field where tabid=? and tablename=? and displaytype in (1,3,4,6) and nectarcrm_field.presence in (0,2)";
			$params = array($tabid, $table_name);
		}
			...
			if (isset($this->column_fields[$fieldname])) {
				if ($uitype == 56) {
					...
					if ($this->column_fields[$fieldname] == null) {
						$fileQuery = $adb->pquery("SELECT filename from nectarcrm_notes WHERE notesid = ?", array($this->id));
						$fldvalue = null;
						if (isset($fileQuery)) {
					...
					if (empty($fldvalue)) {
						$query = "SELECT email1 FROM nectarcrm_users WHERE id = ?";
					...
					if(count($_FILES)) {
						...
						if($_REQUEST['action'] == 'MassSave' || $_REQUEST['action'] == 'MassEditSave') {
							if($IMG_FILES[0]['error'] == 0) {
								...
								$oldAttachmentsRes = $adb->pquery('SELECT nectarcrm_seattachmentsrel.attachmentsid FROM nectarcrm_seattachmentsrel 
									INNER JOIN nectarcrm_attachments ON nectarcrm_seattachmentsrel.attachmentsid = nectarcrm_attachments.attachmentsid 
									INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_attachments.attachmentsid AND setype = ? 
									WHERE nectarcrm_seattachmentsrel.crmid = ?',array($module.' Image',$this->id));
								...
								if(count($oldImageAttachmentIds)) {
									$adb->pquery('DELETE FROM nectarcrm_seattachmentsrel WHERE attachmentsid IN ('.generateQuestionMarks($oldImageAttachmentIds).')',$oldImageAttachmentIds);
									$adb->pquery('DELETE FROM nectarcrm_attachments WHERE attachmentsid IN ('.generateQuestionMarks($oldImageAttachmentIds).')',$oldImageAttachmentIds);
									$adb->pquery('DELETE FROM nectarcrm_crmentity WHERE crmid IN ('.generateQuestionMarks($oldImageAttachmentIds).')',$oldImageAttachmentIds);
								}
							}
						}
					...
					if (($insertion_mode == 'edit' && $skipUpdateForField == false) || $_REQUEST['imgDeleted']) {
						$skipUpdateForField = false;
						$uploadedFileNames = array();
						$getImageNamesSql = 'SELECT name FROM nectarcrm_seattachmentsrel INNER JOIN nectarcrm_attachments ON
						nectarcrm_seattachmentsrel.attachmentsid = nectarcrm_attachments.attachmentsid LEFT JOIN '.$table_name.' ON '.
						$table_name.'.'.$tablekey.' = nectarcrm_seattachmentsrel.crmid WHERE nectarcrm_seattachmentsrel.crmid = ?';
	 ...
	 * The function will get the attachmentsid for the given entityid from nectarcrm_seattachmentsrel table and get the attachmentsname from nectarcrm_attachments table
	...
	function getOldFileName($notesid) {
		...
		global $adb;
		$query1 = "select * from nectarcrm_seattachmentsrel where crmid=?";
		$result = $adb->pquery($query1, array($notesid));
		$noofrows = $adb->num_rows($result);
		if ($noofrows != 0)
			$attachmentid = $adb->query_result($result, 0, 'attachmentsid');
		if ($attachmentid != '') {
			$query2 = "select * from nectarcrm_attachments where attachmentsid=?";
	...
	function retrieve_entity_info($record, $module, $allowDeleted = false) {
		...
		if (isset($this->multirow_tables)) {
			...
			$multirow_tables = array(
				'nectarcrm_campaignrelstatus',
				'nectarcrm_attachments',
				//'nectarcrm_inventoryproductrel',
				//'nectarcrm_cntactivityrel',
				'nectarcrm_email_track'
			);
		}
		...
		if ($cachedModuleFields === false) {
			...
			$sql0 = "SELECT fieldname, fieldid, fieldlabel, columnname, tablename, uitype, typeofdata,presence FROM nectarcrm_field WHERE tabid=?";
		...
		if ($cachedModuleFields) {
			...
			foreach ($cachedModuleFields as $fieldinfo) {
				...
				// Added to avoid picking shipping tax fields for Inventory modules, the shipping tax detail are stored in nectarcrm_inventoryshippingrel
				// table, but in nectarcrm_field table we have set tablename as nectarcrm_inventoryproductrel.
			...
			$column_clause .= 'nectarcrm_crmentity.deleted, nectarcrm_crmentity.label';

			if (isset($required_tables['nectarcrm_crmentity'])) {
				$from_clause  = ' nectarcrm_crmentity';
				unset($required_tables['nectarcrm_crmentity']);
				foreach ($required_tables as $tablename => $tableindex) {
					if (in_array($tablename, $multirow_tables)) {
						// Avoid multirow table joins.
						continue;
					}
					$joinCondition = "($tablename.$tableindex = nectarcrm_crmentity.crmid ";
					if($current_user && nectarcrm_Functions::isUserSpecificFieldTable($tablename, $module)) {
			...
			$where_clause .= ' nectarcrm_crmentity.crmid=?';
			$params[] = $record;
			...
			if (!$result || $adb->num_rows($result) < 1) {
				...
				if(!empty($resultrow['label'])){
					...
					$entityFields = nectarcrm_Functions::getEntityModuleInfo($module);
	...
	function save($module_name, $fileid = '') {
		...
		if(!self::isBulkSaveMode()) {
			...
			$em->triggerEvent("nectarcrm.entity.beforesave.modifiable", $entityData);
			$em->triggerEvent("nectarcrm.entity.beforesave", $entityData);
			$em->triggerEvent("nectarcrm.entity.beforesave.final", $entityData);
		}
		...
		if($em) {
			//Event triggering code
			$em->triggerEvent("nectarcrm.entity.aftersave", $entityData);
			$em->triggerEvent("nectarcrm.entity.aftersave.final", $entityData);
			//Event triggering code ends
		}
	}
...
//Added by Richie
	...
	function mark_deleted($id) {
		global $current_user;
		$date_var = date("Y-m-d H:i:s");
		$query = "UPDATE nectarcrm_crmentity set deleted=1,modifiedtime=?,modifiedby=? where crmid=?";
	...
	// the keys are user defined, and don't directly map to the bean's nectarcrm_fields
	// the value is the method name within that bean that will do extra
	// processing for that nectarcrm_field. example: 'full_name'=>'get_names_from_full_name'
	...
	/**
	 * Function to check if the custom nectarcrm_field nectarcrm_table exists
	...
	/**
	 * function to construct the query to fetch the custom nectarcrm_fields
	 * return the query to fetch the custom nectarcrm_fields
	 */
	function constructCustomQueryAddendum($tablename, $module) {
		global $adb;
		$tabid = getTabid($module);
		$sql1 = "select columnname,fieldlabel from nectarcrm_field where generatedtype=2 and tabid=? and nectarcrm_field.presence in (0,2)";
	...
	function track_view($user_id, $current_module, $id = '') {
		$this->log->debug("About to call nectarcrm_tracker (user_id, module_name, item_id)($user_id, $current_module, $this->id)");
	...
	function initRequiredFields($module) {
		...
		$tabid = getTabId($module);
		$sql = "select * from nectarcrm_field where tabid= ? and typeofdata like '%M%' and uitype not in ('53','70') and nectarcrm_field.presence in (0,2)";
	...
	function trash($module, $id) {
		...
		if(!self::isBulkSaveMode()) {
			...
			$em->triggerEvent("nectarcrm.entity.beforedelete", $entityData);
		}
		...
		$sql_recentviewed = 'DELETE FROM nectarcrm_tracker WHERE user_id = ? AND item_id = ?';
		$this->db->pquery($sql_recentviewed, array($current_user->id, $id));

		if($em){
			$em->triggerEvent("nectarcrm.entity.afterdelete", $entityData);
		}
	}
	...
	function unlinkDependencies($module, $id) {
		...
		$fieldRes = $this->db->pquery('SELECT tabid, tablename, columnname FROM nectarcrm_field WHERE fieldid IN (
			SELECT fieldid FROM nectarcrm_fieldmodulerel WHERE relmodule=?)', array($module));
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Documents') {
			...
			$query = 'DELETE FROM nectarcrm_crmentityrel WHERE (crmid=? AND relmodule=? AND relcrmid=?) OR (relcrmid=? AND module=? AND crmid=?)';
			$params = array($id, $return_module, $return_id, $id, $return_module, $return_id);
			$this->db->pquery($query, $params);

			$fieldRes = $this->db->pquery('SELECT tabid, tablename, columnname FROM nectarcrm_field WHERE fieldid IN (SELECT fieldid FROM nectarcrm_fieldmodulerel WHERE module=? AND relmodule=?)', array($currentModule, $return_module));
			$numOfFields = $this->db->num_rows($fieldRes);
			for ($i = 0; $i < $numOfFields; $i++) {
				$tabId = $this->db->query_result($fieldRes, $i, 'tabid');
				$tableName = $this->db->query_result($fieldRes, $i, 'tablename');
	...
	function restore($module, $id) {
		...
		$em->triggerEvent("nectarcrm.entity.beforerestore", $entityData);

		$date_var = date("Y-m-d H:i:s");
		$query = 'UPDATE nectarcrm_crmentity SET deleted=0,modifiedtime=?,modifiedby=? WHERE crmid = ?';
		...
		//Event triggering code
		$em->triggerEvent("nectarcrm.entity.afterrestore", $entityData);
		//Event triggering code ends
	...
	function restoreRelatedRecords($module, $record) {
		...
		$result = $this->db->pquery('SELECT * FROM nectarcrm_relatedlists_rb WHERE entityid = ?', array($record));
		...
		for ($i = 0; $i < $numRows; $i++) {
			...
			if (strtoupper($action) == RB_RECORD_UPDATED) {
				...
				if ($rel_table == 'nectarcrm_crmentity' && $rel_column == 'deleted') {
				...
				if ($rel_table == 'nectarcrm_seproductsrel') {
		...
		$this->db->pquery('DELETE FROM nectarcrm_relatedlists_rb WHERE entityid = ?', array($record));
	}
	...
	function initSortByField($module) {
		...
		$sql = "SELECT columnname FROM nectarcrm_field " .
				" WHERE (fieldname not like '%\_id' OR fieldname in ('assigned_user_id'))" .
				" AND tabid in (" . generateQuestionMarks($tabid) . ") and nectarcrm_field.presence in (0,2)";
		$params = array($tabid);
	...
	function setModuleSeqNumber($mode, $module, $req_str = '', $req_no = '') {
		...
		if ($mode == "configure" && $req_no != '') {
			$check = $adb->pquery("select cur_id from nectarcrm_modentity_num where semodule=? and prefix = ?", array($module, $req_str));
			if ($adb->num_rows($check) == 0) {
				$numid = $adb->getUniqueId("nectarcrm_modentity_num");
				$active = $adb->pquery("select num_id from nectarcrm_modentity_num where semodule=? and active=1", array($module));
				$adb->pquery("UPDATE nectarcrm_modentity_num SET active=0 where num_id=?", array($adb->query_result($active, 0, 'num_id')));

				$adb->pquery("INSERT into nectarcrm_modentity_num values(?,?,?,?,?,?)", array($numid, $module, $req_str, $req_no, $req_no, 1));
				return true;
				...
				if ($req_no < $num_check) {
					...
					$adb->pquery("UPDATE nectarcrm_modentity_num SET active=0 where active=1 and semodule=?", array($module));
					$adb->pquery("UPDATE nectarcrm_modentity_num SET cur_id=?, active = 1 where prefix=? and semodule=?", array($req_no, $req_str, $module));
					return true;
				}
			}
		} else if ($mode == "increment") {
			//when we save new invoice we will increment the invoice id and write
			$check = $adb->pquery("select cur_id,prefix from nectarcrm_modentity_num where semodule=? and active = 1", array($module));
			...
			$req_no.= $temp . ($curid + 1);
			$adb->pquery("UPDATE nectarcrm_modentity_num SET cur_id=? where cur_id=? and active=1 AND semodule=?", array($req_no, $curid, $module));
	...
	function isModuleSequenceConfigured($module) {
		...
		$result = $adb->pquery('SELECT 1 FROM nectarcrm_modentity_num WHERE semodule = ? AND active = 1', array($module));
	...
	function updateMissingSeqNumber($module) {
		...
		$tabid = getTabid($module);
		$fieldinfo = $adb->pquery("SELECT tablename, columnname FROM nectarcrm_field WHERE tabid = ? AND uitype = 4", Array($tabid));
			...
			if ($fld_table == $this->table_name) {
				...
				if ($records && $adb->num_rows($records)) {
					...
					if ($old_cur_id != $cur_id) {
						$adb->pquery("UPDATE nectarcrm_modentity_num set cur_id=? where semodule=? and active=1", Array($cur_id, $module));
					}
				}
			} else {
	...
	function get_attachments($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>'nectarcrm_users.first_name',
														'last_name' => 'nectarcrm_users.last_name'), 'Users');
		$query = "select case when (nectarcrm_users.user_name not like '') then $userNameSql else nectarcrm_groups.groupname end as user_name," .
				...
				nectarcrm_seattachmentsrel.attachmentsid attachmentsid, nectarcrm_crmentity.smownerid smownerid, nectarcrm_notes.notesid crmid,
				nectarcrm_notes.notecontent description,nectarcrm_notes.*
	...
	function isViewed($crmid = false) {
		...
		if ($crmid) {
			global $adb;
			$result = $adb->pquery("SELECT viewedtime,modifiedtime,smcreatorid,smownerid,modifiedby FROM nectarcrm_crmentity WHERE crmid=?", Array($crmid));
	...
	function markAsViewed($userid) {
		global $adb;
		$adb->pquery("UPDATE nectarcrm_crmentity set viewedtime=? WHERE crmid=? AND smownerid=?", Array(date('Y-m-d H:i:s', time()), $this->id, $userid));
	}
	...
	function save_related_module($module, $crmid, $with_module, $with_crmid) {
		...
		foreach ($with_crmid as $relcrmid) {
			...
			if ($with_module == 'Documents') {
				$checkpresence = $adb->pquery("SELECT crmid FROM nectarcrm_senotesrel WHERE crmid = ? AND notesid = ?", Array($crmid, $relcrmid));
				// Relation already exists? No need to add again
				if ($checkpresence && $adb->num_rows($checkpresence))
					continue;

				$adb->pquery("INSERT INTO nectarcrm_senotesrel(crmid, notesid) VALUES(?,?)", array($crmid, $relcrmid));
			} else {
				$checkpresence = $adb->pquery("SELECT crmid FROM nectarcrm_crmentityrel WHERE
					crmid = ? AND module = ? AND relcrmid = ? AND relmodule = ?", Array($crmid, $module, $relcrmid, $with_module));
				// Relation already exists? No need to add again
				if ($checkpresence && $adb->num_rows($checkpresence))
					continue;

				$adb->pquery("INSERT INTO nectarcrm_crmentityrel(crmid, module, relcrmid, relmodule) VALUES(?,?,?,?)", Array($crmid, $module, $relcrmid, $with_module));
			}
		}
	}
	...
	function delete_related_module($module, $crmid, $with_module, $with_crmid) {
		...
		foreach ($with_crmid as $relcrmid) {
			...
			if ($with_module == 'Documents') {
				$adb->pquery("DELETE FROM nectarcrm_senotesrel WHERE crmid=? AND notesid=?", Array($crmid, $relcrmid));
			} else {
				$adb->pquery("DELETE FROM nectarcrm_crmentityrel WHERE (crmid=? AND module=? AND relcrmid=? AND relmodule=?) OR (relcrmid=? AND relmodule=? AND crmid=? AND module=?)",
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$query .= " INNER JOIN nectarcrm_crmentityrel ON (nectarcrm_crmentityrel.relcrmid = nectarcrm_crmentity.crmid OR nectarcrm_crmentityrel.crmid = nectarcrm_crmentity.crmid)";
		$query .= $more_relation;
		...
		$query .= " WHERE nectarcrm_crmentity.deleted = 0 AND (nectarcrm_crmentityrel.crmid = $id OR nectarcrm_crmentityrel.relcrmid = $id)";
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $relationId) {
		...
		if(empty($relationFieldId)) {
			$dependentFieldSql = $this->db->pquery("SELECT tabid, fieldname, columnname,tablename FROM nectarcrm_field WHERE uitype='10' AND" .
					" fieldid IN (SELECT fieldid FROM nectarcrm_fieldmodulerel WHERE relmodule=? AND module=?)", array($currentModule, $related_module));
		} else {
			$dependentFieldSql = $this->db->pquery("SELECT tabid, fieldname, columnname,tablename FROM nectarcrm_field WHERE uitype='10' AND" .
					" fieldid IN (SELECT relationfieldid FROM nectarcrm_relatedlists WHERE relation_id=?)", array($relationId));
		}
		...
		if ($numOfFields > 0) {
			...
			$query .= " WHERE nectarcrm_crmentity.deleted = 0 AND $this->table_name$this->moduleName.$this->table_index = $id";
			if($related_module == 'Leads') {
				$query .= " AND nectarcrm_leaddetails.converted=0 ";
			}
			$return_value = GetRelatedList($currentModule, $related_module, $other, $query, $button, $returnset);
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		foreach ($transferEntityIds as $transferId) {
			...
			$relatedRecords = $adb->pquery("SELECT relcrmid, relmodule FROM nectarcrm_crmentityrel WHERE crmid=? AND module=?" .
					" AND relcrmid NOT IN (SELECT relcrmid FROM nectarcrm_crmentityrel WHERE crmid=? AND module=?)", array($transferId, $module, $entityId, $module));
			...
			for ($i = 0; $i < $numOfRecords; $i++) {
				...
				$adb->pquery("UPDATE nectarcrm_crmentityrel SET crmid=? WHERE relcrmid=? AND relmodule=? AND crmid=? AND module=?", array($entityId, $relcrmid, $relmodule, $transferId, $module));
			}
			...
			$parentRecords = $adb->pquery("SELECT crmid, module FROM nectarcrm_crmentityrel WHERE relcrmid=? AND relmodule=?" .
					" AND crmid NOT IN (SELECT crmid FROM nectarcrm_crmentityrel WHERE relcrmid=? AND relmodule=?)", array($transferId, $module, $entityId, $module));
			...
			for ($i = 0; $i < $numOfRecords; $i++) {
				...
				$adb->pquery("UPDATE nectarcrm_crmentityrel SET relcrmid=? WHERE crmid=? AND module=? AND relcrmid=? AND relmodule=?", array($entityId, $parcrmid, $parmodule, $transferId, $module));
			}
			$adb->pquery("UPDATE nectarcrm_modcomments SET related_to = ? WHERE related_to = ?", array($entityId, $transferId));
		}
	...
	function generateReportsQuery($module, $queryPlanner) {
		...
		$fields_query = $adb->pquery("SELECT nectarcrm_field.fieldname,nectarcrm_field.tablename,nectarcrm_field.fieldid from nectarcrm_field INNER JOIN nectarcrm_tab on nectarcrm_tab.name = ? WHERE nectarcrm_tab.tabid=nectarcrm_field.tabid AND nectarcrm_field.uitype IN (10) and nectarcrm_field.presence in (0,2)", array($module));
		...
		if ($adb->num_rows($fields_query) > 0) {
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				$ui10_modules_query = $adb->pquery("SELECT relmodule FROM nectarcrm_fieldmodulerel WHERE fieldid=?", array($field_id));

				if ($adb->num_rows($ui10_modules_query) > 0) {

					// Capture the forward table dependencies due to dynamic related-field
					$crmentityRelModuleFieldTable = "nectarcrm_crmentityRel$module$field_id";
					...
					if ($queryPlanner->requireTable($crmentityRelModuleFieldTable, $matrix)) {
						$relquery.= " left join nectarcrm_crmentity as $crmentityRelModuleFieldTable on $crmentityRelModuleFieldTable.crmid = $tab_name.$field_name and nectarcrm_crmentityRel$module$field_id.deleted=0";
					}
		...
		$query = "from $moduletable inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid=$moduletable.$moduleindex";
	...
	function generateReportsSecQuery($module, $secmodule,$queryPlanner) {
		...
		$fields_query = $adb->pquery("SELECT nectarcrm_field.fieldname,nectarcrm_field.tablename,nectarcrm_field.fieldid from nectarcrm_field INNER JOIN nectarcrm_tab on nectarcrm_tab.name = ? WHERE nectarcrm_tab.tabid=nectarcrm_field.tabid AND nectarcrm_field.uitype IN (10) and nectarcrm_field.presence in (0,2)", array($secmodule));
		...
		if ($adb->num_rows($fields_query) > 0) {
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				if ($adb->num_rows($ui10_modules_query) > 0) {
					// Capture the forward table dependencies due to dynamic related-field
					$crmentityRelSecModuleTable = "nectarcrm_crmentityRel$secmodule$field_id";
					...
					if ($queryPlanner->requireTable($crmentityRelSecModuleTable, $matrix)) {
						$relquery .= " left join nectarcrm_crmentity as $crmentityRelSecModuleTable on $crmentityRelSecModuleTable.crmid = $tab_name.$field_name and $crmentityRelSecModuleTable.deleted=0";
					}
					for ($j = 0; $j < $adb->num_rows($ui10_modules_query); $j++) {
						$rel_mod = $adb->query_result($ui10_modules_query, $j, 'relmodule');
						$rel_obj = CRMEntity::getInstance($rel_mod);
		...
		$matrix->setDependency("nectarcrm_crmentity$secmodule", array("nectarcrm_groups$secmodule", "nectarcrm_users$secmodule", "nectarcrm_lastModifiedBy$secmodule"));
		$matrix->addDependency($tablename, "nectarcrm_crmentity$secmodule");
		...
		if ($queryPlanner->requireTable("nectarcrm_crmentity$secmodule", $matrix)) {
			$query .= " left join nectarcrm_crmentity as nectarcrm_crmentity$secmodule on nectarcrm_crmentity$secmodule.crmid = $tablename.$tableindex AND nectarcrm_crmentity$secmodule.deleted=0";
		}

		// Add the pre-joined custom table query
		$query .= " ".$cfquery;

		if ($queryPlanner->requireTable("nectarcrm_groups$secmodule")) {
			$query .= " left join nectarcrm_groups as nectarcrm_groups" . $secmodule . " on nectarcrm_groups" . $secmodule . ".groupid = nectarcrm_crmentity$secmodule.smownerid";
		}
		if ($queryPlanner->requireTable("nectarcrm_users$secmodule")) {
			$query .= " left join nectarcrm_users as nectarcrm_users" . $secmodule . " on nectarcrm_users" . $secmodule . ".id = nectarcrm_crmentity$secmodule.smownerid";
		}
		if ($queryPlanner->requireTable("nectarcrm_lastModifiedBy$secmodule")) {
			$query .= " left join nectarcrm_users as nectarcrm_lastModifiedBy" . $secmodule . " on nectarcrm_lastModifiedBy" . $secmodule . ".id = nectarcrm_crmentity" . $secmodule . ".modifiedby";
		}
		if ($queryPlanner->requireTable('nectarcrm_createdby'.$secmodule)) {
			$query .= " LEFT JOIN nectarcrm_users AS nectarcrm_createdby$secmodule ON nectarcrm_createdby$secmodule.id=nectarcrm_crmentity.smcreatorid";
		}
		// Add the pre-joined relation table query
		$query .= " " . $relquery;
	...
	function getReportsUiType10Query($module, $queryPlanner){
		...
		$fields_query = $adb->pquery("SELECT nectarcrm_field.fieldname,nectarcrm_field.tablename,nectarcrm_field.fieldid from nectarcrm_field INNER JOIN nectarcrm_tab on nectarcrm_tab.name IN (".  generateQuestionMarks($params).") WHERE nectarcrm_tab.tabid=nectarcrm_field.tabid AND nectarcrm_field.uitype IN (10) AND nectarcrm_field.presence IN (0,2)", $params);
	...
	function getListViewSecurityParameter($module) {
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1
			...
			$sec_query .= " and (nectarcrm_crmentity.smownerid in($current_user->id) or nectarcrm_crmentity.smownerid
					in (select nectarcrm_user2role.userid from nectarcrm_user2role
							...
							where nectarcrm_role.parentrole like '" . $current_user_parent_role_seq . "::%') or nectarcrm_crmentity.smownerid
					in(select shareduserid from nectarcrm_tmp_read_user_sharing_per
			...
			$sec_query .= " nectarcrm_crmentity.smownerid in(select nectarcrm_tmp_read_group_sharing_per.sharedgroupid
						from nectarcrm_tmp_read_group_sharing_per where userid=" . $current_user->id . " and tabid=" . $tabid . "))) ";
		}
		return $sec_query;
	}
	...
	function getRelationQuery($module, $secmodule, $table_name, $column_name, $queryPlanner) {
		...
		$secQueryFrom = " FROM $table_name INNER JOIN nectarcrm_crmentity ON " .
				"nectarcrm_crmentity.crmid=$table_name.$column_name AND nectarcrm_crmentity.deleted=0 ";
		...
		if($pritablename != $table_name) {
			...
			if (isset($modulecftable)) {
				...
				//eg : nectarcrm_potential has potentialid and nectarcrm_potentialscf has same potentialid
		...
		$query = '';
		if ($pritablename == 'nectarcrm_crmentityrel') {
			...
			$query = " left join nectarcrm_crmentityrel as $tmpname ON (($condvalue={$tmpname}.{$secfieldname} " .
			...
			if($pritablename == 'nectarcrm_seactivityrel') {
				...
				$query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname
					AND $tmpname.activityid IN (SELECT crmid FROM nectarcrm_crmentity WHERE setype='$tmpModule' AND deleted = 0))";
			} else if($pritablename == 'nectarcrm_senotesrel') {
					$query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname
					AND $tmpname.notesid IN (SELECT crmid FROM nectarcrm_crmentity WHERE setype='Documents' AND deleted = 0))";
			} else if($pritablename == 'nectarcrm_inventoryproductrel' && ($module =="Products" || $module =="Services") && ($secmodule == "Invoice" || $secmodule == "SalesOrder" || $secmodule == "PurchaseOrder" || $secmodule == "Quotes")) {
				/** In nectarcrm_inventoryproductrel table, we'll have same product related to quotes/invoice/salesorder/purchaseorder
				...
				$query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname AND $tmpname.id in 
						(select crmid from nectarcrm_crmentity where setype='$secmodule' and deleted=0))";
			} else if($pritablename == 'nectarcrm_cntactivityrel') {
				if($queryPlanner->requireTable('nectarcrm_cntactivityrel') && $secmodule == 'Contacts') {
					$tmpname = 'nectarcrm_cntactivityrel';
					$condition = "$table_name.$column_name=$tmpname.$secfieldname";
				} else {
					$query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname)";
				}
			...
			// To get all the Contacts from nectarcrm_contpotentialrel table
			$condition .= " OR $table_name.potentialid = nectarcrm_contpotentialrel.potentialid";
			$query .= " left join nectarcrm_contpotentialrel on  nectarcrm_contpotentialrel.contactid = nectarcrm_contactdetails.contactid";
		}else if($module == "Potentials" && $secmodule == "Contacts"){
			// To get all the Potentials from nectarcrm_contpotentialrel table
			$condition .= " OR $table_name.contactid = nectarcrm_contpotentialrel.contactid";
			$query .= " left join nectarcrm_contpotentialrel on nectarcrm_potential.potentialid = nectarcrm_contpotentialrel.potentialid";
		}
		if ($secmodule == "Emails") {
			$table_name .="Emails";
		}
	...
	function add_related_to($module, $fieldname) {
		...
		$query = "SELECT nectarcrm_crmentity.deleted, $focus1->table_name.*
					FROM $focus1->table_name
					INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid=$focus1->table_name.$focus1->table_index
						where $entityName=? and nectarcrm_crmentity.deleted=0";
	...
	function filterInactiveFields($module) {
		...
		if ($cachedModuleFields) {
			foreach ($cachedModuleFields as $fieldinfo) {
				...
				$tableName = str_replace("nectarcrm_", "", $fieldinfo['tablename']);
	...
	function buildSearchQueryForFieldTypes($uitypes, $value=false) {
		...
		$query .=" INNER JOIN nectarcrm_crmentity ON $this->table_name.$this->table_index = nectarcrm_crmentity.crmid AND deleted = 0 ";
	...
	public function getJoinClause($tableName) {
		if (strripos($tableName, 'rel') === (strlen($tableName) - 3)) {
			return 'LEFT JOIN';
		}  else if (nectarcrm_Functions::isUserSpecificFieldTable($tableName, $this->moduleName)) {
			return 'LEFT JOIN';
		}
		else {
			return 'INNER JOIN';
	...
	function getNonAdminUserAccessQuery($user, $parentRole, $userGroups) {
		$query = "(SELECT $user->id as id) UNION (SELECT nectarcrm_user2role.userid AS userid FROM " .
				"nectarcrm_user2role INNER JOIN nectarcrm_users ON nectarcrm_users.id=nectarcrm_user2role.userid " .
				"INNER JOIN nectarcrm_role ON nectarcrm_role.roleid=nectarcrm_user2role.roleid WHERE " .
				"nectarcrm_role.parentrole like '$parentRole::%')";
	...
	function getNonAdminUserGroupAccessQuery($userGroups) {
		$query .= " UNION (SELECT groupid FROM nectarcrm_groups WHERE groupid IN (".implode(",", $userGroups)."))";
		return $query;
	}
	...
	function getNonAdminModuleAccessQuery($module, $user) {
		...
		if (!empty($sharingRuleInfo) && (count($sharingRuleInfo['ROLE']) > 0 ||
			...
			$query = " (SELECT shareduserid FROM nectarcrm_tmp_read_user_sharing_per " .
					"WHERE userid=$user->id AND tabid=$tabId) UNION (SELECT " .
					"nectarcrm_tmp_read_group_sharing_per.sharedgroupid FROM " .
					"nectarcrm_tmp_read_group_sharing_per WHERE userid=$user->id AND tabid=$tabId)";
		}
		return $query;
	}
			...
			// for secondary module we should join the records even if record is not there(primary module without related record)
				if($scope == ''){
					$query = " INNER JOIN $tableName $tableName$scope ON $tableName$scope.id = " .
							"nectarcrm_crmentity$scope.smownerid ";
				}else{
					$query = " INNER JOIN $tableName $tableName$scope ON $tableName$scope.id = " .
							"nectarcrm_crmentity$scope.smownerid OR nectarcrm_crmentity$scope.smownerid IS NULL";
				}
			}
		return $query;
	}
	...
	function setRelationTables($secmodule) {
		$rel_tables = array(
			"Documents" => array("nectarcrm_senotesrel" => array("crmid", "notesid"),
	...
	function trackLinkedInfo($module, $crmid, $with_module, $with_crmid) {
		...
		$adb->pquery('UPDATE nectarcrm_crmentity SET modifiedtime = ?, modifiedby = ? WHERE crmid = ?', array($currentTime, $current_user->id, $crmid));
	...
	function get_comments($relatedRecordId = false) {
		...
		if($relatedRecordId){
			$query .= " AND related_to = ".$relatedRecordId." ORDER BY nectarcrm_crmentity.createdtime DESC";
		}
		return $query;
	}
}
