...
jQuery.Class("nectarcrm_Helper_Js",{
	...
	requestToShowComposeEmailForm : function(selectedId,fieldname,fieldmodule){
		...
		var emailsMassEditInstance = nectarcrm_Helper_Js.getEmailMassEditInstance();
	...
	getInternalMailer  : function(selectedId,fieldname,fieldmodule){
		var module = 'Emails';
		var cacheResponse = nectarcrm_Helper_Js.checkServerConfigResponseCache;
		var  checkServerConfigPostOperations = function (data) {
			if(data == true){
				nectarcrm_Helper_Js.requestToShowComposeEmailForm(selectedId,fieldname,fieldmodule);
			} else {
				alert(app.vtranslate('JS_EMAIL_SERVER_CONFIGURATION'));
			}
		}
		if(cacheResponse === ''){
			var checkServerConfig = nectarcrm_Helper_Js.checkServerConfig(module);
			checkServerConfig.then(function(data){
				nectarcrm_Helper_Js.checkServerConfigResponseCache = data;
				checkServerConfigPostOperations(nectarcrm_Helper_Js.checkServerConfigResponseCache);
			});
		} else {
			checkServerConfigPostOperations(nectarcrm_Helper_Js.checkServerConfigResponseCache);
		}
