...
class nectarcrm_Viewer extends SmartyBC {
	...
	public function getTemplatePath($templateName, $moduleName='') {
		...
		if(!empty($moduleName) && file_exists($completeFilePath)) {
			...
			return "modules/nectarcrm/$templateName";
		}
	}
	 ...
	 * @return nectarcrm_Viewer instance
	 */
...
function vtemplate_path($templateName, $moduleName='') {
	$viewerInstance = nectarcrm_Viewer::getInstance();
	$args = func_get_args();
	return call_user_func_array(array($viewerInstance, 'getTemplatePath'), $args);
}

/**
 * Generated cache friendly resource URL linked with version of nectarcrm
 */
function vresource_url($url) {
    global $nectarcrm_current_version;
    if (stripos($url, '://') === false) {
        $url = $url .'?v='.$nectarcrm_current_version;
    }
    return $url;
}
