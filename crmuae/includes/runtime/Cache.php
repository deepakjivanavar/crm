...
class nectarcrm_Cache {
	...
	private function __construct() {
		$this->connector = nectarcrm_Cache_Connector::getInstance();
	}
	...
	public function getBlockInstance($block, $moduleName){
		if(nectarcrm_Cache::get('BlockInstance',$moduleName.'-'.$block)){
			return nectarcrm_Cache::get('BlockInstance',$moduleName.'-'.$block);
		}
		return false;
	}
	...
	public function setBlockInstance($block, $moduleName, $instance){
		if(isset($instance) && isset($block)){
			nectarcrm_Cache::set('BlockInstance',$moduleName.'-'.$block,$instance);
		}
	}
	...
	public function getBlockFields($moduleName,$blockId){
		if(nectarcrm_Cache::get('BlockFields',$moduleName.'_'.$blockId)){
			return nectarcrm_Cache::get('BlockFields',$moduleName.'_'.$blockId);
		}
		return false;
	}

	public function setBlockFields($moduleName,$blockId,$fields){
		nectarcrm_Cache::set('BlockFields',$moduleName.'_'.$blockId,$fields);
	}
	...
	public static function flushModuleCache($moduleName) {
		...
		nectarcrm_Cache::delete('module', $moduleName);
		nectarcrm_Cache::delete('module', $module->id);

		$moduleBlocks = $module->getBlocks();
		foreach ($moduleBlocks as $label => $block) {
			nectarcrm_Cache::delete('BlockInstance', $module->id.'-'.$label);
			nectarcrm_Cache::delete('BlockInstance', $module->id.'-'.$block->id);
		}

		nectarcrm_Cache::delete('ModuleFieldInfo', $moduleName);
		nectarcrm_Cache::delete('ModuleFieldInfo', $module->id);

		nectarcrm_Cache::delete('ModuleFields', $module->id);
		nectarcrm_Cache::delete('ModuleFields', $moduleName);

		nectarcrm_Cache::delete('ModuleBlocks', $moduleName);
		nectarcrm_Cache::delete('ModuleBlocks', $module->id);
	}
	...
	public static function flushPicklistCache($pickListName, $rolesList = false) {
		nectarcrm_Cache::delete('PicklistValues', $pickListName);
		nectarcrm_Cache::delete('EditablePicklistValues', $pickListName);
		nectarcrm_Cache::delete('NonEditablePicklistValues', $pickListName);
		nectarcrm_Cache::delete('AllPicklistValues', $pickListName);
		nectarcrm_Cache::delete('PicklistDetails', $pickListName);

		if ($rolesList) {
			foreach ($rolesList as $key => $roleId) {
				nectarcrm_Cache::delete('PicklistRoleBasedValues', $pickListName.$roleId);
			}
		}
	}
	...
	public static function flushModuleandBlockFieldsCache($module, $blockId = false) {
		nectarcrm_Cache::delete('ModuleFieldInfo', $module->name);
		nectarcrm_Cache::delete('ModuleFields', $module->id);

		if ($blockId) {
			nectarcrm_Cache::delete('BlockFields', $module->name.'_'.$blockId);
		} else {
			$blocks = $module->getBlocks();
			foreach ($blocks as $label => $block) {
				nectarcrm_Cache::delete('BlockFields', $module->name.'_'.$block->id);
			}
		}
	}
	...
	static function flushModuleBlocksCache($moduleInstance, $block = null) {
		if ($block == null) {
			...
			nectarcrm_Cache::delete('ModuleBlocks', $moduleInstance->id);
			foreach ($moduleBlocks as $label => $block) {
				nectarcrm_Cache::delete('BlockInstance', $moduleInstance->id.'-'.$label);
				nectarcrm_Cache::delete('BlockInstance', $moduleInstance->id.'-'.$block->id);
				nectarcrm_Cache::delete('BlockFields', $moduleInstance->name.'_'.$block->label);
			}
		} else {
			nectarcrm_Cache::delete('ModuleBlocks', $moduleInstance->id);
			nectarcrm_Cache::delete('BlockInstance', $moduleInstance->id.'-'.$block->label);
			nectarcrm_Cache::delete('BlockInstance', $moduleInstance->id.'-'.$block->id);
			nectarcrm_Cache::delete('BlockFields', $moduleInstance->name.'_'.$block->id);
		}
	}
