 ...
 */
class ModComments_ListView_Model extends nectarcrm_ListView_Model {
	...
	public function getListViewLinks($linkParams) {
		...
		if($currentUserModel->isAdminUser()) {
			$settingsLink = array(
					...
					'linkicon' => nectarcrm_Theme::getImagePath('EditWorkflows.png')
			);
	...
	public function getListViewEntries($pagingModel) {
		...
		if(!empty($relatedToArray)) {
			$result = $db->pquery("SELECT crmid, deleted, setype FROM nectarcrm_crmentity WHERE crmid IN (".generateQuestionMarks($relatedToArray).")", $relatedToArray);
		...
		foreach($listViewRecordModels as $key => $listViewRecordModel) {
			...
			if($data[$relatedTo]['deleted'] == '0') {
				$relatedToModel = nectarcrm_Record_Model::getCleanInstance($data[$relatedTo]['setype'])->setId($relatedTo);
