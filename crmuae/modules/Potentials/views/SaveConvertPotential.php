...
class Potentials_SaveConvertPotential_View extends nectarcrm_View_Controller {
	...
	public function process(nectarcrm_Request $request) {
		...
		$recordModel = nectarcrm_Record_Model::getInstanceById($recordId, $request->getModule());
				...
				foreach ($convertPotentialFields[$module] as $fieldModel) {
					...
					if ($fieldModel->getFieldDataType() === 'currency') {
						if($fieldModel->get('uitype') == 72){
							...
							$fieldValue = nectarcrm_Currency_UIType::convertToDBFormat($fieldValue, null, true);
						} else {
							$fieldValue = nectarcrm_Currency_UIType::convertToDBFormat($fieldValue);
						}
