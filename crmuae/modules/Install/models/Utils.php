...
class Install_Utils_Model {
	...
	/**
	 * Function returns the php.ini file settings required for installing nectarcrmCRM
	 * @return <Array>
	 */
	...
	/**
	 * Variable has the recommended php settings for smooth running of nectarcrmCRM
	 * @var <Array>
	 */
	...
	/**
	 * Returns the recommended php settings for nectarcrmCRM
	 * @return type
	 */
	...
	/**
	 * Function checks for nectarcrmCRM installation prerequisites
	 * @return <Array>
	 */
		...
		if($db_type) {
			...
			if(@$conn->Connect($db_hostname,$db_username,$db_password)) {
				...
				if(@$conn->Connect($db_hostname, $db_username, $db_password, $db_name)) {
					...
					if(!$db_utf8_support) {
						$db_utf8_support = nectarcrm_Util_Helper::checkDbUTF8Support($conn);
					}
				}
				$conn->Close();
			}
	...
	public static function installModules() {
		...
		$moduleFolders = array('packages/nectarcrm/mandatory', 'packages/nectarcrm/optional', 'packages/nectarcrm/marketplace');
