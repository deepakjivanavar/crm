...
class Settings_Picklist_Field_Model extends nectarcrm_Field_Model {
    ...
    public function getPicklistValues($roleIdList, $groupMode='INTERSECTION') {
        ...
        if(nectarcrm_Cache::get('PicklistRoleBasedValues',$this->getName().implode('_', $roleIdList))){
            return nectarcrm_Cache::get('PicklistRoleBasedValues',$this->getName().implode('_', $roleIdList));
        }
        ...
        $query .= ' FROM  nectarcrm_role2picklist INNER JOIN '.$tableName.' ON nectarcrm_role2picklist.picklistvalueid = '.$tableName.'.picklist_valueid'.
        ...
        nectarcrm_Cache::set('PicklistRoleBasedValues', $fieldName.implode('_', $roleIdList), $pickListValues);
	...
	public static function getEditablePicklistValues($fieldName){
        ...
        $query="SELECT $primaryKey ,$fieldName FROM nectarcrm_$fieldName WHERE presence=1";
        $values = array();
	...
	public static function getNonEditablePicklistValues($fieldName){
        ...
        $columnIndex = nectarcrm_Util_Helper::getPickListId($fieldName);
        $query = "select $columnIndex, $fieldName from nectarcrm_$fieldName where presence=0";
        $values = array();
        $result = $db->pquery($query, array());
        $num_rows = $db->num_rows($result);
        for($i=0; $i<$num_rows; $i++) {
