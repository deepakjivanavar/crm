...
class Settings_LayoutEditor_Field_Model extends nectarcrm_Field_Model {
	...
	public function delete() {
		...
		$adb->pquery("delete from nectarcrm_cvcolumnlist where columnname = ? ", array($deletecolumnname));
		$adb->pquery("delete from nectarcrm_cvstdfilter where columnname = ?", array($column_cvstdfilter));
		$adb->pquery("delete from nectarcrm_cvadvfilter where columnname = ?", array($deletecolumnname));
		$adb->pquery("delete from nectarcrm_selectcolumn where columnname = ?", array($select_columnname));
		$adb->pquery("delete from nectarcrm_relcriteria where columnname = ?", array($select_columnname));
		$adb->pquery("delete from nectarcrm_reportsortcol where columnname = ?", array($select_columnname));
		$adb->pquery("delete from nectarcrm_reportdatefilter where datecolumnname = ?", array($column_cvstdfilter));
		$adb->pquery("delete from nectarcrm_reportsummary where columnname like ?", array('%'.$reportsummary_column.'%'));

		//Deleting from convert lead mapping nectarcrm_table- Jaguar
		if($fld_module=="Leads") {
			$deletequery = 'delete from nectarcrm_convertleadmapping where leadfid=?';
			...
			$map_del_q = "update nectarcrm_convertleadmapping set ".$map_del_id[$fld_module]."=0 where ".$map_del_id[$fld_module]."=?";
		...
		if($this->getFieldDataType() == 'picklist' || $this->getFieldDataType() == 'multipicklist') {
			$deltablequery = 'drop table nectarcrm_'.$adb->sql_escape_string($columnname);
			$adb->pquery($deltablequery, array());
			//To Delete Sequence Table 
			$deltableseqquery = 'drop table nectarcrm_'.$adb->sql_escape_string($columnname).'_seq'; 
			$adb->pquery($deltableseqquery, array()); 
			$adb->pquery("delete from  nectarcrm_picklist_dependency where sourcefield=? or targetfield=?", array($columnname,$columnname));
		}
	}
	...
	public function move($fieldNewDetails, $fieldOlderDetails) {
		...
		if ($olderBlockId == $newBlockId) {
			if ($newSequence > $olderSequence) {
				$updateQuery = 'UPDATE nectarcrm_field SET sequence = sequence-1 WHERE sequence > ? AND sequence <= ? AND block = ?';
			...
			$query = 'UPDATE nectarcrm_field SET sequence = ? WHERE fieldid = ?';
			$params = array($newSequence, $this->getId());
			$db->pquery($query, $params);
		} else {
			$updateOldBlockQuery = 'UPDATE nectarcrm_field SET sequence = sequence-1 WHERE sequence > ? AND block = ?';
			$params = array($olderSequence, $olderBlockId);
			$db->pquery($updateOldBlockQuery, $params);

			$updateNewBlockQuery = 'UPDATE nectarcrm_field SET sequence = sequence+1 WHERE sequence >= ? AND block = ?';
			$params = array($newSequence, $newBlockId);
			$db->pquery($updateNewBlockQuery, $params);

			$query = 'UPDATE nectarcrm_field SET sequence = ?, block = ? WHERE fieldid = ?';
	...
	public static function makeFieldActive($fieldIdsList=array(), $blockId, $moduleName = false) {
		$db = PearDatabase::getInstance();
		$maxSequenceQuery = "SELECT MAX(sequence) AS maxsequence FROM nectarcrm_field WHERE block = ? AND presence IN (0,2) ";
		$res = $db->pquery($maxSequenceQuery,array($blockId));
		$maxSequence = $db->query_result($res,0,'maxsequence');

		$query = 'UPDATE nectarcrm_field SET presence = 2, sequence = CASE';
		...
		// Clearing cache
		$moduleModel = nectarcrm_Module::getInstance($moduleName);
	...
	public function isDefaultValueOptionDisabled() {
		...
		if (in_array($this->getName(), $defaultValueRestrictedFields) 
				...
				|| $this->getFieldDataType() == nectarcrm_Field_Model::REFERENCE_TYPE
	 ...
	 * @param nectarcrm_Field_Model $fieldObject
	 * @return <Settings_LayoutEditor_Field_Model>
	 */
	public static function getInstanceFromFieldObject(nectarcrm_Field $fieldObject) {
			$objectProperties = get_object_vars($fieldObject);
		$fieldModel = new self();
		foreach ($objectProperties as $properName => $propertyValue) {
			$fieldModel->$properName = $propertyValue;
	...
	public static function getDetailsForMove($fieldIdsList = array()) {
		if ($fieldIdsList) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery('SELECT fieldid, sequence, block, fieldlabel FROM nectarcrm_field WHERE fieldid IN ('. generateQuestionMarks($fieldIdsList) .')', $fieldIdsList);
	 ...
	 * @param <nectarcrm_Module_Model> $moduleInstance
	...
	public static function getInstanceFromBlockIdList($blockId, $moduleInstance= false) {
		...
		$query = 'SELECT * FROM nectarcrm_field WHERE block IN('. generateQuestionMarks($blockId) .') AND nectarcrm_field.displaytype IN (1,2,4,5) ORDER BY sequence';
	...
	public static function getCleanInstance() {
		...
		$fieldInstance->block = new nectarcrm_Block();
		$fieldInstance->block->module = new nectarcrm_Module();
