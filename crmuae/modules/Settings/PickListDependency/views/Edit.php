...
class Settings_PickListDependency_Edit_View extends Settings_nectarcrm_Index_View {
	...
	public function getDependencyGraph(nectarcrm_Request $request) {
		...
		foreach($sourcePicklistValues as $key => $value) {
			$safeHtmlSourcePicklistValues[$key] = nectarcrm_Util_Helper::toSafeHTML($key);
		}
		...
		foreach($targetPicklistValues as $key => $value) {
			$safeHtmlTargetPicklistValues[$key] = nectarcrm_Util_Helper::toSafeHTML($key);
		}
