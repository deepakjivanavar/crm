...
/**
 * Sharng Access nectarcrm Module Model Class
 */
class Settings_SharingAccess_Rule_Model extends nectarcrm_Base_Model {
	...
	static $dataShareTableColArr = array (
		...
		self::RULE_TYPE_ROLE => array (
			self::RULE_TYPE_GROUPS => array (
				'table' => 'nectarcrm_datashare_role2group',
				'source_id' => 'share_roleid',
				'target_id' => 'to_groupid'
			),
			self::RULE_TYPE_ROLE => array (
				'table' => 'nectarcrm_datashare_role2role',
				'source_id' => 'share_roleid',
				'target_id' => 'to_roleid'
			),
			self::RULE_TYPE_ROLE_AND_SUBORDINATES => array (
				'table' => 'nectarcrm_datashare_role2rs',
				'source_id' => 'share_roleid',
				'target_id' => 'to_roleandsubid'
			),
		),
		self::RULE_TYPE_ROLE_AND_SUBORDINATES => array (
			self::RULE_TYPE_GROUPS => array (
				'table' => 'nectarcrm_datashare_rs2grp',
				'source_id' => 'share_roleandsubid',
				'target_id' => 'to_groupid'
			),
			self::RULE_TYPE_ROLE => array (
				'table' => 'nectarcrm_datashare_rs2role',
				'source_id' => 'share_roleandsubid',
				'target_id' => 'to_roleid'
			),
			self::RULE_TYPE_ROLE_AND_SUBORDINATES => array (
				'table' => 'nectarcrm_datashare_rs2rs',
				'source_id' => 'share_roleandsubid',
				'target_id' => 'to_roleandsubid'
			),
		),
	...
	public function save() {
		...
		if(!$ruleId) {
			$ruleId = $db->getUniqueId('nectarcrm_datashare_module_rel');
			$this->set('shareid', $ruleId);

			$db->pquery("INSERT INTO nectarcrm_datashare_module_rel(shareid, tabid) VALUES(?,?)",
		...
		$sql = 'UPDATE nectarcrm_datashare_module_rel SET relationtype=? WHERE shareid=?';
	...
	public function delete() {
		...
		$db->pquery('DELETE FROM nectarcrm_datashare_module_rel WHERE shareid=?', array($ruleId));
        Settings_SharingAccess_Module_Model::recalculateSharingRules();
	}
	...
	public static function getInstance($moduleModel, $ruleId) {
		...
		$sql = 'SELECT * FROM nectarcrm_datashare_module_rel WHERE tabid = ? AND shareid = ?';
	...
	public static function getAllByModule($moduleModel) {
		...
		$sql = 'SELECT * FROM nectarcrm_datashare_module_rel WHERE tabid = ?';
