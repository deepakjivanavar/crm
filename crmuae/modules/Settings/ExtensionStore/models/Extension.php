...
vimport('~~/vtlib/nectarcrm/Package.php');
vimport('~/libraries/PHPMarkdown/Michelf/Markdown.inc.php');

class Settings_ExtensionStore_Extension_Model extends nectarcrm_Base_Model {
	...
	public function __construct() {
		parent::__construct();
		$this->EXTENSIONSTORE_LOOKUP_URL = 'https://marketplace.nectarcrm.com/api';
		global $site_URL;
		if (empty($site_URL)) {
	 ...
	 * @return <nectarcrm_Package> package object
	 */
	public function getPackage() {
		$packageModel = new nectarcrm_Package();
	...
	/**
	 * Function to check whether it is compatible with nectarcrm or not
	 * @return <boolean> true/false
	 */
	public function isnectarcrmCompatible() {
		vimport('~~/vtlib/nectarcrm/Version.php');
		$nectarcrmVersion = $this->get('nectarcrmVersion');
		$nectarcrmMaxVersion = $this->get('nectarcrmMaxVersion');

		if ((nectarcrm_Version::check($nectarcrmVersion, '>=') && $nectarcrmMaxVersion && nectarcrm_Version::check($nectarcrmMaxVersion, '<')) || nectarcrm_Version::check($nectarcrmVersion, '=')) {
			return true;
		}
		return false;
	}
	...
	public static function getLanguageInstance($lang) {
		$sql = 'SELECT id,name,prefix FROM nectarcrm_language WHERE name = ? OR prefix = ?';
	 ...
	 */
	public function logoutMarketPlace(nectarcrm_Request $request) {
		$sql = 'DELETE FROM nectarcrm_extnstore_users';
		$db = PearDatabase::getInstance();
		$db->pquery($sql, array());
	}
	...
	public function getInstanceFromArray($listing) {
		...
		foreach ($listing as $key => $value) {
			switch ($key) {
				...
				case 'minrange'		:	$key = 'nectarcrmVersion';		break;
				case 'maxrange'		:	$key = 'nectarcrmMaxVersion';	break;
	...
	public static function getModuleFromExtnName($extnName) {
		$moduleModel = nectarcrm_Module_Model::getInstance($extnName);
		if ($moduleModel) {
			$moduleModel->set('extnType', 'module');
		}
		if (!$moduleModel) {
			if (self::getLanguageInstance($extnName)) {
				$moduleModel = new nectarcrm_Module_Model();
	...
	public function getLocationUrl($extensionId, $extensionName) {
		...
		if (is_admin($current_user)) {
			...
			return 'https://marketplace.nectarcrm.com/app/listings?id='.$extensionId;
		}
	}
