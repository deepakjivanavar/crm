...
class Settings_ExtensionStore_Basic_Action extends Settings_nectarcrm_IndexAjax_View {
	...
	protected function postReview(nectarcrm_Request $request) {
	...
	protected function getDateString($date) {
		$dateString = nectarcrm_Util_Helper::formatDateTimeIntoDayString($date);
		return $dateString;
	}

	protected function logoutMarketPlace(nectarcrm_Request $request) {
		$modelInstance = $this->getModelInstance();
		$modelInstance->logoutMarketPlace($request);
	}

	protected function uninstallExtension(nectarcrm_Request $request) {
		...
		if ($extnType == 'module') {
			...
			if ($languageInstance) {
				...
				nectarcrm_Language::deregister($langPrefix);
				//remove files
	...
	protected function registerAccount(nectarcrm_Request $request) {
		$options = array();
	...
	protected function updateTrialMode(nectarcrm_Request $request) {
		$response = new nectarcrm_Response();
		$importedModuleName = $request->get('extensionName');
		$trial = $request->get('trial');
		$db = PearDatabase::getInstance();
		$db->pquery('UPDATE nectarcrm_tab SET trial = ? WHERE name = ?', array($trial, $importedModuleName));
		$response->setResult(array('success' => true, 'message' => 'Extension Store Installed'));
		$response->emit();
	}

	protected function updateCardDetails(nectarcrm_Request $request) {
	...
	public function forgotPassword(nectarcrm_Request $request) {
		$response = new nectarcrm_Response();
