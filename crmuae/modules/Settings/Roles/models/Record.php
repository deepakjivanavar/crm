 ...
 */
class Settings_Roles_Record_Model extends Settings_nectarcrm_Record_Model {
	...
	public function getChildren() {
		...
		if(!$this->children) {
			...
			$sql = 'SELECT * FROM nectarcrm_role WHERE parentrole LIKE ? AND depth = ?';
	...
	public function getAllChildren() {
		...
		$sql = 'SELECT * FROM nectarcrm_role WHERE parentrole LIKE ? ORDER BY depth';
    ...
    public function getProfileIdList(){
        ...
        $db = PearDatabase::getInstance();
        $query = 'SELECT profileid FROM nectarcrm_role2profile WHERE roleid=?';
    ...
    public function getDirectlyRelatedProfileId() {
        ...
        $query = 'SELECT directly_related_to_role, nectarcrm_profile.profileid FROM nectarcrm_role2profile 
                  INNER JOIN nectarcrm_profile ON nectarcrm_profile.profileid = nectarcrm_role2profile.profileid 
                  WHERE nectarcrm_role2profile.roleid=?';
...
//	}
	...
	public function save() {
		...
		if(empty($roleId)) {
			$mode = '';
			$roleIdNumber = $db->getUniqueId('nectarcrm_role');
		...
		if($mode == 'edit') {
			$sql = 'UPDATE nectarcrm_role SET rolename=?, parentrole=?, depth=?, allowassignedrecordsto=? WHERE roleid=?';
			$params = array($this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto'), $roleId);
			$db->pquery($sql, $params);
		} else {
			$sql = 'INSERT INTO nectarcrm_role(roleid, rolename, parentrole, depth, allowassignedrecordsto) VALUES (?,?,?,?,?)';
			$params = array($roleId, $this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto'));
			$db->pquery($sql, $params);
			$picklist2RoleSQL = "INSERT INTO nectarcrm_role2picklist SELECT '".$roleId."',picklistvalueid,picklistid,sortid
					FROM nectarcrm_role2picklist WHERE roleid = ?";
		...
		if(!empty($profileIds)) {
			...
			if($noOfProfiles > 0) {
				$db->pquery('DELETE FROM nectarcrm_role2profile WHERE roleid=?', array($roleId));

				$sql = 'INSERT INTO nectarcrm_role2profile(roleid, profileid) VALUES (?,?)';
	...
	public function delete($transferToRole) {
		...
		$usersResult = $db->pquery('SELECT userid FROM nectarcrm_user2role WHERE roleid = ?', array($roleId));
		...
		$db->pquery('UPDATE nectarcrm_user2role SET roleid=? WHERE roleid=?', array($transferRoleId, $roleId));

		$db->pquery('DELETE FROM nectarcrm_role2profile WHERE roleid=?', array($roleId));
		$db->pquery('DELETE FROM nectarcrm_group2role WHERE roleid=?', array($roleId));
		$db->pquery('DELETE FROM nectarcrm_group2rs WHERE roleandsubid=?', array($roleId));

		//delete handling for sharing rules
		deleteRoleRelatedSharingRules($roleId);

		$db->pquery('DELETE FROM nectarcrm_role WHERE roleid=?', array($roleId));
	...
	public static function getAll($baseRole = false) {
		...
		$sql = 'SELECT * FROM nectarcrm_role';
		if (!$baseRole) {
			$sql .= ' WHERE depth != ?';
			$params[] = 0;
		}
	...
	public static function getInstanceById($roleId) {
        ...
        $instance = nectarcrm_Cache::get('roleById',$roleId);
        if($instance){
            return $instance;
        }
        
        $sql = 'SELECT * FROM nectarcrm_role WHERE roleid = ?';
        $params = array($roleId);
        $result = $db->pquery($sql, $params);
        if($db->num_rows($result) > 0) {
            $instance =  self::getInstanceFromQResult($result, 0);
            nectarcrm_Cache::set('roleById',$roleId,$instance);
            return $instance;
        }
		return null;
	}
	...
	public static function getBaseRole() {
		...
		$sql = 'SELECT * FROM nectarcrm_role WHERE depth=0 LIMIT 1';
		$params = array();
		$result = $db->pquery($sql, $params);
		if($db->num_rows($result) > 0) {
   ...
   public static function getInstanceByName($name, $excludedRecordId = array()) {
       $db = PearDatabase::getInstance();
       $sql = 'SELECT * FROM nectarcrm_role WHERE rolename=?';
       $params = array($name);
   ...
   public function getUsers() {
	   $db = PearDatabase::getInstance();
	   $result = $db->pquery('SELECT userid FROM nectarcrm_user2role WHERE roleid = ?', array($this->getId()));
