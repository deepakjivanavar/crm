...
class Settings_CustomerPortal_Module_Model extends Settings_nectarcrm_Module_Model {
	...
	public function getCurrentPortalUser() {
		...
		$result = $db->pquery("SELECT prefvalue FROM nectarcrm_customerportal_prefs WHERE prefkey = 'userid' AND tabid = 0", array());
	...
	public function getCurrentDefaultAssignee() {
		...
		$result = $db->pquery("SELECT default_assignee FROM nectarcrm_customerportal_settings", array());
		if ($db->num_rows($result)) {
	 ...
	 * @return <Array> list of portal modules <nectarcrm_Module_Model>
	 */
	public function getModulesList() {
		if (!$this->portalModules) {
			$db = PearDatabase::getInstance();

			$query = "SELECT nectarcrm_customerportal_tabs.*, nectarcrm_tab.name FROM nectarcrm_customerportal_tabs
					INNER JOIN nectarcrm_tab ON nectarcrm_customerportal_tabs.tabid = nectarcrm_tab.tabid AND nectarcrm_tab.presence = 0 ORDER BY nectarcrm_customerportal_tabs.sequence";
	 ...
	 * @return <Array> list of Contact Related Modules <nectarcrm_Module_Model>
	 */
	public function getContactRelatedModulesList() {
		$contacModuleModel = nectarcrm_Module_Model::getInstance('Contacts');
		$relationModules = nectarcrm_Relation_Model::getAllRelations($contacModuleModel);
	...
	public function save() {
		...
		foreach ($enableModules as $moduleId => $visibility) {
			...
			$db->pquery('INSERT INTO nectarcrm_customerportal_tabs(tabid,visible) VALUES(?,?) ON DUPLICATE KEY UPDATE visible = ?', array($tabid, $visibility, $visibility));
		}

		$updateSequenceQuery = " UPDATE nectarcrm_customerportal_tabs SET sequence = ? WHERE tabid = ?";
		...
		if ($dashboardWidgets) {
			$db->pquery('UPDATE nectarcrm_customerportal_settings SET default_assignee=?, support_notification=?, announcement=?, widgets=?', array($defaultAssignee, $renewalPeriod, $announcement, $dashboardWidgets));
		}
		...
		if (!empty($relatedModuleList)) {
			foreach ($relatedModuleList as $module => $info) {
				$tabid = getTabid($module);
				$db->pquery('INSERT INTO nectarcrm_customerportal_relatedmoduleinfo(tabid, relatedmodules) VALUES(?,?) ON DUPLICATE KEY UPDATE relatedmodules = ?', array($tabid, $info, $info));
			}
		}
		...
		if (!empty($recordsVisible)) {
			foreach ($recordsVisible as $module => $info) {
				$tabid = getTabid($module);
				if ($info == 'all') {
					$db->pquery('UPDATE nectarcrm_customerportal_fields SET records_visible = ? WHERE tabid = ?', array(1, $tabid));
				} else if ($info == 'onlymine') {
					$db->pquery('UPDATE nectarcrm_customerportal_fields SET records_visible = ? WHERE tabid = ?', array(0, $tabid));
				}
			}
		}

		// clearing mem-cache for CustomerPortal
		nectarcrm_Cache::delete('CustomerPortal', 'activeModules');
		nectarcrm_Cache::delete('CustomerPortal', 'activeFields');
		...
		if (!empty($recordPermissions)) {
			...
			$db->pquery('UPDATE nectarcrm_customerportal_tabs SET createrecord=?,editrecord=? WHERE tabid=?', array($updatedPermissions['create'], $updatedPermissions['edit'], getTabid($updatedPermissions['module'])));
		}
	}

	public function getRelatedModules($sourceModule) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT relatedmodules FROM nectarcrm_customerportal_relatedmoduleinfo WHERE tabid = ? ', array(getTabid($sourceModule)));
	...
	public function getDashboardInfo() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM nectarcrm_customerportal_settings', array());
	...
	public function getSelectedFields($tabId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT fieldinfo FROM nectarcrm_customerportal_fields WHERE tabid = ?', array($tabId));
		if ($db->num_rows($result) > 0) {
	...
	public function updateFields($tabId, $fieldJson) {
		$db = PearDatabase::getInstance();
		$db->pquery('INSERT INTO nectarcrm_customerportal_fields(tabid, fieldinfo) VALUES(?,?) ON DUPLICATE KEY UPDATE fieldinfo = ?', array($tabId, $fieldJson, $fieldJson));
	}

	public function getRecordVisiblity($tabId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT records_visible FROM nectarcrm_customerportal_fields WHERE tabid= ?', array($tabId));
		if ($db->num_rows($result)) {
	...
	public function getRecordPermissions($tabid) {
		$db = PearDatabase::getInstance();
		$permissionResult = $db->pquery('SELECT createrecord,editrecord FROM nectarcrm_customerportal_tabs WHERE tabid=?', array($tabid));
