...
class Settings_Potentials_Mapping_Model extends Settings_Leads_Mapping_Model {
	...
	public function getMapping($editable = false) {
		if (!$this->mapping) {
			...
			$query = 'SELECT * FROM nectarcrm_convertpotentialmapping';
			if ($editable) {
	...
	public function save($mapping) {
		...
		if ($deleteMappingsList) {
			$db->pquery('DELETE FROM nectarcrm_convertpotentialmapping WHERE editable = 1 AND cfmid IN ('. generateQuestionMarks($deleteMappingsList) .')', $deleteMappingsList);
		}

		if ($createMappingsList) {
			$insertQuery = 'INSERT INTO nectarcrm_convertpotentialmapping(potentialfid, projectfid) VALUES ';
		...
		if ($updateMappingsList) {
			...
			$db->pquery("UPDATE nectarcrm_convertpotentialmapping $potentialQuery, $projectQuery WHERE editable = ?", array(1));
		}
	}
	...
	public static function getRestrictedFieldIdsList() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM nectarcrm_convertpotentialmapping WHERE editable = ?', array(0));
		$numOfRows = $db->num_rows($result);
	...
	public static function deleteMapping($mappingIdsList) {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM nectarcrm_convertpotentialmapping WHERE cfmid IN ('. generateQuestionMarks($mappingIdsList). ')', $mappingIdsList);
	}
