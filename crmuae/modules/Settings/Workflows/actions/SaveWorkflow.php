...
class Settings_Workflows_SaveWorkflow_Action extends nectarcrm_Action_Controller {
	...
	public function process(nectarcrm_Request $request) {
		...
		$moduleModel = Settings_nectarcrm_Module_Model::getInstance($request->getModule(false));
		if ($recordId) {
		...
		if ($executionCondition == '6') {
			...
			if ($workflowScheduleType == Workflow::$SCHEDULED_WEEKLY) {
				...
				$currentTime = nectarcrm_Util_Helper::getActiveAdminCurrentDateTime();
		...
		// Added to save the condition only when its changed from nectarcrm6
	...
	function saveTasks($workflowModel, $request) {
		...
		if (!empty($tasks)) {
			foreach ($tasks as $task) {
				...
				$request = new nectarcrm_Request($taskDecodedArray, $taskDecodedArray);
