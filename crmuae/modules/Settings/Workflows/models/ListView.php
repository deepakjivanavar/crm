 ...
 */
class Settings_Workflows_ListView_Model extends Settings_nectarcrm_ListView_Model {
	...
	public function getListViewEntries($pagingModel) {
		...
		$listQuery .= $module->baseIndex . " FROM ". $module->baseTable.
					  ' INNER JOIN nectarcrm_tab ON nectarcrm_tab.name='. $module->baseTable.'.module_name';
		$params = array();
		$sourceModule = $this->get('sourceModule');
		if(!empty($sourceModule)) {
			$listQuery .= ' WHERE nectarcrm_tab.presence IN (0,2) AND module_name = ?';
			$params[] = $sourceModule;
		} else {
			$listQuery .= ' WHERE nectarcrm_tab.presence IN (0,2)';
		}
	...
	public function getListViewCount() {
		...
		$listQuery = 'SELECT count(*) AS count FROM ' . $module->baseTable . ' 
						INNER JOIN nectarcrm_tab ON nectarcrm_tab.name = '. $module->baseTable .'.module_name
						AND nectarcrm_tab.presence IN (0,2)';
