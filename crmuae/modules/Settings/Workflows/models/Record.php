...
class Settings_Workflows_Record_Model extends Settings_nectarcrm_Record_Model {
...
//			return nectarcrm_Language_Handler::getTranslatedString($executionConditionAsLabel, 'Settings:Workflows');
//		}
//		if($key == 'module_name') {
//			$moduleName = parent::get($key);
//			return nectarcrm_Language_Handler::getTranslatedString($moduleName, $moduleName);
//		}
	...
	public function getRecordLinks() {
		...
		$recordLinks = array(
			...
			array(
				...
				'linkurl' => 'javascript:nectarcrm_List_Js.deleteRecord('.$this->getId().');',
				'linkicon' => 'icon-trash'
			)
		);
		...
		if(!empty($conditions)) {
			foreach($conditions as $index => $info) {
				...
				for($i = 0; $i < count($valueArray); $i++) {
					if(nectarcrm_Functions::isDateValue($valueArray[$i])) {
						$isDateValue = true;
	...
	public static function getUpdateFieldTaskIdsForModule($moduleName, $fieldName) {
		$ids = array();
		$db = PearDatabase::getInstance();
		$sql = 'SELECT * FROM com_nectarcrm_workflows
				INNER JOIN com_nectarcrm_workflowtasks ON com_nectarcrm_workflows.workflow_id = com_nectarcrm_workflowtasks.workflow_id
				WHERE module_name = ?
				AND task LIKE ? 
				AND task LIKE ? ';
	...
	public static function updateWorkflowStatus($record, $status){
	  $db = PearDatabase::getInstance();
	  $sql = 'UPDATE com_nectarcrm_workflows SET status = ? WHERE workflow_id = ?';
		...
		if(is_array($wfCond)) {
			for ($k=0; $k<(count($wfCond)); ++$k){
				...
				if(count($matches)==0){
					...
					$fieldModel = nectarcrm_Field_Model::getInstance($fieldName, $referenceModuleModel);
					$referenceFieldModel = nectarcrm_Field_Model::getInstance($referenceField, $moduleModel);
					if($fieldModel) {
