...
class EmailTemplates_Record_Model extends nectarcrm_Record_Model {
	...
	public static function getInstanceById($templateId, $module=null) {
		...
		$sql = 'SELECT * FROM nectarcrm_emailtemplates WHERE templateid = ?';
		...
		throw new Exception(vtranslate('LBL_RECORD_DELETE', 'nectarcrm'), 1);
	}
    
    public static function getAllForEmailTask($moduleName) {
        $db = PearDatabase::getInstance();
		$sql = 'SELECT templateid, templatename, body, deleted FROM nectarcrm_emailtemplates WHERE module = ? ORDER BY templateid DESC';
	...
	public static function getSystemTemplateBySubject($subject) {
        $db = PearDatabase::getInstance();
		$sql = 'SELECT * FROM nectarcrm_emailtemplates WHERE subject = ? AND systemtemplate = ?';
	...
	public function updateImageName($imageName) {
		$db = PearDatabase::getInstance();
		if (!empty($imageName)) {
			$db->pquery('UPDATE nectarcrm_emailtemplates SET templatepath=? WHERE templateid=?', array($imageName . ".png", $this->getId()));
		}
	}
