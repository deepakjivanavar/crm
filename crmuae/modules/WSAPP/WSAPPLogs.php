...
class WSAPP_Logs {
	const basetable = 'nectarcrm_wsapp_logs_basic';
	const detailstable = 'nectarcrm_wsapp_logs_details';
	...
	static function add($syncRecords) {
		...
		$id = $adb->getUniqueID('nectarcrm_wsapp_logs_basic');
		...
		$params1 = array($id, $extensiontabid, $extensionModule, $syncTime, $recordsCount['app']['create'],
						...
						$recordsCount['nectarcrm']['create'], $recordsCount['nectarcrm']['update'],
						$recordsCount['nectarcrm']['delete'], $recordsCount['nectarcrm']['skipped'], $userId);
	...
	function getSyncRecordsDetails($syncRecords) {
		$a = $b = $c = $d = $i = $j = $k = $l = 0;
		$recordCount = array('nectarcrm'	=> array('update' => 0, 'create' => 0, 'delete' => 0, 'skipped' => 0), 
				...
				foreach ($records as $record) {
					foreach ($record as $type => $data) {
						...
						if ($type == 'source') {
							...
							switch($data->getMode()) {
								case WSAPP_SyncRecordModel::WSAPP_UPDATE_MODE	:	$recordCount['nectarcrm']['update']++;
																					$recordDetails['vt_record']['update'][$i++] = $recordInfo['id'];
																					break;
								case WSAPP_SyncRecordModel::WSAPP_CREATE_MODE	:	$recordCount['nectarcrm']['create']++;
																					$recordDetails['vt_record']['create'][$j++] = $recordInfo['id'];
																					break;
								case WSAPP_SyncRecordModel::WSAPP_DELETE_MODE	:	$recordCount['nectarcrm']['delete']++;
																					$recordDetails['vt_record']['delete'][$k++] = $source['_serverid'];
																					break;
								case 'skipped'									:	if(empty($recordInfo['messageidentifier'])) {
																						$moduleModel = nectarcrm_Module_Model::getInstance($extensionModule);
