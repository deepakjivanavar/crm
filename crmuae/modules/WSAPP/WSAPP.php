...
class WSAPP {
	...
	function registerCustomWebservices( $operations ) {
		...
		foreach($operations as $operation_name => $operation_info) {	
			$checkres = $adb->pquery("SELECT operationid FROM nectarcrm_ws_operation WHERE name=?", array($operation_name));
			if($checkres && $adb->num_rows($checkres) < 1) {
				$operation_id = $adb->getUniqueId('nectarcrm_ws_operation');
			
				$operation_res = $adb->pquery(
					"INSERT INTO nectarcrm_ws_operation (operationid, name, handler_path, handler_method, type, prelogin) 
					VALUES (?,?,?,?,?,?)",
				...
				foreach($operation_parameters as $parameter_name => $parameter_type) {
					$adb->pquery(
						"INSERT INTO nectarcrm_ws_operation_parameters (operationid, name, type, sequence) 
						VALUES(?,?,?,?)", array($operation_id, $parameter_name, $parameter_type, ($parameter_index+1))
					);
					++$parameter_index;
				}
				nectarcrm_Utils::Log("Opearation $operation_name enabled successfully.");
			} else {
				nectarcrm_Utils::Log("Operation $operation_name already exists.");
			}
		}
	}
    ...
    function registerHandlers(){
        ...
        $appTypehandler = array();
        $appTypehandler['type'] = "nectarcrmCRM";
        $appTypehandler['handlerclass'] = "nectarcrmCRMHandler";
        $appTypehandler['handlerpath'] = "modules/WSAPP/Handlers/nectarcrmCRMHandler.php";
        $handlerDetails[] = $appTypehandler;

        foreach($handlerDetails as $appHandlerDetails)
             $adb->pquery("INSERT INTO nectarcrm_wsapp_handlerdetails VALUES(?,?,?)",array($appHandlerDetails['type'],$appHandlerDetails['handlerclass'],$appHandlerDetails['handlerpath']));
    }
    
    function registernectarcrmCRMApp(){
       $db = PearDatabase::getInstance();
        $appName = "nectarcrmCRM";
        $type  ="user";
        $uid = uniqid();
        $db->pquery("INSERT INTO nectarcrm_wsapp (name, appkey,type) VALUES(?,?,?)", array($appName, $uid,$type));
    }
	...
	function registerWsappWorkflowhandler(){
		...
		$em->registerHandler('nectarcrm.entity.aftersave', 'modules/WSAPP/WorkFlowHandlers/WSAPPAssignToTracker.php', 'WSAPPAssignToTracker','',$dependentEventHandlersJson);
	}

	function registerSynclibEventHandler(){
		$className='WSAPP_nectarcrmSyncEventHandler';
		$path = 'modules/WSAPP/synclib/handlers/nectarcrmSyncEventHandler.php';
		$type = 'nectarcrmSyncLib';
