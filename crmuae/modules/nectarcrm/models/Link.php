...
/**
 * nectarcrm Link Model Class
 */
class nectarcrm_Link_Model extends nectarcrm_Link {
	 ...
	 * @return nectarcrm_Link_Model instance
	 */
	...
	public function getIconPath() {
		...
		return nectarcrm_Theme::getImagePath($this->getIcon());
	}
	 ...
	 * @param nectarcrm_Link_Model $link - link model
	 * @result nectarcrm_Link_Model - current Instance;
	 */
	public function addChildLink(nectarcrm_Link_Model $link) {
	 ...
	 * @result <array> - list of nectarcrm_Link_Model instances
	 */
	...
	public function convertToNativeLink() {
		...
		if(!$this->isPageLoadLink()){
		   ...
		   $url = nectarcrm_Util_Helper::toSafeHTML($url);
		   return $url;
		}
		...
		if(!empty($module) && !empty ($sourceModule) && !empty($sourceRecord)) {
			...
			$relatedModuleModel = nectarcrm_Module_Model::getInstance($module);
			...
			if($relationModel->isDirectRelation()){
				...
				foreach($fieldList as $fieldName=>$fieldModel) {
					if($fieldModel->getFieldDataType() == nectarcrm_Field_Model::REFERENCE_TYPE) {
	...
	/**
	 * Function to get the instance of nectarcrm Link Model from the given array of key-value mapping
	...
	/**
	 * Function to get the instance of nectarcrm Link Model from a given nectarcrm_Link object
	...
	/**
	 * Function to get all the nectarcrm Link Models for a module of the given list of link types
	 * @param <Number> $tabid
	 * @param <Array> $type
	...
	public static function getAllByType($tabid, $type = false, $parameters = false) {
		$links = nectarcrm_Cache::get('links-'.$tabid, $type);
		if(!$links) {
			$links = parent::getAllByType($tabid, $type, $parameters);
			nectarcrm_Cache::set('links-'.$tabid, $type, $links);
		}
	...
	*/
	...
	public function isExtensionAccessible() {
		...
		$moduleModel = nectarcrm_Module_Model::getInstance($extensionName);
