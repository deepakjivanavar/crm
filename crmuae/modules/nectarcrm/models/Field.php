...
/**
 * nectarcrm Field Model Class
 */
class nectarcrm_Field_Model extends nectarcrm_Field {
	...
	public function getModule() {
		if(!$this->module) {
			...
			$this->module = nectarcrm_Module_Model::getInstanceFromModuleObject($moduleObj);
		}
		return $this->module;
	}
	...
	public function getDisplayValue($value, $record=false, $recordInstance = false) {
		if(!$this->uitype_instance) {
			$this->uitype_instance = nectarcrm_Base_UIType::getInstanceFromField($this);
		}
	...
	public function isNameField() {
		$nameFieldObject = nectarcrm_Cache::get('EntityField',$this->getModuleName());
		if(!$nameFieldObject){
	 ...
	 * @return nectarcrm_Base_UIType or UI Type specific model instance
	 */
	public function getUITypeModel() {
		return nectarcrm_Base_UIType::getInstanceFromField($this);
	}
	...
	public function getPicklistValues() {
		...
		if($fieldDataType == 'picklist' || $fieldDataType == 'multipicklist') {
			...
			if($this->isRoleBased()) {
				...
				$picklistValues = nectarcrm_Util_Helper::getRoleBasedPicklistValues($fieldName, $userModel->get('roleid'));
			}else{
				$picklistValues = nectarcrm_Util_Helper::getPickListValues($fieldName);
			}
	...
	/**
	 * Static Function to get the instance fo nectarcrm Field Model from a given nectarcrm_Field object
	 * @param nectarcrm_Field $fieldObj - vtlib field object
	 * @return nectarcrm_Field_Model instance
	 */
	public static function getInstanceFromFieldObject(nectarcrm_Field $fieldObj) {
		$objectProperties = get_object_vars($fieldObj);
		$className = nectarcrm_Loader::getComponentClassName('Model', 'Field', $fieldObj->getModuleName());
	...
	public function getReportFilterColumnName() {
		...
		if($tableName == 'nectarcrm_crmentity' && $columnName !='smownerid'){
			$tableName = 'nectarcrm_crmentity'.$moduleName;
	 ...
	 * @param <nectarcrm_Module_Model> $blockModel - block instance
	 * @return <array> List of field model
	 */
	public static function getAllForModule($moduleModel){
		$fieldModelList = nectarcrm_Cache::get('ModuleFields',$moduleModel->id);
		if(!$fieldModelList){
			...
			foreach($fieldObjects as $fieldObject){
				...
				nectarcrm_Cache::set('field-'.$moduleModel->getId(),$fieldModelObject->getId(),$fieldModelObject);
				nectarcrm_Cache::set('field-'.$moduleModel->getId(),$fieldModelObject->getName(),$fieldModelObject);
			}

			nectarcrm_Cache::set('ModuleFields',$moduleModel->id,$fieldModelList);
		}
	...
	public static function  getInstance($value, $module = false) {
		$fieldObject = null;
		if($module){
			$fieldObject = nectarcrm_Cache::get('field-'.$module->getId(), $value);
		}
		if(!$fieldObject){
			$fieldObject = parent::getInstance($value, $module);
			if($module){
				nectarcrm_Cache::set('field-'.$module->getId(),$value,$fieldObject);
			}
		}
	...
	function getDocumentFolders() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM nectarcrm_attachmentsfolder', array());
	...
	*/
	...
	public function getCurrencyList() {
		...
		$result = $db->pquery('SELECT id, currency_name FROM nectarcrm_currency_info WHERE currency_status = ? AND deleted=0', array('Active'));
	...
	public function getPermissions($accessmode = 'readonly') {
		...
		if ($privileges->hasGlobalReadPermission()) {
			...
			$modulePermission = nectarcrm_Cache::get('modulePermission-'.$accessmode, $this->getModuleId());
			if (!$modulePermission) {
		...
		$profilelist = $privileges->get('profiles');
				...
				if (count($profilelist) > 0) {
					if ($accessmode == 'readonly') {
						$query = "SELECT nectarcrm_profile2field.visible,nectarcrm_field.fieldid FROM nectarcrm_field INNER JOIN nectarcrm_profile2field ON nectarcrm_profile2field.fieldid=nectarcrm_field.fieldid INNER JOIN nectarcrm_def_org_field ON nectarcrm_def_org_field.fieldid=nectarcrm_field.fieldid WHERE nectarcrm_field.tabid=? AND nectarcrm_profile2field.visible=0 AND nectarcrm_def_org_field.visible=0  AND nectarcrm_profile2field.profileid in (" . generateQuestionMarks($profilelist) . ") AND nectarcrm_field.presence in (0,2) GROUP BY nectarcrm_field.fieldid";
					} else {
						$query = "SELECT nectarcrm_profile2field.visible,nectarcrm_field.fieldid FROM nectarcrm_field INNER JOIN nectarcrm_profile2field ON nectarcrm_profile2field.fieldid=nectarcrm_field.fieldid INNER JOIN nectarcrm_def_org_field ON nectarcrm_def_org_field.fieldid=nectarcrm_field.fieldid WHERE nectarcrm_field.tabid=? AND nectarcrm_profile2field.visible=0 AND nectarcrm_profile2field.readonly=0 AND nectarcrm_def_org_field.visible=0  AND nectarcrm_profile2field.profileid in (" . generateQuestionMarks($profilelist) . ") AND nectarcrm_field.presence in (0,2) GROUP BY nectarcrm_field.fieldid";
					}
					$params = array($tabid, $profilelist);
				} else {
					if ($accessmode == 'readonly') {
						$query = "SELECT nectarcrm_profile2field.visible,nectarcrm_field.fieldid FROM nectarcrm_field INNER JOIN nectarcrm_profile2field ON nectarcrm_profile2field.fieldid=nectarcrm_field.fieldid INNER JOIN nectarcrm_def_org_field ON nectarcrm_def_org_field.fieldid=nectarcrm_field.fieldid WHERE nectarcrm_field.tabid=? AND nectarcrm_profile2field.visible=0 AND nectarcrm_def_org_field.visible=0  AND nectarcrm_field.presence in (0,2) GROUP BY nectarcrm_field.fieldid";
					} else {
						$query = "SELECT nectarcrm_profile2field.visible,nectarcrm_field.fieldid FROM nectarcrm_field INNER JOIN nectarcrm_profile2field ON nectarcrm_profile2field.fieldid=nectarcrm_field.fieldid INNER JOIN nectarcrm_def_org_field ON nectarcrm_def_org_field.fieldid=nectarcrm_field.fieldid WHERE nectarcrm_field.tabid=? AND nectarcrm_profile2field.visible=0 AND nectarcrm_profile2field.readonly=0 AND nectarcrm_def_org_field.visible=0  AND nectarcrm_field.presence in (0,2) GROUP BY nectarcrm_field.fieldid";
					}
					$params = array($tabid);
				}
				...
				nectarcrm_Cache::set('modulePermission-'.$accessmode,$tabid,$modulePermission);
	...
	public function __update() {
		$db = PearDatabase::getInstance();
		$query = 'UPDATE nectarcrm_field SET typeofdata=?,presence=?,quickcreate=?,masseditable=?,defaultvalue=?,summaryfield=?,headerfield=?';
	...
	public static function getInstanceFromFieldId($fieldId, $moduleTabId) {
		...
		$query = 'SELECT * FROM nectarcrm_field WHERE fieldid IN ('.generateQuestionMarks($fieldId).') AND tabid=?';
		...
		foreach ($referenceList as $referenceModuleName) {
			...
			if($autoFillData) {
				foreach($autoFillData as $data) {
					...
					$referenceModuleModel = nectarcrm_Module_Model::getInstance($referenceModuleName);
	...
	public function getOneToOneRelationField(){
		...
		return nectarcrm_Field_Model::getInstance($this->get('related_field_id'));
	}
	 ...
	 * @return nectarcrm_Relation_Model / false;
	 */
	public function getRelationShipForThisField() {
		return nectarcrm_Relation_Model::getInstanceFromRelationFied($this->getId());
	}
	...
	public function getPicklistColors() {
		...
		if (in_array($fieldDataType, array('picklist', 'multipicklist'))) {
			...
			if (!in_array($fieldName, array('hdnTaxType', 'region_id')) && !in_array($this->getModuleName(), array('Users'))) {
				$db = PearDatabase::getInstance();
				$picklistValues = $this->getPicklistValues();
				$tableName = "nectarcrm_$fieldName";
				if (nectarcrm_Utils::CheckTable($tableName)) {
