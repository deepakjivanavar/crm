...
class nectarcrm_MiniList_Model extends nectarcrm_Widget_Model {
	...
	public function getTargetModuleModel() {
		if (!$this->targetModuleModel) {
			$this->targetModuleModel = nectarcrm_Module_Model::getInstance($this->getTargetModule());
		}
	...
	public function getTitle($prefix='') {
		...
		$suffix = '';
		$customviewrs = $db->pquery('SELECT viewname FROM nectarcrm_customview WHERE cvid=?', array($this->widgetModel->get('filterid')));
	...
	public function getHeaders() {
		...
		if (!$this->listviewHeaders) {
			...
			foreach ($this->listviewController->getListViewHeaderFields() as $fieldName => $webserviceField) {
				$fieldObj = nectarcrm_Field::getInstance($webserviceField->getFieldId());
				$headerFieldModels[$fieldName] = nectarcrm_Field_Model::getInstanceFromFieldObject($fieldObj);
			}
	...
	public function getRecords() {
		...
		if (!$this->listviewRecords) {
			...
			$query = str_replace(" FROM ", ",nectarcrm_crmentity.crmid as id FROM ", $query);
            if($this->getTargetModule() == 'Calendar') {
                $query = str_replace(" WHERE ", " WHERE nectarcrm_crmentity.setype = 'Calendar' AND ", $query);
            }
