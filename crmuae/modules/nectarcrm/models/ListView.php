...
/**
 * nectarcrm ListView Model Class
 */
class nectarcrm_ListView_Model extends nectarcrm_Base_Model {
	...
	public function getSideBarLinks($linkParams) {
		...
		$listLinks = nectarcrm_Link_Model::getAllByType($this->getModule()->getId(), $listLinkTypes);
	...
	public function getListViewMassActions($linkParams) {
		...
		if($currentUserModel->hasModuleActionPermission($moduleModel->getId(), 'EditView')) {
			$massActionLinks[] = array(
				...
				'linklabel' => 'LBL_EDIT',
				'linkurl' => 'javascript:nectarcrm_List_Js.triggerMassEdit("index.php?module='.$moduleModel->get('name').'&view=MassActionAjax&mode=showMassEditForm");',
				'linkicon' => ''
			);
		}
		if($currentUserModel->hasModuleActionPermission($moduleModel->getId(), 'Delete')) {
			$massActionLinks[] = array(
				...
				'linkurl' => 'javascript:nectarcrm_List_Js.massDeleteRecords("index.php?module='.$moduleModel->get('name').'&action=MassDelete");',
				'linkicon' => ''
			);
		}
	...
	public function getListViewHeaders() {
		...
		foreach($headerFields as $fieldName => $webserviceField) {
			...
			if(count($matches) > 0) {
				...
				$referenceFieldModel = nectarcrm_Field_Model::getInstance($referenceFieldName, $referenceModuleModel);
				...
				$fieldInstance = nectarcrm_Field_Model::getInstance($fieldName,$module);
	...
	public function getListViewEntries($pagingModel) {
		...
		if(!empty($orderBy) && $orderByFieldModel) {
			if($orderBy == 'roleid' && $moduleName == 'Users'){
				$listQuery .= ' ORDER BY nectarcrm_role.rolename '.' '. $sortOrder; 
			} else {
	...
	public static function getInstanceForPopup($value) {
		...
		$modelClassName = nectarcrm_Loader::getComponentClassName('Model', 'ListView', $value);
		$instance = new $modelClassName();
		$moduleModel = nectarcrm_Module_Model::getInstance($value);
	...
	public function getAdvancedLinks(){
		...
		if($duplicatePermission && $editPermission) {
			$advancedLinks[] = array(
				...
				'linkurl' => 'Javascript:nectarcrm_List_Js.showDuplicateSearchForm("index.php?module='.$moduleModel->getName().
		...
		if($exportPermission) {
			$advancedLinks[] = array(
					...
					'linkurl' => 'javascript:nectarcrm_List_Js.triggerExportAction("'.$this->getModule()->getExportUrl().'")',
					'linkicon' => ''
				);
		}
