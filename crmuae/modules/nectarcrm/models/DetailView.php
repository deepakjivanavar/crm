...
class nectarcrm_DetailView_Model extends nectarcrm_Base_Model {
	...
	public function getDetailViewLinks($linkParams) {
		...
		if($moduleModel->isDuplicateOptionAllowed('CreateView', $recordId)) {
			...
			$linkModelList['DETAILVIEW'][] = nectarcrm_Link_Model::getInstanceFromValues($duplicateLinkModel);
		}

		if($this->getModule()->isModuleRelated('Emails') && nectarcrm_RecipientPreference_Model::getInstance($this->getModuleName())) {
			$emailRecpLink = array('linktype' => 'DETAILVIEW',
								'linklabel' => vtranslate('LBL_EMAIL_RECIPIENT_PREFS',  $this->getModuleName()),
								'linkurl' => 'javascript:nectarcrm_Index_Js.showRecipientPreferences("'.$this->getModuleName().'");',
								'linkicon' => '');
			$linkModelList['DETAILVIEW'][] = nectarcrm_Link_Model::getInstanceFromValues($emailRecpLink);
		}
		...
		foreach($relatedLinks as $relatedLinkEntry) {
			$relatedLink = nectarcrm_Link_Model::getInstanceFromValues($relatedLinkEntry);
		...
		if($currentUserModel->isAdminUser()) {
			...
			foreach($settingsLinks as $settingsLink) {
				$linkModelList['DETAILVIEWSETTING'][] = nectarcrm_Link_Model::getInstanceFromValues($settingsLink);
			}
		}
	...
	public function getSideBarLinks($linkParams) {
		...
		if($currentUser->getTagCloudStatus()) {
			...
			$linkModel = nectarcrm_Link_Model::getInstanceFromValues($tagWidget);
	...
	public static function getInstance($moduleName,$recordId) {
		$modelClassName = nectarcrm_Loader::getComponentClassName('Model', 'DetailView', $moduleName);
