 ...
 *************************************************************************************/
/**
 * nectarcrm Widget Model Class
 */
class nectarcrm_Widget_Model extends nectarcrm_Base_Model {
	...
	/**
	 * Function to get the instance of nectarcrm Widget Model from the given array of key-value mapping
	 * @param <Array> $valueMap
	 * @return nectarcrm_Widget_Model instance
	 */
	...
	public static function getInstance($linkId, $userId) {
		...
		$result = $db->pquery('SELECT * FROM nectarcrm_module_dashboard_widgets
			INNER JOIN nectarcrm_links ON nectarcrm_links.linkid = nectarcrm_module_dashboard_widgets.linkid
			WHERE linktype = ? AND nectarcrm_links.linkid = ? AND userid = ?', array('DASHBOARDWIDGET', $linkId, $userId));
	...
	public static function updateWidgetPosition($position, $linkId, $widgetId, $userId) {
		...
		$db = PearDatabase::getInstance();
		$sql = 'UPDATE nectarcrm_module_dashboard_widgets SET position=? WHERE userid=?';
	...
	public static function updateWidgetSize($size, $linkId, $widgetId, $userId, $tabId) {
		if ($linkId || $widgetId) {
			$db = PearDatabase::getInstance();
			$sql = 'UPDATE nectarcrm_module_dashboard_widgets SET size=? WHERE userid=?';
	...
	public static function getInstanceWithWidgetId($widgetId, $userId) {
		...
		$result = $db->pquery('SELECT * FROM nectarcrm_module_dashboard_widgets
			INNER JOIN nectarcrm_links ON nectarcrm_links.linkid = nectarcrm_module_dashboard_widgets.linkid
			WHERE linktype = ? AND nectarcrm_module_dashboard_widgets.id = ? AND userid = ?', array('DASHBOARDWIDGET', $widgetId, $userId));
	...
	public function add() {
		...
		$sql = 'SELECT id FROM nectarcrm_module_dashboard_widgets WHERE linkid = ? AND userid = ? AND dashboardtabid=?';
		...
		if(!$db->num_rows($result)) {
			$db->pquery('INSERT INTO nectarcrm_module_dashboard_widgets(linkid, userid, filterid, title, data,dashboardtabid) VALUES(?,?,?,?,?,?)',
	...
	public function remove() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM nectarcrm_module_dashboard_widgets WHERE id = ? AND userid = ?',
	...
	public static function deleteChartReportWidgets($reportId) {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM nectarcrm_module_dashboard_widgets WHERE reportid = ?',
				array($reportId));
	}
	...
	public function getDeleteUrl() {
		$url = 'index.php?module=nectarcrm&action=RemoveWidget&linkid='. $this->get('linkid');
		$widgetid = $this->has('widgetid')? $this->get('widgetid') : $this->get('id');
		if ($widgetid) $url .= '&widgetid=' . $widgetid;
