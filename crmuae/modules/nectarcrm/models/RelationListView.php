...
class nectarcrm_RelationListView_Model extends nectarcrm_Base_Model {
	...
	public function getSelectRelationLinks() {
		...
		foreach($selectLinkList as $selectLink) {
			$selectLinkModel[] = nectarcrm_Link_Model::getInstanceFromValues($selectLink);
		}
		return $selectLinkModel;
	}
	...
	public function getAddRelationLinks() {
		...
		if($relatedModel->get('label') == 'Calendar'){
			...
			$addLinkList[] = array(
				...
				'linkicon' => 'nectarcrm.png',
				'linkdropdowns' => $nectarcrmDocumentTypes,
				'linkclass' => 'addDocumentTonectarcrm',
			);
		}else{
		...
		foreach($addLinkList as $addLink) {
			$addLinkModel[] = nectarcrm_Link_Model::getInstanceFromValues($addLink);
		}
		return $addLinkModel;
	}
	...
	public function getEntries($pagingModel) {
		...
		if ($this->get('whereCondition') && is_array($this->get('whereCondition'))) {
			...
			foreach ($whereCondition as $fieldName => $fieldValue) {
				if (is_array($fieldValue)) {
					...
					$type = $fieldValue[3];
					if ($type == 'time') {
						$searchValue = nectarcrm_Time_UIType::getTimeValueWithSeconds($searchValue);
					}
		...
		if($orderBy) {
			...
			if($orderByFieldModuleModel && $orderByFieldModuleModel->isReferenceField()) {
				...
				$qualifiedOrderBy = 'nectarcrm_crmentity'.$orderByFieldModuleModel->get('column');
				$selectAndFromClause .= ' LEFT JOIN nectarcrm_crmentity AS '.$qualifiedOrderBy.' ON '.
				...
				$query .= ' ORDER BY '.$qualifiedOrderBy.'.label '.$sortOrder;
				 ...
				 $query .= ' ORDER BY COALESCE(CONCAT(nectarcrm_users.first_name,nectarcrm_users.last_name),nectarcrm_groups.groupname) '.$sortOrder;
			} else{
				...
				if($qualifiedOrderBy == 'nectarcrm_activity.date_start' && ($relationModuleName == 'Calendar' || $relationModuleName == 'Emails')) {
					$qualifiedOrderBy = "str_to_date(concat(nectarcrm_activity.date_start,nectarcrm_activity.time_start),'%Y-%m-%d %H:%i:%s')";
				}
			...
			$query .= ' ORDER BY nectarcrm_crmentity.modifiedtime DESC';
		}
		...
		for($i=0; $i< $db->num_rows($result); $i++ ) {
			...
			$record = nectarcrm_Record_Model::getCleanInstance($relationModule->get('name'));
	...
	public function getRelationQuery() {
		...
		$query = $queryComponents[0].' ,nectarcrm_crmentity.crmid FROM '.$queryComponents[1];
	...
	public static function getInstance($parentRecordModel, $relationModuleName, $label=false) {
		...
		$className = nectarcrm_Loader::getComponentClassName('Model', 'RelationListView', $parentModuleName);
		$instance = new $className();

		$parentModuleModel = $parentRecordModel->getModule();
		$relatedModuleModel = nectarcrm_Module_Model::getInstance($relationModuleName);
		$instance->setRelatedModuleModel($relatedModuleModel);

		$relationModel = nectarcrm_Relation_Model::getInstance($parentModuleModel, $relatedModuleModel, $label);
		...
		if(!$relationModel){
			...
			foreach ($referenceFieldOfParentModule as $fieldName=>$fieldModel) {
				...
				if(in_array($relatedModuleName, $refredModulesOfReferenceField)){
					$relationModelClassName = nectarcrm_Loader::getComponentClassName('Model', 'Relation', $parentModuleModel->getName());
	...
	public function getRelatedEntriesCount() {
		...
		if ($position) {
			...
			if($relatedModuleName == 'Calendar') {
				$relationQuery = 'SELECT DISTINCT nectarcrm_crmentity.crmid, nectarcrm_activity.activitytype ';
			} else {
				$relationQuery = 'SELECT COUNT(DISTINCT nectarcrm_crmentity.crmid) AS count';
			}
	...
	public function getCurrencySymbol($recordId, $fieldModel) {
		...
		if(($fieldName == 'unit_price') && ($moduleName == 'Products' || $moduleName == 'Services')) {
			$query = "SELECT currency_symbol FROM nectarcrm_currency_info WHERE id = (";
			if($moduleName == 'Products') 
				$query .= "SELECT currency_id FROM nectarcrm_products WHERE productid = ?)";
			else if($moduleName == 'Services')
				$query .= "SELECT currency_id FROM nectarcrm_service WHERE serviceid = ?)";

			$result = $db->pquery($query, array($recordId));
			return $db->query_result($result, 0, 'currency_symbol');
		} else if(($tableName == 'nectarcrm_invoice' || $tableName == 'nectarcrm_quotes' || $tableName == 'nectarcrm_purchaseorder' || $tableName == 'nectarcrm_salesorder') &&
			...
			$query = "SELECT currency_symbol FROM nectarcrm_currency_info WHERE id = ( SELECT currency_id FROM ".$tableName." WHERE ".$focus->table_index." = ? )";
			$result = $db->pquery($query, array($recordId));
			return $db->query_result($result, 0, 'currency_symbol');
		} else {
