...
/**
 * nectarcrm Module Model Class
 */
class nectarcrm_Module_Model extends nectarcrm_Module {
	...
	public function isCommentEnabled() {
		...
		$commentsModuleModel = nectarcrm_Module_Model::getInstance('ModComments');
		if($commentsModuleModel && $commentsModuleModel->isActive()) {
			$relatedToFieldResult = $db->pquery('SELECT fieldid FROM nectarcrm_field WHERE fieldname = ? AND tabid = ?',
					array('related_to', $commentsModuleModel->getId()));
			$fieldId = $db->query_result($relatedToFieldResult, 0, 'fieldid');
			if(!empty($fieldId)) {
				$relatedModuleResult = $db->pquery('SELECT relmodule FROM nectarcrm_fieldmodulerel WHERE fieldid = ?', array($fieldId));
	...
	public function saveRecord(nectarcrm_Record_Model $recordModel) {
		...
		foreach($fields as $fieldName => $fieldValue) {
			...
			if(is_array($fieldValue)){
				...
				/*
				 * for ajax edit, in nectarcrm_SaveAjax_Action we are setting relatedContact to 
	 ...
	 * @param nectarcrm_Record_Model $recordModel
	 */
	public function deleteRecord(nectarcrm_Record_Model $recordModel) {
		$moduleName = $this->get('name');
	...
	public function getModuleMeta($userModel = false) {
		if(empty($this->moduleMeta)){
			...
			$this->moduleMeta = nectarcrm_ModuleMeta_Model::getInstance($this->get('name'), $userModel);
		}
	...
	public function getAllFilterCvidForModule() {
		...
		$result = $db->pquery("SELECT cvid FROM nectarcrm_customview WHERE viewname = 'All' AND entitytype = ?",
					array($this->getName()));
	...
	public function getRecordFromArray($valueArray, $rawData=false) {
		$modelClassName = nectarcrm_Loader::getComponentClassName('Model', 'Record', $this->get('name'));
	 ...
	 * @return <Array of nectarcrm_Block_Model> - list of block models
	 */
	public function getBlocks() {
		if(empty($this->blocks)) {
			$blocksList = array();
			$moduleBlocks = nectarcrm_Block_Model::getAllForModule($this);
	...
	public function getFields($blockInstance=false) {
		if(empty($this->fields)){
			$moduleBlockFields = nectarcrm_Field_Model::getAllForModule($this);
			$this->fields = array();
	...
	/**
	 * Function gives fields based on the type
	 * @return <nectarcrm_Field_Model> with field label as key
	 */
	 ...
	 * @return <nectarcrm_Field_Model> with field id as key
	 */
	 ...
	 * @return <Array of nectarcrm_Relation_Model>
	 */
	public function getRelations() {
		if(empty($this->relations)) {
			return nectarcrm_Relation_Model::getAllRelations($this);
		}
	...
	public function getField($fieldName){
		return nectarcrm_Field_Model::getInstance($fieldName,$this);
	}
	...
	public function getRecentRecords($limit=10) {
		...
		$query = 'SELECT * FROM nectarcrm_crmentity '.$nonAdminQuery.' WHERE setype=? AND '.$deletedCondition.' AND modifiedby = ? ORDER BY modifiedtime DESC LIMIT ?';
	...
	public function getDeletedRecordCondition() {
		return 'nectarcrm_crmentity.deleted = 0';
	}
	...
	public function hasSequenceNumberField() {
		if(!empty($this->fields)) {
			...
			$db = PearDatabase::getInstance();
			$query = 'SELECT 1 FROM nectarcrm_field WHERE uitype=4 and tabid=?';
	...
	/**
	 * Static Function to get the instance of nectarcrm Module Model for the given id or name
	...
	/**
	 * Function to get the instance of nectarcrm Module Model from a given nectarcrm_Module object
	 * @param nectarcrm_Module $moduleObj
	 * @return nectarcrm_Module_Model instance
	 */
	public static function getInstanceFromModuleObject(nectarcrm_Module $moduleObj){
		$objectProperties = get_object_vars($moduleObj);
		$modelClassName = nectarcrm_Loader::getComponentClassName('Model', 'Module', $objectProperties['name']);
	...
	/**
	 * Function to get the instance of nectarcrm Module Model from a given list of key-value mapping
	 * @param <Array> $valueArray
	 * @return nectarcrm_Module_Model instance
	 */
	public static function getInstanceFromArray($valueArray) {
		$modelClassName = nectarcrm_Loader::getComponentClassName('Model', 'Module', $valueArray['name']);
	 ...
	 * @return <array> List of module models <nectarcrm_Module_Model>
	 */
	public static function getAll($presence = array(), $restrictedModulesList = array()) {
		...
		$cacheKey = 'modules';
		if(!$presence){
			$moduleModels = nectarcrm_Cache::get('nectarcrm', $cacheKey);
		}else{
		...
		if(!$moduleModels){
			...
			for($i=0; $i<$noOfModules; ++$i) {
				...
				nectarcrm_Cache::set('module',$row['tabid'], $moduleModels[$row['tabid']]);
				nectarcrm_Cache::set('module',$row['name'], $moduleModels[$row['tabid']]);
			}
			if(!$presence){
				nectarcrm_Cache::set('nectarcrm',$cacheKey,$moduleModels);
			}else{
				nectarcrm_Cache::set('nectarcrm', $cacheKey,$moduleModels);
			}
		}
	...
	public static function getEntityModules() {
		self::preModuleInitialize2();
		$moduleModels = nectarcrm_Cache::get('nectarcrm','EntityModules');
		if(!$moduleModels){
			$presence = array(0, 2);
			...
			nectarcrm_Cache::set('nectarcrm','EntityModules',$moduleModels);
		}
		return $moduleModels;
	}
	 ...
	 * @return <Array> - List of nectarcrm_Module_Model instances
	 */
	...
	protected static function preModuleInitialize2() {
		if(!nectarcrm_Cache::get('EntityField','all')){
			$db = PearDatabase::getInstance();
			// Initialize meta information - to speed up instance creation (nectarcrm_ModuleBasic::initialize2)
			$result = $db->pquery('SELECT modulename,tablename,entityidfield,fieldname FROM nectarcrm_entityname', array());
			...
			for($index = 0, $len = $db->num_rows($result); $index < $len; ++$index) {
				...
				nectarcrm_Cache::set('EntityField',$modulename,$entiyObj);
				nectarcrm_Cache::set('EntityField','all',true);
			}
		}
	}
	...
	public static function getCleanInstance($moduleName){
		$modelClassName = nectarcrm_Loader::getComponentClassName('Model', 'Module', $moduleName);
		$instance = new $modelClassName();
		$instance->name = $moduleName;
		return $instance;
	}
	...
	public function getDefaultCustomFilter() {
		...
		$result = $db->pquery("SELECT cvid FROM nectarcrm_customview WHERE setdefault = 1 AND entitytype = ?",
					array($this->getName()));
		if ($db->num_rows($result)) {
			return $db->query_result($result, 0, 'cvid');
		}
	...
	public function getComments($pagingModel,$user, $dateFilter='') {
		...
		$sql = 'SELECT nectarcrm_modcomments.*,nectarcrm_crmentity.createdtime AS createdtime,nectarcrm_crmentity.smownerid AS smownerid 
				FROM nectarcrm_modcomments INNER JOIN nectarcrm_crmentity ON nectarcrm_modcomments.modcommentsid = nectarcrm_crmentity.crmid 
				AND nectarcrm_crmentity.deleted = 0 
				INNER JOIN nectarcrm_crmentity crmentity2 ON nectarcrm_modcomments.related_to = crmentity2.crmid 
				AND crmentity2.deleted = 0 AND crmentity2.setype = ? 
				INNER JOIN nectarcrm_modtracker_basic ON nectarcrm_modtracker_basic.crmid = nectarcrm_crmentity.crmid';
		...
		if(!empty($dateFilter)) {
			$sql .= ' AND nectarcrm_modtracker_basic.changedon BETWEEN ? AND ? ';
			$params[] = $dateFilter['start'];
			$params[] = $dateFilter['end'];
		}

		$sql .= ' ORDER BY nectarcrm_crmentity.createdtime DESC LIMIT ?, ?';
		...
		for($i=0; $i<$noOfRows; $i++) {
			$row = $db->query_result_rowdata($result, $i);
			$commentModel = nectarcrm_Record_Model::getCleanInstance('ModComments');
	...
	public function getHistory($pagingModel, $type='', $userId='', $dateFilter='') {
		...
		$db = PearDatabase::getInstance();
				$sql = 'SELECT nectarcrm_modtracker_basic.*
								FROM nectarcrm_modtracker_basic
				...
				$sql .= ' ORDER BY nectarcrm_modtracker_basic.id DESC LIMIT ?, ?';
				$params[] = $pagingModel->getStartIndex();
				$params[] = $pagingModel->getPageLimit();
		$result = $db->pquery($sql,$params);
	...
	function getCalendarActivities($mode, $pagingModel, $user, $recordId = false) {
		...
		$query = "SELECT nectarcrm_crmentity.crmid, crmentity2.crmid AS parent_id, nectarcrm_crmentity.smownerid, nectarcrm_crmentity.setype, nectarcrm_activity.* FROM nectarcrm_activity
					...
					INNER JOIN nectarcrm_crmentity AS crmentity2 ON nectarcrm_seactivityrel.crmid = crmentity2.crmid AND crmentity2.deleted = 0 AND crmentity2.setype = ?
	...
	public function getSpecificRelationQuery($relatedModule) {
		if($relatedModule == 'Documents'){
			return ' AND nectarcrm_notes.filestatus = 1 ';
		}
		return;
	}
		...
		if($currentUser->isAdminUser()) {
			...
			if($this->hasSequenceNumberField()) {
				$settingsLinks[] = array(
					...
					'linkurl' => 'index.php?parent=Settings&module=nectarcrm&view=CustomRecordNumbering&sourceModule='.$this->getName(),
					'linkicon' => ''
				);
			}

			$webForms = nectarcrm_Module_Model::getInstance('Webforms');
	...
	public function getSearchRecordsQuery($searchValue,$searchFields, $parentId=false, $parentModule=false) {
		return "SELECT ".implode(',',$searchFields)." FROM nectarcrm_crmentity WHERE label LIKE '%$searchValue%' AND nectarcrm_crmentity.deleted = 0";
	}
	...
	public function searchRecord($searchValue, $parentId=false, $parentModule=false, $relatedModule=false) {
		...
		if(!empty($searchValue) && empty($parentId) && empty($parentModule)) {
			$matchingRecords = nectarcrm_Record_Model::getSearchResult($searchValue, $this->getName());
	...
	public function vtJsonDependentModules() {
		vimport('~~/modules/com_nectarcrm_workflow/WorkflowComponents.php');
		$db = PearDatabase::getInstance();
	...
	public function getRelatedModuleRecordIds(nectarcrm_Request $request, $recordIds = array(), $nonAdminCheck = false) {
		$db = PearDatabase::getInstance();
		...
		$query = "SELECT relationfieldid,related_tabid
					FROM nectarcrm_relatedlists
					WHERE nectarcrm_relatedlists.tabid=? AND relation_id IN (".generateQuestionMarks($relationIds).")";
		...
		for($i=0 ;$i<$num_rows; $i++) {
			...
			$relatedModuleModel = nectarcrm_Module_Model::getInstance($relatedTabId);
		...
		if(!empty($relationFieldMapping)) {
			...
			//for ($i=0; $i<count($relatedModules); $i++) {
				...
				$sql = "SELECT nectarcrm_crmentity.crmid FROM nectarcrm_crmentity";
				...
				if(empty($relationFieldId)){
					...
					//Fallback to nectarcrm_crmentityrel if both focus and relationfieldid is empty
					if(empty($tablename)) {
						$tablename = 'nectarcrm_crmentityrel';
						$tabIndex = 'crmid';
						$relIndex = 'crmid';
					}
					//END

					if($tablename == 'nectarcrm_crmentityrel'){
						$sql .= " INNER JOIN $tablename ON ($tablename.relcrmid = nectarcrm_crmentity.crmid OR $tablename.crmid = nectarcrm_crmentity.crmid)
							WHERE ($tablename.crmid IN (".  generateQuestionMarks($recordIds).")) OR ($tablename.relcrmid IN (".  generateQuestionMarks($recordIds)."))";
						foreach ($recordIds as $key => $recordId) {
							array_push($params, $recordId);
						}
					} else {
						$sql .= " INNER JOIN $tablename ON $tablename.$tabIndex = nectarcrm_crmentity.crmid
							WHERE $tablename.$relIndex IN (".  generateQuestionMarks($recordIds).")";
					}
				}else{
					$fieldModel = nectarcrm_Field_Model::getInstance($relationFieldId);
					...
					if($tablename == $relatedModuleFocus->table_name){
						...
						$sql .= " INNER JOIN $modulePrimaryTableName ON $modulePrimaryTableName.$modulePrimaryTableIndex = nectarcrm_crmentity.crmid
							INNER JOIN $tablename ON $tablename.$tabIndex = $modulePrimaryTableName.$modulePrimaryTableIndex 
							WHERE $tablename.$relIndex IN (".  generateQuestionMarks($recordIds).")";
					}
				}

				$sql .=' AND nectarcrm_crmentity.deleted = 0';
				foreach ($recordIds as $key => $recordId) {
					array_push($params, $recordId);
				}
	...
	*/
	...
	function getAppName() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT appname,visible FROM nectarcrm_app2tab WHERE tabid = ?', array($this->getId()));
...
//						3 => 'LBL_PREFER_NECTARCRM_RECORD',
	...
	public function getModuleIcon() {
		...
		$imageFilePath = 'layouts/'.nectarcrm_Viewer::getLayoutName()."/modules/$moduleName/$moduleName.png";
