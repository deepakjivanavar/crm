...
class nectarcrm_Util_Helper {
	...
	public static function formatDateDiffInStrings($dateTime, $isUserFormat = FALSE) {
		try{
			...
			if($isUserFormat) {
				$dateTime = nectarcrm_Datetime_UIType::getDBDateTimeValue($dateTime);
			}
	 ...
	 * @return <String> nectarcrm6 displayable data
	 */
	public static function tonectarcrm6SafeHTML($input) {
		$allowableTags = '<a><br>';
	...
	public static function checkRecordExistance($recordId){
		global $adb;
		$query = 'Select deleted from nectarcrm_crmentity where crmid=?';
	...
	public static function formatDateIntoStrings($date, $time = false) {
		...
		$dateTimeInUserFormat = nectarcrm_Datetime_UIType::getDisplayDateTimeValue($date . ' ' . $time);
		...
		if ($currentUser->get('hour_format') === '12') {
			$displayTime = nectarcrm_Time_UIType::getTimeValueInAMorPM($displayTime);
		}

		$today = nectarcrm_Date_UIType::getDisplayDateValue(date('Y-m-d H:i:s'));
		$tomorrow = nectarcrm_Date_UIType::getDisplayDateValue(date('Y-m-d H:i:s', strtotime('tomorrow')));
	...
	public static function getRecordName ($recordId, $checkDelete=false) {
		...
		$query = 'SELECT label from nectarcrm_crmentity where crmid=?';
		if($checkDelete) {
			$query.= ' AND deleted=0';
		}
		$result = $adb->pquery($query,array($recordId));
	...
	public static function getRecordId($recordName, $module = array(), $checkDelete = false) {
		...
		$query = 'SELECT crmid from nectarcrm_crmentity where label=?';
	...
	public static function formatDateTimeIntoDayString($dateTime, $skipConversion = FALSE) {
		...
		if (!$skipConversion) {
			$dateTimeInUserFormat = nectarcrm_Datetime_UIType::getDisplayDateTimeValue($dateTime);
		}
	...
	public static function getPickListValues($fieldName) {
		...
		$primaryKey = nectarcrm_Util_Helper::getPickListId($fieldName);
		$query = 'SELECT '.$primaryKey.', '.$fieldName.' FROM nectarcrm_'.$fieldName.' order by sortorderid';
		$values = array();
	...
	public static function getBaseCurrency() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM nectarcrm_currency_info WHERE defaultid < 0', array());
	...
	public static function getRoleBasedPicklistValues($fieldName, $roleId) {
		if(nectarcrm_Cache::get('PicklistRoleBasedValues',$fieldName.$roleId)){
			return nectarcrm_Cache::get('PicklistRoleBasedValues',$fieldName.$roleId);
		}
		$db = PearDatabase::getInstance();

		$query = "SELECT $fieldName
				  FROM nectarcrm_$fieldName
					  INNER JOIN nectarcrm_role2picklist on nectarcrm_role2picklist.picklistvalueid = nectarcrm_$fieldName.picklist_valueid
				  WHERE roleid=? and picklistid in (select picklistid from nectarcrm_picklist) order by sortorderid";
		...
		nectarcrm_Cache::set('PicklistRoleBasedValues',$fieldName.$roleId,$picklistValues);
...
/**
	...
	public static function getCreator($recordId) {
		...
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT smcreatorid FROM nectarcrm_crmentity WHERE crmid = ?', array($recordId));
	 ...
	 * @param <DateTime> $dateTime
	 * @param <nectarcrm_Users_Model> $userObject
	...
	public static function convertDateTimeIntoUsersDisplayFormat($dateTime, $userObject = null) {
		...
		if ($userModel->get('hour_format') == '12') {
			$time = nectarcrm_Time_UIType::getTimeValueInAMorPM($time);
		}
	...
	public static function getUserCurrencyInfo() {
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$result = $db->pquery('SELECT * FROM nectarcrm_currency_info WHERE id = ?', array($currentUser->get('currency_id'))); 
		if($db->num_rows($result)) return $db->query_result_rowdata($result, 0);
	}
			...
			foreach($groupInfo as $fieldSearchInfo){
				   ...
				   preg_match('/(\w+) ; \((\w+)\) (\w+)/', $fieldName, $matches);
					if (count($matches) != 0) {
						...
						$fieldInfo = nectarcrm_Field_Model::getInstance($referenceFieldName, $referenceModuleModel);
					...
					if(empty($fieldInfo) && $referenceModule == 'Calendar') {
						...
						$fieldInfo = nectarcrm_Field_Model::getInstance($referenceFieldName, $eventsModuleModel);
					}
	...
	*/
	public static function setCalendarDefaultActivityTypesForUser($userId) {
		$db = PearDatabase::getInstance();
		$userEntries = $db->pquery('SELECT 1 FROM nectarcrm_calendar_user_activitytypes WHERE userid=?', array($userId));
		if($db->num_rows($userEntries) <= 0) {
			$queryResult = $db->pquery('SELECT id, defaultcolor FROM nectarcrm_calendar_default_activitytypes WHERE isdefault = ?', array('1'));
			...
			foreach($activityIds as $activityId=>$color) {
				$db->pquery('INSERT INTO nectarcrm_calendar_user_activitytypes (id, defaultid, userid, color) VALUES (?,?,?,?)', array($db->getUniqueID('nectarcrm_calendar_user_activitytypes'), $activityId, $userId, $color));
			}
		}
	...
	public static function isUserDeleted($userid) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT deleted FROM nectarcrm_users WHERE id = ? AND (status=? OR deleted=?)', array($userid, 'Inactive', 1));
   ...
   public static function checkDbUTF8Support($conn) {
	...
	public static function getCalendarReferenceModulesList() {
		$moduleInstance = nectarcrm_Module_Model::getInstance('Calendar');
		$fieldInstance = nectarcrm_Field_Model::getInstance('parent_id', $moduleInstance);
	...
	public static function getBrowserInfo() {
		...
		$browserInfoModel = new nectarcrm_Base_Model($browserInfo);
	...
	static function detectModulenameFromRecordId($wsrecordid) {
		...
		$result = $adb->pquery("SELECT name FROM nectarcrm_ws_entity WHERE id=?", array($idComponents[0]));
	...
	static function detectFieldnamesToResolve($module) {
		...
		$result = $adb->pquery(
			"SELECT DISTINCT fieldname FROM nectarcrm_field WHERE uitype IN(" .
	...
	static function fetchRecordLabelsForIds($recordIds) {
		...
		$sqlResult = $adb->pquery("SELECT crmid,label from nectarcrm_crmentity WHERE crmid IN (".generateQuestionMarks($crmIds).") ;", $crmIds);
	...
	static function getRelatedModuleLabel($relatedModule, $parentModule = "Contacts") {
		...
		$sql = "SELECT nectarcrm_relatedlists.label FROM nectarcrm_relatedlists
				INNER JOIN nectarcrm_tab ON nectarcrm_relatedlists.related_tabid =nectarcrm_tab.tabid WHERE nectarcrm_tab.name=? AND nectarcrm_relatedlists.tabid=?";
		...
		switch ($fieldDataType) {
			...
			case 'reference'	:	$referenceFieldModule = $fieldModel->getReferenceList(true);
									if (count($referenceFieldModule) > 0) {
										...
										$referenceFieldModuleModel = nectarcrm_Module_Model::getInstance($referenceModule);
										...
										foreach ($mandatoryFieldModels as $mandatoryFieldModel) {
											...
											$fieldValue = nectarcrm_Util_Helper::fillMandatoryFields($fieldName, $referenceModule);
										...
										$query = "SELECT crmid FROM nectarcrm_crmentity WHERE label = ? AND deleted = ? AND setype = ?";
