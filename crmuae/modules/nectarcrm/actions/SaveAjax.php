...
class nectarcrm_SaveAjax_Action extends nectarcrm_Save_Action {
		...
		try {
			...
			foreach ($fieldModelList as $fieldName => $fieldModel) {
				...
				if ($fieldModel->getFieldDataType() == 'currency') {
					$displayValue = nectarcrm_Currency_UIType::transformDisplayValue($fieldValue);
				}
	...
	public function getRecordModelFromRequest(nectarcrm_Request $request) {
		...
		if(!empty($recordId)) {
			...
			foreach ($fieldModelList as $fieldName => $fieldModel) {
				...
				// To support Inline Edit in nectarcrm7
				...
				if($fieldName === 'contact_id' && isRecordExists($fieldValue)) {
					$contactRecord = nectarcrm_Record_Model::getInstanceById($fieldValue, 'Contacts');
