...
class nectarcrm_ExportData_Action extends nectarcrm_Mass_Action {
	...
	function getExportQuery(nectarcrm_Request $request) {
		...
		if ($mode !== 'ExportAllData') {
			...
			$searchParams = nectarcrm_Util_Helper::transferListSearchParamsToFilterCondition($searchParams, $this->moduleInstance);
			...
			if ($orderBy && $orderByFieldModel) {
				if ($orderByFieldModel->getFieldDataType() == nectarcrm_Field_Model::REFERENCE_TYPE || $orderByFieldModel->getFieldDataType() == nectarcrm_Field_Model::OWNER_TYPE) {
		...
		switch($mode) {
			...
			case 'ExportCurrentPage' :	$pagingModel = new nectarcrm_Paging_Model();
	 ...
	 * @return <String>
	 */
	function getExportContentType(nectarcrm_Request $request) {
	...
	function sanitizeValues($arr){
		...
		if(empty ($this->fieldArray)){
			...
			foreach($this->fieldArray as $fieldName => $fieldObj){
				...
				if($fieldObj->get('table') == 'nectarcrm_inventoryproductrel' && ($fieldName == 'discount_amount' || $fieldName == 'discount_percent')){
		...
		foreach($arr as $fieldName=>&$value){
			...
			if($fieldname != 'hdnTaxType' && ($uitype == 15 || $uitype == 16 || $uitype == 33)){
				...
				$value = nectarcrm_Util_Helper::getOwnerName($value);
				...
				if (trim($value) && $value != '0000-00-00 00:00:00') {
					$value = nectarcrm_Datetime_UIType::getDisplayDateTimeValue($value);
				}
			}
