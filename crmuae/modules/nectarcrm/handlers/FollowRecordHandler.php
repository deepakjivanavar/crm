	...
	function handleEvent($eventName, $entityData) {
		if ($eventName == 'nectarcrm.entity.aftersave') {
			...
			if (!in_array($moduleName, $restrictedModules)) {
				$tableName = nectarcrm_Functions::getUserSpecificTableName($moduleName);
	...
	public function getChangedFieldString($fieldModels, $changedValues, $userRecordModel) {
		...
		foreach ($fieldModels as $fieldName => $fieldModel) {
			...
			if ($fieldModel->isReferenceField()) {
				$fieldDisplayValue = nectarcrm_Util_Helper::getRecordName($fieldCurrentValue);
				...
				switch ($fieldDataType) {
					...
					case 'time'			:	if ($userRecordModel->get('hour_format') == '12') {
												$fieldDisplayValue = nectarcrm_Time_UIType::getTimeValueInAMorPM($fieldCurrentValue);
											} else {
