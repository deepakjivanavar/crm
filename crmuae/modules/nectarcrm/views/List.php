...
class nectarcrm_List_View extends nectarcrm_Index_View {
	...
	function preProcess(nectarcrm_Request $request, $display=true) {
		...
		$orderParams = nectarcrm_ListView_Model::getSortParamsSession($listViewSessionKey);

		if(empty($listHeaders)) {
			$listHeaders = $orderParams['list_headers'];
		}

		$this->listViewModel = nectarcrm_ListView_Model::getInstance($moduleName, $cvId, $listHeaders);
	...
	public function initializeListViewContents(nectarcrm_Request $request, nectarcrm_Viewer $viewer) {
		...
		if(empty($tag)) {   
			$tagSessionVal = nectarcrm_ListView_Model::getSortParamsSession($tagSessionKey);
			if(!empty($tagSessionVal)) {
				$tag = $tagSessionVal;
			}
		}else{
			nectarcrm_ListView_Model::setSortParamsSession($tagSessionKey, $tag);
		}
		...
		if($request->get('mode') == 'removeAlphabetSearch') {
			nectarcrm_ListView_Model::deleteParamsSession($listViewSessionKey, array('search_key', 'search_value', 'operator'));
			$searchKey = '';
			$searchValue = '';
			$operator = '';
		}
		if($request->get('mode') == 'removeSorting') {
			nectarcrm_ListView_Model::deleteParamsSession($listViewSessionKey, array('orderby', 'sortorder'));
			$orderBy = '';
			$sortOrder = '';
		}
		if(empty($listHeaders)) {
		...
		if(empty($orderBy) && empty($searchValue) && empty($pageNumber)) {
			...
			nectarcrm_ListView_Model::setSortParamsSession($listViewSessionKey, $params);
		}
		if($sortOrder == "ASC"){
			$nextSortOrder = "DESC";
		...
		if(!$this->listViewModel) {
			$listViewModel = nectarcrm_ListView_Model::getInstance($moduleName, $cvId, $listHeaders);
		} else {
		...
		if(!$this->moduleFieldStructure) {
			$recordStructure = nectarcrm_RecordStructure_Model::getInstanceForModule($listViewModel->getModule(), nectarcrm_RecordStructure_Model::RECORD_STRUCTURE_MODE_FILTER);
			$this->moduleFieldStructure = $recordStructure->getStructure();   
		}

		if(!$this->tags) {
			$this->tags = nectarcrm_Tag_Model::getAllAccessible($currentUser->id, $moduleName);
		}
		if(!$this->allUserTags) {
			$this->allUserTags = nectarcrm_Tag_Model::getAllUserTags($currentUser->getId());
		}
		...
		$viewer->assign('GROUPS_IDS', nectarcrm_Util_Helper::getGroupsIdsForUsers($currentUser->getId()));
	...
	protected function assignCustomViews(nectarcrm_Request $request, nectarcrm_Viewer $viewer) {
	...
	public function transferListSearchParamsToFilterCondition($listSearchParams, $moduleModel) {
		return nectarcrm_Util_Helper::transferListSearchParamsToFilterCondition($listSearchParams, $moduleModel);
	}
