...
class Assets extends CRMEntity {
	...
	var $table_name = 'nectarcrm_assets';
	 ...
	 */
	var $customFieldTable = Array('nectarcrm_assetscf', 'assetsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('nectarcrm_crmentity','nectarcrm_assets','nectarcrm_assetscf');
	...
	var $tab_name_index = Array(
		...
		'nectarcrm_assets'=>'assetsid',
		'nectarcrm_assetscf'=>'assetsid');
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		...
		global $adb;
 		...
 		if($eventType == 'module.postinstall') {
			...
			$AssetsModule  = nectarcrm_Module::getInstance('Assets');
			nectarcrm_Access::setDefaultSharing($AssetsModule);

			//Showing Assets module in the related modules in the More Information Tab
			$assetInstance = nectarcrm_Module::getInstance('Assets');
			$assetLabel = 'Assets';
			...
			$productInstance = nectarcrm_Module::getInstance('Products');
			...
			if (!($adb->num_rows($result))) {
				...
				$adb->pquery("INSERT INTO nectarcrm_modentity_num values(?,?,?,?,?,?)", array($adb->getUniqueId("nectarcrm_modentity_num"), $moduleName, 'ASSET', 1, 1, 1));
			}
	...
	function addModuleToCustomerPortal() {
		...
		$assetsResult = $adb->pquery('SELECT tabid FROM nectarcrm_tab WHERE name=?', array('Assets'));
		$assetsTabId = $adb->query_result($assetsResult, 0, 'tabid');
		if(getTabid('CustomerPortal') && $assetsTabId) {
			$checkAlreadyExists = $adb->pquery('SELECT 1 FROM nectarcrm_customerportal_tabs WHERE tabid=?', array($assetsTabId));
			if($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
				...
				$adb->query("INSERT INTO nectarcrm_customerportal_tabs(tabid,visible,sequence) VALUES ($assetsTabId,1,$nextSequence)");
			}
			$checkAlreadyExists = $adb->pquery('SELECT 1 FROM nectarcrm_customerportal_prefs WHERE tabid=?', array($assetsTabId));
			if($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
				$adb->query("INSERT INTO nectarcrm_customerportal_prefs(tabid,prefkey,prefvalue) VALUES ($assetsTabId,'showrelatedinfo',1)");
			}
		}
	}
