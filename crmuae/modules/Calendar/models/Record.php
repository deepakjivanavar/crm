...
class Calendar_Record_Model extends nectarcrm_Record_Model {
...
/**
	...
	function save() {
		...
		$_REQUEST['time_start'] = nectarcrm_Time_UIType::getTimeValueWithSeconds($_REQUEST['time_start']);
		$_REQUEST['time_end'] = nectarcrm_Time_UIType::getTimeValueWithSeconds($_REQUEST['time_end']);
		parent::save();
	}
	...
	public function delete() {
		...
		if(!empty($recurringEditMode) && $recurringEditMode != 'current') {
			...
			foreach($childRecords as $record) {
				...
				$adb->pquery("DELETE FROM nectarcrm_activity_recurring_info WHERE activityid=? AND recurrenceid=?", array($parentRecurringId, $record));
			...
			if($recurringEditMode == 'current') {
				...
				$adb->pquery("DELETE FROM nectarcrm_activity_recurring_info WHERE activityid=? AND recurrenceid=?", array($parentRecurringId, $this->getId()));
			}
	...
	public function getRecurringObject() {
		...
		$query = 'SELECT nectarcrm_recurringevents.*, nectarcrm_activity.date_start, nectarcrm_activity.time_start, nectarcrm_activity.due_date, nectarcrm_activity.time_end FROM nectarcrm_recurringevents
					INNER JOIN nectarcrm_activity ON nectarcrm_activity.activityid = nectarcrm_recurringevents.activityid
					WHERE nectarcrm_recurringevents.activityid = ?';
	...
	public function updateReminderStatus($status=1) {
		$db = PearDatabase::getInstance();
		$db->pquery("UPDATE nectarcrm_activity_reminder_popup set status = ? where recordid = ?", array($status, $this->getId()));
	...
	public function getParentRecurringRecord() {
		$adb = PearDatabase::getInstance();
		$recordId = $this->getId();
		$result = $adb->pquery("SELECT * FROM nectarcrm_activity_recurring_info WHERE activityid=? OR activityid = (SELECT activityid FROM nectarcrm_activity_recurring_info WHERE recurrenceid=?) LIMIT 1", array($recordId, $recordId));
	...
	public function getRecurringRecordsList() {
		...
		$recordId = $this->getId();
		$result = $adb->pquery("SELECT * FROM nectarcrm_activity_recurring_info WHERE activityid=? OR activityid = (SELECT activityid FROM nectarcrm_activity_recurring_info WHERE recurrenceid=?)", array($recordId, $recordId));
