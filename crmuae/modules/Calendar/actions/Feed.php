...
class Calendar_Feed_Action extends nectarcrm_BasicAjax_Action {
	...
	protected function pullDetails($start, $end, &$result, $type, $fieldName, $color = null, $textColor = 'white', $conditions = '') {
		$moduleModel = nectarcrm_Module_Model::getInstance($type);
		...
		if(count($fieldsList) == 2) {
			...
			$query.= " AND nectarcrm_crmentity.smownerid IN (".generateQuestionMarks($userAndGroupIds).")";
	...
	protected function pullEvents($start, $end, &$result, $userid = false, $color = null, $textColor = 'white', $isGroupId = false, $conditions = '') {
		...
		$query.= " AND nectarcrm_activity.activitytype NOT IN ('Emails','Task') AND ";
	...
	protected function pullTasks($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$query.= " AND nectarcrm_activity.activitytype = 'Task' AND ";
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$hideCompleted = $currentUser->get('hidecompletedevents');
		if($hideCompleted)
			$query.= "nectarcrm_activity.status != 'Completed' AND ";
