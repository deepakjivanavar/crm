...
class Calendar_DragDropAjax_Action extends Calendar_SaveAjax_Action {
	...
	public function updateDeltaOnResize(nectarcrm_Request $request){
		...
		try {
			if(isPermitted($moduleName, $actionname, $recordId) === 'no'){
				...
				$record = nectarcrm_Record_Model::getInstanceById($recordId, $moduleName);
				...
				if(!empty($recurringEditMode) && $recurringEditMode != 'current') {
					...
					foreach($childRecords as $childId) {
						$recordModel = nectarcrm_Record_Model::getInstanceById($childId, 'Events');
	...
	public function updateDeltaOnDrop(nectarcrm_Request $request){
		$moduleName = $request->getModule();
		$activityType = $request->get('activitytype');
		$recordId = $request->get('id');
		$dayDelta = $request->get('dayDelta');
