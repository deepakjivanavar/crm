...
class Calendar_SaveFollowupAjax_Action extends Calendar_SaveAjax_Action {
	...
	public function createFollowupEvent(nectarcrm_Request $request) {
        ...
        $followupStartTime = nectarcrm_Time_UIType::getTimeValueWithSeconds($request->get('followup_time_start'));
		$followupStartDateTime = nectarcrm_Datetime_UIType::getDBDateTimeValue($request->get('followup_date_start')." ".$followupStartTime);
    ...
    public function markAsHeldCompleted(nectarcrm_Request $request) {
        $moduleName = $request->getModule();
        $recordId = $request->get('record');
