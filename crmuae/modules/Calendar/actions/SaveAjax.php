...
class Calendar_SaveAjax_Action extends nectarcrm_SaveAjax_Action {
	...
	public function process(nectarcrm_Request $request) {
		...
		try {
			...
			foreach ($fieldModelList as $fieldName => $fieldModel) {
				...
				$fieldValue = $displayValue = nectarcrm_Util_Helper::toSafeHTML($recordFieldValue);
			...
			if($followupMode == 'on') {
				...
				$startTime = nectarcrm_Time_UIType::getTimeValueWithSeconds($request->get('followup_time_start'));
				$startDateTime = nectarcrm_Datetime_UIType::getDBDateTimeValue($request->get('followup_date_start') . " " . $startTime);
	...
	public function getRecordModelFromRequest(nectarcrm_Request $request) {
		...
		if(!empty($startDate)) {
			//Start Date and Time values
			$startTime = nectarcrm_Time_UIType::getTimeValueWithSeconds($request->get('time_start'));
			$startDateTime = nectarcrm_Datetime_UIType::getDBDateTimeValue($startDate." ".$startTime);
			list($startDate, $startTime) = explode(' ', $startDateTime);

			$recordModel->set('date_start', $startDate);
			$recordModel->set('time_start', $startTime);
		} else {
			$startTime = nectarcrm_Time_UIType::getTimeValueWithSeconds($recordModel->get('time_start'));
			$startDateTime = nectarcrm_Datetime_UIType::getDBDateTimeValue($recordModel->get('date_start')." ".$startTime);
			list($startDate, $startTime) = explode(' ', $startDateTime);
		...
		if(!empty($endDate)) {
			...
			$endDate = nectarcrm_Date_UIType::getDBInsertedValue($request->get('due_date'));

			if ($endTime) {
				$endTime = nectarcrm_Time_UIType::getTimeValueWithSeconds($endTime);
				$endDateTime = nectarcrm_Datetime_UIType::getDBDateTimeValue($request->get('due_date')." ".$endTime);
			...
			$endDate = nectarcrm_Date_UIType::getDBInsertedValue($recordModel->get('due_date'));

			if ($endTime) {
				$endTime = nectarcrm_Time_UIType::getTimeValueWithSeconds($endTime);
				$endDateTime = nectarcrm_Datetime_UIType::getDBDateTimeValue($recordModel->get('due_date')." ".$endTime);
				list($endDate, $endTime) = explode(' ', $endDateTime);
			}
