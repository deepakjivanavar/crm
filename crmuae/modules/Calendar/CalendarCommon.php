...
/**
 * To get the lists of nectarcrm_users id who shared their calendar with specified user
 * @param $sharedid -- The shared user id :: Type integer
 * @returns $shared_ids -- a comma seperated nectarcrm_users id  :: Type string
 */
function getSharedCalendarId($sharedid)
{
	global $adb;
	$query = "SELECT * from nectarcrm_sharedcalendar where sharedid=?";
...
function getAcceptInvitationUrl($user_id,$eventRecordModel) {
   ...
   return nectarcrm_ShortURL_Helper::generateURL($options);
}
...
function addAcceptEventLink($body,$user_id,$recordModel) {
    ...
    $acceptLink = '<div class="invitationresponse"><a href="' . 
            $acceptInvitationUrl. '" target="_blank">Accept - Add Event to nectarcrm Calendar</a></div>';
    return substr_replace($body, $acceptLink, strpos($body, '</body>'), 0);
}

/**
 * Function to get the nectarcrm_activity details for mail body
...
function getActivityDetails($description,$user_id,$from='',$recordModel=false) {
	...
	$query='SELECT body FROM nectarcrm_emailtemplates WHERE subject=? AND systemtemplate=?';
	...
	if(!empty($only_for_user)) {
		if($only_for_user != 'ALL') {
			...
			if($only_for_user == $current_user->id) {
				...
				$qcondition = " AND nectarcrm_crmentity.smownerid IN (" . $user_group_ids .")";
			} else {
				$qcondition = " AND nectarcrm_crmentity.smownerid = "  . $adb->sql_escape_string($only_for_user);
			}
		}
	}
	return $qcondition;
