...
Class Inventory_Edit_View extends nectarcrm_Edit_View {
	...
	public function process(nectarcrm_Request $request) {
		...
		if ($request->has('totalProductCount')) {
			...
			foreach ($mandatoryFieldModels as $fieldModel) {
				if ($fieldModel->isReferenceField()) {
					...
					if (nectarcrm_Util_Helper::checkRecordExistance($recordModel->get($fieldName))) {
			...
			if ($request->get('product_id') || $sourceModule === 'Products' || $request->get('productid')) {
				...
				$parentRecordModel = nectarcrm_Record_Model::getInstanceById($sourceRecord, $sourceModule);
		...
		$productModuleModel = nectarcrm_Module_Model::getInstance('Products');
		$viewer->assign('PRODUCT_ACTIVE', $productModuleModel->isActive());

		$serviceModuleModel = nectarcrm_Module_Model::getInstance('Services');
