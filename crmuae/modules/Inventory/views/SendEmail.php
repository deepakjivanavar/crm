...
class Inventory_SendEmail_View extends nectarcrm_ComposeEmail_View {
	 ...
	 */
	public function composeMailData(nectarcrm_Request $request) {
		parent::composeMailData($request);
		$viewer = $this->getViewer($request);
		$inventoryRecordId = $request->get('record');
        $recordModel = nectarcrm_Record_Model::getInstanceById($inventoryRecordId, $request->getModule());
	...
	public function populateTo($request){
		...
		foreach($toEmailConsiderableFields as $fieldName){
			...
			$referenceModule = nectarcrm_Functions::getCRMRecordType($fieldValue);
			$fieldLabel = decode_html(nectarcrm_Util_Helper::getRecordName($fieldValue));
			...
			if(isRecordExists($fieldValue)) {
				$referenceRecordModel = nectarcrm_Record_Model::getInstanceById($fieldValue, $referenceModule);
