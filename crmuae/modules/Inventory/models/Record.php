 ...
 */
class Inventory_Record_Model extends nectarcrm_Record_Model {
	 ...
	 * @return <Model> returns nectarcrm_Record_Model
	 */
	 ...
	 * @param nectarcrm_Record_Model $parentRecordModel
	 * @return Inventory_Record_Model
	 */
	public function setParentRecordData(nectarcrm_Record_Model $parentRecordModel) {
		...
		foreach ($fieldMappingList as $fieldMapping) {
			...
			$fieldModel = nectarcrm_Field_Model::getInstance($parentField, nectarcrm_Module_Model::getInstance($moduleName));
	...
	public function getPDF() {
		...
		$controllerClassName = "nectarcrm_". $moduleName ."PDFController";
	...
	public function getPDFFileName() {
		...
		if ($moduleName == 'Quotes') {
			...
			$controllerClassName = "nectarcrm_QuotePDFController";
		} else {
	...
	public function getParentRecordRelatedLineItems($parentRecordModel) {
		$userCurrencyInfo = nectarcrm_Util_Helper::getUserCurrencyInfo();
	...
	public function getCharges() {
		if (!$this->chargesAndItsTaxes) {
			...
			if ($recordId) {
				...
				$result = $db->pquery('SELECT * FROM nectarcrm_inventorychargesrel WHERE recordid = ?', array($recordId));
	...
	public function getDeductTaxes() {
		...
		if ($record && $deductTaxes) {
			...
			$result = $db->pquery('SELECT '.implode(',', $deductTaxNamesList).' FROM nectarcrm_inventoryproductrel WHERE id = ?', array($record));
			foreach ($deductTaxes as $taxId => $taxInfo) {
	...
	public function convertRequestToProducts(nectarcrm_Request $request) {
		...
		for ($i=1; $i<=$totalProductsCount; $i++) {
			...
			$productIdsList[] = $productId;
			$itemRecordModel = nectarcrm_Record_Model::getInstanceById($productId);
