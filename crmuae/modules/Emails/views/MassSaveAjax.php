...
class Emails_MassSaveAjax_View extends nectarcrm_Footer_View {
	 ...
	 */
	public function massSave(nectarcrm_Request $request) {
		...
		$result = nectarcrm_Util_Helper::transformUploadedFiles($_FILES, true);
		$_FILES = $result['file'];
		...
		if(!empty($parentEmailId) && !empty ($recordId)) {
			$parentEmailModel = nectarcrm_Record_Model::getInstanceById($parentEmailId);
		...
		if ($recordModel->checkUploadSize($documentIds)) {
			...
			foreach ($toMailInfo as $recordId => $emailValueList) {
				...
				if (!empty($relatedModule) && $relatedModule != 'Users') {
					...
					$relationModel = nectarcrm_Relation_Model::getInstance($relatedModuleModel, $recordModel->getModule());
					if ($relationModel) {
			...
			if($flag == 'SENT') {
				...
				if ($status === true) {
					// This is needed to set nectarcrm_email_track table as it is used in email reporting
