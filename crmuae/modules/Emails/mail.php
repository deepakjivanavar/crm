...
/**   Function used to send email
  ...
  *   $from_email	-- currently loggedin nectarcrm_users's email id. you can give as '' if you are not in HelpDesk module
  ...
  *   $attachment	-- whether we want to attach the currently selected file or all nectarcrm_files.[values = current,all] - optional
  *   $emailid		-- id of the email object which will be used to get the nectarcrm_attachments
  */
function send_mail($module,$to_email,$from_name,$from_email,$subject,$contents,$cc='',$bcc='',$attachment='',$emailid='',$logo='', $useGivenFromEmailAddress=false,$useSignature = 'Yes',$inReplyToMessageId='')
{
	...
	//Get the email id of assigned_to user -- pass the value and name, name must be "user_name" or "id"(field names of nectarcrm_users nectarcrm_table)
	...
	if($cachedFromEmail === null) {
		$query = "select from_email_field from nectarcrm_systems where server_type=?";
		$params = array('email');
...
/**	Function to get the user Email id based on column name and column value
  *	$name -- column name of the nectarcrm_users nectarcrm_table
  *	$val  -- column value
  */
function getUserEmailId($name,$val)
{
	...
	if($val != '')
	{
		...
		$sql = "SELECT email1, email2, secondaryemail  from nectarcrm_users WHERE status='Active' AND ". $adb->sql_escape_string($name)." = ?";
...
function addSignature($contents, $fromname, $fromEmail = '') {
	...
	if ($sign == null) {
		...
		$result = $adb->pquery("select signature, first_name, last_name from nectarcrm_users where user_name=? or user_name=? or email1=? or email2=? or secondaryemail=?", array($fromname, $fromEmail, $fromEmail, $fromEmail, $fromEmail));
...
/**	Function to set all the Mailer properties
  ...
  *	$attachment	-- whether we want to attach the currently selected file or all nectarcrm_files.
				[values = current,all] - optional
  *	$emailid	-- id of the email object which will be used to get the nectarcrm_attachments - optional
  */
function setMailerProperties($mail,$subject,$contents,$from_email,$from_name,$to_email,$attachment='',$emailid='',$module='',$logo='')
{
	...
	if(empty($userFullName)) {
		$rs = $adb->pquery("select first_name,last_name from nectarcrm_users where user_name=?", array($from_name));
	...
	//This will add all the nectarcrm_files which are related to this record or email
...
function setMailServerProperties($mail)
{
	...
	$res = $adb->pquery("select * from nectarcrm_systems where server_type=?", array('email'));
...
/**     Function to add all the nectarcrm_files as attachment with the mail object
  *     $mail -- reference of the mail object
  *     $record -- email id ie., record id which is used to get the all nectarcrm_attachments from database
  */
function addAllAttachments($mail,$record)
{
	...
	//Retrieve the nectarcrm_files from database where avoid the file which has been currently selected
	$sql = "select nectarcrm_attachments.* from nectarcrm_attachments inner join nectarcrm_seattachmentsrel on nectarcrm_attachments.attachmentsid = nectarcrm_seattachmentsrel.attachmentsid inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid = nectarcrm_attachments.attachmentsid where nectarcrm_crmentity.deleted=0 and nectarcrm_seattachmentsrel.crmid=?";
...
function MailSend($mail)
{
	...
	{
		 $log->info("Mail has been sent from the nectarcrmCRM system : Status : '".$mail->ErrorInfo."'");
		return 1;
	}
}
...
/**	Function to get the Parent email id from HelpDesk to send the details about the ticket via email
  ...
  *	$parentid -- id of the parent ie., contact or nectarcrm_account
  */
function getParentMailId($parentmodule,$parentid)
{
	...
	$adb->println("Inside the function getParentMailId. \n parent module and id => ".$parentmodule."&".$parentid);
		...
		{
				$tablename = 'nectarcrm_contactdetails';
				$idname = 'contactid';
		$first_email = 'email';
...
function parseEmailErrorString($mail_error_str)
{
	...
	foreach($mail_status as $key => $val)
	{
		...
		if($status_str[1] != 1 && $status_str[1] != '')
		{
			...
			{
				$adb->println("first elseif part - status will be 0 which is the case of assigned to nectarcrm_users's email is empty.");
...
function getDefaultAssigneeEmailIds($groupId) {
	...
	if($groupId != '') {
		...
		$result = $adb->pquery('SELECT email1,email2,secondaryemail FROM nectarcrm_users WHERE nectarcrm_users.id IN
											('.  generateQuestionMarks($userGroups->group_users).') AND nectarcrm_users.status= ?',
