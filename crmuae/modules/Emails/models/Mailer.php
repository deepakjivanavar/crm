...
class Emails_Mailer_Model extends nectarcrm_Mailer {
	...
	public static function retrieveMessageIdFromMailroom($crmId) {
		...
		$result = $db->pquery('SELECT messageid FROM nectarcrm_mailscanner_ids WHERE crmid=?', array($crmId));
	...
	public static function updateMessageIdByCrmId($messageId, $crmId) {
		...
		$existingResultObject = $db->pquery("SELECT refids FROM nectarcrm_mailscanner_ids WHERE crmid=? AND refids != 'null'", array($crmId));
		...
		if ($num_rows > 0) {
			...
			if (is_array($existingResult)) {
				...
				$db->pquery("UPDATE nectarcrm_mailscanner_ids SET refids=? WHERE crmid=? ", array($refIds, $crmId));
			}
		} else {
			$db->pquery("INSERT INTO nectarcrm_mailscanner_ids (messageid, crmid) VALUES(?,?)", array($messageId, $crmId));
		}
	}
