...
class Emails_Record_Model extends nectarcrm_Record_Model {
	...
	public function getDetailViewUrl($parentId = false) {
		...
		return 'Javascript:nectarcrm_Index_Js.showEmailPreview("'.$this->getId().'","'.$parentId.'")';
	}
	...
	public function send($addToQueue = false) {
		...
		foreach($toEmailInfo as $id => $emails) {
			...
			if ($parentModule) {
				...
				$moduleLanguageStrings = nectarcrm_Language_Handler::getModuleStringsFromFile($currentLanguage,$parentModule);
	...
	function getFromEmailAddress() {
		...
		$fromEmail = false;
		$result = $db->pquery('SELECT from_email_field FROM nectarcrm_systems WHERE server_type=?', array('email'));
	...
	function getAttachmentDetails() {
		...
		$attachmentRes = $db->pquery("SELECT * FROM nectarcrm_attachments
						INNER JOIN nectarcrm_seattachmentsrel ON nectarcrm_attachments.attachmentsid = nectarcrm_seattachmentsrel.attachmentsid
						WHERE nectarcrm_seattachmentsrel.crmid = ?", array($this->getId()));
	...
	public function getRelatedDocuments() {
		...
		$documentRes = $db->pquery("SELECT * FROM nectarcrm_senotesrel
						INNER JOIN nectarcrm_crmentity ON nectarcrm_senotesrel.notesid = nectarcrm_crmentity.crmid AND nectarcrm_senotesrel.crmid = ?
						INNER JOIN nectarcrm_notes ON nectarcrm_notes.notesid = nectarcrm_senotesrel.notesid
						INNER JOIN nectarcrm_seattachmentsrel ON nectarcrm_seattachmentsrel.crmid = nectarcrm_notes.notesid
						INNER JOIN nectarcrm_attachments ON nectarcrm_attachments.attachmentsid = nectarcrm_seattachmentsrel.attachmentsid
						WHERE nectarcrm_crmentity.deleted = 0", array($this->getId()));
	...
	public function saveDocumentDetails() {
		...
		for ($i=0; $i<$count; $i++) {
			$db->pquery("INSERT INTO nectarcrm_senotesrel(crmid, notesid) VALUES(?, ?)", array($record, $documentIds[$i]));
		}
	}
	...
	public function deleteDocumentLink($idList = array()){
		$db = PearDatabase::getInstance();
		$query = 'DELETE FROM nectarcrm_senotesrel where crmid=?';
	...
	public function deleteAttachment($emailAttachmentDetails = array()) {
		...
		$db->pquery('UPDATE nectarcrm_crmentity SET deleted=0 WHERE crmid IN('.generateQuestionMarks($attachmentIdList).')',$attachmentIdList);
		$db->pquery('DELETE FROM nectarcrm_attachments WHERE attachmentsid IN('.generateQuestionMarks($attachmentIdList).')',$attachmentIdList);
		$db->pquery('DELETE FROM nectarcrm_seattachmentsrel WHERE crmid=? and attachmentsid IN('.generateQuestionMarks($attachmentIdList).')',
	...
	public function checkUploadSize($documentIds = false) {
		...
		if (!empty ($documentIds)) {
			...
			for ($i=0; $i<$count; $i++) {
				try {
					$documentRecordModel = nectarcrm_Record_Model::getInstanceById($documentIds[$i], 'Documents');
		...
		$uploadLimit = nectarcrm_Util_Helper::getMaxUploadSizeInBytes();
	...
	public function getTrackImageDetails($crmId, $emailTrack = true) {
		...
		if($emailTrack){
			$emailId = $this->getId();
			$imageDetails = nectarcrm_Functions::getTrackImageContent($emailId, $crmId);
			return $imageDetails;
		} else {
			return null;
		}
	...
	public function updateTrackDetails($parentId) {
		...
		$db->pquery("INSERT INTO nectarcrm_email_access(crmid, mailid, accessdate, accesstime) VALUES(?, ?, ?, ?)", array($parentId, $recordId, date('Y-m-d'), date('Y-m-d H:i:s')));

		$result = $db->pquery("SELECT 1 FROM nectarcrm_email_track WHERE crmid = ? AND mailid = ?", array($parentId, $recordId));
		if ($db->num_rows($result)>0) {
			$db->pquery("UPDATE nectarcrm_email_track SET access_count = access_count+1 WHERE crmid = ? AND mailid = ?", array($parentId, $recordId));
		} else {
			$db->pquery("INSERT INTO nectarcrm_email_track(crmid, mailid, access_count) values(?, ?, ?)", array($parentId, $recordId, 1));
		}
	}
	...
	public function getClickCountValue($parentId){
		...
		$result = $db->pquery("SELECT click_count FROM nectarcrm_email_track WHERE crmid = ? AND mailid = ?", array($parentId, $this->getId()));
	...
	public function getAccessCountValue($parentId) {
		...
		$result = $db->pquery("SELECT access_count FROM nectarcrm_email_track WHERE crmid = ? AND mailid = ?", array($parentId, $this->getId()));
	...
	public static function getTrackingInfo($emailIds,$parentId) {
		...
		$sql = 'SELECT mailid, access_count,click_count FROM nectarcrm_email_track WHERE crmid = ? AND mailid IN('.generateQuestionMarks($emailIds).')';
	...
	public function getEmailFlag() {
		if(!array_key_exists('email_flag', $this->getData())) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT email_flag FROM nectarcrm_emaildetails WHERE emailid = ?", array($this->getId()));
			if($db->num_rows($result) > 0) {
	...
	function getEntityType($id) {
		...
		if (!empty($id)) {
			...
			if(!in_array($relatedModule, $emailRelatedModules)){
				$sql = 'SELECT id FROM nectarcrm_users WHERE id=?';
				$result = $db->pquery($sql, array($id));
				if($db->num_rows($result) > 0){
					$relatedModule = 'Users';
				}
	...
	public function trackClicks($parentId) {
		...
		$result = $db->pquery("SELECT 1 FROM nectarcrm_email_track WHERE crmid = ? AND mailid = ?", array($parentId, $recordId));
		if ($db->num_rows($result) > 0) {
			$db->pquery("UPDATE nectarcrm_email_track SET click_count = click_count+1 WHERE crmid = ? AND mailid = ?", array($parentId, $recordId));
		} else {
			$db->pquery("INSERT INTO nectarcrm_email_track(crmid, mailid, click_count) values(?, ?, ?)", array($parentId, $recordId, 1));
		}
	}
	...
	public function getSenderName($relatedModule = false, $relatedRecordId = false) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT from_email,idlists FROM nectarcrm_emaildetails WHERE emailid = ?", array($this->getId()));
		if ($db->num_rows($result) > 0) {
	...
	public function convertUrlsToTrackUrls($content, $crmid, $type = 'html') {
		if ($this->isEmailTrackEnabled()) {
			$extractedUrls = nectarcrm_Functions::getUrlsFromHtml($content);
	...
	function recieveEmailLookup($fieldId, $values) {
		...
		$db->pquery('INSERT INTO nectarcrm_emailslookup
	...
	function deleteEmailLookup($crmid, $fieldid = false) {
		...
		if ($fieldid) {
			...
			$db->pquery('DELETE FROM nectarcrm_emailslookup WHERE crmid=? AND fieldid=?', $params);
		} else {
			$params = array($crmid);
			$db->pquery('DELETE FROM nectarcrm_emailslookup WHERE crmid=?', $params);
		}
	}
	...
	public function updateEmailFlag() {
		$db = PearDatabase::getInstance();
		$query = 'UPDATE nectarcrm_emaildetails SET email_flag="SAVED" WHERE emailid=?';
