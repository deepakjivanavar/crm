...
require_once 'modules/nectarcrm/CRMEntity.php';
require_once 'include/QueryGenerator/QueryGenerator.php';
require_once 'vtlib/nectarcrm/Mailer.php';
require_once 'include/events/include.inc';
vimport('includes.runtime.EntryPoint');

class Import_Data_Action extends nectarcrm_Action_Controller {
	...
	public function getDefaultFieldValues($moduleMeta) {
		...
		if ($className != 'nectarcrmLineItemMeta') {
	...
	public function importData() {
		...
		$moduleModel = nectarcrm_Module_Model::getInstance($this->module);
		// pre fetch the fields and premmisions of module
		nectarcrm_Field_Model::getAllForModule($moduleModel);
		if ($this->user->is_admin == 'off') {
			nectarcrm_Field_Model::preFetchModuleFieldPermission($moduleModel->getId());
		}
	...
	public function createRecords() {
		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
				...
				if (!empty($mergeType) && $mergeType != Import_Utils_Helper::$AUTO_MERGE_NONE) {
					...
					if ($noOfDuplicates > 0) {
						if ($mergeType == Import_Utils_Helper::$AUTO_MERGE_IGNORE) {
							...
							$baseRecordModel = nectarcrm_Record_Model::getInstanceById($baseRecordId);
							...
							for ($index = 0; $index < $noOfDuplicates - 1; ++$index) {
								...
								if ($userPriviligesModel->hasModuleActionPermission($tabId, 'Delete')) {
									...
									if ($moduleName == 'Calendar') {
										$recordModel = nectarcrm_Record_Model::getInstanceById($duplicateRecordId);
										$recordModel->delete();
									} else {
			...
			if ($createRecord || $mergeType == Import_Utils_Helper::$AUTO_MERGE_MERGEFIELDS || $mergeType == Import_Utils_Helper::$AUTO_MERGE_OVERWRITE) {
				...
				if (!empty($recordId)) {
					...
					$recordModel = nectarcrm_Record_Model::getCleanInstance($this->module);
		...
		if (!empty($createdRecords)) {
			$recordModels = nectarcrm_Record_Model::getInstancesFromIds($createdRecords, $this->module);
			$entityInfos = array();
		...
		if ($this->entitydata) {
			...
			$entity->triggerEvent('nectarcrm.batchevent.save', $this->entitydata);
		}
			...
			if ($fieldDataType == 'owner') {
				...
				if (!in_array($picklistValueInLowerCase, $allPicklistValuesInLowerCase) && !empty($picklistValueInLowerCase)) {
					if ($moduleName != 'Calendar') {
						...
						$moduleObject = nectarcrm_Module::getInstance($moduleName);
						$fieldObject = nectarcrm_Field::getInstance($fieldName, $moduleObject);
						...
						nectarcrm_Cache::getInstance()->setPicklistDetails($moduleObject->getId(), $fieldName, $wsFieldDetails);
	...
	public static function runScheduledImport() {
		...
		foreach ($scheduledImports as $scheduledId => $importDataController) {
			...
			$emailSubject = 'nectarcrm CRM - Scheduled Import Report for '.$importDataController->module;
			...
			$emailData = 'nectarcrm CRM has just completed your import process. <br/><br/>' .
						$importResult.'<br/><br/>'.
			...
			$nectarcrmMailer->AddAddress($userEmail, $userName);
			$nectarcrmMailer->Subject = $emailSubject;
			$nectarcrmMailer->Body    = $emailData;
			$nectarcrmMailer->Send();

			$importDataController->finishImport();
		}
		nectarcrm_Mailer::dispatchQueue(null);
	}
	...
	public static function getScheduledImportCount() {
		$adb = PearDatabase::getInstance();
		$result = $adb->pquery("SELECT count(*) AS count FROM nectarcrm_import_queue WHERE status = ?", array(Import_Queue_Action::$IMPORT_STATUS_SCHEDULED));
	...
	public static function getImportDetails($user, $moduleName) {
		...
		if ($result) {
			...
			for ($i = 0; $i < $noOfRows; ++$i) {
				...
				$record = new nectarcrm_Base_Model();
			...
			if ($recordData['activitytype']) {
				...
				foreach ($recordData as $fieldName => $fieldValue) {
					if ($fieldValue) {
						...
						if (in_array($fieldDataType, array('reference', 'owner'))) {
							...
							$recordData[$fieldName] = nectarcrm_Time_UIType::getTimeValueWithSeconds($fieldValue);
						} else if($fieldDataType == 'boolean') {
				...
				foreach ($recordData as $fieldName => $fieldValue) {
					if (in_array($fieldName, array('date_start', 'due_date'))) {
						...
						$dateTime = nectarcrm_Datetime_UIType::getDBDateTimeValue($dateParts[0].' '.$recordData[$timeField]);
				...
				if(!empty($recordData['createdtime'])) {
					$recordData['createdtime'] = nectarcrm_Datetime_UIType::getDBDateTimeValue($recordData['createdtime']);
				}
				if ($operation != 'create') {
				...
				try {
					if ($recordData['id']) {
						$recordModel = nectarcrm_Record_Model::getInstanceById($recordData['id'], $moduleName);
					} else {
						$recordModel = nectarcrm_Record_Model::getCleanInstance($moduleName);
					}
					...
					$webServiceObj = nectarcrmWebserviceObject::fromName($db, $moduleName);
