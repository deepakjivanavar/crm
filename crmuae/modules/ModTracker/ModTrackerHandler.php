...
class ModTrackerHandler extends VTEventHandler {
	...
	function handleEvent($eventName, $data) {
			...
			if(is_array($delta)) {
				...
				foreach($delta as $fieldName => $values) {
					if($fieldName != 'modifiedtime') {
						if(!$inserted) {
							$checkRecordPresentResult = $adb->pquery('SELECT * FROM nectarcrm_modtracker_basic WHERE crmid = ? AND status = ?', array($recordId, ModTracker::$CREATED));
							if(!$adb->num_rows($checkRecordPresentResult) && $data->isNew()) {
								$status = ModTracker::$CREATED;
							} else {
								$status = ModTracker::$UPDATED;
							}
							$this->id = $adb->getUniqueId('nectarcrm_modtracker_basic');
							$changedOn = $newerColumnFields['modifiedtime'];
							if($moduleName == 'Users') {
								$date_var = date("Y-m-d H:i:s");
								$changedOn =  $adb->formatDate($date_var,true);
							}
							$adb->pquery('INSERT INTO nectarcrm_modtracker_basic(id, crmid, module, whodid, changedon, status)
										VALUES(?,?,?,?,?,?)', Array($this->id, $recordId, $moduleName,
										$current_user->id, $changedOn, $status));
							$inserted = true;
						}
						$adb->pquery('INSERT INTO nectarcrm_modtracker_detail(id,fieldname,prevalue,postvalue) VALUES(?,?,?,?)',
		...
		if($eventName == 'nectarcrm.entity.beforedelete') {
			$recordId = $data->getId();
			$columnFields = $data->getData();
