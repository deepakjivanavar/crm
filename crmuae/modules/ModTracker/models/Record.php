...
class ModTracker_Record_Model extends nectarcrm_Record_Model {
	...
	public static function getUpdates($parentRecordId, $pagingModel,$moduleName) {
		...
		$listQuery = "SELECT * FROM nectarcrm_modtracker_basic WHERE crmid = ? AND module = ? ".
	...
	function setParent($id, $moduleName) {
		if(!nectarcrm_Util_Helper::checkRecordExistance($id)) {
			$this->parent = nectarcrm_Record_Model::getInstanceById($id, $moduleName);
		} else {
			$this->parent = nectarcrm_Record_Model::getCleanInstance($moduleName);
			$this->parent->id = $id;
	...
	function getFieldInstances() {
		...
		if($this->isCreate() || $this->isUpdate()) {
			$result = $db->pquery('SELECT * FROM nectarcrm_modtracker_detail WHERE id = ?', array($id));
			$rows = $db->num_rows($result);
			for($i=0; $i<$rows; $i++) {
				...
				$fieldModel = nectarcrm_Field_Model::getInstance($row['fieldname'], $this->getParent()->getModule());
	...
	function getRelationInstance() {
		...
		if($this->isRelationLink() || $this->isRelationUnLink()) {
			$result = $db->pquery('SELECT * FROM nectarcrm_modtracker_relations WHERE id = ?', array($id));
	...
	public function getTotalRecordCount($recordId) {
    	$db = PearDatabase::getInstance();
        $result = $db->pquery("SELECT COUNT(*) AS count FROM nectarcrm_modtracker_basic WHERE crmid = ?", array($recordId));
