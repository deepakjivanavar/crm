	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($current_user) {
			...
			if (!empty($parentId)) {
				...
				$baseModule = nectarcrmWebserviceObject::fromId($adb, $recordId)->getEntityName();
			...
			if ($module == 'ModComments' && !empty($recordId)) {
				global $adb;
				$relatedModule = nectarcrmWebserviceObject::fromId($adb, $recordId)->getEntityName();
				...
				if (is_array($result)) {
					foreach ($result as $index => $value) {
						...
						if (!empty($attachmentIds)) {
							$attachmentsResult = $adb->pquery('SELECT attachmentsid,name FROM nectarcrm_attachments WHERE attachmentsid IN ('.generateQuestionMarks($attachmentIds).')', $attachmentIds);
