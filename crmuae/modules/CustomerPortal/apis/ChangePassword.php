...
class CustomerPortal_ChangePassword extends CustomerPortal_API_Abstract {
	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($current_user) {
			...
			$sql = "UPDATE nectarcrm_portalinfo SET user_password=? WHERE id=? AND user_name=?";
			$adb->pquery($sql, array(nectarcrm_Functions::generateEncryptedPassword($newPassword), $current_customer->id, $username));
