...
class CustomerPortal_FetchHistory extends CustomerPortal_FetchRecord {
	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($current_user) {
			...
			$sql = 'SELECT nectarcrm_modtracker_basic.* FROM nectarcrm_modtracker_basic
		            INNER JOIN nectarcrm_crmentity ON nectarcrm_modtracker_basic.crmid = nectarcrm_crmentity.crmid WHERE 
                    nectarcrm_modtracker_basic.module = ? AND nectarcrm_crmentity.deleted = ? AND nectarcrm_modtracker_basic.crmid IN ('.generateQuestionMarks($recordIds).')
	...
	protected function resolveReferences(&$items, $module, $user) {
		...
		$labels = nectarcrm_Util_Helper::fetchRecordLabelsForIds($ids);
		...
		foreach ($items as &$item) {
			...
			foreach ($values as $field => $value) {
				...
				if (CustomerPortal_Utils::isReferenceType($field, $describe)) {
					...
					if (!empty($previous)) {
						$value['previous'] = nectarcrm_Util_Helper::getRecordName($previous, true);
					}
					$value['current'] = nectarcrm_Util_Helper::getRecordName($current, true);
				}
