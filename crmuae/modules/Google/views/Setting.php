...
class Google_Setting_View extends nectarcrm_PopupAjax_View {
	...
	public function emitCalendarSyncSettingUI(nectarcrm_Request $request) {
	...
	public function emitContactsSyncSettingUI(nectarcrm_Request $request) {
	   echo $this->intializeContactsSyncSettingParameters($request);
	}

	public function intializeContactsSyncSettingParameters(nectarcrm_Request $request) {
		$user = Users_Record_Model::getCurrentUserModel();
		$connector = new Google_Contacts_Connector(FALSE);
		...
		$contactsModuleModel = nectarcrm_Module_Model::getInstance($request->get('sourcemodule'));
		...
		$viewer = $this->getViewer($request);
		if($request->get('onlyGoogleTonectarcrm')) {
			$viewer->assign('ONLY_GOOGLE_TO_NECTARCRM', true);
		}
		...
		$viewer->assign('NECTARCRM_EMAIL_FIELDS',$emailFields);
		$viewer->assign('NECTARCRM_PHONE_FIELDS',$phoneFields);
		$viewer->assign('NECTARCRM_URL_FIELDS',$urlFields);
		$viewer->assign('NECTARCRM_OTHER_FIELDS',$otherFields);
		$viewer->assign('IS_SYNC_READY',$isSyncReady);
