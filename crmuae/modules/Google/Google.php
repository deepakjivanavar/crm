...
class Google {
    ...
    function vtlib_handler($moduleName, $eventType) {
        ...
        if ($eventType == 'module.postinstall') {
			...
			$adb->pquery('UPDATE nectarcrm_settings_field SET active=1 WHERE name=?',array($this->LBL_GOOGLE));
        } else if ($eventType == 'module.enabled') {
            $this->addMapWidget($forModules);
            $this->addWidgetforSync($syncModules);
			$adb->pquery('UPDATE nectarcrm_settings_field SET active=0 WHERE name=?',array($this->LBL_GOOGLE));
