...
Class Google_Calendar_Connector extends WSAPP_TargetConnector {
    ...
    protected $eventCalendarFieldMappingTableName = 'nectarcrm_google_event_calendar_mapping';
    protected $calendars;
    ...
    public function emailLookUp($emailIds) {
        ...
        $sql = 'SELECT crmid FROM nectarcrm_emailslookup WHERE setype = "Contacts" AND value IN (' .  generateQuestionMarks($emailIds) . ')';
    ...
    /**
     * Tarsform Google Records to nectarcrm Records
    ...
    public function transformToSourceRecord($targetRecords, $user = false) {
        ...
        foreach ($targetRecords as $googleRecord) {
            if ($googleRecord->getMode() != WSAPP_SyncRecordModel::WSAPP_DELETE_MODE) {
                ...
                $entity = nectarcrm_Functions::getMandatoryReferenceFields('Events');
    ...
    public function getCalendar($SyncState, $user = false) {
        ...
        if (count($calendarRecords) > 0) {
            $maxModifiedTime = date('Y-m-d H:i:s', strtotime(Google_Contacts_Model::nectarcrmFormat(end($calendarRecords)->getUpdated())) + 1);
        }
    ...
    protected function putGoogleEventCalendarMap($event_ids, $calendar_id, $user) {
        if(is_array($event_ids) && count($event_ids)) {
            $db = PearDatabase::getInstance();
            $user_id = $user->getId();
            $sql = 'INSERT INTO nectarcrm_google_event_calendar_mapping (event_id, calendar_id, user_id) VALUES ';
            $sqlParams = array();
            ...
            $db->pquery('DELETE FROM nectarcrm_google_event_calendar_mapping WHERE event_id IN ('.generateQuestionMarks($event_ids).')',$event_ids);
    ...
    protected function getGoogleEventCalendarMap($user) {
        $db = PearDatabase::getInstance();
        $map = array();
        $sql = 'SELECT event_id, calendar_id FROM nectarcrm_google_event_calendar_mapping WHERE user_id = ?';
    ...
    /**
     * Push the nectarcrm records to google
     * @param <array> $records nectarcrm records to be pushed to google
    ...
    /**
     * Tarsform  nectarcrm Records to Google Records
     * @param <array> $vtEvents 
     * @return <array> tranformed nectarcrm Records
     */
