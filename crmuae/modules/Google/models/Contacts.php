...
class Google_Contacts_Model extends WSAPP_SyncRecordModel {
    ...
    public function getModifiedTime() {
        return $this->nectarcrmFormat($this->data['entity']['updated']['$t']);
    }
    ...
    function getAccountName($userId) {
        ...
        if(empty($orgName)) {
            $contactsModel = nectarcrm_Module_Model::getInstance('Contacts');
            $accountFieldInstance = nectarcrm_Field_Model::getInstance('account_id', $contactsModel);
        ...
        if(!empty($orgName)) {
            ...
            $result = $db->pquery("SELECT crmid FROM nectarcrm_crmentity WHERE label = ? AND deleted = ? AND setype = ?", array($orgName, 0, 'Accounts'));
            if($db->num_rows($result) < 1) {
				try {
					$accountModel = nectarcrm_Module_Model::getInstance('Accounts');
					$recordModel = nectarcrm_Record_Model::getCleanInstance('Accounts');
				
					$fieldInstances = nectarcrm_Field_Model::getAllForModule($accountModel);
     ...
     * @return <date> nectarcrm date Format
     */
    public function nectarcrmFormat($date) {
