...
class Google_Utils_Helper {
    ...
    const settings_table_name = 'nectarcrm_google_sync_settings';
    
    const fieldmapping_table_name = 'nectarcrm_google_sync_fieldmapping';
    ...
    public static function updateSyncTime($sourceModule, $modifiedTime = false, $user = false) {
        ...
        if (!self::getSyncTime($sourceModule, $user)) {
            if ($modifiedTime) {    
                $db->pquery('INSERT INTO nectarcrm_google_sync (googlemodule,user,synctime,lastsynctime) VALUES (?,?,?,?)', array($sourceModule, $user->id, $modifiedTime, date('Y-m-d H:i:s')));
            }
        } else {
            $db->pquery('UPDATE nectarcrm_google_sync SET synctime = ?,lastsynctime = ? WHERE user=? AND googlemodule=?', array($modifiedTime, date('Y-m-d H:i:s'), $user->id, $sourceModule));
        }
    }
    ...
    private static function intialiseUpdateSchema() {
        if (!nectarcrm_Utils::CheckTable('nectarcrm_google_sync')) {
            nectarcrm_Utils::CreateTable('nectarcrm_google_sync', '(googlemodule varchar(50),user int(10), synctime datetime,lastsynctime datetime)', true);
        }
    }
    ...
    public static function getSyncTime($sourceModule, $user = false) {
        ...
        if(!$user)
            $user = Users_Record_Model::getCurrentUserModel();
        $result = $db->pquery('SELECT synctime FROM nectarcrm_google_sync WHERE user=? AND googlemodule=?', array($user->id, $sourceModule));
    ...
    public static function getLastSyncTime($sourceModule) {
        ...
        self::intialiseUpdateSchema();
        $user = Users_Record_Model::getCurrentUserModel();
        $result = $db->pquery('SELECT lastsynctime FROM nectarcrm_google_sync WHERE user=? AND googlemodule=?', array($user->id, $sourceModule));
        if ($result && $db->num_rows($result) > 0) {
            $row = $db->fetch_array($result);
    ...
    public static function getSyncUsers(){
        global $adb;
        $users = array();
        $result = $adb->pquery("SELECT DISTINCT userid FROM nectarcrm_google_oauth2", array());
    ...
    static function saveFieldMappings($sourceModule, $fieldMappings) {
        ...
        $sql = 'INSERT INTO ' . self::fieldmapping_table_name . ' (nectarcrm_field,google_field,google_field_type,google_custom_label,user) VALUES ';
    ...
    static function getFieldMappingForUser($user = false) {
        ...
        $sql = 'SELECT nectarcrm_field,google_field,google_field_type,google_custom_label FROM ' . self::fieldmapping_table_name . ' WHERE user = ?';
        $result = $db->pquery($sql,array($user->getId()));
        for($i=0;$i<$db->num_rows($result);$i++) {  
            $row = $db->fetch_row($result);
            $fieldmapping[$row['nectarcrm_field']] = array(
