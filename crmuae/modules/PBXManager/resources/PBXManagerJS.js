...
var nectarcrm_PBXManager_Js = {
	/**
	 * Function registers PBX for popups
	 */
	registerPBXCall : function() {
		nectarcrm_PBXManager_Js.requestPBXgetCalls();
	},
	...
	registerPBXOutboundCall : function(number,record) {
		nectarcrm_PBXManager_Js.makeOutboundCall(number,record);
	},
	/**
			...
			if(result) {
				for(i=0; i< result.length; i++) {
					...
					if(jQuery('#pbxcall_'+record.pbxmanagerid+'').size()== 0 )
						nectarcrm_PBXManager_Js.showPBXIncomingCallPopup(record);
					else
						nectarcrm_PBXManager_Js.updatePBXIncomingCallPopup(record);
					}
				}
		});
		nectarcrm_PBXManager_Js.removeCompletedCallPopup();
	},
	...
	showPBXIncomingCallPopup : function(record) {
		...
		if(record.user) {
			if(record.user != record.current_user_id) {
				nectarcrm_PBXManager_Js.removeCallPopup(record.pbxmanagerid);
			}
		}

		// To check if it is new or existing contact
		nectarcrm_PBXManager_Js.checkIfRelatedModuleRecordExist(record);
		...
		jQuery('#pbxcontactsave_'+record.pbxmanagerid+'').bind('click', function(e) {
			...
			nectarcrm_PBXManager_Js.createRecord(e, record);
			...
			app.request.get({url: url}).then(function(e, result){
				if(result){
					if(result!='in-progress' && result!='ringing'){
						nectarcrm_PBXManager_Js.removeCallPopup(callid);
					}
				}
			});
		}
	...
	makeOutboundCall : function(number, record){
	 ...
	 registerEvents : function(){
		...
		app.request.get({url: url}).then(function(e, result){
			if(result) {
				nectarcrm_PBXManager_Js.registerPBXCall();
				//setInterval("nectarcrm_PBXManager_Js.registerPBXCall()", 3000);
			}
		});
	}
...
jQuery(window).load(function() {
	nectarcrm_PBXManager_Js.registerEvents();
});
