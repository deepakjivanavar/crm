...
class Campaigns_RelationListView_Model extends nectarcrm_RelationListView_Model {

	/**
	 * Function to get the links for related list
	 * @return <Array> List of action models <nectarcrm_Link_Model>
	 */
	public function getLinks() {
		...
		if (array_key_exists($relatedModuleName, $relationModel->getEmailEnabledModulesInfoForDetailView())) {
			...
			if ($currentUserPriviligesModel->hasModulePermission(getTabid('Emails'))) {
				$emailLink = nectarcrm_Link_Model::getInstanceFromValues(array(
	 ...
	 * @return <array> List of record models <nectarcrm_Record_Model>
	 */
	public function getEntries($pagingModel) {
		...
		if (array_key_exists($relatedModuleName, $emailEnabledModulesInfo) && $relatedRecordModelsList) {
			...
			$query = "SELECT campaignrelstatus, $fieldName FROM $tableName
						INNER JOIN nectarcrm_campaignrelstatus ON nectarcrm_campaignrelstatus.campaignrelstatusid = $tableName.campaignrelstatusid
