...
class Mobile_WS_Utils {
	...
	static function getnectarcrmVersion() {
	...
	static function getVersion() {
		global $adb;
		$versionResult = $adb->pquery("SELECT version FROM nectarcrm_tab WHERE name='Mobile'", array());
	...
	static function getEntityModuleWSId($moduleName) {
		...
		if (!isset(self::$moduleWSIdCache[$moduleName])) {
			global $adb;
			$result = $adb->pquery("SELECT id FROM nectarcrm_ws_entity WHERE name=?", array($moduleName));
	...
	static function getEntityModuleWSIds($ignoreNonModule = true) {
		...
		$result = false;
		if($ignoreNonModule) {
			$result = $adb->pquery("SELECT id, name FROM nectarcrm_ws_entity WHERE ismodule=1", array());
		} else {
			$result = $adb->pquery("SELECT id, name FROM nectarcrm_ws_entity", array());
		}
	...
	static function getEntityFieldnames($module) {
		global $adb;
		$result = $adb->pquery("SELECT fieldname FROM nectarcrm_entityname WHERE modulename=?", array($module));
		$fieldnames = array();
		if($result && $adb->num_rows($result)) {
	...
	static function getModuleColumnTableByFieldNames($module, $fieldnames) {
		global $adb;
		$result = $adb->pquery("SELECT fieldname,columnname,tablename FROM nectarcrm_field WHERE tabid=? AND fieldname IN (".
	...
	static function gatherModuleFieldGroupInfo($module) {
		...
		$result = $adb->pquery(
			"SELECT fieldname, fieldlabel, blocklabel, uitype FROM nectarcrm_field INNER JOIN
			nectarcrm_blocks ON nectarcrm_blocks.tabid=nectarcrm_field.tabid AND nectarcrm_blocks.blockid=nectarcrm_field.block 
			WHERE nectarcrm_field.tabid=? AND nectarcrm_field.presence != 1 ORDER BY nectarcrm_blocks.sequence, nectarcrm_field.sequence", array(getTabid($module))
		);
	...
	static function documentFoldersInfo() {
		global $adb;
		$folders = $adb->pquery("SELECT folderid, foldername FROM nectarcrm_attachmentsfolder", array());
		$folderOptions = array();
	...
	static function getRelatedFunctionHandler($sourceModule, $targetModule) {
		global $adb;
		$relationResult = $adb->pquery("SELECT name FROM nectarcrm_relatedlists WHERE tabid=? and related_tabid=? and presence=0", array(getTabid($sourceModule), getTabid($targetModule)));
		$functionName = false;
	...
	static function querySecurityFromSuffix($module, $current_user) {
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 
			&& $defaultOrgSharingPermission[$tabid] == 3) {
				...
				$querySuffix .= " AND (nectarcrm_crmentity.smownerid in($current_user->id) OR nectarcrm_crmentity.smownerid IN 
					(
					...
					if(sizeof($current_user_groups) > 0) {
						$querySuffix .= " nectarcrm_groups.groupid IN (". implode(",", $current_user_groups) .") OR ";
					}
					$querySuffix .= " nectarcrm_groups.groupid IN 
						(
