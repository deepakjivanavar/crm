...
class Mobile_WS_TaxByType extends Mobile_WS_Controller{
    ...
    protected function getCharges($recordId) {
		...
		if ($recordId) {
			$result = $adb->pquery('SELECT * FROM nectarcrm_inventorychargesrel WHERE recordid = ?', array($recordId));
    ...
    protected function getTableName($taxType){
        switch($taxType){
            case 'shipping':
                return 'nectarcrm_shippingtaxinfo';
                break;
            case 'inventory':
                return 'nectarcrm_inventorytaxinfo';
                break;
            case 'charges':
                return 'nectarcrm_inventorychargesrel';
                break;
        }
    }
}
