...
class Mobile_WS_AlertModel_PotentialsDueIn5Days extends Mobile_WS_AlertModel {
	...
	function query() {
		$sql = Mobile_WS_Utils::getModuleListQuery('Potentials', 
					"nectarcrm_potential.sales_stage not like 'Closed%' AND 
					DATEDIFF(nectarcrm_potential.closingdate, CURDATE()) <= 5"
				);
		return preg_replace("/^SELECT count\(\*\) as count(.*)/i", "SELECT crmid $1", nectarcrm_Functions::mkCountQuery($sql));
	}
