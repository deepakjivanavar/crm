...
class Users_Module_Model extends nectarcrm_Module_Model {
	/**
	...
	public function getQueryByModuleField($sourceModule, $field, $record, $listQuery) {
		if($sourceModule == 'Users' && $field == 'reports_to_id') {
			...
			if(!empty($record)){
				...
				$overRideQuery = $overRideQuery. " AND nectarcrm_users.id != ". $record;
				$allSubordinates = $currentUser->getAllSubordinatesByReportsToField($record);
				if(count($allSubordinates) > 0) {
					$overRideQuery .= " AND nectarcrm_users.id NOT IN (". implode(',',$allSubordinates) .")"; // do not allow the subordinates
				}
			}
	...
	public function searchRecord($searchValue, $parentId=false, $parentModule=false, $relatedModule=false) {
		if(!empty($searchValue)) {
			...
			$query = 'SELECT * FROM nectarcrm_users WHERE (first_name LIKE ? OR last_name LIKE ?) AND status = ?';
			$currentUser = Users_Record_Model::getCurrentUserModel();
			...
			if(count($allSubordinates) > 0) {
				$query .= " AND nectarcrm_users.id NOT IN (". implode(',',$allSubordinates) .")";
			}
			...
			for($i=0; $i<$noOfRows; ++$i) {
				...
				$modelClassName = nectarcrm_Loader::getComponentClassName('Model', 'Record', 'Users');
	...
	public function checkDuplicateUser($userName){
		...
		$db = PearDatabase::getInstance();
		$query = 'SELECT user_name FROM nectarcrm_users WHERE user_name = ?';
	...
	*/
	public function updateBaseCurrency($currencyName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT currency_code, currency_symbol FROM nectarcrm_currencies WHERE currency_name = ?', array($currencyName));
	...
	*/
	...
	public function getBaseCurrencyName() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT currency_name FROM nectarcrm_currency_info WHERE id=1",array());
	...
	public function saveLoginHistory($username){
		...
		$query = "INSERT INTO nectarcrm_loginhistory (user_name, user_ip, logout_time, login_time, status) VALUES (?,?,?,?,?)";
	...
	public function saveLogoutHistory(){
		...
		$loginIdQuery = "SELECT MAX(login_id) AS login_id FROM nectarcrm_loginhistory WHERE user_name=? AND user_ip=?";
		...
		if (!empty($loginid)){
			$query = "UPDATE nectarcrm_loginhistory SET logout_time =?, status=? WHERE login_id = ?";
	...
	public static function savePackagesInfo($packagesList) {
		...
		if ($disabledModulesList) {
			$updateQuery = 'UPDATE nectarcrm_tab SET presence = CASE WHEN name IN (' . generateQuestionMarks($disabledModulesList) . ') THEN 1 ';
			$updateQuery .= 'ELSE 0 END WHERE presence != 2 ';
		} else {
			$updateQuery = 'UPDATE nectarcrm_tab SET presence = 0 WHERE presence != 2';
		}
	...
	*/
	public function getCurrenciesList() {
		...
		$currency_query = 'SELECT currency_name, currency_code, currency_symbol FROM nectarcrm_currencies ORDER BY currency_name';
	...
	public function getTimeZonesList() {
		...
		$timezone_query = 'SELECT time_zone FROM nectarcrm_time_zone';
	...
	public function getLanguagesList() {
		...
		$language_query = 'SELECT prefix, label FROM nectarcrm_language';
