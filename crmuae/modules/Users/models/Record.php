...
class Users_Record_Model extends nectarcrm_Record_Model {
    ...
    /**
     * Funtion to add default Dashboard Tab in nectarcrm7 Home Page
     */
    function addDashboardTabs(){
        ...
        foreach($tabs as $seq => $tabName){
            ...
            $db->pquery("INSERT INTO nectarcrm_dashboard_tabs(tabname,userid,isdefault,sequence,appname,modulename) VALUES(?,?,?,?,?,?) ON DUPLICATE KEY UPDATE tabname=?,userid=?,appname=?,modulename=?",
	...
	public static function getAll($onlyActive=true, $excludeDefaultAdmin = true) {
		...
		$sql = 'SELECT id FROM nectarcrm_users';
		$params = array();
		if($onlyActive) {
			$sql .= ' WHERE status = ?';
			$params[] = 'Active';
    ...
    function getUserRoleName(){
        ...
        $query = "SELECT rolename from nectarcrm_role WHERE roleid = ?";
	...
	public function getImageDetails() {
		...
		if ($recordId) {
			...
			$query = "SELECT nectarcrm_attachments.attachmentsid, nectarcrm_attachments.path, nectarcrm_attachments.name FROM nectarcrm_attachments
                                  LEFT JOIN nectarcrm_salesmanattachmentsrel ON nectarcrm_salesmanattachmentsrel.attachmentsid = nectarcrm_attachments.attachmentsid
                                  WHERE nectarcrm_salesmanattachmentsrel.smid=?";
	...
	public function getAccessibleUsers($private="",$module = false) {
		...
		$accessibleUser = nectarcrm_Cache::get('nectarcrm-'.$this->getRole().'-'.$currentUserRoleModel->get('allowassignedrecordsto'), 'accessibleusers');
        if(empty($accessibleUser)) {
			...
			nectarcrm_Cache::set('nectarcrm-'.$this->getRole().'-'.$currentUserRoleModel->get('allowassignedrecordsto'), 'accessibleusers',$accessibleUser);
		}
	...
	public function getAllUsersOnRoles($roles) {
		...
		$sql = 'SELECT userid FROM nectarcrm_user2role WHERE roleid IN ('.  generateQuestionMarks($roleIds).')';
		...
		if($noOfUsers > 0) {
			...
			$query = 'SELECT id, first_name, last_name FROM nectarcrm_users WHERE status = ? AND id IN ('.  generateQuestionMarks($userIds).')';
	...
	public function getAccessibleGroups($private="",$module = false) {
        ...
        $accessibleGroups = nectarcrm_Cache::get('nectarcrm-'.$private, 'accessiblegroups');
        if(!$accessibleGroups){
            $accessibleGroups = get_group_array(false, "ACTIVE", "", $private,$module);
            nectarcrm_Cache::set('nectarcrm-'.$private, 'accessiblegroups',$accessibleGroups);
        }
	...
	public function deleteImage($imageId) {
		...
		$checkResult = $db->pquery('SELECT smid FROM nectarcrm_salesmanattachmentsrel WHERE attachmentsid = ?', array($imageId));
		$smId = $db->query_result($checkResult, 0, 'smid');

		if ($this->getId() === $smId) {
			$db->pquery('DELETE FROM nectarcrm_attachments WHERE attachmentsid = ?', array($imageId));
			$db->pquery('DELETE FROM nectarcrm_salesmanattachmentsrel WHERE attachmentsid = ?', array($imageId));
            $db->pquery('DELETE FROM nectarcrm_crmentity WHERE crmid = ?',array($imageId));
			return true;
		}
		return false;
	}
	...
	function saveTagCloud() {
		$db = PearDatabase::getInstance();
		$db->pquery("UPDATE nectarcrm_homestuff SET visible = ? WHERE userid=? AND stufftype='Tag Cloud'",
    ...
    public static function getAllUserGroups() {
        if (empty(self::$allUserGroups)) {
            $db = PearDatabase::getInstance();
            $query = "SELECT * FROM nectarcrm_users2group";
    ...
    public static function getCount($onlyActive = false) {
        $db = PearDatabase::getInstance();
        $query = 'SELECT 1 FROM nectarcrm_users ';
        $params = array();
	...
	public static function getInstanceByName($userName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT id FROM nectarcrm_users WHERE user_name = ?', array($userName));
	...
	public function isAccountOwner() {
		$db = PearDatabase::getInstance();
		$query = 'SELECT is_owner FROM nectarcrm_users WHERE id = ?';
	...
	public function getActiveAdminUsers() {
		...
		$sql = 'SELECT id FROM nectarcrm_users WHERE status=? AND is_admin=?';
	...
	public function getUserHash() {
		$db = PearDatabase::getInstance();
		$query = 'SELECT user_hash FROM nectarcrm_users WHERE id = ?';
	...
	public function deleteUserPermanently($userId, $newOwnerId) {
		...
		$sql = "UPDATE nectarcrm_crmentity SET smcreatorid=?,smownerid=?,modifiedtime=? WHERE smcreatorid=? AND setype=?";
		$db->pquery($sql, array($newOwnerId, $newOwnerId, date('Y-m-d H:i:s'), $userId,'ModComments'));

		// Update creator Id in nectarcrm_crmentity table
		$sql = "UPDATE nectarcrm_crmentity SET smcreatorid = ? WHERE smcreatorid = ? AND setype <> ?";
		$db->pquery($sql, array($newOwnerId, $userId,'ModComments'));

		//update history details in nectarcrm_modtracker_basic 
		$sql ="update nectarcrm_modtracker_basic set whodid=? where whodid=?"; 
		$db->pquery($sql, array($newOwnerId, $userId)); 

		//update comments details in nectarcrm_modcomments 
		$sql ="update nectarcrm_modcomments set userid=? where userid=?"; 
		$db->pquery($sql, array($newOwnerId, $userId));

		$sql = "DELETE FROM nectarcrm_users WHERE id=?";
    ...
    public function getAllSubordinatesByReportsToField($forUserId) {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT id, reports_to_id FROM nectarcrm_users where status = ?', array('Active'));
