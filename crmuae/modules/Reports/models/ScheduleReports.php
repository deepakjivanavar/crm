...
class Reports_ScheduleReports_Model extends nectarcrm_Base_Model {
	...
	public static function getInstanceById($recordId) {
		...
		if (!empty($recordId)) {
			$scheduledReportResult = $db->pquery('SELECT * FROM nectarcrm_schedulereports WHERE reportid = ?', array($recordId));
	...
	public function saveScheduleReport() {
		...
		if ($isReportScheduled == '0' || $isReportScheduled == '' || $isReportScheduled == false) {
			$deleteScheduledReportSql = "DELETE FROM nectarcrm_schedulereports WHERE reportid=?";
			$adb->pquery($deleteScheduledReportSql, array($reportid));
		} else {
			$checkScheduledResult = $adb->pquery('SELECT next_trigger_time FROM nectarcrm_schedulereports WHERE reportid=?', array($reportid));
			if ($adb->num_rows($checkScheduledResult) > 0) {
				$scheduledReportSql = 'UPDATE nectarcrm_schedulereports SET scheduleid=?, recipients=?, schdate=?, schtime=?, schdayoftheweek=?, schdayofthemonth=?, schannualdates=?, specificemails=?, next_trigger_time=?, fileformat = ? WHERE reportid=?';
				...
				$scheduleReportSql = 'INSERT INTO nectarcrm_schedulereports (reportid,scheduleid,recipients,schdate,schtime,schdayoftheweek,schdayofthemonth,schannualdates,next_trigger_time,specificemails, fileformat) VALUES (?,?,?,?,?,?,?,?,?,?,?)';
	...
	public function getRecipientEmails() {
		...
		if (!empty($recipientsList) && count($recipientsList) > 0) {
			foreach ($recipientsList as $userId) {
				if(!nectarcrm_Util_Helper::isUserDeleted($userId)) {
	...
	public function sendEmail() {
		require_once 'vtlib/nectarcrm/Mailer.php';
		$currentUserModel = Users_Record_Model::getCurrentUserModel();

		$nectarcrmMailer = new nectarcrm_Mailer();

		$recipientEmails = $this->getRecipientEmails();
		nectarcrm_Utils::ModuleLog('ScheduleReprots', $recipientEmails);
		foreach ($recipientEmails as $name => $email) {
			$nectarcrmMailer->AddAddress($email, decode_html($name));
		}
		vimport('~modules/Report/models/Record.php');
		$reportRecordModel = Reports_Record_Model::getInstanceById($this->get('reportid'));
		$currentTime = date('Y-m-d H:i:s');
		nectarcrm_Utils::ModuleLog('ScheduleReprots Send Mail Start ::', $currentTime);
		$reportname = decode_html($reportRecordModel->getName());
		$subject = $reportname;
		nectarcrm_Utils::ModuleLog('ScheduleReprot Name ::', $reportname);
		if(empty($nectarcrmMailer->From)) {
			$fromName = trim($currentUserModel->get('first_name').' '.$currentUserModel->get('last_name'));
			$nectarcrmMailer->ConfigSenderInfo($currentUserModel->get('email1'), $fromName);
		}
		$nectarcrmMailer->Subject = $subject;
		$nectarcrmMailer->Body = $this->getEmailContent($reportRecordModel);
		$plainBody = decode_html($nectarcrmMailer->Body);
		$plainBody = preg_replace(array("/<p>/i","/<br>/i","/<br \/>/i"),array("\n","\n","\n"),$plainBody);
		$plainBody = strip_tags($plainBody);
		$plainBody = Emails_Mailer_Model::convertToAscii($plainBody);
		$nectarcrmMailer->AltBody = $plainBody;

		$nectarcrmMailer->IsHTML();
		...
		if($reportType != 'chart') {
			...
			foreach ($attachments as $attachmentName => $path) {
				$nectarcrmMailer->AddAttachment($path, decode_html($attachmentName));
			}
		}
		$status = $nectarcrmMailer->Send(true);
	...
	public function updateNextTriggerTime() {
		$adb = PearDatabase::getInstance();
		$nextTriggerTime = $this->getNextTriggerTime();
		nectarcrm_Utils::ModuleLog('ScheduleReprot Next Trigger Time >> ', $nextTriggerTime);
		$adb->pquery('UPDATE nectarcrm_schedulereports SET next_trigger_time=? WHERE reportid=?', array($nextTriggerTime, $this->get('reportid')));
		nectarcrm_Utils::ModuleLog('ScheduleReprot', 'Next Trigger Time updated');
	}
	...
	public static function getScheduledReports() {
		...
		$result = $adb->pquery("SELECT reportid FROM nectarcrm_schedulereports
								INNER JOIN nectarcrm_reportmodules ON nectarcrm_reportmodules.reportmodulesid = nectarcrm_schedulereports.reportid
	...
	public static function runScheduledReports() {
		...
		foreach ($scheduledReports as $reportId => $scheduledReport) {
			...
			nectarcrm_Utils::ModuleLog('ScheduleReprot Send Mail Status ', $status);
	...
	public function getNextTriggerTimeInUserFormat() {
		...
		$value = $valueParts[0].' '.nectarcrm_Time_UIType::getDisplayValue($valueParts[1]);
		return $value;
	}
