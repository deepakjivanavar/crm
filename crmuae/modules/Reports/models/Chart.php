...
class Reports_Chart_Model extends nectarcrm_Base_Model {

	public static function getInstanceById($reportModel) {
		$self = new self();
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM nectarcrm_reporttype WHERE reportid = ?', array($reportModel->getId()));
...
abstract class Base_Chart extends nectarcrm_Base_Model{
	...
	function generateLink($field, $value) {
		...
		if(($dataFieldInfo[4] == 'D' || $dataFieldInfo[4] == 'DT') && !empty($dataFieldInfo[5])) {
			...
			$value = nectarcrm_Date_UIType::getDisplayDateTimeValue($value);
		}
				...
				if ($fieldDataType == 'picklist') {
					...
					if($row[strtolower($legendField->get('reportlabel'))]) {
						...
						if ($groupByDataField[5] == 'M' || $groupByDataField[5] == 'Y' || $groupByDataField[5] == 'MY') {
							...
							$label = nectarcrm_Date_UIType::getDisplayDateValue($row[strtolower($legendField->get('reportlabel'))]);
						}
					} else {
						$label = '--';
					}
					...
					if($row[strtolower($legendField->get('reportlabel'))]) {
						...
						if ($groupByDataField[5] == 'M' || $groupByDataField[5] == 'Y' || $groupByDataField[5] == 'MY') {
							...
							$label = nectarcrm_Date_UIType::getDisplayDateTimeValue($row[strtolower($legendField->get('reportlabel'))]);
						}
					} else {
						$label = '--';
					}
					...
					if ($fieldDataType == 'picklist') {
						...
						if($row[strtolower($gFieldModel->get('reportlabel'))] != null) {
							...
							if ($groupByDataField[5] == 'M' || $groupByDataField[5] == 'Y' || $groupByDataField[5] == 'MY') {
								...
								$label = nectarcrm_Date_UIType::getDisplayDateValue($row[strtolower($gFieldModel->get('reportlabel'))]);
							}
						} else {
							$label = '--';
						}
