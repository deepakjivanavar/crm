...
class Reports_Record_Model extends nectarcrm_Record_Model {
	/**
	...
	public static function getInstanceById($recordId) {
		...
		$self = new self();
		$reportResult = $db->pquery('SELECT * FROM nectarcrm_report WHERE reportid = ?', array($recordId));
	...
	function initialize() {
		$reportId = $this->getId();
		$this->report = nectarcrm_Report_Model::getInstance($reportId);
	}
	...
	function getPrimaryModuleFields() {
		...
		//need to add this nectarcrm_crmentity:crmid:".$module."_ID:crmid:I
	...
	function isRecordHasViewAccess($reportType){
		...
		$current_user = vglobal('current_user');
			$params = array();
			$sql = ' SELECT nectarcrm_report.reportid,nectarcrm_report.reportname FROM nectarcrm_report ';
			...
			if($reportType == 'Private'){
				$sql .= " WHERE ( ( (".$non_admin_query.") OR nectarcrm_report.sharingtype='Public' OR "
			...
			$sql = $queryObj->query. ' AND nectarcrm_report.reportid = '.$this->getId();
	...
	function getSelectedFields() {
		...
		$result = $db->pquery("SELECT nectarcrm_selectcolumn.columnname FROM nectarcrm_report
					INNER JOIN nectarcrm_selectquery ON nectarcrm_selectquery.queryid = nectarcrm_report.queryid
					INNER JOIN nectarcrm_selectcolumn ON nectarcrm_selectcolumn.queryid = nectarcrm_selectquery.queryid
					WHERE nectarcrm_report.reportid = ? ORDER BY nectarcrm_selectcolumn.columnindex", array($this->getId()));
	...
	function getSelectedCalculationFields() {
		...
		$result = $db->pquery('SELECT nectarcrm_reportsummary.columnname FROM nectarcrm_reportsummary
					INNER JOIN nectarcrm_report ON nectarcrm_report.reportid = nectarcrm_reportsummary.reportsummaryid
					WHERE nectarcrm_report.reportid=?', array($this->getId()));
	...
	function getSelectedSortFields() {
		...
		$result = $db->pquery('SELECT nectarcrm_reportsortcol.* FROM nectarcrm_report
					INNER JOIN nectarcrm_reportsortcol ON nectarcrm_report.reportid = nectarcrm_reportsortcol.reportid
					WHERE nectarcrm_report.reportid = ? ORDER BY nectarcrm_reportsortcol.sortcolid',array($this->getId()));
	...
	function getSelectedStandardFilter() {
		...
		$result = $db->pquery('SELECT * FROM nectarcrm_reportdatefilter WHERE datefilterid = ? AND startdate != ? AND enddate != ?',
	...
	function save() {
		...
		if(empty($reportId)) {
			$reportId = $db->getUniqueID("nectarcrm_selectquery");
			$this->setId($reportId);

			$db->pquery('INSERT INTO nectarcrm_selectquery(queryid, startindex, numofobjects) VALUES(?,?,?)',
					array($reportId, 0, 0));
			...
			$db->pquery('INSERT INTO nectarcrm_report(reportid, folderid, reportname, description,
								reporttype, queryid, state, owner, sharingtype) VALUES(?,?,?,?,?,?,?,?,?)', $reportParams);


			$secondaryModule = $this->getSecondaryModules();
			$db->pquery('INSERT INTO nectarcrm_reportmodules(reportmodulesid, primarymodule, secondarymodules) VALUES(?,?,?)',
			...
			$reportId = $this->getId();
			$db->pquery('DELETE FROM nectarcrm_selectcolumn WHERE queryid = ?', array($reportId));
			$this->saveSelectedFields();

			$db->pquery("DELETE FROM nectarcrm_reportsharing WHERE reportid = ?", array($reportId));
			$this->saveSharingInformation();


			$db->pquery('UPDATE nectarcrm_reportmodules SET primarymodule = ?,secondarymodules = ? WHERE reportmodulesid = ?',
					array($this->getPrimaryModule(), $this->getSecondaryModules(), $reportId));

			$db->pquery('UPDATE nectarcrm_report SET reportname = ?, description = ?, reporttype = ?, folderid = ?,sharingtype = ? WHERE
			...
			$db->pquery('DELETE FROM nectarcrm_reportsortcol WHERE reportid = ?', array($reportId));
			$db->pquery('DELETE FROM nectarcrm_reportgroupbycolumn WHERE reportid = ?',array($reportId));
			$this->saveSortFields();

			$db->pquery('DELETE FROM nectarcrm_reportsummary WHERE reportsummaryid = ?', array($reportId));
			$this->saveCalculationFields();

			$db->pquery('DELETE FROM nectarcrm_reportdatefilter WHERE datefilterid = ?', array($reportId));
			$this->saveStandardFilter();
	...
	function saveSortFields() {
		...
		if(!empty($sortFields)){
			...
			foreach($sortFields as $fieldInfo) {
				$db->pquery('INSERT INTO nectarcrm_reportsortcol(sortcolid, reportid, columnname, sortorder) VALUES (?,?,?,?)',
						array($i, $this->getId(), $fieldInfo[0], $fieldInfo[1]));
				if(IsDateField($fieldInfo[0])) {
					if(empty($fieldInfo[2])){
						$fieldInfo[2] = 'None';
					}
					$db->pquery("INSERT INTO nectarcrm_reportgroupbycolumn(reportid, sortid, sortcolname, dategroupbycriteria)
	...
	function saveCalculationFields() {
		...
		for ($i=0; $i<count($calculationFields); $i++) {
			$db->pquery('INSERT INTO nectarcrm_reportsummary (reportsummaryid, summarytype, columnname) VALUES (?,?,?)',
	...
	function saveStandardFilter() {
		...
		if(!empty($standardFilter)) {
			$db->pquery('INSERT INTO nectarcrm_reportdatefilter (datefilterid, datecolumnname, datefilter, startdate, enddate)
	...
	function saveSharingInformation() {
		...
		for($i=0; $i<count($sharingInfo); $i++) {
			$db->pquery('INSERT INTO nectarcrm_reportsharing(reportid, shareid, setype) VALUES (?,?,?)',
		...
		if(!empty($members)) {
			...
			$db->pquery('DELETE FROM nectarcrm_report_shareusers WHERE reportid=?',array($reportId));
			$db->pquery('DELETE FROM nectarcrm_report_sharegroups WHERE reportid=?',array($reportId));
			$db->pquery('DELETE FROM nectarcrm_report_sharerole WHERE reportid=?',array($reportId));
			$db->pquery('DELETE FROM nectarcrm_report_sharers WHERE reportid=?',array($reportId));
			...
			for ($i = 0; $i < $noOfMembers; ++$i) {
				...
				if ($idComponents && count($idComponents) == 2) {
					...
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_USERS) {
						$db->pquery('INSERT INTO nectarcrm_report_shareusers(userid, reportid) VALUES (?,?)', array($memberId, $reportId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_GROUPS) {
						$db->pquery('INSERT INTO nectarcrm_report_sharegroups(groupid, reportid) VALUES (?,?)', array($memberId, $reportId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLES) {
						$db->pquery('INSERT INTO nectarcrm_report_sharerole(roleid, reportid) VALUES (?,?)', array($memberId, $reportId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLE_AND_SUBORDINATES) {
						$db->pquery('INSERT INTO nectarcrm_report_sharers(rsid, reportid) VALUES (?,?)', array($memberId, $reportId));
					}
				}
			}
		}
		...
		if(!empty($selectedFields)){
		   for($i=0 ;$i<count($selectedFields);$i++) {
				if(!empty($selectedFields[$i])) {
					$db->pquery("INSERT INTO nectarcrm_selectcolumn(queryid, columnindex, columnname) VALUES (?,?,?)",
	...
	function saveAdvancedFilters() {
		...
		if(!empty($advancedFilter)) {
			...
			$db->pquery('DELETE FROM nectarcrm_relcriteria WHERE queryid = ?', array($reportId));
			$db->pquery('DELETE FROM nectarcrm_relcriteria_grouping WHERE queryid = ?', array($reportId));
			...
			foreach($advancedFilter as $groupIndex => $groupInfo) {
				...
				foreach($groupColumns as $columnIndex => $columnCondition) {
					...
					if($fieldType == 'currency') {
						if($field->getUIType() == '72') {
							...
							$advFilterValue = nectarcrm_Currency_UIType::convertToDBFormat($advFilterValue, null, true);
						} else {
							$advFilterValue = nectarcrm_Currency_UIType::convertToDBFormat($advFilterValue);
						}
					}
						...
						for($i=0; $i<count($tempVal); $i++) {
							if(trim($tempVal[$i]) != '') {
								...
								if($columnInfo[4] == 'D') {
									...
									$val[$i] = nectarcrm_Time_UIType::getTimeValueWithSeconds($tempVal[$i]);
								} else {
					...
					$db->pquery('INSERT INTO nectarcrm_relcriteria (queryid, columnindex, columnname, comparator, value,
				...
				$db->pquery("INSERT INTO nectarcrm_relcriteria_grouping(groupid, queryid, group_condition, condition_expression) VALUES (?,?,?,?)",
	...
	function saveScheduleInformation() {
		...
		$db->pquery('INSERT INTO nectarcrm_scheduled_reports(reportid, recipients, schedule, format, next_trigger_time) VALUES
	...
	function deleteScheduling() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM nectarcrm_scheduled_reports WHERE reportid = ?', array($this->getId()));
	}
	...
	function move($folderId) {
		...
		$db->pquery("UPDATE nectarcrm_report SET folderid = ? WHERE reportid = ?", array($folderId, $this->getId()));
	}
	...
	public function checkDuplicate() {
		...
		$query = "SELECT 1 FROM nectarcrm_report WHERE reportname = ?";
	...
	function showLineItemFieldsInFilter($calculationFields=false) {
		...
		if(!empty($calculationFields)) {
			foreach($calculationFields as $field) {
				if(stripos($field, 'cb:nectarcrm_inventoryproductrel') !== false) {
					return true;
				}
			}
			return false;
	...
	public function getRecordsListFromRequest(nectarcrm_Request $request) {
	...
	function saveReportType(){
		...
		if(!empty($data)){
			$db->pquery('DELETE FROM nectarcrm_reporttype WHERE reportid = ?', array($this->getId()));
			$db->pquery("INSERT INTO nectarcrm_reporttype(reportid, data) VALUES (?,?)",
	...
	function getReportTypeInfo() {
		...
		$result = $db->pquery("SELECT data FROM nectarcrm_reporttype WHERE reportid = ?", array($this->getId()));
	...
	function getPrimaryModuleFieldsForAdvancedReporting() {
		...
		$primaryModuleModel = nectarcrm_Module_Model::getInstance($primaryModule);
		...
		if(is_array($fields)) foreach($fields as $module => $blocks) {
			if(is_array($blocks)) foreach($blocks as $blockLabel => $blockFields) {
				if(is_array($blockFields)) foreach($blockFields as $reportFieldInfo => $fieldLabel) {
					...
					if(empty($fieldInstance) || $fieldInfo[0] == 'nectarcrm_inventoryproductrel' || $fieldInstance->getFieldDataType() == 'email'
		...
		if(is_array($secondaryModules)) {
			...
			foreach($secondaryModules as $secondaryModule) {
				if(!empty($secondaryModule)) {
					...
					$secondaryModuleModel = nectarcrm_Module_Model::getInstance($secondaryModule);
		...
		if(is_array($fields)) foreach($fields as $module => $blocks) {
			if(is_array($blocks)) foreach($blocks as $blockLabel => $blockFields) {
				if(is_array($blockFields)) foreach($blockFields as $reportFieldInfo => $fieldLabel) {
					...
					if(empty($fieldInstance) || $fieldInfo[0] == 'nectarcrm_inventoryproductrel'
	...
	public function isPinnedToDashboard() {
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$result = $db->pquery("SELECT 1 FROM nectarcrm_module_dashboard_widgets WHERE reportid = ? AND userid = ?", array($this->getId(), $currentUser->getId()));
		if($db->num_rows($result)) {
			return true;
		}
		return false;
	...
	function isEditableBySharing() {
		...
		$ownerResult = $db->pquery("SELECT owner FROM nectarcrm_report WHERE reportid = ?", array($this->getId()));
		...
		if($currentUserId == $reportOnwer) {
			...
			$reportId = $this->getId();
			$query = "SELECT 1 FROM nectarcrm_report_sharegroups WHERE reportid = ? "
					. "UNION SELECT 1 FROM nectarcrm_report_sharerole WHERE reportid = ? "
					. "UNION SELECT 1 FROM nectarcrm_report_sharers WHERE reportid = ? "
					. "UNION SELECT 1 FROM nectarcrm_report_shareusers WHERE reportid = ?";
