...
$report_std_filter = new nectarcrmCRM_Smarty; 
...
function getVisibleCriteria($recordid='')
{
	...
	if($recordid!='')
	{
		$result = $adb->pquery("select sharingtype from nectarcrm_report where reportid=?",array($recordid));
		$selcriteria=$adb->query_result($result,0,"sharingtype");
	}
	if($selcriteria == ""){
		$selcriteria = 'Public';
	}
	$filter_result = $adb->query("select * from nectarcrm_reportfilters");
...
function getShareInfo($recordid=''){
	global $adb;
	$member_query = $adb->pquery("SELECT nectarcrm_reportsharing.setype,nectarcrm_users.id,nectarcrm_users.user_name FROM nectarcrm_reportsharing INNER JOIN nectarcrm_users on nectarcrm_users.id = nectarcrm_reportsharing.shareid WHERE nectarcrm_reportsharing.setype='users' AND nectarcrm_reportsharing.reportid = ?",array($recordid));
	...
	$member_query = $adb->pquery("SELECT nectarcrm_reportsharing.setype,nectarcrm_groups.groupid,nectarcrm_groups.groupname FROM nectarcrm_reportsharing INNER JOIN nectarcrm_groups on nectarcrm_groups.groupid = nectarcrm_reportsharing.shareid WHERE nectarcrm_reportsharing.setype='groups' AND nectarcrm_reportsharing.reportid = ?",array($recordid));
	$noofrows = $adb->num_rows($member_query);
	if($noofrows > 0){
		for($i=0;$i<$noofrows;$i++){
