...
function getReportFieldValue ($report, $picklistArray, $dbField, $valueArray, $fieldName, $operation = false) {
	...
	if ($report->primarymodule == 'PriceBooks' && $fieldLabel == 'List_Price') {
		$fieldInfo = array(
			...
			'tablename'	=> 'nectarcrm_pricebookproductrel',
			'uitype'	=> 72,
			'typeofdata'=> 'Currency',
			'presence'	=> 0,
		);
		...
		if($module == 'Calendar' && ($field->getFieldName() == 'due_date' || $field->getFieldName() == 'date_start')) {
            if($field->getFieldName() == 'due_date'){
                ...
                if(empty($endTime)) {
                    ...
                    $endTime = getSingleFieldValue('nectarcrm_activity', 'time_end', 'activityid', $recordId);
                }
	...
	if (!empty($searchParams)) {
		...
		foreach ($searchParams as $i => $condition) {
			...
			if ($fieldName == 'reportname' || $fieldName == 'description') {
				$conditionQuery .= " nectarcrm_report.$fieldName LIKE ? ";
				...
				$conditionQuery .= " nectarcrm_report.$fieldName IN (".generateQuestionMarks($searchValue).") ";
				foreach ($searchValue as $value) {
					array_push($params, $value);
				}
			} else if ($fieldName == 'primarymodule') {
				$searchValue = explode(',', $searchValue);
				$conditionQuery .= " nectarcrm_reportmodules.$fieldName IN (".generateQuestionMarks($searchValue).") ";
				foreach ($searchValue as $value) {
					array_push($params, $value);
				}
			}
