...
class HelpDeskHandler extends VTEventHandler {
	...
	function handleEvent($eventName, $entityData) {
		...
		if($eventName == 'nectarcrm.entity.aftersave.final') {
			$moduleName = $entityData->getModuleName();
			if ($moduleName == 'HelpDesk') {
				$ticketId = $entityData->getId();
				$adb->pquery('UPDATE nectarcrm_ticketcf SET from_portal=0 WHERE ticketid=?', array($ticketId));
			}
		}
	}
}
...
function HelpDesk_nofifyOnPortalTicketCreation($entityData) {
	...
	$result = $adb->pquery("SELECT email, concat (firstname,' ',lastname) as name FROM nectarcrm_contactdetails WHERE contactid=?", array($parentId));
...
function HelpDesk_notifyOnPortalTicketComment($entityData) {
	...
	//get the contact email id who creates the ticket from portal and use this email as from email id in email
	$result = $adb->pquery("SELECT lastname, firstname, email FROM nectarcrm_contactdetails WHERE contactid=?", array($parentId));
...
function HelpDesk_notifyParentOnTicketChange($entityData) {
	...
	if(!empty($contactId[0])) {
		$result = $adb->pquery('SELECT email, emailoptout, lastname, firstname FROM nectarcrm_contactdetails WHERE
	...
	//added condition to check the emailoptout(this is for contacts and nectarcrm_accounts.)
	if($emailoptout == 0) {
		if($isPortalUser == 1) {
