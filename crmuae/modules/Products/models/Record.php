...
class Products_Record_Model extends nectarcrm_Record_Model {
	...
	function getTaxes() {
		...
		$result = $db->pquery('SELECT nectarcrm_producttaxrel.*, nectarcrm_inventorytaxinfo.taxname, nectarcrm_inventorytaxinfo.taxlabel, nectarcrm_inventorytaxinfo.compoundon FROM nectarcrm_producttaxrel
								INNER JOIN nectarcrm_inventorytaxinfo ON nectarcrm_inventorytaxinfo.taxid = nectarcrm_producttaxrel.taxid
								INNER JOIN nectarcrm_crmentity ON nectarcrm_producttaxrel.productid = nectarcrm_crmentity.crmid AND nectarcrm_crmentity.deleted = 0
								WHERE nectarcrm_producttaxrel.productid = ? AND nectarcrm_inventorytaxinfo.deleted = 0', array($this->getId()));
		$taxes = array();
		...
		foreach ($taxes as $taxName => $taxInfo) {
			...
			$taxes[$taxName]['regionsList'] = nectarcrm_Util_Helper::toSafeHTML(Zend_Json::encode($regionsInfo));
		}
		return $taxes;
	}
	...
	static function getListPriceValues($id) {
		$db = PearDatabase::getInstance();
		$listPrice = $db->pquery('SELECT * FROM nectarcrm_productcurrencyrel WHERE productid	= ?', array($id)); 
	...
	function getSubProducts($active = false) {
		...
		$query = 'SELECT nectarcrm_products.productid, quantity FROM nectarcrm_products
					INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_products.productid
					LEFT JOIN nectarcrm_seproductsrel ON nectarcrm_seproductsrel.crmid = nectarcrm_products.productid AND nectarcrm_seproductsrel.setype = ?
					...
					WHERE nectarcrm_crmentity.deleted = 0 AND nectarcrm_seproductsrel.productid = ?';

		if ($active) {
			$query .= ' AND nectarcrm_products.discontinued = 1';
		}
		...
		while($rowData = $db->fetch_array($result)) {
			...
			$subProductModel = nectarcrm_Record_Model::getInstanceById($subProductId, 'Products');
	...
	function getDetailsForInventoryModule($focus) {
		...
		$currencyInfo = nectarcrm_Util_Helper::getUserCurrencyInfo();
	...
	public function getImageDetails() {
		...
		if ($recordId) {
			$sql = "SELECT nectarcrm_attachments.*, nectarcrm_crmentity.setype FROM nectarcrm_attachments
						...
						WHERE nectarcrm_crmentity.setype = 'Products Image' AND nectarcrm_seattachmentsrel.crmid = ?";
	...
	public static function getProductsImageDetails($recordIdsList = false) {
		...
		if ($recordIdsList) {
			$sql = "SELECT nectarcrm_attachments.*, nectarcrm_crmentity.setype, nectarcrm_seattachmentsrel.crmid AS productid FROM nectarcrm_attachments
						INNER JOIN nectarcrm_seattachmentsrel ON nectarcrm_seattachmentsrel.attachmentsid = nectarcrm_attachments.attachmentsid
						INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_attachments.attachmentsid
						WHERE nectarcrm_crmentity.setype = 'Products Image' AND nectarcrm_seattachmentsrel.crmid IN (".generateQuestionMarks($recordIdsList).")";
	...
	public static function getSearchResult($searchKey, $module=false) {
		...
		$query = 'SELECT label, crmid, setype, createdtime FROM nectarcrm_crmentity WHERE label LIKE ? AND nectarcrm_crmentity.deleted = 0';
		$params = array("%$searchKey%");

		if($module !== false) {
			$query .= ' AND setype = ?';
			if($module == 'Products'){
				$query = 'SELECT label, crmid, setype, createdtime FROM nectarcrm_crmentity INNER JOIN nectarcrm_products ON 
							nectarcrm_products.productid = nectarcrm_crmentity.crmid WHERE label LIKE ? AND nectarcrm_crmentity.deleted = 0 
							AND nectarcrm_products.discontinued = 1 AND setype = ?';
			}else if($module == 'Services'){
				$query = 'SELECT label, crmid, setype, createdtime FROM nectarcrm_crmentity INNER JOIN nectarcrm_service ON 
							nectarcrm_service.serviceid = nectarcrm_crmentity.crmid WHERE label LIKE ? AND nectarcrm_crmentity.deleted = 0 
							AND nectarcrm_service.discontinued = 1 AND setype = ?';
			}
			$params[] = $module;
		}
	...
	public function getActiveStatusOfRecord(){
		...
		$recordId = $this->getId();
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT discontinued FROM nectarcrm_products WHERE productid = ?',array($recordId));
	...
	public function isBundle() {
		...
		$query = 'SELECT 1 FROM nectarcrm_seproductsrel INNER JOIN nectarcrm_crmentity 
					ON nectarcrm_seproductsrel.crmid = nectarcrm_crmentity.crmid AND nectarcrm_crmentity.deleted = 0 AND nectarcrm_seproductsrel.setype=? 
					WHERE nectarcrm_seproductsrel.productid = ?';
	...
	public function updateShowBundlesOption($value) {
		...
		$query = 'UPDATE nectarcrm_products SET is_subproducts_viewable = ? WHERE productid = ?';
	...
	public function isBundleViewable() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT is_subproducts_viewable FROM nectarcrm_products WHERE productid = ?', array($this->getId()));
	...
	public function updateSubProductQuantity($subProductId, $quantity) {
		...
		$query = 'UPDATE nectarcrm_seproductsrel SET quantity = ? WHERE productid = ? AND crmid = ? AND setype = ?';
