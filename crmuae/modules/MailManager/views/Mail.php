		...
		if ('open' == $this->getOperationArg($request)) {
			...
			if (!empty($to_string)) {
				...
				foreach($toArray as $to) {
					$relatedtos = MailManager::lookupMailInnectarcrm($to, $currentUserModel);
					...
					if ($status === true) {
						...
						if ($realid[1] == -1) {
							$db->pquery('DELETE FROM nectarcrm_salesmanactivityrel WHERE smid=? AND activityid=?',$params);
							$db->pquery('INSERT INTO nectarcrm_salesmanactivityrel VALUES (?,?)', $params);
						} else {
							$db->pquery('DELETE FROM nectarcrm_seactivityrel WHERE crmid=? AND activityid=?', $params);
							$db->pquery('INSERT INTO nectarcrm_seactivityrel VALUES (?,?)', $params);
						}
					}
				}
			}
