...
class MailManager_Relate_Action extends nectarcrm_MailScannerAction {
	...
	public function __CreateNewEmail($mailrecord, $module, $linkfocus) {
		...
		$recordModel = nectarcrm_Record_Model::getCleanInstance('Emails');
		...
		if (is_array($inlineAttachments)) {
			foreach ($inlineAttachments as $index => $att) {
				$cid = $att['cid'];
				$attch_name = nectarcrm_MailRecord::__mime_decode($att['filename']);
				$id = $mailrecord->muid();
	...
	public function __SaveAttachements($mailrecord, $basemodule, $recordModel) {
		...
		foreach($mailrecord->_attachments as $attachmentData) {
			...
			if($issaved) {
				...
				$attachRes = $db->pquery("SELECT * FROM nectarcrm_attachments WHERE attachmentsid = ?", array($attachid));
	...
	public static function getSalesEntityInfo($crmid) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT setype FROM nectarcrm_crmentity WHERE crmid=? AND deleted=0", array($crmid));
	...
	public static function ws_modulename($modulewsid) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT name FROM nectarcrm_ws_entity WHERE id=?", array($modulewsid));
