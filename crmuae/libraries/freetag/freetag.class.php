...
class freetag {
	...
	/**
	 * @access private
	 * @param string The prefix of freetag database nectarcrm_tables.
	 */
	var $_table_prefix = 'nectarcrm_';
	/**
	 * @access private
	...
	/**
	 * @access private
	 * @param string Whether to prevent multiple nectarcrm_users from tagging the same object. By default, set to block (ala Upcoming.org)
	 */
	 ...
	 * - nectarcrm_table_prefix: If you wish to create multiple Freetag databases on the same database, you can put a prefix in front of the nectarcrm_table names and pass separate prefixes to the constructor. [default: '']
	 ...
	 * - block_multiuser_tag_on_object: Set to 0 in order to allow individual nectarcrm_users to all tag the same object with the same tag. Default is 1 to only allow one occurence of a tag per object. [default: 1]
...
/*
	 ...
	 * tagArray parameter. Use this to provide tag combo services to your nectarcrm_users.
	 *
	...
	function safe_tag($tagger_id, $object_id, $tag, $module) {
		...
		// Dynamically switch between allowing duplication between nectarcrm_users on the constructor param 'block_multiuser_tag_on_object'.
		...
		if(!$rs->EOF) {
			...
			// Add new tag! 
			$tag_id = $adb->getUniqueId('nectarcrm_freetags');
	 ...
	 * @param string The raw string form of the tag to delete. See above for nectarcrm_notes.
	 *
	 * @return string Returns the tag in normalized form.
	 */ 
	...
	function get_tag_cloud_tags($max = 100, $tagger_id = NULL,$module = "",$obj_id = NULL){
		...
		$sql = "SELECT tag,tag_id,COUNT(object_id) AS quantity
			...
			ON (${prefix}freetags.id = tag_id) INNER JOIN nectarcrm_tab
			ON ${prefix}freetagged_objects.module = nectarcrm_tab.name
			WHERE nectarcrm_tab.presence != 1
			$tagger_sql
			GROUP BY tag, tag_id
	 ...
	 * Confusing? Think of it like e-commerce's "Other nectarcrm_users who bought this also bought," 
