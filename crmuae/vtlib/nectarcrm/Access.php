...
/**
 * Provides API to control Access like Sharing, Tools etc. for nectarcrm CRM Module
 * @package vtlib
 */
class nectarcrm_Access {
	...
	static function __getDefaultSharingAccessId() {
		global $adb;
		return $adb->getUniqueID('nectarcrm_def_org_share');
	}
	...
	static function allowSharing($moduleInstance, $enable=true) {
		global $adb;
		$ownedby = $enable? 0 : 1;
		$adb->pquery("UPDATE nectarcrm_tab set ownedby=? WHERE tabid=?", Array($ownedby, $moduleInstance->id));
	 ...
	 * @param nectarcrm_Module Instance of the module to use
	 * @access private
	 * @internal This method is called from nectarcrm_Module during creation.
	 */
	static function initSharing($moduleInstance) {
		global $adb;

		$result = $adb->query("SELECT share_action_id from nectarcrm_org_share_action_mapping WHERE share_action_name in
		...
		for($index = 0; $index < $adb->num_rows($result); ++$index) {
			...
			$adb->pquery("INSERT INTO nectarcrm_org_share_action2tab(share_action_id,tabid) VALUES(?,?)", Array($actionid, $moduleInstance->id));
		}
	 ...
	 * @access private
	 * @internal This method is called from nectarcrm_Module during deletion.
	 */
	static function deleteSharing($moduleInstance) {
		global $adb;
		$adb->pquery("DELETE FROM nectarcrm_org_share_action2tab WHERE tabid=?", Array($moduleInstance->id));
	...
	static function setDefaultSharing($moduleInstance, $permission_text='Public_ReadWriteDelete') {
		...
		$result = $adb->pquery("SELECT * FROM nectarcrm_def_org_share WHERE tabid=?", Array($moduleInstance->id));
		if($adb->num_rows($result)) {
			$ruleid = $adb->query_result($result, 0, 'ruleid');
			$adb->pquery("UPDATE nectarcrm_def_org_share SET permission=? WHERE ruleid=?", Array($permission, $ruleid));
		} else {
			$ruleid = self::__getDefaultSharingAccessId();
			$adb->pquery("INSERT INTO nectarcrm_def_org_share (ruleid,tabid,permission,editstatus) VALUES(?,?,?,?)",
	...
	static function updateTool($moduleInstance, $toolAction, $flag, $profileid=false) {
		...
		$result = $adb->pquery("SELECT actionid FROM nectarcrm_actionmapping WHERE actionname=?", Array($toolAction));
		if($adb->num_rows($result)) {
			...
			if($profileid) {
				...
				$profileids = nectarcrm_Profile::getAllIds();
			}

			self::log( ($flag? 'Enabling':'Disabling') . " $toolAction for Profile [", false);

			foreach($profileids as $useprofileid) {
				$result = $adb->pquery("SELECT permission FROM nectarcrm_profile2utility WHERE profileid=? AND tabid=? AND activityid=?",
				...
				if($adb->num_rows($result)) {
					...
					if($curpermission != $permission) {
						$adb->pquery("UPDATE nectarcrm_profile2utility set permission=? WHERE profileid=? AND tabid=? AND activityid=?",
							Array($permission, $useprofileid, $moduleInstance->id, $actionid));
					}
				} else {
					$adb->pquery("INSERT INTO nectarcrm_profile2utility (profileid, tabid, activityid, permission) VALUES(?,?,?,?)",
	...
	static function deleteTools($moduleInstance) {
		global $adb;
		$adb->pquery("DELETE FROM nectarcrm_profile2utility WHERE tabid=?", Array($moduleInstance->id));
