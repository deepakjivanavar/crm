 ...
 * @package vtlib
 */
class nectarcrm_Mailer extends PHPMailer {
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('nectarcrm_mailer_queue');
	}
	...
	function initialize() {
		...
		global $adb;
		$result = $adb->pquery("SELECT * FROM nectarcrm_systems WHERE server_type=?", Array('email'));
...
//			$this->Sender= getReturnPath($this->Host);
	...
	function initFromTemplate($emailtemplate) {
		global $adb;
		$result = $adb->pquery("SELECT * from nectarcrm_emailtemplates WHERE templatename=? AND foldername=?",
	...
	*/
	function addSignature($userId) {
		global $adb;
		$sign = nl2br($adb->query_result($adb->pquery("select signature from nectarcrm_users where id=?", array($userId)),0,"signature"));
		$this->Signature = $sign;
	}
	...
	function ConfigSenderInfo($fromemail, $fromname='', $replyto='') {
		...
		$this->From = $fromemail;
		//fix for (http://trac.nectarcrm.com/cgi-bin/trac.cgi/ticket/8001)
		if($fromname) {
	 ...
	 * @param String nectarcrm CRM Email template name to use
	 */
	...
	function __initializeQueue() {
		if(!$this->_queueinitialized) {
			if(!nectarcrm_Utils::CheckTable('nectarcrm_mailer_queue')) {
				nectarcrm_Utils::CreateTable('nectarcrm_mailer_queue',
			...
			if(!nectarcrm_Utils::CheckTable('nectarcrm_mailer_queueinfo')) {
				nectarcrm_Utils::CreateTable('nectarcrm_mailer_queueinfo',
					'(id INTEGER, name VARCHAR(100), email VARCHAR(100), type VARCHAR(7))',
					true);
			}
			if(!nectarcrm_Utils::CheckTable('nectarcrm_mailer_queueattachments')) {
				nectarcrm_Utils::CreateTable('nectarcrm_mailer_queueattachments',
	...
	function __AddToQueue($linktoid) {
		if($this->__initializeQueue()) {
			...
			$adb->pquery('INSERT INTO nectarcrm_mailer_queue(id,fromname,fromemail,content_type,subject,body,mailer,relcrmid) VALUES(?,?,?,?,?,?,?,?)',
			...
			foreach($this->to as $toinfo) {
				...
				$adb->pquery('INSERT INTO nectarcrm_mailer_queueinfo(id, name, email, type) VALUES(?,?,?,?)',
			...
			foreach($this->attachment as $attachmentinfo) {
				...
				$adb->pquery('INSERT INTO nectarcrm_mailer_queueattachments(id, path, name, encoding, type) VALUES(?,?,?,?,?)',
	 ...
	 */
	static function dispatchQueue(nectarcrm_Mailer_Listener $listener=null) {
		global $adb;
		if(!nectarcrm_Utils::CheckTable('nectarcrm_mailer_queue')) return;

		$mailer = new self();
		$queue = $adb->pquery('SELECT * FROM nectarcrm_mailer_queue', array());
		if($adb->num_rows($queue)) {
			for($index = 0; $index < $adb->num_rows($queue); ++$index) {
				...
				$emails = $adb->pquery('SELECT * FROM nectarcrm_mailer_queueinfo WHERE id=?', Array($queueid));
				...
				$attachments = $adb->pquery('SELECT * FROM nectarcrm_mailer_queueattachments WHERE id=?', Array($queueid));
				...
				if($sent) {
					nectarcrm_Event::trigger('nectarcrm.mailer.mailsent', $relcrmid);
					if($listener) {
						$listener->mailsent($queueid);
					}
					$adb->pquery('DELETE FROM nectarcrm_mailer_queue WHERE id=?', Array($queueid));
					$adb->pquery('DELETE FROM nectarcrm_mailer_queueinfo WHERE id=?', Array($queueid));
					$adb->pquery('DELETE FROM nectarcrm_mailer_queueattachments WHERE id=?', Array($queueid));
				} else {
					if($listener) {
						$listener->mailerror($queueid);
					}
					$adb->pquery('UPDATE nectarcrm_mailer_queue SET failed=?, failreason=? WHERE id=?', Array(1, $mailer->ErrorInfo, $queueid));
				}
			}
		}
	}
 ...
 * @package vtlib
 */
abstract class nectarcrm_Mailer_Listener {
