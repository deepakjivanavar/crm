...
include_once('vtlib/nectarcrm/ModuleBasic.php');
/**
 * Provides API to work with nectarcrm CRM Modules
 * @package vtlib
 */
class nectarcrm_Module extends nectarcrm_ModuleBasic {
	...
	function __getRelatedListUniqueId() {
		global $adb;
		return $adb->getUniqueID('nectarcrm_relatedlists');
	}
	...
	function __getNextRelatedListSequence() {
		global $adb;
		$max_sequence = 0;
		$result = $adb->pquery("SELECT max(sequence) as maxsequence FROM nectarcrm_relatedlists WHERE tabid=?", Array($this->id));
	 ...
	 * @param nectarcrm_Module Instance of target module with which relation should be setup
	 ...
	 *
	 * @internal Creates table nectarcrm_crmentityrel if it does not exists
	 */
	function setRelatedList($moduleInstance, $label = '', $actions = false, $function_name = 'get_related_list', $fieldId = null) {
		...
		if(!nectarcrm_Utils::CheckTable('nectarcrm_crmentityrel')) {
			nectarcrm_Utils::CreateTable(
				'nectarcrm_crmentityrel',
		...
		// Add column to nectarcrm_relatedlists to save extended actions
		nectarcrm_Utils::AddColumn('nectarcrm_relatedlists', 'actions', 'VARCHAR(50)');

		$adb->pquery("INSERT INTO nectarcrm_relatedlists(relation_id,tabid,related_tabid,name,sequence,label,presence,actions,relationfieldid) VALUES(?,?,?,?,?,?,?,?,?)",
	...
	function unsetRelatedList($moduleInstance, $label='', $function_name='get_related_list') {
		...
		$adb->pquery("DELETE FROM nectarcrm_relatedlists WHERE tabid=? AND related_tabid=? AND name=? AND label=?",
	...
	function unsetRelatedListForField($fieldId) {
		...
		$db->pquery("DELETE FROM nectarcrm_relatedlists WHERE relationfieldid=?", array($fieldId));
	}
	...
	function addLink($type, $label, $url, $iconpath='', $sequence=0, $handlerInfo=null) {
		nectarcrm_Link::addLink($this->id, $type, $label, $url, $iconpath, $sequence, $handlerInfo);
	}
	...
	function deleteLink($type, $label, $url=false) {
		nectarcrm_Link::deleteLink($this->id, $type, $label, $url);
	}
	 ...
	 */
	function getLinks() {
		return nectarcrm_Link::getAll($this->id);
	}
	...
	function getLinksForExport() {
		return nectarcrm_Link::getAllForExport($this->id);
	}
	...
	function initWebservice() {
		nectarcrm_Webservice::initialize($this);
	}
	...
	function deinitWebservice() {
		nectarcrm_Webservice::uninitialize($this);
	}
	...
	static function getInstance($value) {
		$instance = nectarcrm_Cache::get('module', $value);
		if (!$instance && $instance !== NULL) {
			$data = nectarcrm_Functions::getModuleData($value);
			if ($data) {
				$instance = new self();
				$instance->initialize($data);
				nectarcrm_Cache::set('module', $instance->id, $instance);
				nectarcrm_Cache::set('module', $instance->name, $instance);
			} else {
				nectarcrm_Cache::set('module', $value, NULL);
			}
		}
	...
	static function getClassInstance($modulename) {
		...
		if(nectarcrm_Utils::checkFileAccessForInclusion($filepath, false)) {
