 ...
 * All Rights Reserved.
 ************************************************************************************/
include_once('vtlib/nectarcrm/PackageImport.php');

/**
 * Provides API to update module into nectarcrm CRM
 * @package vtlib
 */
class nectarcrm_PackageUpdate extends nectarcrm_PackageImport {
	 ...
	 * Constructor
	 */
	function nectarcrm_PackageUpdate() {
		parent::__construct();
	}
	 ...
	 * @param nectarcrm_Module Instance of the module to update
				...
				foreach ($installSequenceArray as $sequence) {
					foreach ($buildModuleArray as $moduleInfo) {
						if($moduleInfo['install_sequence'] == $sequence) {
							$moduleInstance = nectarcrm_Module::getInstance($moduleInfo['name']);
	...
	function update_Module($moduleInstance) {
		...
		nectarcrm_Module::fireEvent($moduleInstance->name, nectarcrm_Module::EVENT_MODULE_PREUPDATE);
		...
		nectarcrm_Module::fireEvent($moduleInstance->name, nectarcrm_Module::EVENT_MODULE_POSTUPDATE);
		nectarcrm_Cache::flushModuleCache($moduleInstance->name);
	}
				...
				if(!empty($migrationnode->tables) && !empty($migrationnode->tables->table)) {
					foreach($migrationnode->tables->table as $tablenode) {
						...
						if(nectarcrm_Utils::IsDestructiveSql($tablesql)) {
							...
							nectarcrm_Utils::ExecuteQuery($tablesql, true);
							self::log("DONE");
						}
					}
				}
	...
	function update_Fields($blocknode, $blockInstance, $moduleInstance) {
		...
		foreach($blocknode->fields->field as $fieldnode) {
			$fieldInstance = nectarcrm_Field::getInstance((string)$fieldnode->fieldname, $moduleInstance);
			if(!$fieldInstance) {
	...
	function update_CustomViews($modulenode, $moduleInstance) {
		...
		foreach($modulenode->customviews->customview as $customviewnode) {
			$filterInstance = nectarcrm_Filter::getInstance($customviewnode->viewname, $moduleInstance);
			if(!$filterInstance) {
	...
	function update_Event($modulenode, $moduleInstance, $eventnode) {
		//nectarcrm_Event::register($moduleInstance, $eventnode->eventname, $eventnode->classname, $eventnode->filename);
	...
	function update_Relatedlist($modulenode, $moduleInstance, $relatedlistnode) {
		$relModuleInstance = nectarcrm_Module::getInstance((string)$relatedlistnode->relatedmodule);
	...
	function update_CronTasks($modulenode) {
		...
		$cronTasks = nectarcrm_Cron::listAllInstancesByModule($modulenode->name);
		foreach ($modulenode->crons->cron as $importCronTask) {
			foreach($cronTasks as $cronTask) {
				if($cronTask->getName() == $importCronTask->name && $importCronTask->handler == $cronTask->getHandlerFile()) {
					nectarcrm_Cron::deregister($importCronTask->name);
				}
			}
			...
			if((empty($importCronTask->sequence))){
				$importCronTask->sequence=nectarcrm_Cron::nextSequence();
			}
			nectarcrm_Cron::register("$importCronTask->name","$importCronTask->handler", "$importCronTask->frequency", "$modulenode->name","$importCronTask->status","$importCronTask->sequence","$importCronTask->description");
		}
	}
}
?>
