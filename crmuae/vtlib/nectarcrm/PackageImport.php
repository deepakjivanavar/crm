 ...
 * All Rights Reserved.
 ************************************************************************************/
include_once('vtlib/nectarcrm/PackageExport.php');
include_once('vtlib/nectarcrm/Unzip.php');
include_once('vtlib/nectarcrm/Module.php');
include_once('vtlib/nectarcrm/Event.php');
include_once('vtlib/nectarcrm/Cron.php');

/**
 * Provides API to import module into nectarcrm CRM
 * @package vtlib
 */
class nectarcrm_PackageImport extends nectarcrm_PackageExport {
	 ...
	 * Constructor
	 */
	function nectarcrm_PackageImport() {
		parent::__construct();
	}
	...
	function checkZip($zipfile) {
		...
		foreach($filelist as $filename=>$fileinfo) {
			...
			if(count($matches)) {
				...
				if($isModuleBundle === 'true' && (!empty($this->_modulexml)) &&
						...
						(!empty($this->_modulexml->dependencies->nectarcrm_version))) {
		...
		if(!empty($this->_modulexml) &&
			!empty($this->_modulexml->dependencies) &&
			!empty($this->_modulexml->dependencies->nectarcrm_version)) {
				$nectarcrmVersion = (string)$this->_modulexml->dependencies->nectarcrm_version;
				if(version_compare($nectarcrmVersion, '6.0.0rc', '>=') === true) {
				$nectarcrmversion_found = true;
			}
		}

		$validzip = false;
		if($manifestxml_found && $languagefile_found && $nectarcrmversion_found)
			$validzip = true;

		if($manifestxml_found && $layoutfile_found && $nectarcrmversion_found)
			$validzip = true;

		if($manifestxml_found && $extensionfile_found && $nectarcrmversion_found)
			$validzip = true;
	...
	function initImport($zipfile, $overwrite=true) {
		...
		if($module != null) {
			...
			$unzip = new nectarcrm_Unzip($zipfile, $overwrite);
	 ...
	 * @access private
	 */
	function getDependentnectarcrmVersion() {
		return $this->_modulexml->dependencies->nectarcrm_version;
	}
	 ...
	 * @access private
	 */
	function getDependentMaxnectarcrmVersion() {
		return $this->_modulexml->dependencies->nectarcrm_max_version;
	}
	...
	function import_Module() {
		...
		$nectarcrmMinVersion = (string)$this->_modulexml->dependencies->nectarcrm_version;
		$nectarcrmMaxVersion = (string)$this->_modulexml->dependencies->nectarcrm_max_version;

		$parentTabs = explode(',', $parenttab);

		$moduleInstance = new nectarcrm_Module();
		...
		$moduleInstance->minversion = (!$nectarcrmMinVersion)? false : $nectarcrmMinVersion;
		$moduleInstance->maxversion = (!$nectarcrmMaxVersion)?  false : $nectarcrmMaxVersion;
		$moduleInstance->save();
		...
		nectarcrm_Module::fireEvent($moduleInstance->name, nectarcrm_Module::EVENT_MODULE_POSTINSTALL);

		$moduleInstance->initWebservice();
		nectarcrm_Cache::delete('module', $this->getModuleName());
	}
	...
	function import_Tables($modulenode, $moduleInstance = false) {
		...
		foreach($modulenode->tables->table as $tablenode) {
			...
			if(nectarcrm_Utils::IsCreateSql($tablesql)) {
				if(!nectarcrm_Utils::checkTable($tablename)) {
					self::log("SQL: $tablesql ... ", false);
					nectarcrm_Utils::ExecuteQuery($tablesql);
					self::log("DONE");
				}
			} else {
				if(nectarcrm_Utils::IsDestructiveSql($tablesql)) {
		...
		$moduleModel = nectarcrm_Module_Model::getInstance($modulenode->name);
		if($moduleInstance && $moduleInstance->isentitytype && $moduleModel->isStarredEnabled()) {
			$moduleUserSpecificTable = nectarcrm_Functions::getUserSpecificTableName($modulenode->name);
	...
	function import_Blocks($modulenode, $moduleInstance) {
		...
		if($moduleInstance->isentitytype) {
			...
			foreach ($modulenode->blocks->block as $blocknode) {
				$blockInstance = nectarcrm_Block::getInstance((string)$blocknode->label, $moduleInstance);
	...
	function import_CustomView($modulenode, $moduleInstance, $customviewnode) {
		...
		$filterInstance = new nectarcrm_Filter();
	...
	function import_Event($modulenode, $moduleInstance, $eventnode) {
		...
		nectarcrm_Event::register($moduleInstance,
	...
	function import_Relatedlist($modulenode, $moduleInstance, $relatedlistnode) {
		$relModuleInstance = nectarcrm_Module::getInstance($relatedlistnode->relatedmodule);
	...
	function import_CronTasks($modulenode){
		...
		foreach ($modulenode->crons->cron as $cronTask){
			if(empty($cronTask->status)){
				$cronTask->status = nectarcrm_Cron::$STATUS_DISABLED;
			} else {
				$cronTask->status = nectarcrm_Cron::$STATUS_ENABLED;
			}
			if((empty($cronTask->sequence))){
				$cronTask->sequence=nectarcrm_Cron::nextSequence();
			}
			nectarcrm_Cron::register("$cronTask->name","$cronTask->handler", "$cronTask->frequency", "$modulenode->name","$cronTask->status","$cronTask->sequence","$cronTask->description");
		}
	}
	...
	function import_mandatoryFields($blocknode, $blockInstance, $moduleInstance) {
		if(nectarcrm_Field::getInstance('source', $moduleInstance) === false) {
		...
		$moduleModel = nectarcrm_Module_Model::getInstance($moduleInstance->name);
		if(nectarcrm_Field::getInstance('starred', $moduleInstance) === false && $moduleModel->isStarredEnabled()) {
			//for starred record 
			$field = new nectarcrm_Field();
			$field->name = 'starred';
			$field->label = 'starred';
			$field->table = nectarcrm_Functions::getUserSpecificTableName($moduleInstance->name);
			$field->presence = 2;
			$field->displaytype = 6;
			$field->readonly = 1;
			$field->uitype = 56;
		...
		if (nectarcrm_Field::getInstance('tags', $moduleInstance) === false && $moduleModel->isTagsEnabled()) {
