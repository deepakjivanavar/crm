...
class nectarcrm_Functions {
	...
	static function getTranslatedString($str, $module='', $language='') {
		return nectarcrm_Language_Handler::getTranslatedString($str, $module, $language);
	}
	...
	static function userCurrencyId($userid) {
		...
		if (!isset(self::$userIdCurrencyIdCache[$userid])) {
			$result = $adb->pquery('SELECT id,currency_id FROM nectarcrm_users', array());
	...
	protected static function getCurrencyInfo($currencyid) {
		...
		if (!isset(self::$currencyInfoCache[$currencyid])) {
			$result = $adb->pquery('SELECT * FROM nectarcrm_currency_info', array());
			while ($row = $adb->fetch_array($result)) {
	...
	static function getCurrencyName($currencyid, $show_symbol = true) {
		...
		if ($show_symbol) {
			return sprintf("%s : %s", nectarcrm_Deprecated::getTranslatedCurrencyString($currencyInfo['currency_name']), $currencyInfo['currency_symbol']);
		}
	...
	protected static function getBasicModuleInfo($mixed) {
		...
		if ($reload) {
			global $adb;
			$result = $adb->pquery('SELECT tabid, name, ownedby FROM nectarcrm_tab', array());
			while ($row = $adb->fetch_array($result)) {
	...
	static function getModuleData($mixed) {
		...
		if ($reload) {
			global $adb;
			$result = $adb->pquery('SELECT * FROM nectarcrm_tab', array());
			while ($row = $adb->fetch_array($result)) {
				self::$moduleIdNameCache[$row['tabid']] = $row;
				self::$moduleNameIdCache[$row['name']]  = $row;
	...
	static function getEntityModuleInfo($mixed) {
		...
		if ($name && !isset(self::$moduleEntityCache[$name])) {
			global $adb;
			$result = $adb->pquery('SELECT fieldname,modulename,tablename,entityidfield,entityidcolumn from nectarcrm_entityname', array());
			while ($row = $adb->fetch_array($result)) {
	...
	protected static function getCRMRecordMetadata($mixedid) {
		...
		if ($missing) {
			$sql = sprintf("SELECT crmid, setype, label FROM nectarcrm_crmentity WHERE %s", implode(' OR ', array_fill(0, count($missing), 'crmid=?')));
	...
	static function updateCRMRecordLabel($module, $id) {
		...
		if ($labelInfo) {
			...
			$adb->pquery('UPDATE nectarcrm_crmentity SET label=? WHERE crmid=?', array($label, $id));
	...
	static function computeCRMRecordLabels($module, $ids) {
		...
		if ($module) {
			...
			if ($ids) {
				...
				if ($module == 'Groups') {
					$metainfo = array('tablename' => 'nectarcrm_groups','entityidfield' => 'groupid','fieldname' => 'groupname');
				} else if ($module == 'DocumentFolders') {
					$metainfo = array('tablename' => 'nectarcrm_attachmentsfolder','entityidfield' => 'folderid','fieldname' => 'foldername'); 
				} else {
	...
	static function getGroupName($id) {
		...
		if (!self::$groupIdNameCache[$id]) {
			$result = $adb->pquery('SELECT groupid, groupname FROM nectarcrm_groups');
			while ($row = $adb->fetch_array($result)) {
	...
	static function getUserName($id) {
		...
		if (!self::$userIdNameCache[$id]) {
			$result = $adb->pquery('SELECT id, user_name FROM nectarcrm_users');
			while ($row = $adb->fetch_array($result)) {
	...
	static function getModuleFieldInfos($mixed) {
		...
		if(nectarcrm_Cache::get('ModuleFieldInfo',$module)){
			return nectarcrm_Cache::get('ModuleFieldInfo',$module);
		}

		if ($module) {
			$result = $adb->pquery('SELECT * FROM nectarcrm_field WHERE tabid=?', array(self::getModuleId($module)));

			while ($row = $adb->fetch_array($result)) {
				$moduleFieldInfo[$module][$row['fieldname']] = $row;
			}
			nectarcrm_Cache::set('ModuleFieldInfo',$module,$moduleFieldInfo[$module]);
		}
	...
	static function getModuleFieldInfoWithId($fieldid) {
		global $adb;
		$result = $adb->pquery('SELECT * FROM nectarcrm_field WHERE fieldid=?', array($fieldid));
	...
	static function getInventoryTermsAndCondition($moduleName) {
		global $adb;
		$sql = 'SELECT tandc FROM nectarcrm_inventory_tandc WHERE type = ?';
	...
	static function getTicketComments($ticketid) {
		...
		$commentlist = '';
		$sql = "SELECT commentcontent FROM nectarcrm_modcomments WHERE related_to = ?";
	...
	static function getTagCloudView($id = "") {
		...
		if ($id == '') {
			...
			$query = "select visible from nectarcrm_homestuff where userid=? and stufftype='Tag Cloud'";
	...
	static function transformFieldTypeOfData($table_name, $column_name, $type_of_data) {
		...
		static $new_field_details = Array(
			...
			"nectarcrm_contactdetails:accountid" => "V",
			"nectarcrm_contactsubdetails:birthday" => "D",
			"nectarcrm_contactdetails:email" => "V",
			"nectarcrm_contactdetails:secondaryemail" => "V",
			//Potential Related Fields
			"nectarcrm_potential:campaignid" => "V",
			//Account Related Fields
			"nectarcrm_account:parentid" => "V",
			"nectarcrm_account:email1" => "V",
			"nectarcrm_account:email2" => "V",
			//Lead Related Fields
			"nectarcrm_leaddetails:email" => "V",
			"nectarcrm_leaddetails:secondaryemail" => "V",
			//Documents Related Fields
			"nectarcrm_senotesrel:crmid" => "V",
			//Calendar Related Fields
			"nectarcrm_seactivityrel:crmid" => "V",
			"nectarcrm_seactivityrel:contactid" => "V",
			"nectarcrm_recurringevents:recurringtype" => "V",
			//HelpDesk Related Fields
			"nectarcrm_troubletickets:parent_id" => "V",
			"nectarcrm_troubletickets:product_id" => "V",
			//Product Related Fields
			"nectarcrm_products:discontinued" => "C",
			"nectarcrm_products:vendor_id" => "V",
			"nectarcrm_products:parentid" => "V",
			//Faq Related Fields
			"nectarcrm_faq:product_id" => "V",
			//Vendor Related Fields
			"nectarcrm_vendor:email" => "V",
			//Quotes Related Fields
			"nectarcrm_quotes:potentialid" => "V",
			"nectarcrm_quotes:inventorymanager" => "V",
			"nectarcrm_quotes:accountid" => "V",
			//Purchase Order Related Fields
			"nectarcrm_purchaseorder:vendorid" => "V",
			"nectarcrm_purchaseorder:contactid" => "V",
			//SalesOrder Related Fields
			"nectarcrm_salesorder:potentialid" => "V",
			"nectarcrm_salesorder:quoteid" => "V",
			"nectarcrm_salesorder:contactid" => "V",
			"nectarcrm_salesorder:accountid" => "V",
			//Invoice Related Fields
			"nectarcrm_invoice:salesorderid" => "V",
			"nectarcrm_invoice:contactid" => "V",
			"nectarcrm_invoice:accountid" => "V",
			//Campaign Related Fields
			"nectarcrm_campaign:product_id" => "V",
			//Related List Entries(For Report Module)
			"nectarcrm_activityproductrel:activityid" => "V",
			"nectarcrm_activityproductrel:productid" => "V",
			"nectarcrm_campaigncontrel:campaignid" => "V",
			"nectarcrm_campaigncontrel:contactid" => "V",
			"nectarcrm_campaignleadrel:campaignid" => "V",
			"nectarcrm_campaignleadrel:leadid" => "V",
			"nectarcrm_cntactivityrel:contactid" => "V",
			"nectarcrm_cntactivityrel:activityid" => "V",
			"nectarcrm_contpotentialrel:contactid" => "V",
			"nectarcrm_contpotentialrel:potentialid" => "V",
			"nectarcrm_pricebookproductrel:pricebookid" => "V",
			"nectarcrm_pricebookproductrel:productid" => "V",
			"nectarcrm_seactivityrel:crmid" => "V",
			"nectarcrm_seactivityrel:activityid" => "V",
			"nectarcrm_senotesrel:crmid" => "V",
			"nectarcrm_senotesrel:notesid" => "V",
			"nectarcrm_seproductsrel:crmid" => "V",
			"nectarcrm_seproductsrel:productid" => "V",
			"nectarcrm_seticketsrel:crmid" => "V",
			"nectarcrm_seticketsrel:ticketid" => "V",
			"nectarcrm_vendorcontactrel:vendorid" => "V",
			"nectarcrm_vendorcontactrel:contactid" => "V",
			"nectarcrm_pricebook:currency_id" => "V",
		);
	...
	static function getPickListValuesFromTableForRole($tablename, $roleid) {
		global $adb;
		$query = "select $tablename from nectarcrm_$tablename inner join nectarcrm_role2picklist on nectarcrm_role2picklist.picklistvalueid = nectarcrm_$tablename.picklist_valueid where roleid=? and picklistid in (select picklistid from nectarcrm_picklist) order by sortid";
	...
	static function getActivityType($id) {
		global $adb;
		$query = "select activitytype from nectarcrm_activity where activityid=?";
		$res = $adb->pquery($query, array($id));
	...
	static function getInvoiceStatus($id) {
		global $adb;
		$result = $adb->pquery("SELECT invoicestatus FROM nectarcrm_invoice where invoiceid=?", array($id));
	...
	*/
	static function getUnitPrice($productid, $module='Products') {
		...
		if($module == 'Services') {
			$query = "select unit_price from nectarcrm_service where serviceid=?";
		} else {
			$query = "select unit_price from nectarcrm_products where productid=?";
		}
	...
	/**
	* Function to fetch the list of nectarcrm_groups from group nectarcrm_table
	* Takes no value as input
	* returns the query result set object
	*/
	static function get_group_options() {
        global $adb, $noof_group_rows;
		$sql = "select groupname,groupid from nectarcrm_groups";
	...
	*/
	...
	public static function getMandatoryReferenceFields($module, $mode = 'label') {
		...
		$moduleInstance = nectarcrm_Module_Model::getInstance($module);
		...
		foreach ($referenceFields as $field => $fieldModel) {
			...
			if ($fieldModel->isMandatory() && !empty($referenceModule) && !in_array($uiType, array(50, 51, 52))) {
				$recordName = '?????';
				$result = $db->pquery("SELECT crmid, label FROM nectarcrm_crmentity WHERE label LIKE ? AND deleted = ? AND setype = ?", array("%$recordName%", 0, $referenceModule));
				if ($db->num_rows($result) < 1) {
					$moduleModel = nectarcrm_Module_Model::getInstance($referenceModule);
					$recordModel = nectarcrm_Record_Model::getCleanInstance($referenceModule);
					...
					foreach ($fieldInstances as $blockInstance) {
						foreach ($blockInstance as $fieldInstance) {
							...
							if ($fieldInstance->isMandatory() && !$defaultValue && !in_array($dataType, array('reference', 'multireference'))) {
								$randomValue = nectarcrm_Util_Helper::getDefaultMandatoryValue($fieldInstance->getFieldDataType());
							...
							$referenceRelationFields = nectarcrm_Functions::getMandatoryReferenceFields($referenceModule, 'id');
					...
					$recordModel->save();
					if ($mode == 'label') {
						$recordName = nectarcrm_Util_Helper::getRecordName($recordModel->getId());
					} else {
	...
	static function setEventsContactIdToRequest($recordId) {
		$db = PearDatabase::getInstance();
		$contactIds = array();
		$result = $db->pquery("SELECT contactid FROM nectarcrm_cntactivityrel WHERE activityid = ?", array($recordId));
	 ...
	 static function getNonQuickCreateSupportedModules() {
		...
		$modules = nectarcrm_Module_Model::getAll(array(0, 2));
	...
	public static function getUserSpecificTableName($moduleName) {
		return 'nectarcrm_crmentity_user_field';
	}
	...
	public static function isUserExist($userId) {
		$adb = PearDatabase::getInstance();
		$query = "SELECT 1 FROM nectarcrm_users WHERE id=? AND deleted=? AND status = ?";
	...
	static function isDocumentsRelated($sourceModule) {
		...
		if (vtlib_isModuleActive($moduleName)) {
			...
			if ($moduleModel && $sourceModuleModel) {
				$relationModel = nectarcrm_Relation_Model::getInstance($sourceModuleModel, $moduleModel);
			}
			if ($relationModel) {
				$isRelated = true;
			}
