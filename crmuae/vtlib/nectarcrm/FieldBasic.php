...
/**
 * Provides basic API to work with nectarcrm CRM Fields
 * @package vtlib
 */
class nectarcrm_FieldBasic {
	 ...
	 * @param Array 
	 * @param nectarcrm_Module Instance of module to which this field belongs
	 * @param nectarcrm_Block Instance of block to which this field belongs
	 * @access private
	 */
	function initialize($valuemap, $moduleInstance=false, $blockInstance=false) {
		...
		$this->block= $blockInstance? $blockInstance : nectarcrm_Block::getInstance($valuemap['block'], $moduleInstance);
	...
	function __handlenectarcrmCoreSchemaChanges() {
		// Add helpinfo column to the nectarcrm_field table
		if (empty(self::$__cacheSchemaChanges['nectarcrm_field.helpinfo'])) {
			nectarcrm_Utils::AddColumn('nectarcrm_field', 'helpinfo', ' TEXT');
			self::$__cacheSchemaChanges['nectarcrm_field.helpinfo'] = true;
		}
		if (empty(self::$__cacheSchemaChanges['nectarcrm_field.summaryfield'])) {
			nectarcrm_Utils::AddColumn('nectarcrm_field', 'summaryfield', ' INT(10) NOT NULL DEFAULT 0');
			self::$__cacheSchemaChanges['nectarcrm_field.summaryfield'] = 0;
		}
		if (empty(self::$__cacheSchemaChanges['nectarcrm_field.headerfield'])) {
			nectarcrm_Utils::AddColumn('nectarcrm_field', 'headerfield', ' INT(1) DEFAULT 0');
			self::$__cacheSchemaChanges['nectarcrm_field.headerfield'] = 0;
		}
	}
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('nectarcrm_field');
	}
	...
	function __getNextSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(sequence) AS max_seq FROM nectarcrm_field WHERE tabid=? AND block=?", Array($this->getModuleId(), $this->getBlockId()));
		$maxseq = 0;
	...
	function __getNextQuickCreateSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(quickcreatesequence) AS max_quickcreateseq FROM nectarcrm_field WHERE tabid=?", Array($this->getModuleId()));
	 ...
	 * @param nectarcrm_Block Instance of the block to use
	 * @access private
	 */
	function __create($blockInstance) {
		...
		if (!empty($this->columntype)) {
			nectarcrm_Utils::AddColumn($this->table, $this->column, $this->columntype);
		}
		...
		$adb->pquery("INSERT INTO nectarcrm_field (tabid, fieldid, columnname, tablename, generatedtype,
		...
		$adb->pquery('UPDATE nectarcrm_field SET masseditable=? WHERE fieldid=?', Array($this->masseditable, $this->id));

		nectarcrm_Profile::initForField($this);
	...
	function __delete() {
		...
		nectarcrm_Profile::deleteForField($this);

		//TODO : should we check if the field is realtion field or not
		$this->getModuleInstance()->unsetRelatedListForField($this->id);

		$adb->pquery("DELETE FROM nectarcrm_field WHERE fieldid=?", Array($this->id));

		$em = new VTEventsManager($adb);
		$em->triggerEvent('nectarcrm.field.afterdelete', $this);
	 ...
	 * @param nectarcrm_Block Instance of block to which this field should be added.
	 */
	function save($blockInstance = false) {
		...
		// Clearing cache
		nectarcrm_Cache::flushModuleandBlockFieldsCache($this->getModuleInstance(), $this->getBlockId());
		return $this->id;
	}
	...
	function setHelpInfo($helptext) {
		...
		global $adb;
		$adb->pquery('UPDATE nectarcrm_field SET helpinfo=? WHERE fieldid=?', Array($helptext, $this->id));
	...
	function setMassEditable($value) {
		global $adb;
		$adb->pquery('UPDATE nectarcrm_field SET masseditable=? WHERE fieldid=?', Array($value, $this->id));
    ...
    function setSummaryField($value) { 
        global $adb; 
        $adb->pquery('UPDATE nectarcrm_field SET summaryfield=? WHERE fieldid=?', Array($value, $this->id)); 
