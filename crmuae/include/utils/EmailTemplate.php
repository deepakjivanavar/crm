			...
			if (count($tableList) > 0 && count($columnListTable) > 0) {
				...
				foreach ($tableList as $index => $tableName) {
					if ($tableName != $tableList[0]) {
						if($tableName == 'nectarcrm_seactivityrel' || $tableName == 'nectarcrm_cntactivityrel') {
							$sql .= ' LEFT JOIN ';
						} else {
							$sql .= ' INNER JOIN ';
						}
				...
				if($module == 'Leads' && !in_array('nectarcrm_leaddetails', $tableList)){
					$sql .=' INNER JOIN nectarcrm_leaddetails ON nectarcrm_leaddetails.leadid = nectarcrm_crmentity.crmid';
				}
			...
			//assuming there can only be one row.
				...
				foreach ($it as $row) {
					foreach ($fieldList as $field) {
						...
						$fieldModel = nectarcrm_Field_Model::getInstance($field, $moduleModel);
...
s								 */
	...
	public function isActive($field, $mod) {
		global $adb;
		$tabid = getTabid($mod);
		$query = 'select * from nectarcrm_field where fieldname = ?  and tabid = ? and presence in (0,2)';
