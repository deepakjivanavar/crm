 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/nectarcrm_crm/sugarcrm/include/utils/EditViewUtils.php,v 1.188 2005/04/29 05:5 * 4:39 rank Exp
...
/** This function returns the nectarcrm_invoice object populated with the details from sales order object.
...
function getConvertSoToInvoice($focus,$so_focus,$soid)
{
	...
	$focus->column_fields['nectarcrm_purchaseorder'] = $so_focus->column_fields['nectarcrm_purchaseorder'];
...
/** This function returns the detailed list of nectarcrm_products associated to a given entity or a record.
...
function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false)
{
	...
	{
		$query="SELECT
					case when nectarcrm_products.productid != '' then nectarcrm_products.productname else nectarcrm_service.servicename end as productname,
					case when nectarcrm_products.productid != '' then nectarcrm_products.product_no else nectarcrm_service.service_no end as productcode,
					case when nectarcrm_products.productid != '' then nectarcrm_products.unit_price else nectarcrm_service.unit_price end as unit_price,
					case when nectarcrm_products.productid != '' then nectarcrm_products.qtyinstock else 'NA' end as qtyinstock,
					case when nectarcrm_products.productid != '' then 'Products' else 'Services' end as entitytype,
					nectarcrm_inventoryproductrel.listprice, nectarcrm_products.is_subproducts_viewable, 
					nectarcrm_inventoryproductrel.description AS product_description, nectarcrm_inventoryproductrel.*,
					nectarcrm_crmentity.deleted FROM nectarcrm_inventoryproductrel
					LEFT JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid=nectarcrm_inventoryproductrel.productid
	...
	{
		$query = '(SELECT nectarcrm_products.productid, nectarcrm_products.productname, nectarcrm_products.product_no AS productcode, nectarcrm_products.purchase_cost,
					nectarcrm_products.unit_price, nectarcrm_products.qtyinstock, nectarcrm_crmentity.deleted, "Products" AS entitytype,
					nectarcrm_products.is_subproducts_viewable, nectarcrm_crmentity.description '.$additionalProductFieldsString.' FROM nectarcrm_products
					INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid=nectarcrm_products.productid
					INNER JOIN nectarcrm_seproductsrel ON nectarcrm_seproductsrel.productid=nectarcrm_products.productid
					INNER JOIN nectarcrm_productcf ON nectarcrm_products.productid = nectarcrm_productcf.productid
					WHERE nectarcrm_seproductsrel.crmid=? AND nectarcrm_crmentity.deleted=0 AND nectarcrm_products.discontinued=1)
					UNION
					(SELECT nectarcrm_service.serviceid AS productid, nectarcrm_service.servicename AS productname, nectarcrm_service.service_no AS productcode,
					nectarcrm_service.purchase_cost, nectarcrm_service.unit_price, "NA" as qtyinstock, nectarcrm_crmentity.deleted,
					"Services" AS entitytype, 1 AS is_subproducts_viewable, nectarcrm_crmentity.description '.$additionalServiceFieldsString.' FROM nectarcrm_service
					INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_service.serviceid
					INNER JOIN nectarcrm_crmentityrel ON nectarcrm_crmentityrel.relcrmid = nectarcrm_service.serviceid
					INNER JOIN nectarcrm_servicecf ON nectarcrm_service.serviceid = nectarcrm_servicecf.serviceid
					WHERE nectarcrm_crmentityrel.crmid=? AND nectarcrm_crmentity.deleted=0 AND nectarcrm_service.discontinued=1)';
			$params = array($seid, $seid);
	}
	elseif ($module == 'Vendors') {
		$query = 'SELECT nectarcrm_products.productid, nectarcrm_products.productname, nectarcrm_products.product_no AS productcode, nectarcrm_products.purchase_cost,
					nectarcrm_products.unit_price, nectarcrm_products.qtyinstock, nectarcrm_crmentity.deleted, "Products" AS entitytype,
					nectarcrm_products.is_subproducts_viewable, nectarcrm_crmentity.description '.$additionalServiceFieldsString.' FROM nectarcrm_products
					INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid=nectarcrm_products.productid
					INNER JOIN nectarcrm_vendor ON nectarcrm_vendor.vendorid = nectarcrm_products.vendor_id
					INNER JOIN nectarcrm_productcf ON nectarcrm_products.productid = nectarcrm_productcf.productid
					WHERE nectarcrm_vendor.vendorid=? AND nectarcrm_crmentity.deleted=0 AND nectarcrm_products.discontinued=1';
			$params = array($seid);
	}
	elseif ($module == 'HelpDesk') {
		$query = 'SELECT nectarcrm_service.serviceid AS productid, nectarcrm_service.servicename AS productname, nectarcrm_service.service_no AS productcode,
					nectarcrm_service.purchase_cost, nectarcrm_service.unit_price, "NA" as qtyinstock, nectarcrm_crmentity.deleted,
					"Services" AS entitytype, 1 AS is_subproducts_viewable, nectarcrm_crmentity.description '.$additionalServiceFieldsString.' FROM nectarcrm_service
					INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_service.serviceid
					INNER JOIN nectarcrm_crmentityrel ON nectarcrm_crmentityrel.relcrmid = nectarcrm_service.serviceid
	...
	elseif($module == 'Products')
	{
		$query = 'SELECT nectarcrm_products.productid, nectarcrm_products.productname, nectarcrm_products.product_no AS productcode, nectarcrm_products.purchase_cost,
					...
					INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid=nectarcrm_products.productid
					INNER JOIN nectarcrm_productcf ON nectarcrm_products.productid = nectarcrm_productcf.productid
					WHERE nectarcrm_crmentity.deleted=0 AND nectarcrm_products.productid=?';
			$params = array($seid);
	}
	elseif($module == 'Services')
	{
		$query='SELECT nectarcrm_service.serviceid AS productid, nectarcrm_service.servicename AS productname, nectarcrm_service.service_no AS productcode,
					nectarcrm_service.purchase_cost, nectarcrm_service.unit_price AS unit_price, "NA" AS qtyinstock, nectarcrm_crmentity.deleted,
					"Services" AS entitytype, 1 AS is_subproducts_viewable, nectarcrm_crmentity.description '.$additionalServiceFieldsString.' FROM nectarcrm_service
					INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid=nectarcrm_service.serviceid
					INNER JOIN nectarcrm_servicecf ON nectarcrm_service.serviceid = nectarcrm_servicecf.serviceid
					WHERE nectarcrm_crmentity.deleted=0 AND nectarcrm_service.serviceid=?';
			$params = array($seid);
	}
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		if (in_array($module, $lineItemSupportedModules) || $module === 'Vendors' || (!$focus->mode && $seid)) {
			$subProductsQuery = 'SELECT nectarcrm_seproductsrel.crmid AS prod_id, quantity FROM nectarcrm_seproductsrel
								 INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_seproductsrel.crmid
								 INNER JOIN nectarcrm_products ON nectarcrm_products.productid = nectarcrm_seproductsrel.crmid
								 WHERE nectarcrm_seproductsrel.productid=? AND nectarcrm_seproductsrel.setype=? AND nectarcrm_products.discontinued=1';
			...
			$subProductsQuery = 'SELECT productid AS prod_id, quantity FROM nectarcrm_inventorysubproductrel WHERE id=? AND sequence_no=?';
	...
	//if taxtype is group then the tax should be same for all products in nectarcrm_inventoryproductrel table
...
/** This function returns the data type of the nectarcrm_fields, with nectarcrm_field label, which is used for javascript validation.
* Param $validationData - array of nectarcrm_fieldnames with datatype
* Return type array
*/
