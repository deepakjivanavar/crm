...
class ListViewController {
	...
	function getListViewRecords($focus, $module, $result) {
		...
		if(count($attachmentIds)) {
			$getAttachmentsNamesSql = 'SELECT attachmentsid,name FROM nectarcrm_attachments WHERE attachmentsid IN (' . generateQuestionMarks($attachmentIds) . ')';
			...
			foreach ($listViewFields as $fieldName) {
				...
				if($module == 'Documents' && $fieldName == 'filename') {
					...
					if($module == 'Calendar' && ($fieldName == 'taskstatus' || $fieldName == 'eventstatus' || $fieldName == 'activitytype')) {
						$value = nectarcrm_Language_Handler::getTranslatedString($value,$module);
						$value = textlength_check($value);
					}
						...
						$value = "<font color='red'>". nectarcrm_Language_Handler::getTranslatedString('LBL_NOT_ACCESSIBLE',
								$module)."</font>";
					} else {
						$value =  nectarcrm_Language_Handler::getTranslatedString($value,$module);
						$value = textlength_check($value);
					...
					if($value != '' && $value != '0000-00-00' && $value != 'NULL') {
						...
						if($fieldDataType == 'datetime' && $value != '0000-00-00 00:00:00') {
							$value = nectarcrm_Datetime_UIType::getDateTimeValue($value);
					...
					if(!empty($value)){
						...
						if($userModel->get('hour_format') == '12'){
							$value = nectarcrm_Time_UIType::getTimeValueInAMorPM($value);
						}
					}
...
//							$value = CurrencyField::appendCurrencySymbol($currencyValue, $currencySymbol);
					...
					if($current_user->internal_mailer == 1){
						...
						$value = "<a class='emailField' data-rawvalue=\"$rawValue\" onclick=\"nectarcrm_Helper_Js.getInternalMailer($recordId,".
					...
					$result1 = $this->db->pquery("SELECT access_count FROM nectarcrm_email_track WHERE ".
