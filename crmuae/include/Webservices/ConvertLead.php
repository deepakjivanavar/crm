...
function vtws_convertlead($entityvalues, $user) {
	...
	$leadObject = nectarcrmWebserviceObject::fromName($adb, 'Leads');
	...
	$getQuote = 'SELECT quoteid from nectarcrm_quotes where contactid=?';
	...
	foreach ($availableModules as $entityName) {
		if ($entityvalues['entities'][$entityName]['create']) {
			...
			$entityObject = nectarcrmWebserviceObject::fromName($adb, $entityvalue['name']);
			...
			try {
				...
				if ($entityvalue['name'] == 'Accounts') {
					$sql = "SELECT nectarcrm_account.accountid FROM nectarcrm_account,nectarcrm_crmentity WHERE nectarcrm_crmentity.crmid=nectarcrm_account.accountid AND nectarcrm_account.accountname=? AND nectarcrm_crmentity.deleted=0";
				...
				if ($create) {
					...
					if(($leadHasImage) && ((($entityName == 'Contacts') || ($entityName == 'Accounts' && !$entityvalues['entities']['Contacts']['create'])))) {
						$imageName = $adb->query_result($adb->pquery('SELECT name FROM nectarcrm_attachments 
					...
					if($leadHasImage && in_array($entityName,array('Accounts','Contacts'))) {
						...
						$adb->pquery('UPDATE nectarcrm_seattachmentsrel SET crmid = ? WHERE attachmentsid = ?',array($crmId,$imageAttachmentId));
						$adb->pquery('UPDATE nectarcrm_crmentity SET setype = ? WHERE crmid = ?',array($entityName.' Image',$imageAttachmentId));
					}
				}
	...
	try {
		...
		if($quoteIds){
			$queryUpdate = 'UPDATE nectarcrm_quotes SET contactid=?, potentialid=? WHERE quoteid IN('. generateQuestionMarks($quoteIds).') ';
			$adb->pquery($queryUpdate, array($contactId, $potentialId, $quoteIds));

			if($accountId){
				$queryUpdate = 'UPDATE nectarcrm_quotes SET accountid=? WHERE quoteid IN('. generateQuestionMarks($quoteIds).')';
	...
	if($leadId[1]) {
		...
		$em->triggerEvent('nectarcrm.lead.convertlead', $entityData);
	}
...
function vtws_updateConvertLeadStatus($entityIds, $leadId, $user) {
	...
	if ($entityIds['Accounts'] != '' || $entityIds['Contacts'] != '') {
		$sql = "UPDATE nectarcrm_leaddetails SET converted = 1 where leadid=?";
		$result = $adb->pquery($sql, array($leadIdComponents[1]));
		if ($result === false) {
		...
		$sql = "DELETE FROM nectarcrm_campaignleadrel WHERE leadid=?";
		$adb->pquery($sql, array($leadIdComponents[1]));

		$sql = "DELETE FROM nectarcrm_tracker WHERE item_id=?";
		$adb->pquery($sql, array($leadIdComponents[1]));

		//update the modifiedtime and modified by information for the record
		$leadModifiedTime = $adb->formatDate(date('Y-m-d H:i:s'), true);
		$crmentityUpdateSql = "UPDATE nectarcrm_crmentity SET modifiedtime=?, modifiedby=? WHERE crmid=?";
