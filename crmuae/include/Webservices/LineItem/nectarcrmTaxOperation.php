...
require_once 'include/Webservices/LineItem/nectarcrmTaxMeta.php';
...
/**
 * Description of nectarcrmTaxOperation
 */
class nectarcrmTaxOperation  extends nectarcrmActorOperation {
	...
	public function __construct($webserviceObject, $user, $adb, $log) {
		...
		$this->meta = new nectarcrmTaxMeta($this->entityTableName,$webserviceObject,$adb,$user);
	...
	public function create($elementType, $taxElement) {
		...
		$sql = "alter table nectarcrm_inventoryproductrel add column $taxName decimal(7,3)";
	...
	private function dropRow($id) {
		$sql = 'delete from nectarcrm_inventorytaxinfo where taxid = ?';
		$params = array($id);
		$result = $this->pearDB->pquery($sql, $params);
	}

	private function getCurrentTaxName() {
		$sql = 'select taxname from nectarcrm_inventorytaxinfo order by taxid desc limit 1';
		$params = array();
		$result = $this->pearDB->pquery($sql, $params);
	...
	private function getTaxName($element) {
		if ($element['taxlabel']) {
			$sql = 'SELECT taxname FROM nectarcrm_inventorytaxinfo WHERE taxlabel = ?';
	...
	private function sanitizeElementForInsert($element) {
		...
		if ($taxFormula) {
			...
			$sql = 'SELECT taxid, method FROM nectarcrm_inventorytaxinfo WHERE taxname IN ('.generateQuestionMarks($taxFormulaElements).')';
