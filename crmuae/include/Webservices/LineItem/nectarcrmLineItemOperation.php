...
require_once "include/Webservices/LineItem/nectarcrmInventoryOperation.php";
require_once("include/events/include.inc");
require_once 'modules/com_nectarcrm_workflow/VTEntityCache.inc';
require_once 'data/CRMEntity.php';
require_once 'include/events/SqlResultIterator.inc';
require_once 'include/Webservices/LineItem/nectarcrmLineItemMeta.php';
...
/**
 * Description of nectarcrmLineItemOperation
 */
class nectarcrmLineItemOperation extends nectarcrmActorOperation {
	...
	public function __construct($webserviceObject,$user,$adb,$log) {
		...
		$this->meta = new nectarcrmLineItemMeta($this->entityTableName,$webserviceObject,$adb,$user);
	...
	public function getAllLineItemForParent($parentId){
		...
		$query = "SELECT nectarcrm_crmentity.label AS productname,nectarcrm_crmentity.setype AS entitytype,nectarcrm_crmentity.deleted AS deleted, {$this->entityTableName}.*
	...
	private function updateTaxes($createdElement){
		if (count($this->taxList) > 0 || (is_array($this->inActiveTaxList) && count($this->inActiveTaxList) > 0)) {
			...
			$id = $id[1];
			$sql = 'UPDATE nectarcrm_inventoryproductrel set ';
	...
	function getProductPrice($productId){
		...
		$sql = "select unit_price from nectarcrm_products where productid=?";
	...
	public function updateParent($createdElement,$parent){
		...
		$result = $this->pearDB->pquery('SELECT * FROM nectarcrm_inventorychargesrel WHERE recordid = ?', array($parentId));
	...
	public function getCollectiveTaxList(){
		$db = PearDatabase::getInstance();
		$sql = 'select * from nectarcrm_inventorytaxinfo where deleted=0';
		$params = array();
		$result = null;
		$transactionSuccessful = vtws_runQueryAsTransaction($sql,$params,$result);
		if(!$transactionSuccessful){
	...
	private function getProductTaxList($productId){
		$db = PearDatabase::getInstance();
		$sql = 'select * from nectarcrm_producttaxrel inner join nectarcrm_inventorytaxinfo on
			nectarcrm_producttaxrel.taxid=nectarcrm_inventorytaxinfo.taxid and deleted=0
			where productid=?';
		$params = array($productId);
		$result = null;
		$transactionSuccessful = vtws_runQueryAsTransaction($sql,$params,$result);
	...
	public function getParentById($parentId){
		if (empty(self::$parentCache[$parentId])) {
			self::$parentCache[$parentId] = nectarcrm_Functions::jsonEncode(vtws_retrieve($parentId, $this->user));
		}
	...
	public function setParent($parentId, $parent) {
		if (is_array($parent) || is_object($parent)) {
			$parent = nectarcrm_Functions::jsonEncode($parent);
		}
