...
require_once 'include/Webservices/nectarcrmModuleOperation.php';
require_once 'include/Webservices/Utils.php';

/**
 * Description of nectarcrmInventoryOperation
 */
class nectarcrmInventoryOperation extends nectarcrmModuleOperation {
	...
	public function create($elementType, $element) {
		...
		if (!empty($lineItems)) {
			...
			$currentBulkSaveMode = vglobal('NECTARCRM_BULK_SAVE_MODE');
			if ($currentBulkSaveMode === NULL) {
				$currentBulkSaveMode = false;
			}
			vglobal('NECTARCRM_BULK_SAVE_MODE', true);
			...
			vglobal('NECTARCRM_BULK_SAVE_MODE', $currentBulkSaveMode);
	...
	public function triggerBeforeSaveEvents($element, $eventManager) {
		...
		if ($eventManager) {
			...
			if (!$NECTARCRM_BULK_SAVE_MODE) {
				$eventManager->triggerEvent("nectarcrm.entity.beforesave.modifiable", $entityData);
				$eventManager->triggerEvent("nectarcrm.entity.beforesave", $entityData);
				$eventManager->triggerEvent("nectarcrm.entity.beforesave.final", $entityData);
			}
		}
	}
	...
	public function triggerAfterSaveEvents($element, $eventManager) {
		...
		if ($eventManager) {
			...
			if (!$NECTARCRM_BULK_SAVE_MODE) {
				...
				$eventManager->triggerEvent("nectarcrm.entity.aftersave", $entityData);
				$eventManager->triggerEvent("nectarcrm.entity.aftersave.final", $entityData);
			}
		}
	}
	...
	public function getChargesElement($elementId) {
		...
		if ($elementId) {
			...
			$id = $ids[1];
			$result = $this->pearDB->pquery('SELECT * FROM nectarcrm_inventorychargesrel WHERE recordid = ?', array($id));
