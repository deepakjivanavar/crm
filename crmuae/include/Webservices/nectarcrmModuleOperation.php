...
class nectarcrmModuleOperation extends WebserviceEntityOperation {
	protected $tabId;
	protected $isEntity = true;
	protected $partialDescribeFields = null;
	
	public function nectarcrmModuleOperation($webserviceObject,$user,$adb,$log){
	...
	protected function getMetaInstance(){
		if(empty(WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id])){
			WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id]  = new nectarcrmCRMObjectMeta($this->webserviceObject,$this->user);
		}
		return WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id];
	}
	
	public function create($elementType,$element){
		$crmObject = new nectarcrmCRMObject($elementType, false);
	...
	public function retrieve($id){
		...
		$crmObject = new nectarcrmCRMObject($this->tabId, true);
    ...
    public function relatedIds($id, $relatedModule, $relatedLabel, $relatedHandler=null) {
		...
		$sourceRecordModel = nectarcrm_Record_Model::getInstanceById($ids[1], $sourceModule);
		$targetModel       = nectarcrm_RelationListView_Model::getInstance($sourceRecordModel, $relatedModule, $relatedLabel);
        $sql = $targetModel->getRelationQuery();

        $relatedWebserviceObject = nectarcrmWebserviceObject::fromName($adb,$relatedModule);
        ...
        $sql = sprintf("SELECT DISTINCT concat('%sx',nectarcrm_crmentity.crmid) as wsid FROM %s", $relatedModuleWSId, $sqlFromPart);
