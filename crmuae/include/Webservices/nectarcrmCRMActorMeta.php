...
class nectarcrmCRMActorMeta extends EntityMeta {
	protected $pearDB;
	protected static $fieldTypeMapping = array();
	protected static $referenceTypeMapping = array();
	
	function nectarcrmCRMActorMeta($tableName,$webserviceObject,$adb,$user){
	...
	protected function getReferenceList($dbField, $tableName){
		...
		if(!isset(nectarcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])){
			$this->getFieldType($dbField, $tableName);
		}
		$fieldTypeData = nectarcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name];
		
		if (empty(nectarcrmCRMActorMeta::$referenceTypeMapping)) {
			$sql = "SELECT * FROM nectarcrm_ws_entity_referencetype";
			...
			for ($index =0, $count = $this->pearDB->num_rows($result); $index < $count; ++$index) {
				...
				nectarcrmCRMActorMeta::$referenceTypeMapping[$row['fieldtypeid']][] = $row['type'];
			}
		}
		
		$referenceTypes = array();
		if (isset(nectarcrmCRMActorMeta::$referenceTypeMapping[$fieldTypeData['fieldtypeid']])) {
			$referenceTypes = nectarcrmCRMActorMeta::$referenceTypeMapping[$fieldTypeData['fieldtypeid']];
		}
	...
	protected function getFieldType($dbField,$tableName){
		...
		if(isset(nectarcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])){
			if(nectarcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name] === 'null'){
				return null;
			}
			$row = nectarcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name];
			return $row['fieldtype'];
		}
		
		if (empty(nectarcrmCRMActorMeta::$fieldTypeMapping)) {
			// Optimization to avoid repeated initialization
			$sql = "select * from nectarcrm_ws_entity_fieldtype";
			$result = $this->pearDB->pquery($sql,array());
			$rowCount = $this->pearDB->num_rows($result);
			while ($rowCount) {
				$row = $this->pearDB->query_result_rowdata($result,$rowCount-1);
				nectarcrmCRMActorMeta::$fieldTypeMapping[$row['table_name']][$row['field_name']] = $row;
				--$rowCount;
			}
		}
		
		if (!isset(nectarcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])) {
			nectarcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name] = 'null';
			return null;
		}
		return nectarcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name]['fieldtype'];
	}
	...
	public function getNameFields(){
		$query = "select name_fields from nectarcrm_ws_entity_name where entity_id = ?";
	...
	public function getEntityDeletedQuery() {
		if($this->getEntityName() == 'Currency'){
			return 'nectarcrm_currency_info.deleted=0';
		}
