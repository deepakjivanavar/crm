...
class nectarcrmCRMObject{
	
	private $moduleName ;
	private $moduleId ;
	private $instance ;
	
	function nectarcrmCRMObject($moduleCredential, $isId=false){
	...
	private function getObjectTypeId($objectName){
		...
		if($tid === false) {
			...
			$sql = "select * from nectarcrm_tab where name=?;";
	...
	function exists($id){
		...
		$exists = false;
		$sql = "select * from nectarcrm_crmentity where crmid=? and deleted=0";
