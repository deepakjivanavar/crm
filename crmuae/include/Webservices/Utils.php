...
/**
 * get current nectarcrm version from the database.
 */
function vtws_getnectarcrmVersion(){
	global $adb;
	$query = 'select * from nectarcrm_version';
...
function vtws_getWebserviceGroupFromGroups($groups){
	global $adb;
	$webserviceObject = nectarcrmWebserviceObject::fromName($adb,'Groups');
...
function getEmailFieldId($meta, $entityId){
	...
	$query="SELECT fieldid,fieldlabel,columnname FROM nectarcrm_field WHERE tabid=?
...
function vtws_getModuleNameList(){
	...
	$sql = "select name from nectarcrm_tab where isentitytype=1 and name not in ('Rss','Webmails',".
...
function vtws_getWebserviceEntities(){
	...
	$sql = "select name,id,ismodule from nectarcrm_ws_entity";
	$res = $adb->pquery($sql, array());
	$moduleArray = Array();
	$entityArray = Array();
	while($row = $adb->fetchByAssoc($res)){
...
/**
 *
 * @param nectarcrmWebserviceObject $webserviceObject
 * @return CRMEntity
 */
...
function vtws_isRecordOwnerUser($ownerId){
	...
	if (!array_key_exists($ownerId, $cache)) {
		$result = $adb->pquery("select first_name from nectarcrm_users where id = ?",array($ownerId));
		$rowCount = $adb->num_rows($result);
...
function vtws_isRecordOwnerGroup($ownerId){
	...
	static $cache = array();
	if (!array_key_exists($ownerId, $cache)) {
		$result = $adb->pquery("select groupname from nectarcrm_groups where groupid = ?",array($ownerId));
		$rowCount = $adb->num_rows($result);
...
function vtws_getCalendarEntityType($id){
	...
	$sql = "select activitytype from nectarcrm_activity where activityid=?";
...
function vtws_getWebserviceEntityId($entityName, $id){
	global $adb;
	$webserviceObject = nectarcrmWebserviceObject::fromName($adb,$entityName);
...
function vtws_addModuleTypeWebserviceEntity($moduleName,$filePath,$className){
	global $adb;
	$checkres = $adb->pquery('SELECT id FROM nectarcrm_ws_entity WHERE name=? AND handler_path=? AND handler_class=?',
		array($moduleName, $filePath, $className));
	if($checkres && $adb->num_rows($checkres) == 0) {
		$isModule=1;
		$entityId = $adb->getUniqueID("nectarcrm_ws_entity");
		$adb->pquery('insert into nectarcrm_ws_entity(id,name,handler_path,handler_class,ismodule) values (?,?,?,?,?)',
...
function vtws_deleteWebserviceEntity($moduleName) {
	global $adb;
	$adb->pquery('DELETE FROM nectarcrm_ws_entity WHERE name=?',array($moduleName));
}

function vtws_addDefaultActorTypeEntity($actorName,$actorNameDetails,$withName = true){
	$actorHandler = array('file'=>'include/Webservices/nectarcrmActorOperation.php',
		'class'=>'nectarcrmActorOperation');
	if($withName == true){
...
function vtws_addActorTypeName($entityId,$fieldNames,$indexColumn,$tableName){
	global $adb;
	$adb->pquery('insert into nectarcrm_ws_entity_name(entity_id,name_fields,index_field,table_name) values (?,?,?,?)',
...
function vtws_getName($id,$user){
	...
	$webserviceObject = nectarcrmWebserviceObject::fromId($adb,$id);
...
function vtws_addWebserviceOperation($name,$handlerFilePath,$handlerMethodName,$requestType,$preLogin = 0){
	global $adb;
	$createOperationQuery = "insert into nectarcrm_ws_operation(operationid,name,handler_path,handler_method,type,prelogin)
		values (?,?,?,?,?,?);";
	...
	$operationId = $adb->getUniqueID("nectarcrm_ws_operation");
...
function vtws_deleteWebServiceOperation($oprationId) {
	global $adb;
	$params = array();
	$sql = 'DELETE from nectarcrm_ws_operation WHERE operationid=?';
	$adb->pquery($sql, array($oprationId));
	$sql = 'DELETE from nectarcrm_ws_operation_parameters WHERE operationid=?';
	$adb->pquery($sql, array($oprationId));
	return true;
}
...
function vtws_addWebserviceOperationParam($operationId,$paramName,$paramType,$sequence){
	...
	$createOperationParamsQuery = "insert into nectarcrm_ws_operation_parameters(operationid,name,type,sequence)
		values (?,?,?,?);";
...
function vtws_getModuleHandlerFromName($name,$user){
	global $adb, $log;
	$webserviceObject = nectarcrmWebserviceObject::fromName($adb,$name);
	$handlerPath = $webserviceObject->getHandlerPath();
	$handlerClass = $webserviceObject->getHandlerClass();
...
function vtws_getActorEntityName ($name, $idList) {
	...
	$entity = nectarcrmWebserviceObject::fromName($db, $name);
...
function vtws_getActorEntityNameById ($entityId, $idList) {
	...
	$nameList = array();
	$webserviceObject = nectarcrmWebserviceObject::fromId($db, $entityId);
	$query = "select * from nectarcrm_ws_entity_name where entity_id = ?";
...
function vtws_isRoleBasedPicklist($name) {
	$db = PearDatabase::getInstance();
	$sql = "select picklistid from nectarcrm_picklist where name = ?";
...
function vtws_getConvertLeadFieldMapping(){
	global $adb;
	$sql = "select * from nectarcrm_convertleadmapping";
...
function vtws_getRelatedNotesAttachments($id,$relatedId) {
	...
	$sql = "select * from nectarcrm_senotesrel where crmid=?";
	$result = $adb->pquery($sql, array($id));
	if($result === false){
		return false;
	}
	...
	$sql="insert into nectarcrm_senotesrel(crmid,notesid) values (?,?)";
	for($i=0; $i<$rowCount;++$i ) {
	...
	$sql = "select * from nectarcrm_seattachmentsrel where crmid=?";
	$result = $adb->pquery($sql, array($id));
	if($result === false){
		return false;
	}
	...
	$sql = "insert into nectarcrm_seattachmentsrel(crmid,attachmentsid) values (?,?)";
	for($i=0;$i<$rowCount;++$i) {
...
function vtws_saveLeadRelatedProducts($leadId, $relatedId, $setype) {
	...
	$result = $adb->pquery("select * from nectarcrm_seproductsrel where crmid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
	for($i = 0; $i < $rowCount; ++$i) {
		...
		$resultNew = $adb->pquery('INSERT INTO nectarcrm_seproductsrel VALUES(?,?,?,?)', array($relatedId, $productId, $setype, 1));
		if($resultNew === false){
			return false;
		}
	}
...
function vtws_saveLeadRelations($leadId, $relatedId, $setype) {
	...
	$result = $adb->pquery("select * from nectarcrm_crmentityrel where crmid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
	for($i = 0; $i < $rowCount; ++$i) {
		...
		$adb->pquery("insert into nectarcrm_crmentityrel values(?,?,?,?)",
		array($relatedId, $setype, $recordId, $recordModule));
		if($resultNew === false){
			return false;
		}
	}
	$result = $adb->pquery("select * from nectarcrm_crmentityrel where relcrmid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
...
function vtws_getRelatedActivities($leadId,$accountId,$contactId,$relatedId) {
	...
	global $adb;
	$sql = "select * from nectarcrm_seactivityrel where crmid=?";
	...
	for($i=0;$i<$rowCount;++$i) {
		...
		$sql ="select setype from nectarcrm_crmentity where crmid=?";
		...
		if($type != "Emails") {
				if(!empty($accountId)){
					$sql = "insert into nectarcrm_seactivityrel(crmid,activityid) values (?,?)";
				...
				if(!empty($contactId)){
					$sql="insert into nectarcrm_cntactivityrel(contactid,activityid) values (?,?)";
...
function vtws_saveLeadRelatedCampaigns($leadId, $relatedId, $seType) {
	...
	$result = $adb->pquery("select * from nectarcrm_campaignleadrel where leadid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
	for($i = 0; $i < $rowCount; ++$i) {
		...
		if($seType == 'Accounts') {
			$resultNew = $adb->pquery("insert into nectarcrm_campaignaccountrel (campaignid, accountid) values(?,?)",
				array($campaignId, $relatedId));
		} elseif ($seType == 'Contacts') {
			$resultNew = $adb->pquery("insert into nectarcrm_campaigncontrel (campaignid, contactid) values(?,?)",
				array($campaignId, $relatedId));
		}
		if($resultNew === false){
			return false;
...
function vtws_transferOwnership($ownerId, $newOwnerId, $delete=true) {
	$db = PearDatabase::getInstance();
	//Updating the smownerid, modifiedby in nectarcrm_crmentity
	$db->pquery('UPDATE nectarcrm_crmentity SET smownerid=?, modifiedtime = ? WHERE smownerid=? AND setype<>?', array($newOwnerId, date('Y-m-d H:i:s'), $ownerId, 'ModComments'));
	$db->pquery('UPDATE nectarcrm_crmentity SET modifiedby=? WHERE modifiedby=?', array($newOwnerId, $ownerId));

	//deleting from nectarcrm_tracker
	$db->pquery('DELETE FROM nectarcrm_tracker WHERE user_id=?', array($ownerId));

	//delete from nectarcrm_homestuff
	$db->pquery('DELETE FROM nectarcrm_homestuff WHERE userid=?', array($ownerId));

	//updating the nectarcrm_import_maps
	$db->pquery('UPDATE nectarcrm_import_maps SET assigned_user_id=? WHERE assigned_user_id=?', array($newOwnerId, $ownerId));

	if (nectarcrm_Utils::CheckTable('nectarcrm_customerportal_prefs')) {
		$query = 'UPDATE nectarcrm_customerportal_prefs SET prefvalue = ? WHERE prefkey IN (?, ?) AND prefvalue = ?';
		$params = array($newOwnerId, 'userid', 'defaultassignee', $ownerId);
		$db->pquery($query, $params);
	}

	$sql = "SELECT tablename,columnname FROM nectarcrm_field 
            LEFT JOIN nectarcrm_fieldmodulerel ON nectarcrm_field.fieldid=nectarcrm_fieldmodulerel.fieldid 
	...
	foreach ($it as $row) {
		...
		if (!in_array($column, $columnList)) {
			...
			} elseif ($row->tablename == 'nectarcrm_users' && $row->columnname == 'reports_to_id') {
	...
	$db->pquery("UPDATE nectarcrm_webforms SET ownerid = ? WHERE ownerid = ?", array($newOwnerId, $ownerId));
	...
	$db->pquery('UPDATE nectarcrm_customview SET userid = ? WHERE userid = ? AND status != ?', array($newOwnerId, $ownerId, 1));
	//transferring private shared filters of deleted user to selected user
	$db->pquery('UPDATE nectarcrm_customview SET userid = ? WHERE userid = ? AND status = ? AND cvid IN (SELECT cvid FROM nectarcrm_cv2users UNION SELECT cvid FROM nectarcrm_cv2group UNION SELECT cvid FROM nectarcrm_cv2role UNION SELECT cvid FROM nectarcrm_cv2rs)', array($newOwnerId, $ownerId, 1));

	if ($delete) {
		//Delete from nectarcrm_users to nectarcrm_role nectarcrm_table
		$db->pquery('DELETE FROM nectarcrm_users2group WHERE userid=?', array($ownerId));

		//Mark user as deleted =1 
		$db->pquery('UPDATE nectarcrm_users SET deleted=? WHERE id=?', array(1, $ownerId));

		//Change the owner for report
		$db->pquery('UPDATE nectarcrm_report SET owner=? WHERE owner=?', array($newOwnerId, $ownerId));
...
function vtws_updateWebformsRoundrobinUsersLists($ownerId, $newOwnerId) {
	$db = PearDatabase::getInstance();
	$sql = 'SELECT id,roundrobin_userid FROM nectarcrm_webforms;';
		...
		if (is_array($usersList)) {
			if (($key = array_search($ownerId, $usersList)) !== false) {
				...
				if (count($usersList) == 0) {
					$db->pquery('UPDATE nectarcrm_webforms SET roundrobin_userid = ?,roundrobin = ? where id =?', array("--None--", 0, $webformId));
				} else {
					$usersList = json_encode($usersList);
					$db->pquery('UPDATE nectarcrm_webforms SET roundrobin_userid = ? where id =?', array($usersList, $webformId));
				}
			}
		}
	}
...
function vtws_transferOwnershipForWorkflowTasks($ownerModel, $newOwnerModel) {
	...
	$query = "SELECT task,task_id,workflow_id FROM com_nectarcrm_workflowtasks where task LIKE '%".$nameSearchValue."%' OR task LIKE '%".$idSearchValue.
	...
	for ($i = 0; $i < $num_rows; $i++) {
		...
		require_once 'modules/com_nectarcrm_workflow/tasks/'.$className.'.inc';
		...
		if(array_key_exists("field_value_mapping",$unserializeTask)) {
			...
			if (!empty($fieldMapping)) {
				...
				$query = 'UPDATE com_nectarcrm_workflowtasks SET task=? where workflow_id=? AND task_id=?';
...
function vtws_transferRelatedPotentialActivities($entityId, $relatedId) {
	...
	$relatedRecordInstance = nectarcrm_Record_Model::getInstanceById($relatedId);
	...
	$sql = "SELECT * FROM nectarcrm_crmentityrel 
			INNER JOIN nectarcrm_crmentity on nectarcrm_crmentity.crmid = nectarcrm_crmentityrel.relcrmid 
			WHERE nectarcrm_crmentityrel.crmid=? AND nectarcrm_crmentityrel.relmodule=? AND nectarcrm_crmentity.setype <> ?";
	...
	for ($i = 0; $i < $rowCount; $i++) {
		...
		if (!empty($relatedId)) {
			$sql = "INSERT INTO nectarcrm_crmentityrel (crmid,module,relcrmid,relmodule) values (?,?,?,?)";
	...
	$sql = 'SELECT activityid,setype FROM nectarcrm_seactivityrel
			INNER JOIN nectarcrm_crmentity on nectarcrm_crmentity.crmid = nectarcrm_seactivityrel.activityid
			WHERE nectarcrm_seactivityrel.crmid=? AND nectarcrm_crmentity.setype <> ?';
...
function vtws_transferRelatedPotentialQuotes($entityId, $relatedId) {
	$db = PearDatabase::getInstance();
	$entityRecordModel = nectarcrm_Record_Model::getInstanceById($entityId);
	$relatedRecordModel = nectarcrm_Record_Model::getInstanceById($relatedId);

	$relatedModel = $relatedRecordModel->getModule();
	$relatedModuleName = $relatedModel->getName();
	$quotesModel = nectarcrm_Module_Model::getInstance('Quotes');

	$query = 'SELECT nectarcrm_quotes.quoteid FROM nectarcrm_quotes
				INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid=nectarcrm_quotes.quoteid
				INNER JOIN nectarcrm_potential on nectarcrm_potential.potentialid=nectarcrm_quotes.potentialid
				WHERE nectarcrm_crmentity.deleted=0 and nectarcrm_potential.potentialid=?';
	$result = $db->pquery($query, array($entityId));
	if ($result == false) {
		return false;
	}
	...
	for ($i = 0; $i < $rowCount; $i++) {
		...
		$relationModel = nectarcrm_Relation_Model::getInstance($relatedModel, $quotesModel);
		$relationModel->addRelation($relatedId, $quoteId);
	}

	$sql = "SELECT * FROM nectarcrm_crmentityrel 
			INNER JOIN nectarcrm_crmentity on nectarcrm_crmentity.crmid = nectarcrm_crmentityrel.relcrmid 
			WHERE nectarcrm_crmentityrel.crmid=? AND nectarcrm_crmentityrel.relmodule=?";
...
function vtws_transferRelatedPotentialDocuments($entityId, $relatedId) {
	...
	$sql = "SELECT nectarcrm_senotesrel.notesid FROM nectarcrm_senotesrel 
			INNER JOIN nectarcrm_crmentity on nectarcrm_crmentity.crmid = nectarcrm_senotesrel.crmid 
			WHERE nectarcrm_senotesrel.crmid=?";
...
function vtws_getCompanyId() {
	$db = PearDatabase::getInstance();
	$result = $db->pquery('SELECT organization_id FROM nectarcrm_organizationdetails', array());
...
function vtws_recordExists($recordId) {
	...
	return !nectarcrm_Util_Helper::CheckRecordExistance($ids[1]);
}
...
function vtws_isDuplicatesAllowed($webserviceObject){
	...
	if ($moduleName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT allowduplicates FROM nectarcrm_tab WHERE name=?', array($moduleName));
