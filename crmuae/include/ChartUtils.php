...
Class ChartUtils {
	...
	public static function generateChartDataFromReports($queryResult, $groupbyField, $fieldDetails='', $reportid='') {
		...
		if ($fieldDetails != '') {
			...
			if ($single == 'D') {
				$dateField = true;
				$query = "SELECT * FROM nectarcrm_reportgroupbycolumn WHERE reportid=? ORDER BY sortid";
			...
			if (!empty($groupFieldValue)) {
				...
				if ($dateField) {
					...
					if ($fieldname == 'currency_id' && in_array($module, $inventorymodules)) {
						$tablename = 'nectarcrm_currency_info';
					} elseif ($refenceModule == 'DocumentFolders' && $fieldname == 'folderid') {
						$tablename = 'nectarcrm_attachmentsfolder';
						$colname = 'foldername';
					} else {
