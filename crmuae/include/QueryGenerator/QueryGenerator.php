...
class QueryGenerator {
	...
	public function initForCustomViewById($viewId) {
		...
		if ($this->customViewColumnList && is_array($this->customViewColumnList)) {
			...
			if(empty($details[2]) && $details[1] == 'crmid' && $details[0] == 'nectarcrm_crmentity') {
					$name = 'id';
	...
	public function getFromClause() {
				...
				foreach($moduleList as $module) {
					if($module == 'Users' && $baseModule != 'Users') {
						if($fieldName == 'created_user_id' || $fieldName == 'modifiedby') {
							$tableJoinCondition[$fieldName]['nectarcrm_users'.$fieldName] = $field->getTableName().
								".".$field->getColumnName()." = nectarcrm_users".$fieldName.".id";
							$tableJoinMapping['nectarcrm_users'.$fieldName] = 'LEFT JOIN nectarcrm_users AS';
							$i++;
						} else {
							$tableJoinCondition[$fieldName]['nectarcrm_users'.$fieldName] = $field->getTableName().
									".".$field->getColumnName()." = nectarcrm_users".$fieldName.".id";
							$tableJoinCondition[$fieldName]['nectarcrm_groups'.$fieldName] = $field->getTableName().
									".".$field->getColumnName()." = nectarcrm_groups".$fieldName.".groupid";
							$tableJoinMapping['nectarcrm_users'.$fieldName] = 'LEFT JOIN nectarcrm_users AS';
							$tableJoinMapping['nectarcrm_groups'.$fieldName] = 'LEFT JOIN nectarcrm_groups AS';
							$i++;
						}
					}
				}
		...
		foreach ($this->whereFields as $fieldName) {
			...
			if($field->getFieldDataType() == 'reference') {
				...
				if($fieldName == 'parent_id' && $field->getTableName() == 'nectarcrm_seactivityrel') {
					$tableJoinMapping[$field->getTableName()] = 'LEFT JOIN';
				} else if($fieldName == 'contact_id' && $field->getTableName() == 'nectarcrm_cntactivityrel') {
					$tableJoinMapping[$field->getTableName()] = "LEFT JOIN";
				} else {
			...
			foreach ($this->referenceModuleField as $index=>$conditionInfo) {
				...
				if(!in_array($tableName, $referenceFieldTableList)) {
					if($referenceFieldObject->getFieldName() == 'parent_id' && ($this->getModule() == 'Calendar' || $this->getModule() == 'Events')) {
						$sql .= ' LEFT JOIN nectarcrm_seactivityrel ON nectarcrm_seactivityrel.activityid = nectarcrm_activity.activityid ';
					}
					...
					if($referenceFieldObject->getFieldName() == 'contact_id' && ($this->getModule() == 'Calendar' || $this->getModule() == 'Events')) {
						$sql .= ' LEFT JOIN nectarcrm_cntactivityrel ON nectarcrm_cntactivityrel.activityid = nectarcrm_activity.activityid ';
					}
		...
		foreach ($this->conditionals as $index=>$conditionInfo) {
			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($fieldName, $this->referenceFieldList)) {
					...
					if($fieldName == 'created_user_id'){
						$concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"nectarcrm_users$fieldName.first_name",'last_name'=>"nectarcrm_users$fieldName.last_name"), 'Users');
						$fieldSql .= "$fieldGlue (trim($concatSql) $valueSql)";
					}else{
						$concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"nectarcrm_users.first_name",'last_name'=>"nectarcrm_users.last_name"), 'Users');
						$fieldSql .= "$fieldGlue (trim($concatSql) $valueSql or "."nectarcrm_groups.groupname $valueSql)";
					}
					...
					if(!empty($otherField))
						...
						$fieldSql .= ')';
				}else if (nectarcrm_Functions::isUserSpecificFieldTable($field->getTableName(), getTabModuleName($field->getTabId())) && $fieldName == "starred"
			...
			foreach ($advfilterlist as $groupindex=>$groupcolumns) {
				...
				if(count($filtercolumns) > 0) {
					...
					foreach ($filtercolumns as $index=>$filter) {
						...
						if(empty($name[2]) && $name[1] == 'crmid' && $name[0] == 'nectarcrm_crmentity') {
							$name = $this->getSQLColumn('id');
						} else {
							$name = $name[2];
						}
