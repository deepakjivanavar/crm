...
require_once 'modules/nectarcrm/helpers/ShortURL.php';
...
if (isset($_REQUEST['username']) && isset($_REQUEST['emailId'])) {
	...
	$result = $adb->pquery('select email1 from nectarcrm_users where user_name= ? ', array($username));
	...
	if (vtlib_purify($_REQUEST['emailId']) == $email) {
		...
		$content = 'Dear Customer,<br><br> 
						You recently requested a password reset for your Nectar CRM Account.<br> 
		...
		$subject = 'Nectar CRM: Password Reset';
