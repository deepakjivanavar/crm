...
function LogintonectarcrmCRM($user_name,$password,$version)
{
	global $log,$adb;
	require_once('modules/Users/Users.php');
	include('nectarcrmversion.php');
	if($version != $nectarcrm_current_version)
	{
...
function create_account($username,$sessionid,$accountname,$email,$phone,$primary_address_street,$primary_address_city,$primary_address_state,$primary_address_postalcode,$primary_address_country)
{
	...
	if(isPermitted("Accounts",'CreateView') == "yes")
	{
		$query = "SELECT accountname FROM nectarcrm_account,nectarcrm_crmentity WHERE accountname =? and nectarcrm_account.accountid = nectarcrm_crmentity.crmid and nectarcrm_crmentity.deleted != 1";
...
function GetPicklistValues($username,$sessionid,$tablename)
{
	...
	{
		$query = "select " . $adb->sql_escape_string($tablename) . " from nectarcrm_". $adb->sql_escape_string($tablename);		
	...
	{
		$query = "select " .$adb->sql_escape_string($tablename) . " from nectarcrm_". $adb->sql_escape_string($tablename) ." inner join nectarcrm_role2picklist on nectarcrm_role2picklist.picklistvalueid = nectarcrm_". $adb->sql_escape_string($tablename) .".picklist_valueid where roleid=? and picklistid in (select picklistid from nectarcrm_". $adb->sql_escape_string($tablename)." ) order by sortid";	
...
function unsetServerSessionId($id)
{
	...
	$adb->query("delete from nectarcrm_soapservice where type='FireFox' and id=$id");
...
function getServerSessionId($id)
{
	...
	$query = "select * from nectarcrm_soapservice where type='FireFox' and id={$id}";
