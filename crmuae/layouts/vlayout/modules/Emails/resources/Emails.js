...
//		return aDeferred.promise();
	...
	//configured ckeditor toolbar for nectarcrm
	var nectarcrm_ckeditor_toolbar = 
	[
    ...
    CKEDITOR.replace( 'description',
	{
		...
		toolbar : nectarcrm_ckeditor_toolbar
	});
	...
	getComposeEmailForm : function(form){
		...
		AppConnector.request(massSendEmailUrl).then(
			function(data) {
				...
				nectarcrm_Emails_Js.loadCkEditor();
				nectarcrm_Emails_Js.registerEvents();
			},
			function(error,err){
	...
	registerComposeEmailStep1Events : function(){
		jQuery('.SendEmailFormStep1').on('submit',"#SendEmailFormStep1",function(e){
			...
			var result = nectarcrm_MultiSelect_Validator_Js.invokeValidation(chosenElement);
			if(result == true){
				nectarcrm_Emails_Js.getComposeEmailForm(form);
			} else {
	...
	registerEvents : function(){
		...
		composeEmailContainer.on('submit','#SendEmailFormStep2',function(e){
			var form = jQuery(e.currentTarget);
			nectarcrm_Emails_Js.SendEmailStep2(form);
			e.preventDefault();
		});
