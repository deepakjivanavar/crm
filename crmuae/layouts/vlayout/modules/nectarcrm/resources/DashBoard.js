...
jQuery.Class("nectarcrm_DashBoard_Js", {
	gridster : false,
	...
	savePositions: function(widgets) {
		...
		AppConnector.request({module: 'nectarcrm', action: 'SaveWidgetPositions', 'positionsmap': widgetRowColPositions}).then(function(data){
		});
	},
	...
	gridsterStop : function() {
		...
		var gridster = nectarcrm_DashBoard_Js.gridster;
							...
							function(response) {
								if (response.success) {
									...
									if (jQuery.inArray(widgetName, nonReversableWidgets) == -1) {
										var data = '<li><a onclick="nectarcrm_DashBoard_Js.addWidget(this, \''+response.result.url+'\')" href="javascript:void(0);"';
	...
	registerFilterInitiater : function() {
		...
		container.on('click', 'a[name="dfilter"]', function(e) {
			...
			nectarcrm_Helper_Js.addClickOutSideEvent(dashboardWidgetHeader, callbackFunction);
