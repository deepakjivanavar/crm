...
jQuery.Class("nectarcrm_List_Js",{

	listInstance : false,

	getRelatedModulesContainer : false,

	massEditPreSave : 'nectarcrm.MassEdit.PreSave',

   getInstance: function(){
		if(nectarcrm_List_Js.listInstance == false){
			...
			if(parentModule == 'Settings'){
				...
				var fallbackClassName = parentModule+"_nectarcrm_List_Js";
				if(typeof window[fallbackClassName] == 'undefined') {
					fallbackClassName = "nectarcrm_List_Js";
				}
			} else {
				moduleClassName = module+"_List_Js";
				fallbackClassName = "nectarcrm_List_Js";
			...
			nectarcrm_List_Js.listInstance = instance;
			return instance;
		}
		return nectarcrm_List_Js.listInstance;
	},
		/*
	...
	triggerSendEmail : function(massActionUrl, module, params){
		...
		if(validationResult != true){
			...
			nectarcrm_Index_Js.showComposeEmailPopup(actionParams);
		} else {
	...
	triggerSendSms : function(massActionUrl, module){
		...
		if(validationResult != true){
			...
			nectarcrm_Helper_Js.checkServerConfig(module).then(function(data){
				...
				if(data == true){
					nectarcrm_List_Js.triggerMassAction(massActionUrl);
				} else {
		...
		if(validationResult != true){
			...
			nectarcrm_Helper_Js.showConfirmationBox({'message' : message}).then(
				function(e) {
                    ...
                    var listViewInstance = nectarcrm_List_Js.getInstance();
	...
	triggerMassEdit : function(massEditUrl) {
		...
		nectarcrm_List_Js.triggerMassAction(massEditUrl, function(container){
			...
			var editInstance = nectarcrm_Edit_Js.getInstance();
	...
	showDuplicateSearchForm : function(url) {
		...
		app.showModalWindow("", url, function() {
			progressIndicatorElement.progressIndicator({'mode' : 'hide'});
			nectarcrm_List_Js.registerDuplicateSearchButtonEvent();
		});
	},
	...
	massActionSave : function(form, isMassEdit){
		...
		if(isMassEdit) {
			...
			if(fieldsChanged == false){
				nectarcrm_Helper_Js.showPnotify(app.vtranslate('NONE_OF_THE_FIELD_VALUES_ARE_CHANGED_IN_MASS_EDIT'));
				form.find('[name="saveButton"]').removeAttr('disabled');
				aDeferred.reject();
				return aDeferred.promise();
			}
			//on submit form trigger the massEditPreSave event
			var massEditPreSaveEvent = jQuery.Event(nectarcrm_List_Js.massEditPreSave);
	...
	registerRecordAccessCheckEvent : function(form) {
		...
		form.on(nectarcrm_List_Js.massEditPreSave, function(e) {
				...
				if(assignedToSelectElement.data('recordaccessconfirmation') == true) {
					...
					if(assignedToSelectElement.data('recordaccessconfirmationprogress') != true) {
						...
						if(recordAccess == false) {
							...
							nectarcrm_Helper_Js.showConfirmationBox({
								'message' : message
							}).then(
								function(e) {
	...
	registerPageNavigationEvents : function(){
		...
		jQuery('#listViewPageJump').on('click',function(e){
            if(typeof nectarcrm_WholeNumberGreaterThanZero_Validator_Js.invokeValidation(jQuery('#pageToJump'))!= 'undefined') {
		...
		jQuery('#listViewPageJumpDropDown').on('click','li',function(e){
			...
			if(e.which == 13){
				...
				var response = nectarcrm_WholeNumberGreaterThanZero_Validator_Js.invokeValidation(element);
				if(typeof response != "undefined"){
					...
					if(newPageNumber == currentPageNumber){
						...
						nectarcrm_Helper_Js.showMessage(params);
						return;
					}
	...
	*/
	...
	registerCreateFilterClickEvent : function(){
		...
		jQuery('#createFilter').on('click',function(event){
			...
			nectarcrm_CustomView_Js.loadFilterView(createUrl);
		});
	},
	...
	registerEditFilterClickEvent : function(){
		...
		if(listViewFilterBlock != false){
			listViewFilterBlock.on('mouseup','li i.editFilter',function(event){
				...
				nectarcrm_CustomView_Js.loadFilterView(editUrl);
	...
	registerDeleteRecordClickEvent: function(){
		...
		listViewContentDiv.on('click','.deleteRecordButton',function(e){
			...
			nectarcrm_List_Js.deleteRecord(recordId);
			e.stopPropagation();
		});
	},
	/*
	...
	registerReferenceFieldsForValidation : function(form){
		...
		form.find('.sourceField').on(nectarcrm_Edit_Js.referenceSelectionEvent,function(e,params){
			var element = jQuery(e.currentTarget);
		...
		form.find('.clearReferenceSelection').on(nectarcrm_Edit_Js.referenceDeSelectionEvent,function(e){
