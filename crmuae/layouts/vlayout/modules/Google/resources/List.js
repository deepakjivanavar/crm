...
jQuery.Class("Contact",{
                        ...
                        AppConnector.request(url).then(
                           function(data) {
                               ...
                               if(jQuery('#norefresh').length == 0){
                                listInstance  =  nectarcrm_List_Js.getInstance();
    ...
    packFieldmappingsForSubmit : function(container) {
        ...
        jQuery.each(rows,function(index,row) {
            var tr = jQuery(row);
            var nectarcrm_field_name = tr.find('.nectarcrm_field_name').not('.select2-container').val();
            ...
            var map = {};
            map['nectarcrm_field_name'] = nectarcrm_field_name;
    ...
    validateFieldMappings : function(container) {
        ...
        var customMapElements = jQuery('select.nectarcrm_field_name');
        ...
        container.find('button#save_syncsetting').on('click',function() {
            thisInstance.validateFieldMappings(container).then(function() {
                ...
                AppConnector.request(serializedFormData).then(function(data) {
                    ...
                    app.hideModalWindow();
                    nectarcrm_Helper_Js.showMessage({text: app.vtranslate('JS_SAVED_SUCCESSFULLY')});
                });
            }, function(inputs) {
    ...
    registerAddCustomFieldMappingEvent : function(container) {
        ...
        jQuery('.addCustomFieldMapping',container).on('click',function(e) {
            ...
            var nectarcrmFields = currentSelectionElement.data('nectarcrmfields');
            
            var nectarcrmFieldSelectElement = '<select class="nectarcrm_field_name" style="width:200px" data-category="'+selectionType+'">';
            if(!Object.keys(nectarcrmFields).length) {
                alert(app.vtranslate('JS_SUITABLE_NECTARCRM_FIELD_NOT_AVAILABLE_FOR_MAPPING'));
                return;
            }
            
            var customMapElements = jQuery('select.nectarcrm_field_name[data-category="'+selectionType+'"]');
            var mappedCustomFields = [];
            jQuery.each(customMapElements,function(i,elem) {
                    mappedCustomFields.push(jQuery(elem).val());
            });
            var numberOfOptions = 0;
            jQuery.each(nectarcrmFields,function(fieldname,fieldLabel) {
                if(jQuery.inArray(fieldname,mappedCustomFields) === -1) {
                    numberOfOptions++;
                    var option = '<option value="'+fieldname+'">'+fieldLabel+'</option>';
                    nectarcrmFieldSelectElement += option;
                }
            });
            ...
            nectarcrmFieldSelectElement += '</select>';
            ...
            if(selectionType != 'custom') {
                ...
                googleTypeSelectElement += '</select>\n\
                                 &nbsp;&nbsp;<input type="text" class="google-custom-label" style="visibility:hidden;width:190px;" data-validation-engine="validate[required,funcCall[nectarcrm_Base_Validator_Js.invokeValidation]]" />';
            } else {
                googleTypeSelectElement = '<input type="hidden" class="google_field_name" value="'+ googleFields[selectionType]['name'] +'" />';
                googleTypeSelectElement += '<input type="hidden" class="google-type" value="'+selectionType+'" />';
                googleTypeSelectElement += '<input type="text" class="google-custom-label" style="width:190px;" data-validation-engine="validate[required,funcCall[nectarcrm_Base_Validator_Js.invokeValidation]]" />';
            }
            var tabRow = '<tr>\n\
                            <td>' + nectarcrmFieldSelectElement + '</td>\n\
            ...
            thisInstance.registernectarcrmFieldSelectOnChangeEvent(container,lastRow.find('select.nectarcrm_field_name'));
            thisInstance.registerGoogleTypeChangeEvent(container,lastRow.find('select.google-type'));
            lastRow.find('select.nectarcrm_field_name').trigger('change');
    ...
    registerDeleteCustomFieldMappingEvent : function(container) {
        jQuery('.deleteCustomMapping',container).on('click',function() {
            ...
            var currentCategory = currentRow.find('select.nectarcrm_field_name').data('category');
            currentRow.remove();
            jQuery('select.nectarcrm_field_name[data-category="'+currentCategory+'"]').trigger('change');
    ...
    removeOptionFromSelectList : function(selectElement,optionValue,category) {
        ...
        var categorisedSelectElements = jQuery('select.nectarcrm_field_name[data-category="'+category+'"]');
    ...
    registernectarcrmFieldSelectOnChangeEvent : function(container,selectElement) {
        var thisInstance = this;
        if(typeof selectElement === 'undefined') {
            selectElement = jQuery('select.nectarcrm_field_name',container);   
        }
        selectElement.on('change', function(e){
            var element = jQuery(e.currentTarget);
            var category = element.data('category');
            
            var allCategorizedSelects = jQuery('select.nectarcrm_field_name[data-category="'+category+'"]');
            var selectedValues = [];
            ...
            jQuery.each(allCategorizedSelects, function(i, selectElement){
                if(e.currentTarget !== selectElement || allCategorizedSelects.length == 1) {
                    var allCategoryFieldLabelValues = jQuery('li.addCustomFieldMapping[data-type="'+category+'"]').data('nectarcrmfields');
    ...
    registerPostSettingRenderEvents : function(container) {
        ...
        this.registernectarcrmFieldSelectOnChangeEvent(container);
        this.registerGoogleTypeChangeEvent(container);

        jQuery('select.nectarcrm_field_name',container).trigger('change');
