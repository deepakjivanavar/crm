                ...
                <div class="row">
                    {foreach item=RELATED_LINK from=$RELATED_LIST_LINKS['LISTVIEWBASIC']}
						...
						{if $RELATED_LINK->get('linkmodule') eq 'Documents'}
                            ...
                            {if $RELATED_LINK->getLabel() eq 'nectarcrm'}
										...
										<div class="dropdown">
											...
											<ul class="dropdown-menu">
												...
												<li id="nectarcrmAction">
													<a href="javascript:Documents_Index_Js.uploadTo('nectarcrm',{$PARENT_ID},'{$MODULE}')">
														<img style="  margin-top: -3px;margin-right: 4%;" title="nectarcrm" alt="nectarcrm" src="layouts/v7/skins//images/nectarcrm.png">
														{vtranslate('LBL_TO_SERVICE', $RELATED_MODULE_NAME, {vtranslate('LBL_NECTARCRM', $RELATED_MODULE_NAME)})}
													</a>
												</li>
                ...
                <tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' 
                    ...
                    <td style="width:100px">&nbsp;&nbsp;&nbsp;
                        <span class="actionImages">
                            ...
                            {assign var="DOCUMENT_RECORD_MODEL" value=nectarcrm_Record_Model::getInstanceById($RECORD_ID)}
                            {if $DOCUMENT_RECORD_MODEL->get('filename') && $DOCUMENT_RECORD_MODEL->get('filestatus')}
                                <a name="viewfile" href="javascript:void(0)" data-filelocationtype="{$DOCUMENT_RECORD_MODEL->get('filelocationtype')}" data-filename="{$DOCUMENT_RECORD_MODEL->get('filename')}" onclick="nectarcrm_Header_Js.previewFile(event)"><i title="{vtranslate('LBL_VIEW_FILE', $RELATED_MODULE_NAME)}" class="fa fa-picture-o alignMiddle"></i></a>&nbsp;&nbsp;
                            {/if}
                    ...
                    {foreach item=HEADER_FIELD from=$RELATED_HEADERS}
                        ...
                        {if $RELATED_MODULE->get('name') eq 'Documents' && $RELATED_HEADERNAME eq 'document_source'}
                            {if $RELATED_RECORD->get($RELATED_HEADERNAME) eq 'nectarcrm' || $RELATED_RECORD->get($RELATED_HEADERNAME) eq 'Google Drive' || $RELATED_RECORD->get($RELATED_HEADERNAME) eq 'Dropbox'}
                            ...
                            <span class= "value textOverflowEllipsis">
                                ...
                                {elseif $RELATED_MODULE_NAME eq 'PriceBooks' AND ($RELATED_HEADERNAME eq 'listprice' || $RELATED_HEADERNAME eq 'unit_price')}
                                    ...
                                    {elseif $HEADER_FIELD->get('uitype') eq '71' or $HEADER_FIELD->get('uitype') eq '72'}
                                        {assign var=CURRENCY_SYMBOL value=nectarcrm_RelationListView_Model::getCurrencySymbol($RELATED_RECORD->get('id'), $HEADER_FIELD)}
                                    ...
                                    {else if $HEADER_FIELD->getFieldDataType() eq 'picklist'}
                                        <span {if !empty($RELATED_LIST_VALUE)} class="picklist-color picklist-{$HEADER_FIELD->getId()}-{nectarcrm_Util_Helper::convertSpaceToHyphen($RELATED_LIST_VALUE)}" {/if}> {$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)} </span>
                                    {else}
