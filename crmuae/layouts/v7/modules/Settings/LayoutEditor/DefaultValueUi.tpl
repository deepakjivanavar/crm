{*<!--
/*+****************************************************************************
* The contents of this file are subject to the nectarcrm CRM Commercial License
* ("License"); You may not use this file except in compliance with the License
* The Initial Developer of the Code is nectarcrm.
* All Rights Reserved. Copyright (C) nectarcrm.
*******************************************************************************/
-->*}
...
{strip}
	{if $FIELD_MODEL->isDefaultValueOptionDisabled() neq "true"}
		<div class="form-group">
			<label class="control-label fieldLabel col-sm-5" style="width:32.666667%!important">
					...
					{if $FIELD_MODEL->getFieldDataType() eq "picklist"}
						...
						<select class="col-sm-9 select2" name="{$NAME_ATTR}">
							{foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
								<option value="{nectarcrm_Util_Helper::toSafeHTML($PICKLIST_NAME)}" {if $DEFAULT_VALUE eq $PICKLIST_NAME} selected {/if}>{vtranslate($PICKLIST_VALUE, $SELECTED_MODULE_NAME)}</option>
							{/foreach}
						</select>
