    ...
    <div class="modal-dialog">
        <div class='modal-content'>
            ...
            <form id="renameItemForm" class="form-horizontal" method="post" action="index.php">
                ...
                <input type="hidden" name="pickListValues" value='{nectarcrm_Util_Helper::toSafeHTML(ZEND_JSON::encode($SELECTED_PICKLISTFIELD_EDITABLE_VALUES))}' />
                <input type="hidden" name="picklistColorMap" value='{nectarcrm_Util_Helper::toSafeHTML(ZEND_JSON::encode(Settings_Picklist_Module_Model::getPicklistColorMap($FIELD_MODEL->get('name'))))}' />
                <div class="modal-body tabbable">
                        ...
                        <div class="controls col-sm-4 col-xs-4">
                            ...
                            <select class="select2 form-control" name="oldValue">
                                {foreach from=$PICKLIST_VALUES key=PICKLIST_VALUE_KEY item=PICKLIST_VALUE}
                                    <option {if $FIELD_VALUE eq $PICKLIST_VALUE} selected="" {/if}value="{nectarcrm_Util_Helper::toSafeHTML($PICKLIST_VALUE)}" data-id={$PICKLIST_VALUE_KEY}>{vtranslate($PICKLIST_VALUE,$SOURCE_MODULE)}</option>
                                {/foreach}
                                {if $SELECTED_PICKLISTFIELD_NON_EDITABLE_VALUES}
                                    {foreach from=$SELECTED_PICKLISTFIELD_NON_EDITABLE_VALUES key=NON_EDITABLE_VALUE_KEY item=NON_EDITABLE_VALUE}
                                        <option data-edit-disabled="true" {if $FIELD_VALUE eq $NON_EDITABLE_VALUE} selected="" {/if}value="{nectarcrm_Util_Helper::toSafeHTML($NON_EDITABLE_VALUE)}" data-id={$NON_EDITABLE_VALUE_KEY}>{vtranslate($NON_EDITABLE_VALUE,$SOURCE_MODULE)}</option>
                                    {/foreach}
                                {/if}
                            </select>	
                        </div><br>
                    ...
                    <div class="form-group">
                        ...
                        <div class="controls col-sm-4 col-xs-4"><input class="form-control" type="text"  name="renamedValue" {if in_array($FIELD_VALUE,$SELECTED_PICKLISTFIELD_NON_EDITABLE_VALUES)} disabled='disabled' {/if} data-rule-required="true" value="{nectarcrm_Util_Helper::toSafeHTML($FIELD_VALUE)}"></div>
                    </div>
                    <div class="form-group">
