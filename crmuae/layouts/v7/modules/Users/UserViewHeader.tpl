        ...
        <div class="detailViewTitle" id="userPageHeader">
            <div class = "row">
                <div class="col-md-5">
                    ...
                    <span class="font-x-x-large headtitle">
                        {$RECORD->getName()}
                    </span>
                </div>
                <div class="pull-right col-md-7 detailViewButtoncontainer">
                    <div class="btn-group pull-right">
                        {foreach item=DETAIL_VIEW_BASIC_LINK from=$DETAILVIEW_LINKS['DETAILVIEWBASIC']}
                            <button class="btn btn-default {if $DETAIL_VIEW_BASIC_LINK->getLabel() eq 'LBL_EDIT'}{/if}" id="{$MODULE}_detailView_basicAction_{nectarcrm_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel())}"
                                ...
                                {foreach item=DETAIL_VIEW_LINK from=$DETAILVIEW_LINKS['DETAILVIEW']}
                                    {if $DETAIL_VIEW_LINK->getLabel() eq "Delete"}
                                        {if $CURRENT_USER_MODEL->isAdminUser() && $CURRENT_USER_MODEL->getId() neq $RECORD->getId()}
                                            <li id="{$MODULE}_detailView_moreAction_{nectarcrm_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_LINK->getLabel())}">
