                    ...
                    {foreach item=RECENT_ACTIVITY from=$RECENT_ACTIVITIES}
                        ...
                        {if $PROCEED}
                            {if $RECENT_ACTIVITY->isCreate()}
                                <li>
                                    <time class="update_time cursorDefault">
                                        <small title="{nectarcrm_Util_Helper::formatDateTimeIntoDayString($RECENT_ACTIVITY->getParent()->get('createdtime'))}">
                                            {nectarcrm_Util_Helper::formatDateDiffInStrings($RECENT_ACTIVITY->getParent()->get('createdtime'))}
                                        </small>
                                    </time>
                                    ...
                                    {if $IMAGE_DETAILS neq '' && $IMAGE_DETAILS[0] neq '' && $IMAGE_DETAILS[0].path eq ''}
                                        <div class="update_icon bg-info">
                                            <i class='update_image vicon-nectarcrmuser'></i>
                                        </div>
                                    {else}
                            ...
                            {else if $RECENT_ACTIVITY->isUpdate()}
                                <li>
                                    <time class="update_time cursorDefault">
                                        <small title="{nectarcrm_Util_Helper::formatDateTimeIntoDayString($RECENT_ACTIVITY->getActivityTime())}">
                                            {nectarcrm_Util_Helper::formatDateDiffInStrings($RECENT_ACTIVITY->getActivityTime())}
                                        </small>
                                    </time>
                                    {assign var=USER_MODEL value=$RECENT_ACTIVITY->getModifiedBy()}
                                    {assign var=IMAGE_DETAILS value=$USER_MODEL->getImageDetails()}
                                    ...
                                    <div class="update_info">
                                        ...
                                        {foreach item=FIELDMODEL from=$RECENT_ACTIVITY->getFieldInstances()}
                                                ...
                                                <div class='font-x-small updateInfoContainer textOverflowEllipsis'>
                                                    <div class='update-name'><span class="field-name">{vtranslate($FIELDMODEL->getName(),$MODULE_NAME)}</span>
                                                        ...
                                                        <div class='update-from'><span class="field-name">{vtranslate('LBL_FROM')}</span>&nbsp;
                                                            <em style="white-space:pre-line;" title="{strip_tags({nectarcrm_Util_Helper::tonectarcrm6SafeHTML($FIELDMODEL->getDisplayValue(decode_html($FIELDMODEL->get('prevalue'))))})}">{nectarcrm_Util_Helper::tonectarcrm6SafeHTML($FIELDMODEL->getDisplayValue(decode_html($FIELDMODEL->get('prevalue'))))}</em>
                                                        </div>
                                                    {else if $FIELDMODEL->get('postvalue') eq '' || ($FIELDMODEL->getFieldInstance()->getFieldDataType() eq 'reference' && $FIELDMODEL->get('postvalue') eq '0')}
                                                        &nbsp;(<del>{nectarcrm_Util_Helper::tonectarcrm6SafeHTML($FIELDMODEL->getDisplayValue(decode_html($FIELDMODEL->get('prevalue'))))}</del> ) {vtranslate('LBL_IS_REMOVED')}</div>
                                            ...
                                            {if $FIELDMODEL->get('postvalue') neq '' && !($FIELDMODEL->getFieldInstance()->getFieldDataType() eq 'reference' && $FIELDMODEL->get('postvalue') eq '0')}
                                                <div class="update-to"><span class="field-name">{vtranslate('LBL_TO')}</span>&nbsp;<em style="white-space:pre-line;">{nectarcrm_Util_Helper::tonectarcrm6SafeHTML($FIELDMODEL->getDisplayValue(decode_html($FIELDMODEL->get('postvalue'))))}</em>
                                                </div>
                                            {/if}
                                            </div>
                                        {/if}
                            ...
                            {else if ($RECENT_ACTIVITY->isRelationLink() || $RECENT_ACTIVITY->isRelationUnLink())}
                                ...
                                <li>
                                    <time class="update_time cursorDefault">
                                        <small title="{nectarcrm_Util_Helper::formatDateTimeIntoDayString($RELATION->get('changedon'))}">
                                            {nectarcrm_Util_Helper::formatDateDiffInStrings($RELATION->get('changedon'))} </small>
                                    </time>
									<div class="update_icon bg-info-{$RELATED_MODULE|strtolower}">
										...
										{else}
											<span class="update_image">{nectarcrm_Module_Model::getModuleIconPath($RELATED_MODULE)}</span>
										{/if}
									</div>
                                    <div class="update_info">
                                        <h5>
