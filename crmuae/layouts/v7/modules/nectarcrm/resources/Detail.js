...
nectarcrm.Class("nectarcrm_Detail_Js",{

	detailInstance : false,
	PreAjaxSaveEvent : 'PreAjaxSaveEvent',
	PostAjaxSaveEvent : 'PostAjaxSaveEvent',
	getInstance: function(){
		if( nectarcrm_Detail_Js.detailInstance == false ){
			var module = app.getModuleName();
			var view = app.view;
			var moduleClassName = module+"_"+view+"_Js";
			var fallbackClassName = nectarcrm_Detail_Js;
			if(typeof window[moduleClassName] != 'undefined'){
				var instance = new window[moduleClassName]();
			}else{
				var instance = new fallbackClassName();
			}
			nectarcrm_Detail_Js.detailInstance = instance;
		}
		return nectarcrm_Detail_Js.detailInstance;
	},
		...
		if(parentModule == 'Settings'){
			...
			var fallbackClassName = parentModule+"_nectarcrm_Detail_Js";
			if(typeof window[fallbackClassName] == 'undefined') {
				fallbackClassName = "nectarcrm_Detail_Js";
			}
		} else {
			moduleClassName = moduleName+"_Detail_Js";
			fallbackClassName = "nectarcrm_Detail_Js";
		...
		app.request.post({data: params}).then(function(err, data) {
			if(data) {
				...
				app.request.get({data: params}).then(function(err, result) {
					...
					nectarcrm_Detail_Js.getInstance().registerEventForActivityWidget();
				});
			}
		});
	},
	...
	triggerSendEmail : function(detailActionUrl, module){
		...
		nectarcrm_Index_Js.showComposeEmailPopup(urlParams);
	},
	...
	previewFile: function (e, recordId,attachmentId) {
		nectarcrm_Index_Js.previewFile(e, recordId,attachmentId);
	}
	...
	widgetPostLoad : 'nectarcrm.Widget.PostLoad',
	_moduleName : false,
	...
	setModuleName : function(module){
		...
		registerOverlayEditEvents: function(module, container) {
				var editInstance = nectarcrm_Edit_Js.getInstanceByModuleName(module);
			...
			app.event.on('post.overLayEditView.loaded',function(e, container){
				jQuery('#EditView').vtValidate({
					submitHandler : function(form){
						...
						var e = jQuery.Event(nectarcrm_Edit_Js.recordPresaveEvent);
						app.event.trigger(e);
	...
	addComponents : function() {
		var emailPreviewClassName = 'nectarcrm_EmailPreview_Js';
	this.addIndexComponent();
		this.addComponent(emailPreviewClassName);
		this.addComponent('nectarcrm_Tag_Js');
	},
	...
	registerEventsForRelatedList : function(){
		...
		app.event.on("post.relatedListLoad.click",function(event, container){
			...
			nectarcrmInstance.registerMultiUpload();
			//For Rollup Comments
	...
	*/
	...
	getRelatedController : function(relatedModuleName) {
		...
		var relatedListClass = 'nectarcrm_RelatedList_Js';
		if(typeof window[relatedListClass] != 'undefined'){
			return nectarcrm_RelatedList_Js.getInstance(recordId, moduleName, selectedTabElement, relatedModuleName);
		}
		return null;
	},
	...
	ajaxEditHandling : function(currentTdElement){
		...
		if(jQuery('.editElement',editElement).length === 0){
			...
			var fieldObject = nectarcrm_Field_Js.getInstance(fieldInfo);
		...
		var nectarcrmInstance = nectarcrm_Index_Js.getInstance();
		nectarcrmInstance.registerAutoCompleteFields(contentHolder);
		nectarcrmInstance.referenceModulePopupRegisterEvent(contentHolder);
		...
		contentHolder.on('click','.inlineAjaxSave',function(e){
			...
			if(previousValue == ajaxEditNewValue) {
				...
				var params = {
					...
					submitHandler : function(form){
						var preAjaxSaveEvent = jQuery.Event(nectarcrm_Detail_Js.PreAjaxSaveEvent);
						...
						thisInstance.saveFieldValues(fieldNameValueMap).then(function(err, response) {
							...
							jQuery('.editAction').removeClass('hide');
							actionElement.show();
							var postAjaxSaveEvent = jQuery.Event(nectarcrm_Detail_Js.PostAjaxSaveEvent);
	...
	registerAjaxEditCancelEvent : function(contentHolder){
		...
		contentHolder.on('click','.inlineAjaxCancel',function(e){
			...
			detailViewValue.css('display', 'inline-block');
			editElement.addClass('hide');
			editElement.find('.inputElement').trigger('nectarcrm.Validation.Hide.Messsage')
			editElement.removeClass('ajaxEdited');
			jQuery('.editAction').removeClass('hide');
			actionElement.show();
		});
	...
	saveComment : function(e) {
		...
		if(commentMode == "edit"){
			...
			postData['filename'] = nectarcrm_Index_Js.files,
			incrementCount = true;
		}
		...
		app.request.post(postData).then(
			function(err,data){
				...
				if(incrementCount){
					...
					var relatedController = new nectarcrm_RelatedList_Js(self.getRecordId(), app.getModuleName(), tabElement, self.getRelatedModuleName());
	...
	toggleRollupComments : function (e) {
		...
		if(viewtype == 'relatedlist') {
			url = 'index.php?module=nectarcrm&view=ModCommentsDetailAjax&parent='+moduleName+'&parentId='+recordId+
		...
		app.request.post({'data': params}).then(
			function(error, data) {
				if(error == null) {
					...
					var tagInstance = self.getComponentInstance('nectarcrm_Tag_Js');
	...
	postTagDeleteActions : function(deletedTagClone) {
		var summaryTagContainer = jQuery('.detailTagList');
		var tagInstance = this.getComponentInstance('nectarcrm_Tag_Js');
		var tagInfo = {
				...
				if(typeof recordUrl != "undefined"){
						...
						app.request.get({data: params}).then(function(err, response) {
							...
							app.helper.loadPageContentOverlay(response, overlayParams).then(function(container) {
								var detailjs = nectarcrm_Detail_Js.getInstanceByModuleName(params.module);
			...
			if(!element.is(":disabled")) {
				...
				dataObj.then(function(){
					...
					self.loadWidget(commentsContainer).then(function() {
						...
						var indexInstance = nectarcrm_Index_Js.getInstance();
	...
	registerPostAjaxSaveEvent: function () {
		var _this = this;
		app.event.on(nectarcrm_Detail_Js.PostAjaxSaveEvent, function (e, fieldBasicData, postSaveRecordDetails,contentHolder) {
			if(typeof contentHolder == 'undefined'){
				return;
			}
