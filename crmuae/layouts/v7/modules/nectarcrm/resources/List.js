...
nectarcrm.Class("nectarcrm_List_Js", {
	listInstance: false,
	filterClick: false,
	...
	triggerSendEmail: function (massActionUrl, module, params) {
		...
		if (listSelectParams) {
			...
			nectarcrm_Index_Js.showComposeEmailPopup(postData);
		}
		else {
			listInstance.noRecordSelectedAlert();
		}
	...
	addComponents: function () {
		...
		this.addComponent('nectarcrm_MergeRecords_Js');
	...
	getRecordSelectTrackerInstance: function () {
		if (this.recordSelectTrackerInstance === false) {
			this.recordSelectTrackerInstance = nectarcrm_RecordSelectTracker_Js.getInstance();
...
//			http://stackoverflow.com/questions/26482645/number-isintegerx-which-is-created-can-not-work-in-ie
	...
	registerInlineEdit: function (currentTrElement) {
		...
		for (var i = 0; i < tdElements.length; i++) {
			...
			var fieldObject = new nectarcrm_Field_Js.getInstance(fieldData, moduleName);
	...
	registerFieldActionEvents: function (editedRow) {
		var nectarcrmInstance = nectarcrm_Index_Js.getInstance();
		nectarcrmInstance.registerAutoCompleteFields(editedRow);
		nectarcrmInstance.registerClearReferenceSelectionEvent(editedRow);
		nectarcrmInstance.referenceModulePopupRegisterEvent(editedRow);
	},
		...
		app.event.on('post.listViewMassAction.loaded', function (e, container) {
			...
			jQuery('#massSave').vtValidate({
				...
				submitHandler: function (form) {
					...
					if (nectarcrm_Index_Js.files) {
						formData.append("filename", nectarcrm_Index_Js.files);
						delete nectarcrm_Index_Js.files;
					}
					var params = {
						url: "index.php",
						type: "POST",
...
//			e.stopPropagation();
	...
	showQuickPreviewForId: function (recordId, appName) {
		var self = this;
		var nectarcrmInstance = nectarcrm_Index_Js.getInstance();
		nectarcrmInstance.showQuickPreviewForId(recordId, self.getModuleName(), appName);
	},
	...
	initializePaginationEvents: function () {
		var thisInstance = this;
		var paginationObj = this.getComponentInstance('nectarcrm_Pagination_Js');
		var listViewContainer = thisInstance.getListViewContainer();
	...
	registerEvents: function () {
		...
		//reference fields quick preview handled in nectarcrm.js
