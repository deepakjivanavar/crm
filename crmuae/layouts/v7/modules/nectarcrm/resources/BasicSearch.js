...
nectarcrm.Class('nectarcrm_BasicSearch_Js',{},{
	...
	_search : function(params) {
		...
		if(typeof params.module == 'undefined') {
			...
			//if you are in Settings then module should be nectarcrm for normal text search
			if(app.getParentModuleName().length > 0) {
				params.module = 'nectarcrm';
			}
		}
	...
	addSearchListener : function () {
		jQuery('.search-link .keyword-input').on('VT_SEARCH_INTIATED',function(e,args){
			...
			var url = '?module=nectarcrm&view=ListAjax&mode=searchAll&value='+encodeURIComponent(val);
			app.helper.showProgress();
			app.request.get({'url': url}).then(function (error, data) {
				if (error == null) {
					...
					app.helper.loadPageOverlay(data).then(function (modal) {
						...
						nectarcrm_SearchList_Js.intializeListInstances(modal);
					});
				}
			});
		});
