...
nectarcrm.Class("nectarcrm_DashBoard_Js",{
	...
	addWidget : function(element, url) {
		...
		var activeTabId = nectarcrm_DashBoard_Js.currentInstance.getActiveTabId();
		...
		nectarcrm_DashBoard_Js.gridster.add_widget(widgetContainer, width, height);
		nectarcrm_DashBoard_Js.currentInstance.loadWidget(widgetContainer);
	},
	...
	init : function() {
		nectarcrm_DashBoard_Js.currentInstance = this;
		this.addComponents();
	},
	...
	getWidgetInstance : function(widgetContainer) {
			...
			if(!(id in this.instancesCache)) {
					...
					this.instancesCache[id] = nectarcrm_Widget_Js.getInstance(widgetContainer, widgetName);
			}
	else{
	...
	registerGridster : function() {
		...
		nectarcrm_DashBoard_Js.gridster = this.getContainer().gridster({
			...
			resize : {
				...
				stop : function(e, ui, widget) {
					...
					nectarcrm_Widget_Js.getInstance(widget, widgetName);
					widget.trigger(nectarcrm_Widget_Js.widgetPostResizeEvent);
		...
		jQuery.each(items , function (i, e) {
			...
			if(item.attr("data-position")=="false"){
				nectarcrm_DashBoard_Js.gridster.add_widget(item, columns, rows);
			} else {
				nectarcrm_DashBoard_Js.gridster.add_widget(item, columns, rows);
			}
		});
	...
	loadWidget : function(widgetContainer) {
		...
		if(mode == 'open') {
			app.request.post({"url":urlParams}).then(function(err,data){
				...
				try {
					widgetContainer.trigger(nectarcrm_Widget_Js.widgetPostLoadEvent);
				} catch (error) {
					...
					function(err,response) {
						if (err == null) {
							...
							parent.fadeOut('slow', function() {
								nectarcrm_DashBoard_Js.gridster.remove_widget(parent);
								parent.remove();
							});
							if (jQuery.inArray(widgetName, nonReversableWidgets) == -1) {
								var data = '<li><a onclick="nectarcrm_DashBoard_Js.addWidget(this, \''+response.url+'\')" href="javascript:void(0);"';
			...
			var callback = function(modalData){
				...
				modalWidgetContainer.find('.widgetData').val(data);
				 if(chartType != ''){
					...
					chartClass('nectarcrm_ChartReportWidget_Widget_Js',{},{
						init : function() {
								this._super(modalWidgetContainer);
							}
					});
				}
				var widgetInstance = nectarcrm_Widget_Js.getInstance(modalWidgetContainer, widgetName);
				modalWidgetContainer.trigger(nectarcrm_Widget_Js.widgetPostLoadEvent);
			}
	...
	registerFilterInitiater : function() {
		...
		container.on('click', 'a[name="dfilter"]', function(e) {
			...
			var helper = new nectarcrm_Helper_Js();
			...
			app.helper.showConfirmationBox({'message' : message, 'htmlSupportEnable' : false}).then(function(e) {
				...
				app.request.post({"data":data}).then(function(err,data){
					...
					if(err == null){
						...
						if(jQuery('.dashboardTab').length < nectarcrm_DashBoard_Js.dashboardTabsLimit ){
	...
	registerDashBoardTabClick : function(){
		...
		dashBoardContainer.on("shown.bs.tab",".dashboardTab",function(e){
			...
			if(typeof jQuery("#tab_"+tabid).find(".dashBoardTabContainer").val() !== 'undefined'){
				...
				nectarcrm_DashBoard_Js.gridster = thisInstance.getContainer(tabid).gridster({ 
