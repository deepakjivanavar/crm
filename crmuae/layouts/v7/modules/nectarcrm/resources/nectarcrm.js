 ...
 * All Rights Reserved.
 *************************************************************************************/
nectarcrm.Class('nectarcrm_Index_Js', {
	files: [],
	hideNC: true,

	getInstance : function() {
		return new nectarcrm_Index_Js();
	},
	...
	showEmailPreview : function(recordId, parentId) {
		...
		var callback = function(data){
			emailPreviewClass = app.getModuleSpecificViewClass('EmailPreview','nectarcrm');
	...
	registerActivityReminder : function() {
		...
		if(activityReminderInterval != '') {
			...
			if(currentTime >= nextActivityReminderCheck) {
				nectarcrm_Index_Js.requestReminder();
			} else {
				var nextInterval = nextActivityReminderCheck - currentTime;
				setTimeout(function() {nectarcrm_Index_Js.requestReminder()}, nextInterval*1000);
			}
		}
	},
	...
	requestReminder : function() {
		...
		setTimeout(function() {nectarcrm_Index_Js.requestReminder()}, activityReminder*1000);
			...
			if(!res.hasOwnProperty('result')) {
				for(i=0; i< res.length; i++) {
					...
					if(typeof record == 'object') {
						nectarcrm_Index_Js.showReminderPopup(record);
					}
				}
			}
		});
	...
	addComponents : function() {
		this.addComponent('nectarcrm_BasicSearch_Js');
	},
	...
	registerListEssentialsToggleEvent : function() {
		jQuery('.main-container').on('click', '.essentials-toggle', function() {
			...
			app.event.trigger("nectarcrm.Post.MenuToggle");
		});
	},
	...
	registerEvents: function() {
		...
		nectarcrm_Index_Js.registerActivityReminder();
			...
			app.request.post({"data":params}).then(function(err,data){
				if(err === null){
					app.helper.loadPageOverlay(data,{'ignoreScroll' : true,'backdrop': 'static'}).then(function(){
						...
						var js = new nectarcrm_TaskManagement_Js();
						js.registerEvents();
					});
				}else{
	...
	registerQuickCreateEvent : function (){
		...
		jQuery("#quickCreateModules").on("click",".quickCreateModule",function(e,params){
			...
			thisInstance.getQuickCreateForm(quickCreateUrl,quickCreateModuleName,params).then(function(data){
				...
				var moduleInstance = nectarcrm_Edit_Js.getInstanceByModuleName(moduleName);
	...
	registerQuickcreateTabEvents : function(form) {
		...
		tabElements.on('shown.bs.tab', function(e) {
			...
			if(form.find('[name="module"]').val()=== 'Calendar') {
				...
				var moduleInstance = nectarcrm_Edit_Js.getInstanceByModuleName('Calendar');
	...
	registerAdvanceSeachIntiator : function () {
		jQuery('#adv-search').on('click',function(e){
			var advanceSearchInstance = new nectarcrm_AdvanceSearch_Js();
...
//			});
	...
	registerAutoCompleteFields : function(container) {
		...
		container.find('input.autoComplete').autocomplete({
			...
			'select' : function(event, ui ){
				...
				sourceField.trigger(nectarcrm_Edit_Js.referenceSelectionEvent,selectedItemData);
				//trigger post reference selection
				sourceField.trigger(nectarcrm_Edit_Js.postReferenceSelectionEvent,{'data':selectedItemData});
			}
		});
	},
	...
	registerClearReferenceSelectionEvent : function(container) {
		...
		container.on('click', '.clearReferenceSelection',function(e){
			...
			element.trigger(nectarcrm_Edit_Js.referenceDeSelectionEvent);
		});
	},
	...
	openPopUp : function(e) {
		...
		var prePopupOpenEvent = jQuery.Event(nectarcrm_Edit_Js.preReferencePopUpOpenEvent);
		...
		app.event.off(nectarcrm_Edit_Js.popupSelectionEvent);
		app.event.one(nectarcrm_Edit_Js.popupSelectionEvent,function(e,data) {
...
//					
			...
			if(isMultiple) {
				sourceFieldElement.trigger(nectarcrm_Edit_Js.refrenceMultiSelectionEvent,{'data':dataList});
			}
			sourceFieldElement.trigger(nectarcrm_Edit_Js.postReferenceSelectionEvent,{'data':responseData});
		});
		popupInstance.showPopup(params,nectarcrm_Edit_Js.popupSelectionEvent,function() {});
	},
	...
	registerFileChangeEvent: function () {
		 var thisInstance = this;
			...
			container.on('change', 'input[name="filename[]"],input[name="imagename[]"]', function(e){
				...
				nectarcrm_Index_Js.files = files_uploaded;
	...
	setReferenceFieldValue : function(container, params) {
		...
		if (id && selectedName) {
			...
			fieldElement.trigger(nectarcrm_Edit_Js.referenceSelectionEvent, {'source_module' : popupReferenceModule, 'record' : id, 'selectedName' : selectedName});
		}
	},
	...
	referenceCreateHandler : function(container) {
		...
		var postQuickCreateSave = function(data) {
			...
			tdElement.find('input[class="sourceField"]').trigger(nectarcrm_Edit_Js.postReferenceQuickCreateSave, {'data' : data});
		}
	...
	*/
	...
	registerPostReferenceEvent : function(container) {
		...
		container.find('.sourceField').on(nectarcrm_Edit_Js.postReferenceSelectionEvent,function(e,result){
