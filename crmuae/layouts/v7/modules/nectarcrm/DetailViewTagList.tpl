...
<div class="tagContainer">
    <div class="tag-contents {if empty($TAGS_LIST)} hide{/if}">
        <div class="detailTagList" data-num-of-tags-to-show="{nectarcrm_Tag_Model::NUM_OF_TAGS_DETAIL}">
            {foreach from=$TAGS_LIST item=TAG_MODEL name=tagCounter}
                {if $smarty.foreach.tagCounter.iteration gt nectarcrm_Tag_Model::NUM_OF_TAGS_DETAIL}
                     {break}
                {/if}
            ...
            <a href="javascript:void(0);" class="moreTags {if count($TAGS_LIST) <= nectarcrm_Tag_Model::NUM_OF_TAGS_DETAIL } hide {/if}">
                <span class="tagMoreCount">{count($TAGS_LIST)-nectarcrm_Tag_Model::NUM_OF_TAGS_DETAIL}</span>
