...
{strip}
	...
	<div class="summaryWidgetContainer">
		<div class="widget_header clearfix">
			...
			{assign var=CALENDAR_MODEL value = nectarcrm_Module_Model::getInstance('Calendar')}
							...
							<div class='row'>
								...
								<div class='media-body col-lg-7 col-md-7 col-sm-7'>
									<div class="summaryViewEntries">
										...
										{if $DELETE_PERMITTED == 'yes'}<a onclick="nectarcrm_Detail_Js.deleteRelatedActivity(event);" data-id="{$RECORD->getId()}" data-recurring-enabled="{$RECORD->isRecurringEnabled()}" class="fieldValue"><i class="summaryViewEdit fa fa-trash " title="{vtranslate('LBL_DELETE',$MODULE_NAME)}"></i></a>{/if}
									</div>
								<span><strong title="{nectarcrm_Util_Helper::formatDateTimeIntoDayString("$START_DATE $START_TIME")}">{nectarcrm_Util_Helper::formatDateIntoStrings($START_DATE, $START_TIME)}</strong></span>
							</div>
								...
								<div class="row">
										...
										<div class="pull-right">
											...
											<style>
												...
												{foreach item=PICKLIST_COLOR key=PICKLIST_VALUE from=$PICKLIST_COLOR_MAP}
													...
													{assign var=CONVERTED_PICKLIST_VALUE value=nectarcrm_Util_Helper::convertSpaceToHyphen($PICKLIST_VALUE)}
														.picklist-{$FIELD_MODEL->getId()}-{nectarcrm_Util_Helper::escapeCssSpecialCharacters($CONVERTED_PICKLIST_VALUE)} {
															background-color: {$PICKLIST_COLOR};color: {$PICKLIST_TEXT_COLOR};
														}
												{/foreach}
											</style>
											<strong><span class="value picklist-color picklist-{$FIELD_MODEL->getId()}-{nectarcrm_Util_Helper::convertSpaceToHyphen($RECORD->get('status'))}">{vtranslate($RECORD->get('status'),$MODULE_NAME)}</span></strong>&nbsp&nbsp;
									...
									{else}
										...
										<div class="pull-right">
											...
											<strong><span class="value picklist-color picklist-{$FIELD_MODEL->getId()}-{nectarcrm_Util_Helper::convertSpaceToHyphen($RECORD->get('eventstatus'))}">{vtranslate($RECORD->get('eventstatus'),$MODULE_NAME)}</span></strong>&nbsp&nbsp;
											{if $EDITVIEW_PERMITTED == 'yes'}
												<span class="editStatus cursorPointer"><i class="fa fa-pencil" title="{vtranslate('LBL_EDIT',$MODULE_NAME)}"></i></span>
												<span class="edit hide">
