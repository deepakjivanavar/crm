 ...
 * All Rights Reserved.
 ************************************************************************************/
if (typeof (nectarcrm_Import_Js) == 'undefined') {

    nectarcrm_Import_Js = {
        triggerImportAction: function(url) {
            var params = nectarcrm_Import_Js.getDefaultParams();
            //Only for contacts and Calendar show landing page.
            if(params.module != 'Contacts' && params.module != 'Calendar') {
                nectarcrm_Import_Js.showImportActionStepOne();
                return false;
            }
            params['mode'] = 'landing';
            ...
            app.request.get({data: params}).then(function(err, data) {
                app.helper.loadPageContentOverlay(data).then(function() {
                    ...
                    nectarcrm_Import_Js.registerEvents();
                });
            });
            return false;
        },
        ...
        importActionStep2: function() {
			if(nectarcrm_Import_Js.validateFilePath()){
        ...
        uploadAndParse: function(auto_merge) {
            if (nectarcrm_Import_Js.validateFilePath() && nectarcrm_Import_Js.validateMergeCriteria(auto_merge)) {
                ...
                app.request.post(postParams).then(function(err, response) {
					...
					nectarcrm_Import_Js.loadDefaultValueWidgetForMappedFields();
                    app.helper.hideProgress();
                });
            }
            return false;
        ...
        backToLandingPage: function() {
            nectarcrm_Import_Js.triggerImportAction();
            return false;
        },
        sanitizeAndSubmit: function() {
            if (nectarcrm_Import_Js.sanitizeFieldMapping() && nectarcrm_Import_Js.validateCustomMap()) {
                    ...
                    if(!err){
                        if (jQuery('#scheduleImportStatus').length > 0) {
                            app.event.one('post.overlayPageContent.hide', function(container) {
                                clearTimeout(nectarcrm_Import_Js.timer);
                                nectarcrm_Import_Js.isReloadStatusPageStopped = true;
                            });
                            nectarcrm_Import_Js.isReloadStatusPageStopped = false;
                            nectarcrm_Import_Js.timer = setTimeout(nectarcrm_Import_Js.scheduledImportRunning, 5000);
                        } else {
        ...
        undoImport: function(url) {
            var params = nectarcrm_Import_Js.getParamsFromURL(url);
            nectarcrm_Import_Js.showOverLayModal(params);
        },
        loadSavedMap: function() {
            ...
            fieldsList.each(function(i, element) {
                ...
                nectarcrm_Import_Js.loadDefaultValueWidget(fieldElement.attr('id'));
            });
        },
        ...
        validateMergeCriteria: function(auto_merge) {
			if (auto_merge == 1) {
				...
				nectarcrm_Import_Js.convertOptionsToJSONArray('#selected_merge_fields', '#merge_fields');
			}
            return true;
        },
        ...
        validateFilePath: function() {
            ...
            if (!nectarcrm_Import_Js.uploadFilter("import_file", fileFormats)) {
                return false;
            }
            if (!nectarcrm_Import_Js.uploadFileSize("import_file")) {
                return false;
            }
            return true;
        },
        ...
        checkFileType: function(e) {
            ...
            if (filePath != '') {
                ...
                nectarcrm_Import_Js.handleFileTypeChange();
            } else {
        ...
        scheduledImportRunning: function() {
			...
			app.request.post(postParams).then(function(err, response) {
				if(!nectarcrm_Import_Js.isReloadStatusPageStopped) {
					app.helper.loadPageContentOverlay(response);
					if (jQuery('#scheduleImportStatus').length > 0) {
						if (!nectarcrm_Import_Js.isReloadStatusPageStopped) {
							nectarcrm_Import_Js.timer = setTimeout(nectarcrm_Import_Js.scheduledImportRunning, 50000);
						}
					}
				}
			});
            ...
            app.request.get({data: params}).then(function(err, data) {
                ...
                app.helper.hidePageContentOverlay().then(function(){
                    app.helper.loadPageContentOverlay(data).then(function(){
                        ...
                        nectarcrm_Import_Js.registerAuthorizeButton(container);
                        nectarcrm_Import_Js.registerSyncNowButton(container, googleSettingInstance);
                    });    
                });
            });
        },
        ...
        registerImportEvents: function() {
            ...
            importContainer.on('click', '#vcfImport', function(e) {
                nectarcrm_Import_Js.showImportActionStepOne('vcf');
            });

			importContainer.on('click', '#icsImport', function(e) {
                nectarcrm_Import_Js.showImportActionStepOne('ics');
            });
            
            importContainer.on('click', '#googleImport', function(e) {
                nectarcrm_Import_Js.googleImportHandler(e);
            });
        },
        registerAuthorizeButton: function(container) {
            container.on('click', '#authorizeButton', function(e) {
                ...
                window.sync = function() {
                    nectarcrm_Import_Js.googleImportHandler();
                };
                            ...
                            app.helper.hidePageContentOverlay().then(function(){
                                ...
                                app.helper.hideProgress();
                                    app.helper.loadPageContentOverlay(data).then(function(overlayPageContent){
                                        ...
                                        var extensionCommonJs = new nectarcrm_ExtensionCommon_Js;
        ...
        finishUndoOperation: function(){
            nectarcrm_Import_Js.loadListRecords();
        },
        loadListRecords : function(){
			...
			if(app.getModuleName() == 'Users') {
				...
				listInstance = new nectarcrm_List_Js();
			}
        ...
        registerEvents: function() {
            nectarcrm_Import_Js.registerImportEvents();
        }
    }
