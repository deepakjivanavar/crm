				...
				<table id="listview-table"  class="table {if $LISTVIEW_ENTRIES_COUNT eq '0'}listview-table-norecords {/if} listview-table">
					<thead>
						<tr class="listViewContentHeader">
							...
							{assign var="LISTVIEW_HEADERS" value=$LISTVIEW_MODEL->getListViewHeadersFornectarcrm7({$VIEWNAME})}
										...
										<span class="fieldValue">
											<span class="value textOverflowEllipsis">
												...
												{else if $LISTVIEW_HEADERNAME eq 'primarymodule'}
													{nectarcrm_Util_Helper::tosafeHTML(decode_html(vtranslate($LISTVIEW_ENTRY_VALUE, $LISTVIEW_ENTRY_VALUE)))}
												{else if $LISTVIEW_HEADERNAME eq 'foldername'}
													{nectarcrm_Util_Helper::tosafeHTML(vtranslate($LISTVIEW_ENTRY_VALUE, $MODULE))}
												{else}
													{nectarcrm_Util_Helper::tosafeHTML($LISTVIEW_ENTRY_VALUE)}
												{/if}
											</span>
										</span>
										</span>
