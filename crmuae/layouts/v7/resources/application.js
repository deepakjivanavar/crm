...
window.app = (function () {
	...
	return {
		storage: {
			NSKey: function (key) {
				return 'nectarcrm6.'+key; // retain the existing cached data
			},
		...
		request: new Request(),
		event: new Event(),
		helper: new nectarcrm_Helper_Js(),
		/**
		...
		getModuleSpecificViewClass: function (view, module, parentModule) {
			...
			if (parentModule == 'Settings') {
				...
				if (typeof window[handlerClass] == 'undefined') {
					handlerClass = parentModule+'_nectarcrm_'+view+'_Js';
				}
			}
			if (typeof window[handlerClass] != 'undefined') {
				controllerClass = handlerClass;
			} else {
				handlerClass = 'nectarcrm_'+view+'_Js';
				if (typeof window[handlerClass] != 'undefined') {
					controllerClass = handlerClass;
				}
			}
