...
class nectarcrm_Loader {
	...
	static function resolveNameToPath($qualifiedName, $fileExtension='php') {
		...
		// TO handle loading nectarcrm files
	 ...
	 * @throws AppException
	 */
	public static function getComponentClassName($componentType, $componentName, $moduleName='nectarcrm') {
		...
		$fallBackModuleDir = $fallBackModuleClassPath = 'nectarcrm';
		...
		if(strpos($moduleName, ':') > 0) {
			...
			if($actualModule != 'Users') {
				...
				if($baseModule == 'Settings')  $baseModule = 'Settings:nectarcrm';
		...
		$moduleSpecificComponentFilePath = nectarcrm_Loader::resolveNameToPath('modules.'.$moduleDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		if(!empty($firstFallBackDir) && !empty($firstFallBackClassPath)) {
			$fallBackComponentFilePath = nectarcrm_Loader::resolveNameToPath('modules.'.$firstFallBackDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		if(!empty($secondFallBackModuleDir) && !empty($secondFallBackModuleClassPath)) {
			$fallBackComponentFilePath = nectarcrm_Loader::resolveNameToPath('modules.'.$secondFallBackModuleDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		$fallBackComponentFilePath = nectarcrm_Loader::resolveNameToPath('modules.'.$fallBackModuleDir.'.'.$componentTypeDirectory.'.'.$componentName);
	...
	public static function autoLoad($className) {
		...
		if($noOfParts > 2) {
            ...
            return nectarcrm_Loader::includeOnce($filePath);
		}
		return false;
	}
}
...
function vimport($qualifiedName) {
	return nectarcrm_Loader::includeOnce($qualifiedName);
}

function vimport_try($qualifiedName) {
	return nectarcrm_Loader::includeOnce($qualifiedName, true);
}

function vimport_path($qualifiedName) {
	return nectarcrm_Loader::includePath($qualifiedName);
}

spl_autoload_register('nectarcrm_Loader::autoLoad');