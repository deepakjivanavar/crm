...
class nectarcrm_Theme extends nectarcrm_Viewer {
	...
	public static function getStylePath($fileName='', $theme=''){
		...
		$completeLessFilePath = nectarcrm_Loader::resolveNameToPath('~'.$lessFilePath);
		$completeFallBackPath = nectarcrm_Loader::resolveNameToPath('~'.$fallbackPath);
	...
	public static function getImagePath($imageFileName){
		...
		$completeImageFilePath = nectarcrm_Loader::resolveNameToPath('~'.$imageFilePath);
		$completeFallBackThemePath = nectarcrm_Loader::resolveNameToPath('~'.$fallbackPath);
	...
	public static function getThemePath($theme=''){
		...
		$completeSelectedThemePath = nectarcrm_Loader::resolveNameToPath('~'.$selectedThemePath);
		$completeFallBackThemePath = nectarcrm_Loader::resolveNameToPath('~'.$fallBackThemePath);
	...
	public static function getAllSkins(){
		return nectarcrm_Util_Helper::getAllSkins();
	}
	...
	public static function getv7AppStylePath($appTheme = false) {
		...
		return nectarcrm_Theme::getStylePath('', strtolower($appTheme));
	}
}

function vimage_path($imageName) {
	$args = func_get_args();
	return call_user_func_array(array('nectarcrm_Theme', 'getImagePath'), $args);
}
