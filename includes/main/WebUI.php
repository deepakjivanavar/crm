...
class nectarcrm_WebUI extends nectarcrm_EntryPoint {
	 ...
	 * @throws AppException
	 */
	protected function checkLogin (nectarcrm_Request $request) {
		if (!$this->hasLogin()) {
			...
			if($return_params && !$_SESSION['return_params']) {
				...
				nectarcrm_Session::set('return_params', $return_params);
			}
	...
	function getLogin() {
		...
		if (!$user && isset($_SESSION['authenticated_user_id'])) {
			$userid = nectarcrm_Session::get('AUTHUSERID', $_SESSION['authenticated_user_id']);
	...
	function process (nectarcrm_Request $request) {
		...
		if ($currentUser && $qualifiedModuleName) {
			$moduleLanguageStrings = nectarcrm_Language_Handler::getModuleStringsFromFile($currentLanguage,$qualifiedModuleName);
		...
		try {
			...
			if(empty($module)) {
				if ($this->hasLogin()) {
					...
					$moduleModel = nectarcrm_Module_Model::getInstance($defaultModule);
			...
			$handlerClass = nectarcrm_Loader::getComponentClassName($componentType, $componentName, $qualifiedModuleName);
			...
			if ($handler) {
				...
				$skipList = array('Users', 'Home', 'CustomView', 'Import', 'Export', 'Inventory', 'nectarcrm', 'PriceBooks', 'Migration', 'Install');
