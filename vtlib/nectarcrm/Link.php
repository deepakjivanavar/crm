...
include_once('vtlib/nectarcrm/Utils/StringTemplate.php');
include_once 'vtlib/nectarcrm/LinkData.php';

/**
 * Provides API to handle custom links
 * @package vtlib
 */
class nectarcrm_Link {
	var $tabid;
	var $linkid;
	var $linktype;
	var $linklabel;
	...
	static function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('nectarcrm_links');
	}
	...
	static function __initSchema() {
		/* nectarcrm_links is already core product table */
		/*if(empty(self::$__cacheSchemaChanges['nectarcrm_links'])) {
			if(!nectarcrm_Utils::CheckTable('nectarcrm_links')) {
				nectarcrm_Utils::CreateTable(
					...
					true);
				nectarcrm_Utils::ExecuteQuery(
					'CREATE INDEX link_tabidtype_idx on nectarcrm_links(tabid,linktype)');
			}
			self::$__cacheSchemaChanges['nectarcrm_links'] = true;
		}*/
	}
	...
	static function addLink($tabid, $type, $label, $url, $iconpath='',$sequence=0, $handlerInfo=null, $parentLink=null) {
		global $adb;
		self::__initSchema();
		$checkres = $adb->pquery('SELECT linkid FROM nectarcrm_links WHERE tabid=? AND linktype=? AND linkurl=? AND linkicon=? AND linklabel=?',
			Array($tabid, $type, $url, $iconpath, $label));
		if(!$adb->num_rows($checkres)) {
			$uniqueid = self::__getUniqueId();
			$sql = 'INSERT INTO nectarcrm_links (linkid,tabid,linktype,linklabel,linkurl,linkicon,'.
			'sequence';
	...
	static function deleteLink($tabid, $type, $label, $url=false) {
		...
		self::__initSchema();
		if($url) {
			$adb->pquery('DELETE FROM nectarcrm_links WHERE tabid=? AND linktype=? AND linklabel=? AND linkurl=?',
				Array($tabid, $type, $label, $url));
			self::log("Deleting Link ($type - $label - $url) ... DONE");
		} else {
			$adb->pquery('DELETE FROM nectarcrm_links WHERE tabid=? AND linktype=? AND linklabel=?',
	...
	static function deleteAll($tabid) {
		global $adb;
		self::__initSchema();
		$adb->pquery('DELETE FROM nectarcrm_links WHERE tabid=?', Array($tabid));
	...
	static function getAllByType($tabid, $type=false, $parameters=false) {
		...
		if($type) {
			...
			if(is_array($type)) { 
				...
				if($tabid === self::IGNORE_MODULE) {
					$sql = 'SELECT * FROM nectarcrm_links WHERE linktype IN ('.
						nectarcrm_Utils::implodestr('?', count($type), ',') .') ';
					$params = $type;
					...
					if(count($permittedTabIdList) > 0 && $current_user->is_admin !== 'on') {
						...
						$sql .= ' and tabid IN ('.
							nectarcrm_Utils::implodestr('?', count($permittedTabIdList), ',').')';
						$params[] = $permittedTabIdList;
					}
					$result = $adb->pquery($sql, Array($adb->flatten_array($params)));
				} else {
					$result = $adb->pquery('SELECT * FROM nectarcrm_links WHERE (tabid=? OR tabid=0) AND linktype IN ('.
						nectarcrm_Utils::implodestr('?', count($type), ',') .')',
				...
				if($tabid === self::IGNORE_MODULE) {
					$result = $adb->pquery('SELECT * FROM nectarcrm_links WHERE linktype=?', Array($type));
				} else {
					$result = $adb->pquery('SELECT * FROM nectarcrm_links WHERE (tabid=? OR tabid=0) AND linktype=?', Array($tabid, $type));
				}
			}
		} else {
			$result = $adb->pquery('SELECT * FROM nectarcrm_links WHERE tabid=?', Array($tabid));
		}

		$strtemplate = new nectarcrm_StringTemplate();
		if($parameters) {
		...
		while($row = $adb->fetch_array($result)){
			...
			if(!empty($row['handler_path']) && isFileAccessible($row['handler_path'])) {
				...
				$linkData = new nectarcrm_LinkData($instance, $current_user);
	 ...
	 * @param nectarcrm_LinkData $linkData
	 * @return Boolean
	 */
	...
	static function updateLink($tabId, $linkId, $linkInfo = array()) {
		if ($linkInfo && is_array($linkInfo)) {
			...
			$result = $db->pquery('SELECT 1 FROM nectarcrm_links WHERE tabid=? AND linkid=?', array($tabId, $linkId));
			if ($db->num_rows($result)) {
				$columnsList = $db->getColumnNames('nectarcrm_links');
				$isColumnUpdate = false;

				$sql = 'UPDATE nectarcrm_links SET ';
