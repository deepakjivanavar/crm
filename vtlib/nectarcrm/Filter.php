...
/**
 * Provides API to work with nectarcrm CRM Custom View (Filter)
 * @package vtlib
 */
class nectarcrm_Filter {
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('nectarcrm_customview');
	}

	/**
	 * Initialize this filter instance
	 * @param nectarcrm_Module Instance of the module to which this filter is associated.
	 * @access private
	 */
	function initialize($valuemap, $moduleInstance=false) {
		$this->id = $valuemap[cvid];
		$this->name= $valuemap[viewname];
		$this->module=$moduleInstance? $moduleInstance: nectarcrm_Module::getInstance($valuemap[tabid]);
	}

	/**
	 * Create this instance
	 * @param nectarcrm_Module Instance of the module to which this filter should be associated with
	 * @access private
	 */
	function __create($moduleInstance) {
		...
		$adb->pquery("INSERT INTO nectarcrm_customview(cvid,viewname,setdefault,setmetrics,entitytype) VALUES(?,?,?,?,?)", 
		...
		if(!$this->status) {
			...
			$adb->pquery("UPDATE nectarcrm_customview SET status=? WHERE cvid=?", Array($this->status, $this->id));
	...
	function __delete() {
		global $adb;
		$adb->pquery("DELETE FROM nectarcrm_cvadvfilter WHERE cvid=?", Array($this->id));
		$adb->pquery("DELETE FROM nectarcrm_cvcolumnlist WHERE cvid=?", Array($this->id));
		$adb->pquery("DELETE FROM nectarcrm_customview WHERE cvid=?", Array($this->id));
	}
	...
	function addField($fieldInstance, $index=0) {
		...
		$adb->pquery("UPDATE nectarcrm_cvcolumnlist SET columnindex=columnindex+1 WHERE cvid=? AND columnindex>=? ORDER BY columnindex DESC", 
			Array($this->id, $index));
		$adb->pquery("INSERT INTO nectarcrm_cvcolumnlist(cvid,columnindex,columnname) VALUES(?,?,?)", Array($this->id, $index, $cvcolvalue));
	...
	function addRule($fieldInstance, $comparator, $comparevalue, $index=0, $group=1, $condition='and') {
		...
		$adb->pquery("UPDATE nectarcrm_cvadvfilter set columnindex=columnindex+1 WHERE cvid=? AND columnindex>=? ORDER BY columnindex DESC",
			Array($this->id, $index));		
		$adb->pquery("INSERT INTO nectarcrm_cvadvfilter(cvid, columnindex, columnname, comparator, value, groupid, column_condition) VALUES(?,?,?,?,?,?,?)",
			Array($this->id, $index, $cvcolvalue, $comparator, $comparevalue, $group, $condition));

		nectarcrm_Utils::Log("Adding Condition " . self::translateComparator($comparator,true) ." on $fieldInstance->name of $this->name filter ... DONE");
	 ...
	 * @access private
	 * @internal Used from nectarcrm_PackageExport also
	 */
	 ...
	 * @param nectarcrm_Module Instance of the module to use when filtername is used
	 */
	static function getInstance($value, $moduleInstance=false) {
		...
		if(nectarcrm_Utils::isNumber($value)) {
			$query = "SELECT * FROM nectarcrm_customview WHERE cvid=?";
			$queryParams = Array($value);
		} else {
			$query = "SELECT * FROM nectarcrm_customview WHERE viewname=? AND entitytype=?";
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM nectarcrm_customview WHERE entitytype=?";
	...
	static function deleteForModule($moduleInstance) {
		...
		$cvidres = $adb->pquery("SELECT cvid FROM nectarcrm_customview WHERE entitytype=?", Array($moduleInstance->name));
		if($adb->num_rows($cvidres)) {
			...
			if(!empty($cvids)) {
				$adb->pquery("DELETE FROM nectarcrm_cvadvfilter WHERE cvid  IN (" . implode(',', $cvids) . ")", array());
				$adb->pquery("DELETE FROM nectarcrm_cvcolumnlist WHERE cvid IN (" . implode(',', $cvids) . ")", array());
				$adb->pquery("DELETE FROM nectarcrm_customview WHERE cvid   IN (" . implode(',', $cvids) . ")", array());
			}
		}
	}
}
