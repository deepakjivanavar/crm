...
/**
 * Provides API to work with nectarcrm CRM Module Blocks
 * @package vtlib
 */
class nectarcrm_Block {
	...
	function __getUniqueId() {
		...
		$maxblockid = $adb->getUniqueID('nectarcrm_blocks');
		return $maxblockid;
	}
	...
	function __getNextSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(sequence) as max_sequence from nectarcrm_blocks where tabid = ?", Array($this->module->id));
		$maxseq = 0;
	 ...
	 * @param nectarcrm_Module Instance of module to which this block is associated
	 * @access private
	 */
	function initialize($valuemap, $moduleInstance=false) {
		...
		$this->module= $moduleInstance ? $moduleInstance : nectarcrm_Module::getInstance($tabid);
	}

	/**
	 * Create nectarcrm CRM block
	 * @access private
	 */
	function __create($moduleInstance) {
		...
		$adb->pquery("INSERT INTO nectarcrm_blocks(blockid,tabid,blocklabel,sequence,show_title,visible,create_view,edit_view,detail_view,iscustom)
	...
	function __delete() {
		...
		$adb->pquery("DELETE FROM nectarcrm_blocks WHERE blockid=?", Array($this->id));
		self::log("DONE");
	}

	/**
	 * Save this block instance
	 * @param nectarcrm_Module Instance of the module to which this block is associated
	 */
	...
	function delete($recursive=true) {
		if($recursive) {
			$fields = nectarcrm_Field::getAllForBlock($this);
	 ...
	 * @param nectarcrm_Field Instance of field to add to this block.
	 ...
	 * @param nectarcrm_Module Instance of the module if block label is passed
	 */
	static function getInstance($value, $moduleInstance=false) {
		...
		if(nectarcrm_Utils::isNumber($value)) {
			$query = "SELECT * FROM nectarcrm_blocks WHERE blockid=?";
			$queryParams = Array($value);
		} else {
			$query = "SELECT * FROM nectarcrm_blocks WHERE blocklabel=? AND tabid=?";
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM nectarcrm_blocks WHERE tabid=? ORDER BY sequence";
	 ...
	 * @param nectarcrm_Module Instnace of module to use
	...
	static function deleteForModule($moduleInstance, $recursive=true) {
		global $adb;
		if($recursive) nectarcrm_Field::deleteForModule($moduleInstance);
		$adb->pquery("DELETE FROM nectarcrm_blocks WHERE tabid=?", Array($moduleInstance->id));
