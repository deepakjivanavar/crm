...
require_once 'include/Webservices/DescribeObject.php';
	...
	function vtws_sync($mtime,$elementType,$syncType,$user){
		...
		if($typed){
		 ...
		 $baseCRMTable = " nectarcrm_crmentity ";
		...
		foreach($accessableModules as $elementType){
			...
			$fromClause.= ' ) nectarcrm_ws_sync ON (nectarcrm_crmentity.crmid = nectarcrm_ws_sync.crmid)';
			if ($elementType == 'Events') {
				// If we have more than one contact attached to nectarcrm Event then we are getting duplicates
	...
	function vtws_getEmailFromClause(){
		$q = "FROM nectarcrm_activity
				...
				LEFT JOIN nectarcrm_seattachmentsrel ON nectarcrm_activity.activityid = nectarcrm_seattachmentsrel.crmid
				LEFT JOIN nectarcrm_attachments ON nectarcrm_seattachmentsrel.attachmentsid = nectarcrm_attachments.attachmentsid
				LEFT JOIN nectarcrm_email_track ON nectarcrm_activity.activityid = nectarcrm_email_track.mailid
				INNER JOIN nectarcrm_emaildetails ON nectarcrm_activity.activityid = nectarcrm_emaildetails.emailid
				LEFT JOIN nectarcrm_users nectarcrm_users2 ON nectarcrm_emaildetails.idlists = nectarcrm_users2.id
				LEFT JOIN nectarcrm_groups nectarcrm_groups2 ON nectarcrm_emaildetails.idlists = nectarcrm_groups2.groupid";
		return $q;
	}

	function getSyncQueryBaseTable($elementType){
		if($elementType!="Calendar" && $elementType!="Events"){
			return "nectarcrm_crmentity";
		}
		else{
			$activityCondition = getCalendarTypeCondition($elementType);
			$query = "nectarcrm_crmentity INNER JOIN nectarcrm_activity ON (nectarcrm_crmentity.crmid = nectarcrm_activity.activityid and $activityCondition)";
			return $query;
		}
	}
	...
	function getCalendarTypeCondition($elementType){
		if($elementType == "Events")
			$activityCondition = "nectarcrm_activity.activitytype !='Task' and nectarcrm_activity.activitytype !='Emails'";
		else
			$activityCondition = "nectarcrm_activity.activitytype ='Task'";
