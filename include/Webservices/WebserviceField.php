...
class WebserviceField{
	...
	public function getReferenceList($hideDisabledModules = true){
		...
		if($this->referenceList === null){
			...
			if($this->getUIType() != $this->genericUIType){
				$sql = "select type from nectarcrm_ws_referencetype where fieldtypeid=?";
				$params = array($fieldTypeData['fieldtypeid']);
			}else{
				$sql = 'SELECT relmodule AS type FROM nectarcrm_fieldmodulerel WHERE fieldid=? ORDER BY sequence ASC';
	...
	private function getFieldTypeFromUIType(){
		...
		if(empty(self::$fieldTypeMapping)) {
			$result = $this->pearDB->pquery("select * from nectarcrm_ws_fieldtype", array());
	...
	function getPickListOptions(){
		...
		$language = nectarcrm_Language_Handler::getLanguage();

		$default_charset = VTWS_PreserveGlobal::getGlobal('default_charset');
		$options = array();
		$sql = "select * from nectarcrm_picklist where name=?";
		...
		if($numRows == 0){
			$sql = "select * from nectarcrm_$fieldName";
