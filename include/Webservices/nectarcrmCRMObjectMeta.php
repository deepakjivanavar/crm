...
class nectarcrmCRMObjectMeta extends EntityMeta {
	...
	function nectarcrmCRMObjectMeta($webserviceObject,$user){
		...
		if(in_array('nectarcrm_crmentity',$instance->tab_name)){
			$this->defaultTableList = array('nectarcrm_crmentity');
		}else{
	...
	private function computeAccess(){
		...
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0){
			...
			$sql = "select * from nectarcrm_profile2globalpermissions where profileid in (".generateQuestionMarks($profileList).");";
			...
			$sql = 'select * from nectarcrm_profile2tab where profileid in ('.generateQuestionMarks($profileList).') and tabid = ? and permissions = ?';
			...
			$sql = "select * from nectarcrm_profile2standardpermissions where profileid in (".generateQuestionMarks($profileList).") and tabid=?";
	...
	function hasAssignPrivilege($webserviceId){
		...
		$webserviceObject = nectarcrmWebserviceObject::fromId($adb,$ownerTypeId);
	...
	private function retrieveMetaForBlock($block){
		...
		require('user_privileges/user_privileges_'.$this->user->id.'.php');
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0){
			$sql = "select *, '0' as readonly from nectarcrm_field where tabid =? and block in (".generateQuestionMarks($block).") and displaytype in (1,2,3,4,5,6)";
			$params = array($tabid, $block);	
		}else{
			$profileList = getCurrentUserProfileList();
			
			if (count($profileList) > 0) {
				$sql = "SELECT nectarcrm_field.*, nectarcrm_profile2field.readonly
						...
						WHERE nectarcrm_field.tabid =? AND nectarcrm_profile2field.visible = 0 
						AND nectarcrm_profile2field.profileid IN (". generateQuestionMarks($profileList) .")
						AND nectarcrm_def_org_field.visible = 0 and nectarcrm_field.block in (".generateQuestionMarks($block).") and nectarcrm_field.displaytype in (1,2,3,4,5,6) and nectarcrm_field.presence in (0,2) group by columnname";
				...
				$sql = "SELECT nectarcrm_field.*, nectarcrm_profile2field.readonly
						...
						AND nectarcrm_profile2field.visible = 0 
						AND nectarcrm_def_org_field.visible = 0 and nectarcrm_field.block in (".generateQuestionMarks($block).") and nectarcrm_field.displaytype in (1,2,3,4,5,6) and nectarcrm_field.presence in (0,2) group by columnname";
				$params = array($tabid, $block);
			}
		}
	...
	function getObjectEntityName($webserviceId){
		...
		if($this->objectName == 'Users'){
			$sql = "select user_name from nectarcrm_users where id=? and deleted=0";
			...
			$sql = "select setype from nectarcrm_crmentity where crmid=? and deleted=0";
			$result = $adb->pquery($sql , array($id));
			if($result != null && isset($result)){
				if($adb->num_rows($result)>0){
	...
	function exists($recordId){
		...
		if($this->objectName == 'Users'){
			if (array_key_exists($recordId, $user_exists_cache)) {
				...
				$sql = "select 1 from nectarcrm_users where id=? and deleted=0 and status='Active'";
			}
			
		}else{
			$sql = "select 1 from nectarcrm_crmentity where crmid=? and deleted=0 and setype='".
				$this->getTabName()."'";
		}
