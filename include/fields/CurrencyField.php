...
class CurrencyField {
	...
	public static function getDBCurrencyId() {
		...
		$result = $adb->pquery('SELECT id FROM nectarcrm_currency_info WHERE defaultid < 0', array());
	...
	public static function getCurrencyInfoFromName($currencyName) {
		...
		$currencyInfo = array();
		$result = $db->pquery('SELECT * FROM nectarcrm_currency_info WHERE currency_name=?', array(trim($currencyName)));
