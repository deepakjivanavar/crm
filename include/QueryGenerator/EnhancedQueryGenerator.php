			...
			if (count($filtercolumns) > 0) {
				...
				foreach ($filtercolumns as $index => $filter) {
					...
					if (!in_array($filter['comparator'], array('e', 'n'))) {
						$filter['value'] = nectarcrm_Util_Helper::escapeSqlString($filter['value']);
					}
		...
		foreach ($this->fields as $field) {
			...
			if ($this->meta->getEntityName() == 'Calendar' && ($field == 'date_start' || $field == 'due_date' || $field == 'taskstatus' || $field == 'eventstatus')) {
				if ($field == 'date_start') {
					...
					$sql = "CASE WHEN (nectarcrm_activity.status not like '') THEN nectarcrm_activity.status ELSE nectarcrm_activity.eventstatus END AS ";
					if ($field == 'taskstatus') {
						$sql .= "status";
					} else {
						$sql .= $field;
	...
	public function getFromClause() {
		...
		foreach ($fieldList as $fieldName) {
			...
			if (empty($referenceParentFieldName)) {
				...
				if ($fieldType == 'owner') {
					$tableList['nectarcrm_users'] = 'nectarcrm_users';
					$tableList['nectarcrm_groups'] = 'nectarcrm_groups';
					$tableJoinMapping['nectarcrm_users'] = 'LEFT JOIN';
					$tableJoinMapping['nectarcrm_groups'] = 'LEFT JOIN';
				}
				...
				if ($fieldName == 'roleid' && $baseModule == 'Users') {
					$tableJoinMapping['nectarcrm_role'] = 'INNER JOIN';
					$tableList['nectarcrm_role'] = 'nectarcrm_role';
				}
			} else {
		...
		foreach ($this->whereFields as $fieldName) {
			...
			if (empty($referenceParentFieldName)) {
				...
				if ($fieldType == 'reference') {
					...
					foreach ($moduleList as $module) {
						if ($module == 'Users' && $baseModule != 'Users') {
							$tableJoinMapping['nectarcrm_users'.$fieldName] = 'LEFT JOIN nectarcrm_users AS';
							$tableJoinCondition[$fieldName]['nectarcrm_users'.$fieldName] = $fieldTable.'.'.$field->getColumnName().' = nectarcrm_users'.$fieldName.'.id';
						} else if ($module == 'Currency') {
							$tableJoinMapping['nectarcrm_currency_info'.$fieldName] = 'LEFT JOIN nectarcrm_currency_info AS';
							$tableJoinCondition[$fieldName]['nectarcrm_currency_info'.$fieldName] = $fieldTable.'.'.$field->getColumnName().' = nectarcrm_currency_info'.$fieldName.'.id';
						} else {
							$tableJoinMapping['nectarcrm_crmentity'.$fieldName] = 'LEFT JOIN nectarcrm_crmentity AS';
							$tableJoinCondition[$fieldName]['nectarcrm_crmentity'.$fieldName] = 'nectarcrm_crmentity'.$fieldName.'.crmid' .
				...
				if ($fieldName == 'tags') {
					$tableList['nectarcrm_freetagged_objects'] = 'nectarcrm_freetagged_objects';
					$tableJoinMapping['nectarcrm_freetagged_objects'] = 'INNER JOIN';
				}
			} else {
				$referenceParentFieldModel = $field->parentReferenceField;
				...
				if ($fieldType == 'owner') {
					// Need to join with nectarcrm_crmentity table
					...
					$tableJoinMapping['nectarcrm_users'.$referenceParentFieldName.$fieldName] = 'LEFT JOIN nectarcrm_users AS ';
					$tableJoinCondition[$referenceParentFieldName.$fieldName]['nectarcrm_users'.$referenceParentFieldName.$fieldName] = 'nectarcrm_users'.$referenceParentFieldName.$fieldName.'.id = '.$fieldTable.$referenceParentFieldName.'.'.$field->getColumnName();

					$tableJoinMapping['nectarcrm_groups'.$referenceParentFieldName.$fieldName] = 'LEFT JOIN nectarcrm_groups AS ';
					$tableJoinCondition[$referenceParentFieldName.$fieldName]['nectarcrm_groups'.$referenceParentFieldName.$fieldName] = 'nectarcrm_groups'.$referenceParentFieldName.$fieldName.'.groupid = '.$fieldTable.$referenceParentFieldName.'.'.$field->getColumnName();
					...
					foreach ($moduleList as $module) {
						if ($module == 'Users' && $baseModule != 'Users') {// && ($fieldName == 'created_user_id' || $fieldName == 'modifiedby')) {
							...
							// Need to join with nectarcrm_crmentity table if its not joined earlier
							if (!array_key_exists($fieldTable.$referenceParentFieldName, $tableJoinMapping)) {
								$tableJoinMapping[$fieldTable.$referenceParentFieldName] = 'LEFT JOIN '.$fieldTable.' AS ';
							...
							$tableJoinMapping['nectarcrm_currency_info'.$referenceParentFieldName.$fieldName] = 'LEFT JOIN nectarcrm_currency_info AS';
							$tableJoinCondition[$fieldName]['nectarcrm_currency_info'.$referenceParentFieldName.$fieldName] = $fieldTable.$referenceParentFieldName.'.'.$field->getColumnName().' = nectarcrm_currency_info'.$referenceParentFieldName.$fieldName.'.id';
						} else {
							if (!array_key_exists($fieldTable.$referenceParentFieldName, $tableJoinMapping)) {
								$tableJoinMapping[$fieldTable.$referenceParentFieldName] = 'LEFT JOIN '.$fieldTable.' AS ';
								$tableJoinCondition[$referenceParentFieldName.$fieldName][$fieldTable.$referenceParentFieldName] = $fieldTable.$referenceParentFieldName.'.'.$referenceModuleColumnIndex[$fieldTable].' = ' .
							...
							$tableAlias = 'nectarcrm_crmentity'.$referenceParentFieldName.$fieldName;
							if (!array_key_exists($tableAlias, $tableJoinMapping)) {
								$tableJoinMapping[$tableAlias] = 'LEFT JOIN nectarcrm_crmentity AS ';
		...
		foreach ($tableList as $tableName) {
			...
			} elseif ($tableName == 'nectarcrm_groups') {
				$field = $moduleFields[$ownerField];
				$sql .= " $tableJoinMapping[$tableName] $tableName ON ".$field->getTableName()."." .
						$field->getColumnName()." = $tableName.groupid";
			} elseif ($tableName == 'nectarcrm_freetagged_objects') {
				$sql .= " $tableJoinMapping[$tableName] $tableName ON $baseTable.$baseTableIndex = $tableName.object_id " .
						"INNER JOIN nectarcrm_freetags ON $tableName.tag_id = nectarcrm_freetags.id ";
			} elseif ($tableName == 'nectarcrm_role') {
				$sql .= " $tableJoinMapping[$tableName] $tableName ON nectarcrm_role.roleid = nectarcrm_user2role.roleid";
			} else {
				$tableCondition = $tableName.'.'.$moduleTableIndexList[$tableName];
				if (nectarcrm_Functions::isUserSpecificFieldTable($tableName, $this->getModule())) {
		...
		if ($this->meta->getTabName() == 'Documents') {
			$tableJoinCondition['folderid'] = array(
				'nectarcrm_attachmentsfolderfolderid' => "$baseTable.folderid = nectarcrm_attachmentsfolderfolderid.folderid"
			);
			$tableJoinMapping['nectarcrm_attachmentsfolderfolderid'] = 'INNER JOIN nectarcrm_attachmentsfolder';
		}
	...
	public function getWhereClause() {
		...
		foreach ($this->conditionals as $index => $conditionInfo) {
			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($baseFieldName, $this->referenceFieldList)) {
					if ($conditionInfo['operator'] == 'y') {
						...
						if (in_array('Users', $moduleList)) {
							$columnSqlTable = 'nectarcrm_users'.$parentReferenceField.$fieldName;
							...
							$columnSql = "nectarcrm_attachmentsfolder".$fieldName.".foldername";
						} else if (in_array('Currency', $moduleList)) {
							$columnSql = "nectarcrm_currency_info$parentReferenceField$fieldName.currency_name";
						} else if ($baseFieldName == 'roleid') {
							$columnSql = 'nectarcrm_role.rolename';
						} else {
							$columnSql = 'nectarcrm_crmentity'.$parentReferenceField.$fieldName.'.label';
						}
					...
					if ($parentReferenceField)
						...
						$ownerTableName = '';
					$concatSql = getSqlForNameInDisplayFormat(array('first_name' => 'nectarcrm_users'.$ownerTableName.'.first_name',
						'last_name' => 'nectarcrm_users'.$ownerTableName.'.last_name'), 'Users');
					if ($conditionInfo['operator'] == 'y') {
						//if both user name and group name empty, then only should list in isempty condition
						$fieldSql .= "$fieldGlue ((trim($concatSql) $valueSql) AND (nectarcrm_groups$ownerTableName.groupname $valueSql))";
					} else {
						$fieldSql .= "$fieldGlue (trim($concatSql) $valueSql or nectarcrm_groups$ownerTableName.groupname $valueSql)";
					}
					...
					if ($fieldName == 'date_start') {
						$dateFieldColumnName = 'nectarcrm_activity.date_start';
						$timeFieldColumnName = 'nectarcrm_activity.time_start';
					} else {
						$dateFieldColumnName = 'nectarcrm_activity.due_date';
						$timeFieldColumnName = 'nectarcrm_activity.time_end';
					}
					if ($operator == 'bw') {
				...
				}else if (nectarcrm_Functions::isUserSpecificFieldTable($field->getTableName(), getTabModuleName($field->getTabId())) && $fieldName == "starred" && $conditionInfo['value'] != 1) {
					...
					$fieldSql .= " $fieldGlue ( nectarcrm_freetags.id ".$valueSql.' AND ' .
							'( nectarcrm_freetagged_objects.tagger_id = '.$this->user->id.' OR nectarcrm_freetags.visibility = "public")) ';
				} else {
			...
			$tmpTableName = 'nectarcrm_crmentity'.$parentReferenceField;
	...
	function getOrderByColumn($fieldName) {
		...
		if ($orderByFieldModel && $orderByFieldModel->getFieldDataType() == 'reference') {
			...
			if (in_array('DocumentFolders', $referenceModules)) {
				$orderByColumn = "nectarcrm_attachmentsfolder".$orderByFieldModel->getFieldName().".foldername";
			} else if (in_array('Currency', $referenceModules)) {
				if ($parentReferenceField) {
					$orderByColumn = 'nectarcrm_currency_info'.$parentReferenceField.$orderByFieldModel->getFieldName().'.currency_name';
				} else {
					$orderByColumn = 'nectarcrm_currency_info'.$fieldName.'.currency_name';
				}
				...
				$orderByColumn = 'nectarcrm_crmentity'.$parentReferenceField.$orderByFieldModel->getFieldName().'.label'; //.$fieldModel->get('column');
			}
		} else if ($orderByFieldModel && $orderByFieldModel->getFieldDataType() == 'owner') {
			if ($parentReferenceField) {
				$userTableName = 'nectarcrm_users'.$parentReferenceField.$orderByFieldModel->getFieldName();
				$groupTableName = 'nectarcrm_groups'.$parentReferenceField.$orderByFieldModel->getFieldName();
				$orderByColumn = "COALESCE(CONCAT($userTableName.first_name,$userTableName.last_name),$groupTableName.groupname)";
			} else {
				$orderByColumn = 'COALESCE(CONCAT(nectarcrm_users.first_name,nectarcrm_users.last_name),nectarcrm_groups.groupname)';
			}
