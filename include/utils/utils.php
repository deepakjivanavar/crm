...
require_once 'vtlib/nectarcrm/Functions.php';
require_once 'vtlib/nectarcrm/Deprecated.php';

require_once 'includes/runtime/Cache.php';
require_once 'modules/nectarcrm/helpers/Util.php';
require_once 'vtlib/nectarcrm/AccessControl.php';
...
function get_user_array($add_blank=true, $status="Active", $assigned_user="",$private="",$module=false)
{
	...
	if($user_array == null)
	{
		...
		$temp_result = Array();
		// Including deleted nectarcrm_users for now.
		if (empty($status)) {
				$query = "SELECT id, user_name from nectarcrm_users";
				$params = array();
		}
		else {
				if($private == 'private')
				{
					...
					$query = "select id as id,user_name as user_name,first_name,last_name from nectarcrm_users where id=? and status='Active' union select nectarcrm_user2role.userid as id,nectarcrm_users.user_name as user_name ,
							  nectarcrm_users.first_name as first_name ,nectarcrm_users.last_name as last_name
							  from nectarcrm_user2role inner join nectarcrm_users on nectarcrm_users.id=nectarcrm_user2role.userid inner join nectarcrm_role on nectarcrm_role.roleid=nectarcrm_user2role.roleid where nectarcrm_role.parentrole like ? and status='Active' union
							  select shareduserid as id,nectarcrm_users.user_name as user_name ,
							  nectarcrm_users.first_name as first_name ,nectarcrm_users.last_name as last_name  from nectarcrm_tmp_write_user_sharing_per inner join nectarcrm_users on nectarcrm_users.id=nectarcrm_tmp_write_user_sharing_per.shareduserid where status='Active' and nectarcrm_tmp_write_user_sharing_per.userid=? and nectarcrm_tmp_write_user_sharing_per.tabid=? and (user_name != 'admin' OR is_owner=1)";
					$params = array($current_user->id, $current_user_parent_role_seq."::%", $current_user->id, getTabid($module));
				}
				else
				{
					$log->debug("Sharing is Public. All nectarcrm_users should be listed");
					$query = "SELECT id, user_name,first_name,last_name from nectarcrm_users WHERE status=? and (user_name != 'admin' OR is_owner=1)";
					$params = array($status);
				}
		}
...
function get_group_array($add_blank=true, $status="Active", $assigned_user="",$private="",$module = false)
{
	...
	if($group_array == null)
	{
		...
		// Including deleted nectarcrm_users for now.
		$log->debug("Sharing is Public. All nectarcrm_users should be listed");
		$query = "SELECT groupid, groupname from nectarcrm_groups";
		$params = array();
		...
		if($private == 'private'){
			...
			if(count($current_user_groups) != 0) {
				$query .= " OR nectarcrm_groups.groupid in (".generateQuestionMarks($current_user_groups).")";
				array_push($params, $current_user_groups);
			}
			$log->debug("Sharing is Private. Only the current user should be listed");
			$query .= " union select nectarcrm_group2role.groupid as groupid,nectarcrm_groups.groupname as groupname from nectarcrm_group2role inner join nectarcrm_groups on nectarcrm_groups.groupid=nectarcrm_group2role.groupid inner join nectarcrm_role on nectarcrm_role.roleid=nectarcrm_group2role.roleid where nectarcrm_role.parentrole like ?";
			array_push($params, $current_user_parent_role_seq."::%");

			if(count($current_user_groups) != 0) {
				$query .= " union select nectarcrm_groups.groupid as groupid,nectarcrm_groups.groupname as groupname from nectarcrm_groups inner join nectarcrm_group2rs on nectarcrm_groups.groupid=nectarcrm_group2rs.groupid where nectarcrm_group2rs.roleandsubid in (".generateQuestionMarks($parent_roles).")";
				array_push($params, $parent_roles);
			}

			$query .= " union select sharedgroupid as groupid,nectarcrm_groups.groupname as groupname from nectarcrm_tmp_write_group_sharing_per inner join nectarcrm_groups on nectarcrm_groups.groupid=nectarcrm_tmp_write_group_sharing_per.sharedgroupid where nectarcrm_tmp_write_group_sharing_per.userid=?";
			array_push($params, $current_user->id);

			$query .= " and nectarcrm_tmp_write_group_sharing_per.tabid=?";
...
function return_app_list_strings_language($language) {
	return nectarcrm_Deprecated::return_app_list_strings_language($language);
}
...
function return_module_language($language, $module) {
	return nectarcrm_Deprecated::getModuleTranslationStrings($language, $module);
}
...
function return_specified_module_language($language, $module) {
	return nectarcrm_Deprecated::return_app_list_strings_language($language, $module);
}
 ...
 */
function get_themes() {
	return nectarcrm_Theme::getAllSkins();
   }
...
function to_html($string, $encode=true) {
    ...
    if(is_string($string)) {
		// In nectarcrm5 ajax request are treated specially and the data is encoded
		if ($doconvert == true) {
...
function getTabname($tabid)
{
	...
	if (!isset($cache[$tabid])) {
		$sql = "select tablabel from nectarcrm_tab where tabid=?";
...
{
	return nectarcrm_Functions::getModuleName($tabid);
}
...
function getColumnFields($module)
{
	...
	if($cachedModuleFields === false) {
		...
		FROM nectarcrm_field WHERE tabid in (" . generateQuestionMarks($tabid) . ")";
...
function getUserEmail($userid)
{
	...
	$log->info("in getUserEmail ".$userid);
        ...
        if($userid != '')
        {
                $sql = "select email1 from nectarcrm_users where id=?";
...
function getUserId_Ol($username)
{
	...
	global $adb;
	$sql = "select id from nectarcrm_users where user_name=?";
...
function getActionid($action)
{
	...
	{
		$query="select * from nectarcrm_actionmapping where actionname=?";
...
function getActionname($actionid)
{
	...
	else
	{
		...
		$query="select * from nectarcrm_actionmapping where actionid=? and securitycheck=0";
...
function getRecordOwnerId($record)
{
	...
	if($ownerId === false) {
		$query="select smownerid from nectarcrm_crmentity where crmid = ?";
	...
	if($ownerId)
	{
		...
		if($count === false) {
			$sql_result = $adb->pquery('SELECT 1 FROM nectarcrm_users WHERE id = ?', array($ownerId));
...
function insertProfile2field($profileid)
{
	...
	$fld_result = $adb->pquery("select * from nectarcrm_field where generatedtype=1 and displaytype in (1,2,3) and nectarcrm_field.presence in (0,2) and tabid != 29", array());
    ...
    for($i=0; $i<$num_rows; $i++) {
         ...
         $adb->pquery("insert into nectarcrm_profile2field values (?,?,?,?,?)", $params);
	}
...
function insert_def_org_field()
{
	...
	$fld_result = $adb->pquery("select * from nectarcrm_field where generatedtype=1 and displaytype in (1,2,3) and nectarcrm_field.presence in (0,2) and tabid != 29", array());
        ...
        for($i=0; $i<$num_rows; $i++)
        {
                 ...
                 $adb->pquery("insert into nectarcrm_def_org_field values (?,?,?,?)", $params);
	}
...
function updateProductQty($product_id, $upd_qty)
{
	...
	global $adb;
	$query= "update nectarcrm_products set qtyinstock=? where productid=?";
...
function addToProductStock($productId,$qty)
{
	...
	$sql = "UPDATE nectarcrm_products set qtyinstock=? where productid=?";
...
function addToProductDemand($productId,$qty)
{
	...
	$updQty=$qtyInStck + $qty;
	$sql = "UPDATE nectarcrm_products set qtyindemand=? where productid=?";
	$adb->pquery($sql, array($updQty, $productId));
...
function getProductQtyInStock($product_id)
{
	...
	global $adb;
        $query1 = "select qtyinstock from nectarcrm_products where productid=?";
...
function getProductQtyInDemand($product_id)
{
	...
	global $adb;
        $query1 = "select qtyindemand from nectarcrm_products where productid=?";
        $result = $adb->pquery($query1, array($product_id));
...
/**     Function to get the nectarcrm_table name from 'field' nectarcrm_table for the input nectarcrm_field based on the module
 *      @param  : string $module - current module value
 *      @param  : string $fieldname - nectarcrm_fieldname to which we want the nectarcrm_tablename
 *      @return : string $tablename - nectarcrm_tablename in which $fieldname is a column, which is retrieved from 'field' nectarcrm_table per $module basis
  */
function getTableNameForField($module,$fieldname)
{
	...
	$sql = "select tablename from nectarcrm_field where tabid in (". generateQuestionMarks($tabid) .") and nectarcrm_field.presence in (0,2) and columnname like ?";
...
function getPotentialsRelatedAccounts($record_id)
{
	...
	global $adb;
	$query="select related_to from nectarcrm_potential where potentialid=?";
...
function getEmailsRelatedAccounts($record_id)
{
	...
	global $adb;
	$query = "select nectarcrm_seactivityrel.crmid from nectarcrm_seactivityrel inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_seactivityrel.crmid where nectarcrm_crmentity.setype='Accounts' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getEmailsRelatedLeads($record_id)
{
	...
	global $adb;
	$query = "select nectarcrm_seactivityrel.crmid from nectarcrm_seactivityrel inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_seactivityrel.crmid where nectarcrm_crmentity.setype='Leads' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getHelpDeskRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select parent_id from nectarcrm_troubletickets inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_troubletickets.parent_id where ticketid=? and nectarcrm_crmentity.setype='Accounts'";
        $result=$adb->pquery($query, array($record_id));
...
function getQuotesRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from nectarcrm_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getQuotesRelatedPotentials($record_id)
{
	...
	global $adb;
        $query="select potentialid from nectarcrm_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getSalesOrderRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from nectarcrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getSalesOrderRelatedPotentials($record_id)
{
	...
	global $adb;
        $query="select potentialid from nectarcrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $potid=$adb->query_result($result,0,'potentialid');
...
function getSalesOrderRelatedQuotes($record_id)
{
	...
	global $adb;
        $query="select quoteid from nectarcrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getInvoiceRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from nectarcrm_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getInvoiceRelatedSalesOrder($record_id)
{
	...
	global $adb;
        $query="select salesorderid from nectarcrm_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getAccessPickListValues($module)
{
	...
	$id = getTabid($module);
	$query = "select fieldname,columnname,fieldid,fieldlabel,tabid,uitype from nectarcrm_field where tabid = ? and uitype in ('15','33','55') and nectarcrm_field.presence in (0,2)";
	...
	for($i=0;$i < $adb->num_rows($result);$i++)
{
		...
		if (count($roleids) > 1)
	{
			$mulsel="select distinct $fieldname from nectarcrm_$fieldname inner join nectarcrm_role2picklist on nectarcrm_role2picklist.picklistvalueid = nectarcrm_$fieldname.picklist_valueid where roleid in (\"". implode($roleids,"\",\"") ."\") and picklistid in (select picklistid from nectarcrm_$fieldname) order by sortid asc";
	}
	else
	{
			$mulsel="select distinct $fieldname from nectarcrm_$fieldname inner join nectarcrm_role2picklist on nectarcrm_role2picklist.picklistvalueid = nectarcrm_$fieldname.picklist_valueid where roleid ='".$roleid."' and picklistid in (select picklistid from nectarcrm_$fieldname) order by sortid asc";
	}
...
function transferUserCurrency($old_cur, $new_cur) {
	...
	$sql = 'UPDATE nectarcrm_users SET currency_id=? WHERE currency_id=?';
...
function transferProductCurrency($old_cur, $new_cur) {
	...
	$prod_res = $adb->pquery("select productid from nectarcrm_products where currency_id = ?", array($old_cur));
	...
	if(count($prod_ids) > 0) {
		...
		for($i=0;$i<count($prod_ids);$i++) {
			...
			$query = "update nectarcrm_products set currency_id=?, unit_price=? where productid=?";
...
function transferPriceBookCurrency($old_cur, $new_cur) {
	...
	$pb_res = $adb->pquery("select pricebookid from nectarcrm_pricebook where currency_id = ?", array($old_cur));
...
function transferServicesCurrency($old_cur, $new_cur) {
	...
	$ser_res = $adb->pquery('SELECT serviceid FROM nectarcrm_service WHERE currency_id = ?', array($old_cur));
    ...
    if (count($ser_ids) > 0) {
        ...
        for ($i = 0; $i < count($ser_ids); $i++) {
			...
			$query = 'UPDATE nectarcrm_service SET currency_id=?, unit_price=? WHERE serviceid=?';
...
/**
 * this function searches for a given number in nectarcrm and returns the callerInfo in an array format
...
function get_use_asterisk($id){
	...
	$sql = "select * from nectarcrm_asteriskextensions where userid = ?";
...
function addToCallHistory($userExtension, $callfrom, $callto, $status, $adb, $useCallerInfo){
	$sql = "select * from nectarcrm_asteriskextensions where asterisk_extension=?";
	$result = $adb->pquery($sql,array($userExtension));
	$userID = $adb->query_result($result, 0, "userid");
	if(empty($userID)) {
		// we have observed call to extension not configured in nectarcrm will returns NULL
		return;
	}
	if(empty($callfrom)){
		$callfrom = "Unknown";
	...
	$crmID = $adb->getUniqueID('nectarcrm_crmentity');
	$timeOfCall = date('Y-m-d H:i:s');

	$query = "INSERT INTO nectarcrm_crmentity (crmid,smcreatorid,smownerid,modifiedby,setype,description,createdtime,
	...
	$sql = "insert into nectarcrm_pbxmanager (pbxmanagerid,callfrom,callto,timeofcall,status)values (?,?,?,?,?)";
...
function getRelationTables($module,$secmodule){
	...
	$ui10_query = $adb->pquery("SELECT nectarcrm_field.tabid AS tabid,nectarcrm_field.tablename AS tablename, nectarcrm_field.columnname AS columnname FROM nectarcrm_field INNER JOIN nectarcrm_fieldmodulerel ON nectarcrm_fieldmodulerel.fieldid = nectarcrm_field.fieldid WHERE (nectarcrm_fieldmodulerel.module=? AND nectarcrm_fieldmodulerel.relmodule=?) OR (nectarcrm_fieldmodulerel.module=? AND nectarcrm_fieldmodulerel.relmodule=?)",array($module,$secmodule,$secmodule,$module));
	...
	if(is_array($reltables) && !empty($reltables)){
		...
		$rel_array = array("nectarcrm_crmentityrel"=>array("crmid","relcrmid"),"".$primary_obj->table_name."" => "".$primary_obj->table_index."");
	}
	return $rel_array;
}
...
function installVtlibModule($packagename, $packagepath, $customized=false) {
	global $log, $nectarcrm_Utils_Log, $_installOrUpdateVtlibModule;
	...
	$nectarcrm_Utils_Log = defined('INSTALLATION_MODE_DEBUG')? INSTALLATION_MODE_DEBUG : true;
	...
	if($package->isLanguageType()) {
		require_once('vtlib/nectarcrm/Language.php');
		$languagePack = new nectarcrm_Language();
	...
	} else if(nectarcrm_Module::getInstance($module)) {
...
function com_vtGetModules($adb) {
	$sql="select distinct nectarcrm_field.tabid, name
		from nectarcrm_field
		inner join nectarcrm_tab
			on nectarcrm_field.tabid=nectarcrm_tab.tabid
		where nectarcrm_field.tabid not in(9,10,16,15,29) and nectarcrm_tab.presence = 0 and nectarcrm_tab.isentitytype=1";
...
function isRecordExists($recordId) {
	global $adb;
	$query = "SELECT crmid FROM nectarcrm_crmentity where crmid=? AND deleted=0";
...
function getTabInfo($tabId) {
	...
	$tabInfoResult = $adb->pquery('SELECT prefname, prefvalue FROM nectarcrm_tab_info WHERE tabid=?', array($tabId));
	$tabInfo = array();
...
function getBlockName($blockid) {
	...
	if(!empty($blockid) && $blockname === false){
		$block_res = $adb->pquery('SELECT blocklabel FROM nectarcrm_blocks WHERE blockid = ?',array($blockid));
...
function isLeadConverted($leadId) {
	...
	$query = 'SELECT converted FROM nectarcrm_leaddetails WHERE converted = 1 AND leadid=?';
	$params = array($leadId);
...
function getSelectAllQuery($input,$module) {
	...
	if($module == "Calendar") {
		...
		if($module == 'Documents') {
			...
			$query .= " AND nectarcrm_notes.folderid in (".$folderid.")";
		}
	}
...
function getCampaignAccountIds($id) {
	global $adb;
	$sql="SELECT nectarcrm_account.accountid as id FROM nectarcrm_account
		INNER JOIN nectarcrm_campaignaccountrel ON nectarcrm_campaignaccountrel.accountid = nectarcrm_account.accountid
		LEFT JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_account.accountid
		WHERE nectarcrm_campaignaccountrel.campaignid = ? AND nectarcrm_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
...
function getCampaignContactIds($id) {
	global $adb;
	$sql="SELECT nectarcrm_contactdetails.contactid as id FROM nectarcrm_contactdetails
		INNER JOIN nectarcrm_campaigncontrel ON nectarcrm_campaigncontrel.contactid = nectarcrm_contactdetails.contactid
		LEFT JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_contactdetails.contactid
		WHERE nectarcrm_campaigncontrel.campaignid = ? AND nectarcrm_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
...
function getCampaignLeadIds($id) {
	global $adb;
	$sql="SELECT nectarcrm_leaddetails.leadid as id FROM nectarcrm_leaddetails
		INNER JOIN nectarcrm_campaignleadrel ON nectarcrm_campaignleadrel.leadid = nectarcrm_leaddetails.leadid
		LEFT JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_leaddetails.leadid
		WHERE nectarcrm_campaignleadrel.campaignid = ? AND nectarcrm_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
...
function getRecordOwnerReportsToId($record) {
    ...
    if ($ownerArr['Users']) {
        $query = "SELECT reports_to_id FROM nectarcrm_users WHERE id = ?";
...
function getRecordGroupId($record) {
	...
	if ($groupId === false) {
		$query = "SELECT smgroupid FROM nectarcrm_crmentity WHERE crmid = ?";
		$result = $adb->pquery($query, array($record));
		if ($adb->num_rows($result) > 0) {
...
function sendMailToUserOnDuplicationPrevention($moduleName, $fieldData, $mailBody, $userModel = '') {
	...
	$mailer->Subject = vtranslate('LBL_NECTARCRM_NOTIFICATION');
	$body = $mailBody;
		...
		if ($fieldModel->isUniqueField() && $fieldModel->isViewable()) {
			...
			switch($fieldModel->getFieldDataType()) {
				case 'reference'		:	list($refModuleId, $refRecordId) = vtws_getIdComponents($fieldValue);
											$fieldValue = nectarcrm_Functions::getCRMRecordLabel($refRecordId);
											break;
				case 'date'				:
				case 'datetime'			:
				case 'currency'			:
...
function getDuplicatesPreventionMessage($moduleName, $duplicateRecordsList) {
	...
	for ($i=0; $i<$duplicateRecordsCount && $i<5; $i++) {
		...
		$dupliRecordModel = new nectarcrm_Record_Model();
		$dupliRecordModel->setId($dupliRecordId)->setModuleFromInstance($moduleModel);
		$message .= '<a href="'.$dupliRecordModel->getDetailViewUrl().'" target="_blank" style="color:#15c !important">'.nectarcrm_Functions::getCRMRecordLabel($dupliRecordId).'</a><br>';
	}
	...
	if ($duplicateRecordsCount === 6) {
		...
		foreach ($uniqueFields as $fieldModel) {
			...
			switch($fieldDataType) {
				case 'reference'		:	$fieldValue = nectarcrm_Functions::getCRMRecordLabel($fieldValue);
											break;
				case 'date'				:
				case 'datetime'			:
				case 'currency'			:
