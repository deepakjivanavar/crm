...
function is_admin($user) {
	return nectarcrm_Functions::userIsAdministrator($user);
}

function parse_calendardate($local_format) {
	return nectarcrm_Functions::currentUserJSDateFormat($local_format);
}

function from_html($string, $encode = true) {
	return nectarcrm_Functions::fromHTML($string, $encode);
}

function fck_from_html($string) {
	return nectarcrm_Functions::fromHTML_FCK($string);
}

function popup_from_html($string, $encode = true) {
	return nectarcrm_Functions::fromHTML_Popup($string, $encode);
}

function fetchCurrency($id) {
	return nectarcrm_Functions::userCurrencyId($id);
}

function getCurrencyName($currencyid, $show_symbol = true) {
	return nectarcrm_Functions::getCurrencyName($currencyid, $show_symbol);
}

function getTabid($module) {
	return nectarcrm_Functions::getModuleId($module);
	}

function getFieldid($tabid, $fieldname, $onlyactive = true) {
	return nectarcrm_Functions::getModuleFieldId($tabid, $fieldname, $onlyactive);
}

function getTabOwnedBy($module) {
	return nectarcrm_Functions::getModuleOwner($module);
}

function getSalesEntityType($crmid) {
	return nectarcrm_Functions::getCRMRecordType($crmid);
}

function getAccountName($account_id) {
	return nectarcrm_Functions::getCRMRecordLabel($account_id);
}

function getProductName($product_id) {
	return nectarcrm_Functions::getCRMRecordLabel($product_id);
}

function getPotentialName($potential_id) {
	return nectarcrm_Functions::getCRMRecordLabel($potential_id);
}

function getContactName($contact_id) {
	return nectarcrm_Functions::getCRMRecordLabel($contact_id);
}

function getFullNameFromQResult($result, $row_count, $module) {
	return nectarcrm_Deprecated::getFullNameFromQResult($result, $row_count, $module);
}

function getFullNameFromArray($module, $fieldValues) {
	return nectarcrm_Deprecated::getFullNameFromArray($module, $fieldValues);
}

function getCampaignName($campaign_id) {
	return nectarcrm_Functions::getCRMRecordLabel($campaign_id);
}

function getVendorName($vendor_id) {
	return nectarcrm_Functions::getCRMRecordLabel($vendor_id);
}

function getQuoteName($quote_id) {
	return nectarcrm_Functions::getCRMRecordLabel($quote_id);
}

function getPriceBookName($pricebookid) {
	return nectarcrm_Functions::getCRMRecordLabel($pricebookid);
}

function getSoName($so_id) {
	return nectarcrm_Functions::getCRMRecordLabel($so_id);
}

function getGroupName($groupid) {
	return nectarcrm_Functions::getGroupName($groupid);
}

function getUserName($userid) {
	return nectarcrm_Functions::getUserName($userid);
}

function getUserFullName($userid) {
	return nectarcrm_Functions::getUserRecordLabel($userid);
}

function getParentName($parent_id) {
	return nectarcrm_Functions::getCRMRecordLabel($parent_id);
	}

function getValidDisplayDate($cur_date_val) {
	return nectarcrm_Functions::currentUserDisplayDate($cur_date_val);
}

function getNewDisplayDate() {
	return nectarcrm_Functions::currentUserDisplayDateNew();
}

/** This function returns the conversion rate and nectarcrm_currency symbol
 * in array format for a given id.
 * param $id - nectarcrm_currency id.
 */
function getCurrencySymbolandCRate($id) {
	return nectarcrm_Functions::getCurrencySymbolandRate($id);
}
...
function getTermsAndConditions($moduleName) {
	return nectarcrm_Functions::getInventoryTermsAndCondition($moduleName);
}
 ...
 */
function br2nl($str) {
	return nectarcrm_Functions::br2nl($str);
}
...
function getBlockId($tabid, $label) {
	return nectarcrm_Deprecated::getBlockId($tabid, $label);
}
...
function getParentTabFromModule($module) {
	return nectarcrm_Deprecated::getParentTabFromModule($module);
}

/**
 * This function is used to get the Parent Tab name for a given module.
 * Takes no parameter but gets the nectarcrm_parenttab value from form request
 * This returns value string type
 */
function getParentTab() {
	return nectarcrm_Deprecated::getParentTab();
}
...
function setObjectValuesFromRequest($focus) {
	return nectarcrm_Deprecated::copyValuesFromRequest($focus);
}

function create_tab_data_file() {
	return nectarcrm_Deprecated::createModuleMetaFile();
}

/**
 * Function to write the nectarcrm_parenttabid and name to a flat file parent_tabdata.txt so that the data
 * is obtained from the file instead of repeated queries
 * returns null
 */
function create_parenttab_data_file() {
	return nectarcrm_Deprecated::createModuleGroupMetaFile();
}

function getEntityName($module, $ids_list, $compute=true) {
	if ($compute) {
		return nectarcrm_Functions::computeCRMRecordLabels($module, $ids_list);
	} else {
		return nectarcrm_Functions::getCRMRecordLabels($module, $ids_list);
	}
}
...
function decideFilePath() {
	return nectarcrm_Functions::initStorageFileDirectory();
}

/**
 * 	This function is used to check whether the attached file is a image file or not
 * 	@param string $file_details  - nectarcrm_files array which contains all the uploaded file details
...
function validateImageFile($file_details) {
	return nectarcrm_Functions::validateImage($file_details);
}
...
function getTemplateDetails($templateid) {
	return nectarcrm_Deprecated::getTemplateDetails($templateid);
}
...
function getMergedDescription($description, $id, $parent_type, $removeTags = false) {
	return nectarcrm_Functions::getMergedDescription($description, $id, $parent_type, $removeTags);
}
...
function getSingleFieldValue($tablename, $fieldname, $idname, $id) {
	return nectarcrm_Functions::getSingleFieldValue($tablename, $fieldname, $idname, $id);
}
...
function get_announcements() {
	return nectarcrm_Deprecated::getAnnouncements();
}
...
function getrecurringObjValue() {
	return nectarcrm_Functions::getRecurringObjValue();
}

function getTranslatedString($str, $module = 'nectarcrm', $language = '') {
	return nectarcrm_Functions::getTranslatedString($str, $module, $language);
}
...
function getTranslatedCurrencyString($str) {
	return nectarcrm_Deprecated::getTranslatedCurrencyString($str);
	}

function getTicketComments($ticketid) {
	return nectarcrm_Functions::getTicketComments($ticketid);
}

function makeRandomPassword() {
	return nectarcrm_Functions::generateRandomPassword();
}
...
function getCvIdOfAll($module) {
	return nectarcrm_Deprecated::getIdOfCustomViewByNameAll($module);
}
...
function getTagCloudView($id = "") {
	return nectarcrm_Functions::getTagCloudView($id);
}
...
function SaveTagCloudView($id = "") {
	return nectarcrm_Deprecated::SaveTagCloudView($id);
}
 ...
 * *	        -------------------+------------+----------------------+------------------+
 * *		Potential Name     | I~O        | nectarcrm_quotes        | potentialid      |
 * *
...
function ChangeTypeOfData_Filter($table_name, $column_name, $type_of_data) {
	return nectarcrm_Functions::transformFieldTypeOfData($table_name, $column_name, $type_of_data);
}
...
function clear_smarty_cache($path = null) {
	nectarcrm_Deprecated::clearSmartyCompiledFiles($path);
}
...
function get_smarty_compiled_file($template_file, $path = null) {
	return nectarcrm_Deprecated::getSmartyCompiledTemplateFile($template_file, $path);
}

/** Function to carry out all the necessary actions after migration */
function perform_post_migration_activities() {
	nectarcrm_Deprecated::postApplicationMigrationTasks();
}
...
function getPickListValues($tablename, $roleid) {
	return nectarcrm_Functions::getPickListValuesFromTableForRole($tablename, $roleid);
}
...
function checkFileAccessForInclusion($filepath) {
	nectarcrm_Deprecated::checkFileAccessForInclusion($filepath);
}

/** Function to check the file deletion within the deletable (safe) directories*/
function checkFileAccessForDeletion($filepath) {
	nectarcrm_Deprecated::checkFileAccessForDeletion($filepath);
}

/** Function to check the file access is made within web root directory. */
function checkFileAccess($filepath) {
	nectarcrm_Deprecated::checkFileAccess($filepath);
}
...
function isFileAccessible($filepath) {
	return nectarcrm_Deprecated::isFileAccessible($filepath);
}
...
function getActivityType($id) {
	return nectarcrm_Functions::getActivityType($id);
}

/** Function to get owner name either user or group */
function getOwnerName($id) {
	return nectarcrm_Functions::getOwnerRecordLabel($id);
}

/** Function to get owner name either user or group */
function getOwnerNameList($idList) {
	return nectarcrm_Functions::getOwnerRecordLabels($idList);
}
...
function getSettingsBlockId($label) {
	return nectarcrm_Deprecated::getSettingsBlockId($label);
}
...
function getEntityField($module) {
	return nectarcrm_Functions::getEntityModuleSQLColumnString($module);
}

/**
 * this function returns the entity information for a given module; for e.g. for Contacts module
 * it returns the information of tablename, modulename, fieldsname and id gets from nectarcrm_entityname
 * @param string $module - the module name
 * @return array $data - the entity information for the module
 */
function getEntityFieldNames($module) {
	return nectarcrm_Functions::getEntityModuleInfoFieldsFormatted($module);
}
...
function getEntityFieldNameDisplay($module, $fieldsName, $fieldValues) {
	return nectarcrm_Deprecated::getCurrentUserEntityFieldNameDisplay($module, $fieldsName, $fieldValues);
}

// nectarcrm cache utility
require_once('include/utils/VTCacheUtils.php');

// vtlib customization: Extended nectarcrm CRM utlitiy functions
require_once('include/utils/VtlibUtils.php');

// END

function vt_suppressHTMLTags($string) {
	return nectarcrm_Functions::suppressHTMLTags($string);
}

function getSqlForNameInDisplayFormat($input, $module, $glue = ' ') {
	return nectarcrm_Deprecated::getSqlForNameInDisplayFormat($input, $module, $glue);
}

function getModuleSequenceNumber($module, $recordId) {
	return nectarcrm_Deprecated::getModuleSequenceNumber($module, $recordId);
	}

function getInvoiceStatus($invoiceId) {
	return nectarcrm_Functions::getInvoiceStatus($invoiceId);
}

function decimalFormat($value){
	return nectarcrm_Functions::formatDecimal($value);
}

function updateRecordLabel($module,$recordId){
	return nectarcrm_Functions::updateCRMRecordLabel($module, $recordId);
}

function get_group_options() {
    return nectarcrm_Functions::get_group_options();
}
