 ...
 * Param $ext_prod_arr - existing nectarcrm_products
...
function sendPrdStckMail($product_id,$upd_qty,$prod_name,$qtyinstk,$qty,$module)
{
	...
	{	
		...
		$query = "select * from nectarcrm_inventorynotification where notificationname=?";
...
function getPrdReOrderLevel($product_id)
{
	...
	global $adb;
	$query1 = "SELECT reorderlevel FROM nectarcrm_products WHERE productid = ?";
 ...
 *	return int   $taxid - taxid corresponding to the Tax type from nectarcrm_inventorytaxinfo nectarcrm_table
 */
function getTaxId($type)
{
	...
	$res = $adb->pquery("SELECT taxid FROM nectarcrm_inventorytaxinfo WHERE taxname=?", array($type));
 ...
 *	@param string $type       - tax type (VAT or Sales or Service)
 *	return int $taxpercentage - taxpercentage corresponding to the Tax type from nectarcrm_inventorytaxinfo nectarcrm_table
 */
function getTaxPercentage($type)
{
	...
	$res = $adb->pquery("SELECT percentage FROM nectarcrm_inventorytaxinfo WHERE taxname = ?", array($type));
...
function getProductTaxPercentage($type,$productid,$default='')
{
	...
	$res = $adb->pquery("SELECT taxpercentage, nectarcrm_producttaxrel.regions FROM nectarcrm_inventorytaxinfo
							INNER JOIN nectarcrm_producttaxrel ON nectarcrm_inventorytaxinfo.taxid = nectarcrm_producttaxrel.taxid
							WHERE nectarcrm_producttaxrel.productid = ? AND nectarcrm_inventorytaxinfo.taxname = ?", array($productid, $type));
...
function addInventoryHistory($module, $id, $relatedname, $total, $history_fldval)
{
	...
	$history_table_array = Array(
					"PurchaseOrder"=>"nectarcrm_postatushistory",
					"SalesOrder"=>"nectarcrm_sostatushistory",
					"Quotes"=>"nectarcrm_quotestagehistory",
					"Invoice"=>"nectarcrm_invoicestatushistory"
				    );
...
function getAllTaxes($available='all', $sh='',$mode='',$id='')
{
	...
	if($sh != '' && $sh == 'sh') {
		...
		$value_table='nectarcrm_inventoryshippingrel';
		if($mode == 'edit' && $id != '') {
		...
		$value_table='nectarcrm_inventoryproductrel';
		if($mode == 'edit' && $id != '' ) {
...
function getTaxDetailsForProduct($productid, $available='all')
{
	...
	if($productid != '')
	{
		...
		if($available != 'all' && $available == 'available')
		{
			$where = ' and nectarcrm_inventorytaxinfo.deleted=0';
		}
		if($available != 'all' && $available == 'available_associated')
		{
			$query = "SELECT nectarcrm_producttaxrel.*, nectarcrm_inventorytaxinfo.*, nectarcrm_inventorytaxinfo.regions AS taxregions, nectarcrm_producttaxrel.regions AS productregions FROM nectarcrm_inventorytaxinfo left JOIN nectarcrm_producttaxrel ON nectarcrm_inventorytaxinfo.taxid = nectarcrm_producttaxrel.taxid WHERE (nectarcrm_producttaxrel.productid = ? or nectarcrm_inventorytaxinfo.deleted=0) AND method != 'Deducted' GROUP BY nectarcrm_inventorytaxinfo.taxid";
		}
		else
		{
			$query = "SELECT nectarcrm_producttaxrel.*, nectarcrm_inventorytaxinfo.*, nectarcrm_inventorytaxinfo.regions AS taxregions, nectarcrm_producttaxrel.regions AS productregions FROM nectarcrm_inventorytaxinfo INNER JOIN nectarcrm_producttaxrel ON nectarcrm_inventorytaxinfo.taxid = nectarcrm_producttaxrel.taxid WHERE nectarcrm_producttaxrel.productid = ? AND method != 'Deducted' $where";
		}
...
function deleteInventoryProductDetails($focus)
{
	...
	$product_info = $adb->pquery("SELECT productid, quantity, sequence_no, incrementondel from nectarcrm_inventoryproductrel WHERE id=?",array($focus->id));
	...
	for($index = 0;$index <$numrows;$index++){
		...
		if($incrementondel){
			...
			$subProdQuery = $adb->pquery('SELECT productid, quantity from nectarcrm_inventorysubproductrel WHERE id=? AND sequence_no=?', array($focus->id,$sequence_no));
    ...
    $adb->pquery("delete from nectarcrm_inventoryproductrel where id=?", array($focus->id));
    $adb->pquery("delete from nectarcrm_inventorysubproductrel where id=?", array($focus->id));
    $adb->pquery("delete from nectarcrm_inventoryshippingrel where id=?", array($focus->id));
...
function updateInventoryProductRel($entity) {
	...
	if($statusChanged && $moduleName === 'Invoice') {
		if($recordDetails[$statusFieldName] == $statusFieldValue) {
			$adb->pquery("UPDATE nectarcrm_inventoryproductrel SET incrementondel=0 WHERE id=?",array($entity_id));
	...
	if($updateInventoryProductRel_deduct_stock) {
		$adb->pquery("UPDATE nectarcrm_inventoryproductrel SET incrementondel=1 WHERE id=?",array($entity_id));

		$product_info = $adb->pquery("SELECT productid,sequence_no, quantity from nectarcrm_inventoryproductrel WHERE id=?",array($entity_id));
		$numrows = $adb->num_rows($product_info);
		for($index = 0;$index <$numrows;$index++) {
			$productid = $adb->query_result($product_info,$index,'productid');
			$qty = $adb->query_result($product_info,$index,'quantity');
			...
			$sub_prod_query = $adb->pquery("SELECT productid, quantity from nectarcrm_inventorysubproductrel WHERE id=? AND sequence_no=?",array($entity_id,$sequence_no));
	...
	if ($moduleName === 'PurchaseOrder') {
		...
		if ($statusFieldValue === 'Cancelled') {
			...
			if ($statusChanged && $itemQuantitiesList) {
				...
				$query = "SELECT postvalue FROM nectarcrm_modtracker_detail
							INNER JOIN nectarcrm_modtracker_basic ON nectarcrm_modtracker_basic.id = nectarcrm_modtracker_detail.id
							WHERE crmid = ? AND fieldname = ? ORDER BY nectarcrm_modtracker_detail.id DESC";
...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
		...
		/*else{
			$desc_duery = "select nectarcrm_crmentity.description AS product_description from nectarcrm_crmentity where nectarcrm_crmentity.crmid=?";
		...
		$query = 'INSERT INTO nectarcrm_inventoryproductrel(id, productid, sequence_no, quantity, listprice, comment, description, purchase_cost, margin)
					VALUES(?,?,?,?,?,?,?,?,?)';
		...
		if (!empty($sub_prod_str)) {
			 ...
			 foreach ($sub_prod as $subProductInfo) {
				 ...
				 $query = 'INSERT INTO nectarcrm_inventorysubproductrel VALUES(?, ?, ?, ?)';
				 if (!$subProductQty) {
					 $subProductQty = 1;
				 }
		...
		$updatequery = "update nectarcrm_inventoryproductrel set ";
		$updateparams = array();
	...
	//to save the S&H tax details in nectarcrm_inventoryshippingrel table
	$adb->pquery('DELETE FROM nectarcrm_inventorychargesrel WHERE recordid = ?', array($focus->id));
	$chargesInfo = array();
	if (isset($_REQUEST['charges'])) {
		$chargesInfo = $_REQUEST['charges'];
	}
	$adb->pquery('INSERT INTO nectarcrm_inventorychargesrel VALUES (?, ?)', array($focus->id, Zend_Json::encode($chargesInfo)));
...
function getInventoryCurrencyInfo($module, $id)
{
	...
	$res = $adb->pquery("select currency_id, $inventory_table.conversion_rate as conv_rate, nectarcrm_currency_info.* from $inventory_table
						inner join nectarcrm_currency_info on $inventory_table.currency_id = nectarcrm_currency_info.id
...
function getInventoryProductTaxValue($id, $productId, $taxName, $lineItemId = 0)
{
	...
	$query = "SELECT $taxName FROM nectarcrm_inventoryproductrel WHERE id = ? AND productid = ?";
...
function getInventorySHTaxPercent($id, $taxname)
{
	...
	$res = $adb->pquery("select $taxname from nectarcrm_inventoryshippingrel where id= ?", array($id));
...
function getAllCurrencies($available='available') {
	...
	$sql = "select * from nectarcrm_currency_info";
	if ($available != 'all') {
...
function getPriceDetailsForProduct($productid, $unit_price, $available='available', $itemtype='Products')
{
	...
	if($productid != '')
	{
		...
		if ($available == 'available_associated') {
			$query = "select nectarcrm_currency_info.*, nectarcrm_productcurrencyrel.converted_price, nectarcrm_productcurrencyrel.actual_price
					from nectarcrm_currency_info
					inner join nectarcrm_productcurrencyrel on nectarcrm_currency_info.id = nectarcrm_productcurrencyrel.currencyid
					where nectarcrm_currency_info.currency_status = 'Active' and nectarcrm_currency_info.deleted=0
					and nectarcrm_productcurrencyrel.productid = ? and nectarcrm_currency_info.id != ?";
			$params = array($productid, $product_currency_id);
		} else { // Edit View
			$query = "select nectarcrm_currency_info.*, nectarcrm_productcurrencyrel.converted_price, nectarcrm_productcurrencyrel.actual_price
					from nectarcrm_currency_info
					left join nectarcrm_productcurrencyrel
					on nectarcrm_currency_info.id = nectarcrm_productcurrencyrel.currencyid and nectarcrm_productcurrencyrel.productid = ?
					where nectarcrm_currency_info.currency_status = 'Active' and nectarcrm_currency_info.deleted=0";
			$params = array($productid);
		}
	...
	else
	{
		if($available == 'available') { // Create View
			...
			$query = "select nectarcrm_currency_info.* from nectarcrm_currency_info
					where nectarcrm_currency_info.currency_status = 'Active' and nectarcrm_currency_info.deleted=0";
			$params = array();
...
function getProductBaseCurrency($productid,$module='Products') {
	global $adb, $log;
	if ($module == 'Services') {
		$sql = "select currency_id from nectarcrm_service where serviceid=?";
	} else {
		$sql = "select currency_id from nectarcrm_products where productid=?";
	}
	$params = array($productid);
...
function getBaseConversionRateForProduct($productid, $mode='edit', $module='Products') {
	...
	if ($mode == 'edit') {
		if ($module == 'Services') {
			$sql = "select conversion_rate from nectarcrm_service inner join nectarcrm_currency_info
					on nectarcrm_service.currency_id = nectarcrm_currency_info.id where nectarcrm_service.serviceid=?";
		} else {
			$sql = "select conversion_rate from nectarcrm_products inner join nectarcrm_currency_info
					on nectarcrm_products.currency_id = nectarcrm_currency_info.id where nectarcrm_products.productid=?";
		}
		$params = array($productid);
	} else {
		$sql = "select conversion_rate from nectarcrm_currency_info where id=?";
...
function getPricesForProducts($currencyid, $product_ids, $module='Products', $skipActualPrice = false) {
	...
	if (count($product_ids) > 0) {
		if ($module == 'Services') {
			$query = "SELECT nectarcrm_currency_info.id, nectarcrm_currency_info.conversion_rate, " .
					"nectarcrm_service.serviceid AS productid, nectarcrm_service.unit_price, " .
					"nectarcrm_productcurrencyrel.actual_price " .
					"FROM (nectarcrm_currency_info, nectarcrm_service) " .
					"left join nectarcrm_productcurrencyrel on nectarcrm_service.serviceid = nectarcrm_productcurrencyrel.productid " .
					"and nectarcrm_currency_info.id = nectarcrm_productcurrencyrel.currencyid " .
					"where nectarcrm_service.serviceid in (". generateQuestionMarks($product_ids) .") and nectarcrm_currency_info.id = ?";
		} else {
			$query = "SELECT nectarcrm_currency_info.id, nectarcrm_currency_info.conversion_rate, " .
					"nectarcrm_products.productid, nectarcrm_products.unit_price, " .
					"nectarcrm_productcurrencyrel.actual_price " .
					"FROM (nectarcrm_currency_info, nectarcrm_products) " .
					"left join nectarcrm_productcurrencyrel on nectarcrm_products.productid = nectarcrm_productcurrencyrel.productid " .
					"and nectarcrm_currency_info.id = nectarcrm_productcurrencyrel.currencyid " .
					"where nectarcrm_products.productid in (". generateQuestionMarks($product_ids) .") and nectarcrm_currency_info.id = ?";
		}
...
function getPriceBookCurrency($pricebook_id) {
	global $adb;
	$result = $adb->pquery("select currency_id from nectarcrm_pricebook where pricebookid=?", array($pricebook_id));
...
function deductProductsFromStock($recordId) {
	global $adb;
	$adb->pquery("UPDATE nectarcrm_inventoryproductrel SET incrementondel=1 WHERE id=?",array($recordId));

	$product_info = $adb->pquery("SELECT productid,sequence_no, quantity from nectarcrm_inventoryproductrel WHERE id=?",array($recordId));
	$numrows = $adb->num_rows($product_info);
	for($index = 0;$index <$numrows;$index++) {
		$productid = $adb->query_result($product_info,$index,'productid');
		$qty = $adb->query_result($product_info,$index,'quantity');
		...
		$upd_qty = $qtyinstk-$qty;
		updateProductQty($productid, $upd_qty);
		$sub_prod_query = $adb->pquery("SELECT productid, quantity FROM nectarcrm_inventorysubproductrel WHERE id=? AND sequence_no=?",array($recordId,$sequence_no));
		if($adb->num_rows($sub_prod_query)>0) {
			for($j=0;$j<$adb->num_rows($sub_prod_query);$j++) {
				$sub_prod_id = $adb->query_result($sub_prod_query,$j,"productid");
				$subProductQty = $adb->query_result($sub_prod_query, $j, 'quantity'); 
...
function createRecords($obj) {
		...
		for ($j = 0; $j < $count; ++$j) {
			...
			if ($subjectRow['productid'] == '' || $subjectRow['quantity'] == '' || $subjectRow['listprice'] == '') {
				...
				foreach ($fieldMapping as $fieldName => $index) {
					if($moduleFields[$fieldName]->getTableName() == 'nectarcrm_inventoryproductrel') {
	...
	if ($createdRecords) {
        ...
        $query = "UPDATE nectarcrm_crmentity SET label= CASE crmid";
...
function importRecord($obj, $inventoryFieldData, $lineItemDetails) {
	...
	$webserviceObject = nectarcrmWebserviceObject::fromName($adb, $moduleName);
	$inventoryOperation = new nectarcrmInventoryOperation($webserviceObject, $obj->user, $adb, $log);
...
function getInventoryFieldsForExport($tableName) {
	...
	$sql .= 'nectarcrm_currency_info.currency_name as "Currency" ';
...
function getCurrencyId($fieldValue) {
	...
	$sql = 'SELECT id FROM nectarcrm_currency_info WHERE currency_name = ? AND deleted = 0';
...
function getLineItemFields(){
	...
	$sql = 'SELECT DISTINCT columnname FROM nectarcrm_field WHERE tablename=?';
	$result = $adb->pquery($sql, array('nectarcrm_inventoryproductrel'));
...
function getAllCharges() {
	...
	$result = $db->pquery('SELECT * FROM nectarcrm_inventorycharges WHERE deleted = 0', array());
...
function getAllRegions() {
	...
	$result = $db->pquery('SELECT * FROM nectarcrm_taxregions', array());
	while($rowData = $db->fetch_array($result)) {
...
function getCompoundTaxesInfoForInventoryRecord($recordId, $moduleName) {
	...
	$tableName = '';
	switch($moduleName) {
		case 'Quotes'		: $tableName = 'nectarcrm_quotes';			$index = 'quoteid';			break;
		case 'Invoice'		: $tableName = 'nectarcrm_invoice';		$index = 'invoiceid';		break;
		case 'SalesOrder'	: $tableName = 'nectarcrm_salesorder';		$index = 'salesorderid';	break;
		case 'PurchaseOrder': $tableName = 'nectarcrm_purchaseorder';	$index = 'purchaseorderid';	break;
	}
