...
include_once 'vtlib/nectarcrm/PDF/models/Model.php';
include_once 'vtlib/nectarcrm/PDF/inventory/HeaderViewer.php';
include_once 'vtlib/nectarcrm/PDF/inventory/FooterViewer.php';
include_once 'vtlib/nectarcrm/PDF/inventory/ContentViewer.php';
include_once 'vtlib/nectarcrm/PDF/inventory/ContentViewer2.php';
include_once 'vtlib/nectarcrm/PDF/viewers/PagerViewer.php';
include_once 'vtlib/nectarcrm/PDF/PDFGenerator.php';
include_once 'data/CRMEntity.php';

class nectarcrm_InventoryPDFController {
	...
	function getPDFGenerator() {
		return new nectarcrm_PDF_Generator();
	}

	function getContentViewer() {
		if($this->focusColumnValue('hdnTaxType') == "individual") {
			$contentViewer = new nectarcrm_PDF_InventoryContentViewer();
		} else {
			$contentViewer = new nectarcrm_PDF_InventoryTaxGroupContentViewer();
		}
	...
	function getHeaderViewer() {
		$headerViewer = new nectarcrm_PDF_InventoryHeaderViewer();
		$headerViewer->setModel($this->buildHeaderModel());
		return $headerViewer;
	}

	function getFooterViewer() {
		$footerViewer = new nectarcrm_PDF_InventoryFooterViewer();
	...
	function getPagerViewer() {
		$pagerViewer = new nectarcrm_PDF_PagerViewer();
	...
	function buildContentModels() {
		...
		foreach($associated_products as $productLineItem) {
			...
			$contentModel = new nectarcrm_PDF_Model();
	...
	function buildContentLabelModel() {
		$labelModel = new nectarcrm_PDF_Model();
	...
	function buildSummaryModel() {
		...
		$summaryModel = new nectarcrm_PDF_Model();
	...
	function buildHeaderModel() {
		$headerModel = new nectarcrm_PDF_Model();
	...
	function buildHeaderModelColumnLeft() {
		...
		// Company information
		$result = $adb->pquery("SELECT * FROM nectarcrm_organizationdetails", array());
	...
	function buildFooterModel() {
		$footerModel = new nectarcrm_PDF_Model();
		$footerModel->set(nectarcrm_PDF_InventoryFooterViewer::$DESCRIPTION_DATA_KEY, from_html($this->focusColumnValue('description')));
		$footerModel->set(nectarcrm_PDF_InventoryFooterViewer::$TERMSANDCONDITION_DATA_KEY, from_html($this->focusColumnValue('terms_conditions')));
		return $footerModel;
	}

	function buildFooterLabelModel() {
		$labelModel = new nectarcrm_PDF_Model();
		$labelModel->set(nectarcrm_PDF_InventoryFooterViewer::$DESCRIPTION_LABEL_KEY, getTranslatedString('Description',$this->moduleName));
		$labelModel->set(nectarcrm_PDF_InventoryFooterViewer::$TERMSANDCONDITION_LABEL_KEY, getTranslatedString('Terms & Conditions',$this->moduleName));
		return $labelModel;
	}
	...
	function buildWatermarkModel() {
		$watermarkModel = new nectarcrm_PDF_Model();
	...
	function buildCurrencySymbol() {
		...
		if(!empty($currencyId)) {
			$result = $adb->pquery("SELECT currency_symbol FROM nectarcrm_currency_info WHERE id=?", array($currencyId));
