...
include_once 'vtlib/nectarcrm/Functions.php';
...
$NAMESPACE = 'http://www.nectarcrm.com/products/crm';
 ...
 */
class nectarcrm_Soap_CustomerPortal {
...
function _getTicketModComments($ticketId) {
	global $adb;
	$sql = "SELECT * FROM nectarcrm_modcomments
			INNER JOIN nectarcrm_crmentity ON nectarcrm_modcomments.modcommentsid = nectarcrm_crmentity.crmid AND deleted = 0
	...
	for($i=0; $i<$rows; $i++) {
		...
		if(!empty($customer)) {
			$emailResult = $adb->pquery('SELECT * FROM nectarcrm_portalinfo WHERE id = ?', array($customer));
...
function get_combo_values($input_array)
{
	...
	$output = Array();
	$sql = "select  productid, productname from nectarcrm_products inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_products.productid where nectarcrm_crmentity.deleted=0";
	...
	$roleres = $adb->pquery("SELECT roleid from nectarcrm_user2role where userid = ?",array($userid));
	$RowCount = $adb->num_rows($roleres);
	if($RowCount > 0){
		$admin_role = $adb->query_result($roleres,0,'roleid');
	}
	$result1 = $adb->pquery("select nectarcrm_ticketpriorities.ticketpriorities from nectarcrm_ticketpriorities inner join nectarcrm_role2picklist on nectarcrm_role2picklist.picklistvalueid = nectarcrm_ticketpriorities.picklist_valueid and nectarcrm_role2picklist.roleid='$admin_role' order by sortorderid", array());
	for($i=0;$i<$adb->num_rows($result1);$i++)
	{
		$output['ticketpriorities']['ticketpriorities'][$i] = $adb->query_result($result1,$i,"ticketpriorities");
	}

	$result2 = $adb->pquery("select nectarcrm_ticketseverities.ticketseverities from nectarcrm_ticketseverities inner join nectarcrm_role2picklist on nectarcrm_role2picklist.picklistvalueid = nectarcrm_ticketseverities.picklist_valueid and nectarcrm_role2picklist.roleid='$admin_role' order by sortorderid", array());
	for($i=0;$i<$adb->num_rows($result2);$i++)
	{
		$output['ticketseverities']['ticketseverities'][$i] = $adb->query_result($result2,$i,"ticketseverities");
	}

	$result3 = $adb->pquery("select nectarcrm_ticketcategories.ticketcategories from nectarcrm_ticketcategories inner join nectarcrm_role2picklist on nectarcrm_role2picklist.picklistvalueid = nectarcrm_ticketcategories.picklist_valueid and nectarcrm_role2picklist.roleid='$admin_role' order by sortorderid", array());
	...
	if(!vtlib_isModuleActive('ServiceContracts')) {
		...
		$servicequery = "SELECT nectarcrm_servicecontracts.servicecontractsid,nectarcrm_servicecontracts.subject
							FROM nectarcrm_servicecontracts
							INNER JOIN nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_servicecontracts.servicecontractsid
									AND nectarcrm_crmentity.deleted = 0
							WHERE nectarcrm_servicecontracts.sc_related_to = ?";
		$params = array($id);
		$showAll = show_all('HelpDesk');
		if($showAll == 'true') {
			$servicequery .= ' OR nectarcrm_servicecontracts.sc_related_to = (SELECT accountid FROM nectarcrm_contactdetails WHERE contactid=? AND accountid <> 0)
								OR nectarcrm_servicecontracts.sc_related_to IN
											(SELECT contactid FROM nectarcrm_contactdetails WHERE accountid =
													(SELECT accountid FROM nectarcrm_contactdetails WHERE contactid=? AND accountid <> 0))
							';
...
function get_KBase_details($input_array)
{
	...
	$category_query = "select nectarcrm_faqcategories.faqcategories from nectarcrm_faqcategories inner join nectarcrm_role2picklist on nectarcrm_role2picklist.picklistvalueid = nectarcrm_faqcategories.picklist_valueid and nectarcrm_role2picklist.roleid='$admin_role'";
	...
	if($check == true) {
		$product_query = "select productid, productname from nectarcrm_products inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_products.productid where nectarcrm_crmentity.deleted=0";
	...
	$faq_query = "select nectarcrm_faq.*, nectarcrm_crmentity.createdtime, nectarcrm_crmentity.modifiedtime from nectarcrm_faq " .
		"inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_faq.id " .
		"where nectarcrm_crmentity.deleted=0 and nectarcrm_faq.status='Published' order by nectarcrm_crmentity.modifiedtime DESC";
	...
	for($k=0;$k<$faq_noofrows;$k++)
	{
		...
		$faq_comment_query = "select * from nectarcrm_faqcomments where faqid=? order by createdtime DESC";
...
function save_faq_comment($input_array)
{
	...
	if(trim($comment) != '')
	{
		$faq_query = "insert into nectarcrm_faqcomments values(?,?,?,?)";
...
function get_tickets_list($input_array) {
	...
	{
		$contactquery = "SELECT contactid, accountid FROM nectarcrm_contactdetails " .
			" INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_contactdetails.contactid" .
			" AND nectarcrm_crmentity.deleted = 0 " .
			" WHERE (accountid = (SELECT accountid FROM nectarcrm_contactdetails WHERE contactid = ?)  AND accountid != 0) OR contactid = ?";
	...
	$query = "SELECT nectarcrm_troubletickets.*, nectarcrm_crmentity.smownerid,nectarcrm_crmentity.createdtime, nectarcrm_crmentity.modifiedtime, '' AS setype
		FROM nectarcrm_troubletickets
		INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_troubletickets.ticketid AND nectarcrm_crmentity.deleted = 0
		WHERE (nectarcrm_troubletickets.contact_id IN (". generateQuestionMarks($entity_ids_list) .")";
	if($acc_id) {
		$query .= " OR nectarcrm_troubletickets.parent_id = $acc_id) ";
	} else {
		$query .= ')';
	}
...
function create_ticket($input_array)
{
	...
	$accountResult = $adb->pquery('SELECT accountid FROM nectarcrm_contactdetails WHERE contactid = ?', array($parent_id));
	$accountId = $adb->query_result($accountResult, 0, 'accountid');
	if(!empty($accountId)) $ticket->column_fields['parent_id'] = $accountId;

	$ticket->save("HelpDesk");

	$ticketresult = $adb->pquery("select nectarcrm_troubletickets.ticketid from nectarcrm_troubletickets
		inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid = nectarcrm_troubletickets.ticketid inner join nectarcrm_ticketcf on nectarcrm_ticketcf.ticketid = nectarcrm_troubletickets.ticketid
		where nectarcrm_crmentity.deleted=0 and nectarcrm_troubletickets.ticketid = ?", array($ticket->id));
	...
	if($servicecontractid != ''){
		$res = $adb->pquery("insert into nectarcrm_crmentityrel values(?,?,?,?)",
...
function authenticate_user($username,$password,$version,$login = 'true')
{
	...
	$sql = "select id, user_name, user_password,last_login_time, support_start_date, support_end_date, cryptmode
				...
				where nectarcrm_crmentity.deleted=0 and user_name=?
					and isactive=1 and nectarcrm_customerdetails.portal=1
					and nectarcrm_customerdetails.support_start_date <= ? and nectarcrm_customerdetails.support_end_date >= ?";
	...
	if($login != 'false')
	{
		...
		$sql="insert into nectarcrm_soapservice values(?,?,?)";
...
function change_password($input_array)
{
	...
	$sql = "update nectarcrm_portalinfo set user_password=?, cryptmode=? where id=? and user_name=?";
	$result = $adb->pquery($sql, array(nectarcrm_Functions::generateEncryptedPassword($password), 'CRYPT', $id, $username));
...
function send_mail_for_password($mailid)
{
	...
	$sql = 'SELECT * FROM nectarcrm_portalinfo INNER JOIN nectarcrm_contactdetails ON nectarcrm_contactdetails.contactid=nectarcrm_portalinfo.id WHERE user_name = ?';
	...
	if (!empty($adb->query_result($res, 0, 'cryptmode'))) {
		...
		$enc_password = nectarcrm_Functions::generateEncryptedPassword($password);

		$sql = 'UPDATE nectarcrm_portalinfo SET user_password=?, cryptmode=? WHERE id=?';
...
function get_ticket_creator($input_array)
{
	...
	$res = $adb->pquery("select smcreatorid from nectarcrm_crmentity where crmid=?", array($ticketid));
...
function get_picklists($input_array)
{
	...
	$res = $adb->pquery("select nectarcrm_". $picklist_name.".* from nectarcrm_". $picklist_name." inner join nectarcrm_role2picklist on nectarcrm_role2picklist.picklistvalueid = nectarcrm_". $picklist_name.".picklist_valueid and nectarcrm_role2picklist.roleid='$admin_role'", array());
...
function get_ticket_attachments($input_array)
{
	...
	$query = "select nectarcrm_troubletickets.ticketid, nectarcrm_attachments.*,nectarcrm_notes.filename,nectarcrm_notes.filelocationtype from nectarcrm_troubletickets " .
		"left join nectarcrm_senotesrel on nectarcrm_senotesrel.crmid=nectarcrm_troubletickets.ticketid " .
		"left join nectarcrm_notes on nectarcrm_notes.notesid=nectarcrm_senotesrel.notesid " .
		...
		"left join nectarcrm_attachments on nectarcrm_attachments.attachmentsid = nectarcrm_seattachmentsrel.attachmentsid " .
		"and nectarcrm_crmentity.deleted = 0 where nectarcrm_troubletickets.ticketid =?";
...
function get_filecontent($input_array)
{
	...
	$query = 'SELECT nectarcrm_attachments.path FROM nectarcrm_attachments
	INNER JOIN nectarcrm_seattachmentsrel ON nectarcrm_seattachmentsrel.attachmentsid = nectarcrm_attachments.attachmentsid
	INNER JOIN nectarcrm_notes ON nectarcrm_notes.notesid = nectarcrm_seattachmentsrel.crmid
	INNER JOIN nectarcrm_senotesrel ON nectarcrm_senotesrel.notesid = nectarcrm_notes.notesid
	INNER JOIN nectarcrm_troubletickets ON nectarcrm_troubletickets.ticketid = nectarcrm_senotesrel.crmid
	WHERE nectarcrm_troubletickets.ticketid = ? AND nectarcrm_attachments.name = ? AND nectarcrm_attachments.attachmentsid = ?';
...
function add_ticket_attachment($input_array)
{
	...
	$attachmentid = $adb->getUniqueID("nectarcrm_crmentity");
	...
	$crmquery = "insert into nectarcrm_crmentity (crmid,setype,description,createdtime) values(?,?,?,?)";
	$crmresult = $adb->pquery($crmquery, array($attachmentid, 'HelpDesk Attachment', $description, $date_var));

	$attachmentquery = "insert into nectarcrm_attachments(attachmentsid,name,description,type,path) values(?,?,?,?,?)";
	$attachmentreulst = $adb->pquery($attachmentquery, array($attachmentid, $filename, $description, $filetype, $upload_filepath));

	$relatedquery = "insert into nectarcrm_seattachmentsrel values(?,?)";
	...
	$related_doc = 'insert into nectarcrm_seattachmentsrel values (?,?)';
	$res = $adb->pquery($related_doc,array($focus->id,$attachmentid));

	$tic_doc = 'insert into nectarcrm_senotesrel values(?,?)';
...
function getServerSessionId($id)
{
	...
	$sessionid = nectarcrm_Soap_CustomerPortal::lookupSessionId($id);
	if($sessionid === false) {
		$query = "select * from nectarcrm_soapservice where type='customer' and id=?";
		$result = $adb->pquery($query, array($id));
		if($adb->num_rows($result) > 0) {
			$sessionid = $adb->query_result($result,0,'sessionid');
			nectarcrm_Soap_CustomerPortal::updateSessionId($id, $sessionid);
		}
	}
	return $sessionid;
}
...
function unsetServerSessionId($id)
{
	...
	$id = (int) $id;
	nectarcrm_Soap_CustomerPortal::updateSessionId($id, false);

	$adb->pquery("delete from nectarcrm_soapservice where type='customer' and id=?", array($id));
...
function get_account_name($accountid)
{
	...
	$res = $adb->pquery("select accountname from nectarcrm_account where accountid=?", array($accountid));
...
function get_contact_name($contactid)
{
	...
	if($contactid != '')
	{
		$sql = "select firstname,lastname from nectarcrm_contactdetails where contactid=?";
...
function get_check_account_id($id)
{
	...
	$res = $adb->pquery("select accountid from nectarcrm_contactdetails where contactid=?", array($id));
...
function get_vendor_name($vendorid)
{
	...
	$res = $adb->pquery("select vendorname from nectarcrm_vendor where vendorid=?", array($vendorid));
...
function get_list_values($id,$module,$sessionid,$only_mine='true')
{
	...
	if($module == 'Quotes')
	{
		$query = "select distinct nectarcrm_quotes.*,nectarcrm_crmentity.smownerid,
		case when nectarcrm_quotes.contactid is not null then nectarcrm_quotes.contactid else nectarcrm_quotes.accountid end as entityid,
		case when nectarcrm_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype,
		nectarcrm_potential.potentialname,nectarcrm_account.accountid
		from nectarcrm_quotes left join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_quotes.quoteid
		...
		where nectarcrm_crmentity.deleted=0 and (nectarcrm_quotes.accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
	...
	{
		$query ="select distinct nectarcrm_invoice.*,nectarcrm_crmentity.smownerid,
		case when nectarcrm_invoice.contactid !=0 then nectarcrm_invoice.contactid else nectarcrm_invoice.accountid end as entityid,
		case when nectarcrm_invoice.contactid !=0 then 'Contacts' else 'Accounts' end as setype
		from nectarcrm_invoice
		left join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_invoice.invoiceid
		where nectarcrm_crmentity.deleted=0 and (accountid in (". generateQuestionMarks($entity_ids_list) .") or contactid in  (". generateQuestionMarks($entity_ids_list) ."))";
		$params = array($entity_ids_list,$entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}
	else if ($module == 'Documents')
	{
		$query ="select nectarcrm_notes.*, nectarcrm_crmentity.*, nectarcrm_senotesrel.crmid as entityid, '' as setype,nectarcrm_attachmentsfolder.foldername from nectarcrm_notes " .
		"inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid = nectarcrm_notes.notesid " .
		"left join nectarcrm_senotesrel on nectarcrm_senotesrel.notesid=nectarcrm_notes.notesid " .
		"LEFT JOIN nectarcrm_attachmentsfolder ON nectarcrm_attachmentsfolder.folderid = nectarcrm_notes.folderid " .
		"where nectarcrm_crmentity.deleted = 0 and  nectarcrm_senotesrel.crmid in (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}else if ($module == 'Contacts'){
		$query = "select nectarcrm_contactdetails.*,nectarcrm_crmentity.smownerid from nectarcrm_contactdetails
		 inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_contactdetails.contactid
		 where nectarcrm_crmentity.deleted = 0 and contactid IN (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
	}else if ($module == 'Assets') {
		$accountRes = $adb->pquery("SELECT accountid FROM nectarcrm_contactdetails
						INNER JOIN nectarcrm_crmentity ON nectarcrm_contactdetails.contactid = nectarcrm_crmentity.crmid
		...
		$query = "select nectarcrm_assets.*, nectarcrm_assets.account as entityid , nectarcrm_crmentity.smownerid from nectarcrm_assets
						inner join nectarcrm_crmentity on nectarcrm_assets.assetsid = nectarcrm_crmentity.crmid
						left join nectarcrm_account on nectarcrm_account.accountid = nectarcrm_assets.account
						left join nectarcrm_products on nectarcrm_products.productid = nectarcrm_assets.product
						where nectarcrm_crmentity.deleted = 0 and account = ?";
		$params = array($accountid);
		$fields_list['Related To'] = 'entityid';
		}
	}else if ($module == 'Project') {
		$query = "SELECT nectarcrm_project.*, nectarcrm_crmentity.smownerid
					FROM nectarcrm_project
					INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_project.projectid
					WHERE nectarcrm_crmentity.deleted = 0 AND nectarcrm_project.linktoaccountscontacts IN (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
	...
	for( $j= 0;$j < $noofdata; $j++)
	{
		...
		foreach($fields_list as $fieldlabel =>$fieldname ) {
			...
			if($fieldname == 'product' && $module == 'Assets'){
				...
				$fres = $adb->pquery('select nectarcrm_products.productname from nectarcrm_products where productid=?',array($crmid));
...
function get_filecontent_detail($id,$folderid,$module,$customerid,$sessionid)
{
	...
	if($module == 'Documents')
	{
		$query="SELECT filetype FROM nectarcrm_notes WHERE notesid =?";
		$res = $adb->pquery($query, array($id));
		$filetype = $adb->query_result($res, 0, "filetype");
		updateDownloadCount($id);

		$fileidQuery = 'select attachmentsid from nectarcrm_seattachmentsrel where crmid = ?';
		$fileres = $adb->pquery($fileidQuery,array($id));
		$fileid = $adb->query_result($fileres,0,'attachmentsid');

		$filepathQuery = 'select path,name from nectarcrm_attachments where attachmentsid = ?';
	...
	{
		$query ='select nectarcrm_attachments.*,nectarcrm_seattachmentsrel.* from nectarcrm_attachments inner join nectarcrm_seattachmentsrel on nectarcrm_seattachmentsrel.attachmentsid=nectarcrm_attachments.attachmentsid where nectarcrm_seattachmentsrel.crmid =?';
...
function updateDownloadCount($id){
	...
	$updateDownloadCount = "UPDATE nectarcrm_notes SET filedownloadcount = filedownloadcount+1 WHERE notesid = ?";
...
function get_salesorder_name($id)
{
	...
	$res = $adb->pquery(" select subject from nectarcrm_salesorder where salesorderid=?", array($id));
...
function get_invoice_detail($id,$module,$customerid,$sessionid)
{
	...
	$fieldquery = "SELECT fieldname, columnname, fieldlabel,block,uitype FROM nectarcrm_field WHERE tabid = ? AND displaytype in (1,2,4) ORDER BY block,sequence";
	$fieldres = $adb->pquery($fieldquery,array(getTabid($module)));
	$nooffields = $adb->num_rows($fieldres);
	$query = "select nectarcrm_invoice.*,nectarcrm_crmentity.* ,nectarcrm_invoicebillads.*,nectarcrm_invoiceshipads.*,
		nectarcrm_invoicecf.* from nectarcrm_invoice
		inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid = nectarcrm_invoice.invoiceid
		LEFT JOIN nectarcrm_invoicebillads ON nectarcrm_invoice.invoiceid = nectarcrm_invoicebillads.invoicebilladdressid
		LEFT JOIN nectarcrm_invoiceshipads ON nectarcrm_invoice.invoiceid = nectarcrm_invoiceshipads.invoiceshipaddressid
		INNER JOIN nectarcrm_invoicecf ON nectarcrm_invoice.invoiceid = nectarcrm_invoicecf.invoiceid
		where nectarcrm_invoice.invoiceid=?";
	$res = $adb->pquery($query, array($id));
	...
	for($i=0;$i<$nooffields;$i++)
	{
		...
		$blocknameQuery = "select blocklabel from nectarcrm_blocks where blockid = ?";
...
function get_product_list_values($id,$modulename,$sessionid,$only_mine='true')
{
	...
	$query = array();
	$params = array();
	$query[] = "SELECT nectarcrm_products.*,nectarcrm_seproductsrel.crmid as entityid, nectarcrm_seproductsrel.setype FROM nectarcrm_products
		INNER JOIN nectarcrm_crmentity on nectarcrm_products.productid = nectarcrm_crmentity.crmid
		LEFT JOIN nectarcrm_seproductsrel on nectarcrm_seproductsrel.productid = nectarcrm_products.productid
		WHERE nectarcrm_seproductsrel.crmid in (". generateQuestionMarks($entity_ids_list).") and nectarcrm_crmentity.deleted = 0 ";
	$params[] = array($entity_ids_list);

	$checkQuotes = checkModuleActive('Quotes');
	if($checkQuotes == true){
		$query[] = "select distinct nectarcrm_products.*,
			case when nectarcrm_quotes.contactid is not null then nectarcrm_quotes.contactid else nectarcrm_quotes.accountid end as entityid,
			case when nectarcrm_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype
			from nectarcrm_quotes INNER join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_quotes.quoteid
			...
			where nectarcrm_inventoryproductrel.productid = nectarcrm_products.productid AND nectarcrm_crmentity.deleted=0 and (accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
		$params[] = array($entity_ids_list,$entity_ids_list);
	}
...
function get_details($id,$module,$customerid,$sessionid)
{
	...
	if($module == 'Quotes'){
		$query =  "SELECT
			nectarcrm_quotes.*,nectarcrm_crmentity.*,nectarcrm_quotesbillads.*,nectarcrm_quotesshipads.*,
			nectarcrm_quotescf.* FROM nectarcrm_quotes
			...
			WHERE nectarcrm_quotes.quoteid=(". generateQuestionMarks($id) .") AND nectarcrm_crmentity.deleted = 0";

	}
	else if($module == 'Documents'){
		$query =  "SELECT
			nectarcrm_notes.*,nectarcrm_crmentity.*,nectarcrm_attachmentsfolder.foldername,nectarcrm_notescf.*
			...
			WHERE nectarcrm_notes.notesid=(". generateQuestionMarks($id) .") AND nectarcrm_crmentity.deleted=0";
	}
	else if($module == 'HelpDesk'){
		$query ="SELECT
			nectarcrm_troubletickets.*,nectarcrm_crmentity.smownerid,nectarcrm_crmentity.createdtime,nectarcrm_crmentity.modifiedtime,
			nectarcrm_ticketcf.*,nectarcrm_crmentity.description  FROM nectarcrm_troubletickets
			...
			WHERE (nectarcrm_troubletickets.ticketid=(". generateQuestionMarks($id) .") AND nectarcrm_crmentity.deleted = 0)";
	}
	else if($module == 'Services'){
		$query ="SELECT nectarcrm_service.*,nectarcrm_crmentity.*,nectarcrm_servicecf.*  FROM nectarcrm_service
			INNER JOIN nectarcrm_crmentity
				ON nectarcrm_crmentity.crmid = nectarcrm_service.serviceid AND nectarcrm_crmentity.deleted = 0
			LEFT JOIN nectarcrm_servicecf
				ON nectarcrm_service.serviceid = nectarcrm_servicecf.serviceid
			WHERE nectarcrm_service.serviceid= (". generateQuestionMarks($id) .")";
	}
	else if($module == 'Contacts'){
		$query = "SELECT nectarcrm_contactdetails.*,nectarcrm_contactaddress.*,nectarcrm_contactsubdetails.*,nectarcrm_contactscf.*" .
			" ,nectarcrm_crmentity.*,nectarcrm_customerdetails.*
			...
			WHERE nectarcrm_contactdetails.contactid = (". generateQuestionMarks($id) .") AND nectarcrm_crmentity.deleted = 0";
	}
	else if($module == 'Accounts'){
		$query = "SELECT nectarcrm_account.*,nectarcrm_accountbillads.*,nectarcrm_accountshipads.*,nectarcrm_accountscf.*,
			nectarcrm_crmentity.* FROM nectarcrm_account
			INNER JOIN nectarcrm_crmentity
		...
		" WHERE nectarcrm_account.accountid = (". generateQuestionMarks($id) .") AND nectarcrm_crmentity.deleted = 0";
	}
	else if ($module == 'Products'){
		$query = "SELECT nectarcrm_products.*,nectarcrm_productcf.*,nectarcrm_crmentity.* " .
		"FROM nectarcrm_products " .
		"INNER JOIN nectarcrm_crmentity " .
			"ON nectarcrm_crmentity.crmid = nectarcrm_products.productid " .
		"LEFT JOIN nectarcrm_productcf " .
			"ON nectarcrm_productcf.productid = nectarcrm_products.productid " .
		"LEFT JOIN nectarcrm_vendor
			ON nectarcrm_vendor.vendorid = nectarcrm_products.vendor_id " .
		"WHERE nectarcrm_products.productid = (". generateQuestionMarks($id) .") AND nectarcrm_crmentity.deleted = 0";
	} else if($module == 'Assets') {
		$query = "SELECT nectarcrm_assets.*, nectarcrm_assetscf.*, nectarcrm_crmentity.*
		FROM nectarcrm_assets
		INNER JOIN nectarcrm_crmentity
		ON nectarcrm_assets.assetsid = nectarcrm_crmentity.crmid
		INNER JOIN nectarcrm_assetscf
		ON nectarcrm_assetscf.assetsid = nectarcrm_assets.assetsid
		WHERE nectarcrm_crmentity.deleted = 0 AND nectarcrm_assets.assetsid = (". generateQuestionMarks($id) .")";
	} else if ($module == 'Project') {
		$query = "SELECT nectarcrm_project.*, nectarcrm_projectcf.*, nectarcrm_crmentity.*
					FROM nectarcrm_project
					INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_project.projectid
					LEFT JOIN nectarcrm_projectcf ON nectarcrm_projectcf.projectid = nectarcrm_project.projectid
					WHERE nectarcrm_project.projectid = ? AND nectarcrm_crmentity.deleted = 0";
	}

	$params = array($id);
	$res = $adb->pquery($query,$params);

	$fieldquery = "SELECT fieldname,columnname,fieldlabel,blocklabel,uitype FROM nectarcrm_field
		INNER JOIN  nectarcrm_blocks on nectarcrm_blocks.blockid=nectarcrm_field.block WHERE nectarcrm_field.tabid = ? AND displaytype in (1,2,4)
		ORDER BY nectarcrm_field.block,nectarcrm_field.sequence";
	...
	for($i=0;$i<$nooffields;$i++)
	{
		...
		if($module == 'Assets' ){
			if($fieldname == 'account'){
				...
				$accountres = $adb->pquery("select nectarcrm_account.accountname from nectarcrm_account where accountid=?",array($accountid));
				$accountname = $adb->query_result($accountres,0,'accountname');
				$fieldvalue = $accountname;
			}
			if($fieldname == 'product'){
				$productid = $adb->query_result($res,0,'product');
				$productres = $adb->pquery("select nectarcrm_products.productname from nectarcrm_products where productid=?",array($productid));
			...
			if($fieldname == 'invoiceid'){
				...
				$invoiceres = $adb->pquery("select nectarcrm_invoice.subject from nectarcrm_invoice where invoiceid=?",array($invoiceid));
...
function check_permission($customerid, $module, $entityid) {
	...
	$faqquery = "select id from nectarcrm_faq";
	...
	switch($module) {
		case 'Products'	: 	$query = "SELECT nectarcrm_seproductsrel.productid FROM nectarcrm_seproductsrel
								INNER JOIN nectarcrm_crmentity
								ON nectarcrm_seproductsrel.productid=nectarcrm_crmentity.crmid
								WHERE nectarcrm_seproductsrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts).")
									AND nectarcrm_crmentity.deleted=0
									AND nectarcrm_seproductsrel.productid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
							$query = "SELECT nectarcrm_inventoryproductrel.productid, nectarcrm_inventoryproductrel.id
													FROM nectarcrm_inventoryproductrel
													INNER JOIN nectarcrm_crmentity
													ON nectarcrm_inventoryproductrel.productid=nectarcrm_crmentity.crmid
													LEFT JOIN nectarcrm_quotes
													ON nectarcrm_inventoryproductrel.id = nectarcrm_quotes.quoteid
													WHERE nectarcrm_crmentity.deleted=0
														AND (nectarcrm_quotes.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or nectarcrm_quotes.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
														AND nectarcrm_inventoryproductrel.productid = ?";
							...
							$query = "SELECT nectarcrm_inventoryproductrel.productid, nectarcrm_inventoryproductrel.id
													...
													INNER JOIN nectarcrm_crmentity
													ON nectarcrm_inventoryproductrel.productid=nectarcrm_crmentity.crmid
													LEFT JOIN nectarcrm_invoice
													ON nectarcrm_inventoryproductrel.id = nectarcrm_invoice.invoiceid
													WHERE nectarcrm_crmentity.deleted=0
														AND (nectarcrm_invoice.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or nectarcrm_invoice.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
														AND nectarcrm_inventoryproductrel.productid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
		...
		case 'Quotes'	:	$query = "SELECT nectarcrm_quotes.quoteid
								FROM nectarcrm_quotes
								INNER JOIN nectarcrm_crmentity
								ON nectarcrm_quotes.quoteid=nectarcrm_crmentity.crmid
								WHERE nectarcrm_crmentity.deleted=0
									AND (nectarcrm_quotes.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or nectarcrm_quotes.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
									AND nectarcrm_quotes.quoteid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Invoice'	:	$query = "SELECT nectarcrm_invoice.invoiceid
								FROM nectarcrm_invoice
								INNER JOIN nectarcrm_crmentity
								ON nectarcrm_invoice.invoiceid=nectarcrm_crmentity.crmid
								WHERE nectarcrm_crmentity.deleted=0
									AND (nectarcrm_invoice.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or nectarcrm_invoice.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
									AND nectarcrm_invoice.invoiceid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Documents'	: 	$query = "SELECT nectarcrm_senotesrel.notesid FROM nectarcrm_senotesrel
									INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_senotesrel.notesid AND nectarcrm_crmentity.deleted = 0
									WHERE nectarcrm_senotesrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
									AND nectarcrm_senotesrel.notesid = ?";
								$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
								if ($adb->num_rows($res) > 0) {
									return true;
								}
								if(checkModuleActive('Project')) {
									$query = "SELECT nectarcrm_senotesrel.notesid FROM nectarcrm_senotesrel
										INNER JOIN nectarcrm_project ON nectarcrm_project.projectid = nectarcrm_senotesrel.crmid
										WHERE nectarcrm_project.linktoaccountscontacts IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
										AND nectarcrm_senotesrel.notesid = ?";
									$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
									if ($adb->num_rows($res) > 0) {
										return true;
								...
								$query = "SELECT nectarcrm_senotesrel.notesid FROM nectarcrm_senotesrel
															INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_senotesrel.notesid AND nectarcrm_crmentity.deleted = 0
															WHERE nectarcrm_senotesrel.crmid IN (". generateQuestionMarks($faq_id) .")
															AND nectarcrm_senotesrel.notesid = ?";
		...
		case 'HelpDesk'	:	if($acc_id) $accCondition = "OR nectarcrm_troubletickets.parent_id = $acc_id";
							$query = "SELECT nectarcrm_troubletickets.ticketid FROM nectarcrm_troubletickets
									INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_troubletickets.ticketid AND nectarcrm_crmentity.deleted = 0
									WHERE (nectarcrm_troubletickets.contact_id IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") $accCondition )
									AND nectarcrm_troubletickets.ticketid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
							...
							$query = "SELECT nectarcrm_troubletickets.ticketid FROM nectarcrm_troubletickets
									...
									WHERE nectarcrm_crmentity.deleted = 0 AND
											(nectarcrm_crmentityrel.crmid IN
												(SELECT projectid FROM nectarcrm_project WHERE linktoaccountscontacts
													IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") AND nectarcrm_crmentityrel.relcrmid = $entityid)
											OR nectarcrm_crmentityrel.relcrmid IN
												(SELECT projectid FROM nectarcrm_project WHERE linktoaccountscontacts
													IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") AND nectarcrm_crmentityrel.crmid = $entityid)
										AND nectarcrm_troubletickets.ticketid = ?)";
		...
		case 'Services'	:	$query = "SELECT nectarcrm_service.serviceid FROM nectarcrm_service
									INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_service.serviceid AND nectarcrm_crmentity.deleted = 0
									LEFT JOIN nectarcrm_crmentityrel ON (nectarcrm_crmentityrel.relcrmid=nectarcrm_service.serviceid OR nectarcrm_crmentityrel.crmid=nectarcrm_service.serviceid)
									WHERE (nectarcrm_crmentityrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")  OR " .
		 							"(nectarcrm_crmentityrel.relcrmid IN (".generateQuestionMarks($allowed_contacts_and_accounts).") AND nectarcrm_crmentityrel.module = 'Services'))
									AND nectarcrm_service.serviceid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts,$allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Accounts' : 	$query = "SELECT nectarcrm_account.accountid FROM nectarcrm_account " .
									"INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_account.accountid " .
									"INNER JOIN nectarcrm_contactdetails ON nectarcrm_contactdetails.accountid = nectarcrm_account.accountid " .
									"WHERE nectarcrm_crmentity.deleted = 0 and nectarcrm_contactdetails.contactid = ? and nectarcrm_contactdetails.accountid = ?";
		...
		case 'Assets' : $query = "SELECT nectarcrm_assets.assetname FROM nectarcrm_assets
								INNER JOIN nectarcrm_crmentity ON  nectarcrm_assets.assetsid = nectarcrm_crmentity.crmid
								WHERE nectarcrm_crmentity.deleted = 0 and nectarcrm_assets.account = ? ";
						$accountid = '';
						$accountRes = $adb->pquery("SELECT accountid FROM nectarcrm_contactdetails
								INNER JOIN nectarcrm_crmentity ON nectarcrm_contactdetails.contactid = nectarcrm_crmentity.crmid
		...
		case 'Project'	:	$query = "SELECT nectarcrm_project.projectid FROM nectarcrm_project
									INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_project.projectid AND nectarcrm_crmentity.deleted = 0
									WHERE nectarcrm_project.linktoaccountscontacts IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
									AND nectarcrm_project.projectid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
...
function get_documents($id,$module,$customerid,$sessionid)
{
	...
	$query ="select nectarcrm_notes.title,'Documents' ActivityType, nectarcrm_notes.filename,
		crm2.createdtime,nectarcrm_notes.notesid,nectarcrm_notes.folderid,
		nectarcrm_notes.notecontent description, nectarcrm_users.user_name, nectarcrm_notes.filelocationtype
		from nectarcrm_notes
		LEFT join nectarcrm_senotesrel on nectarcrm_senotesrel.notesid= nectarcrm_notes.notesid
		INNER join nectarcrm_crmentity on nectarcrm_crmentity.crmid= nectarcrm_senotesrel.crmid
		LEFT join nectarcrm_crmentity crm2 on crm2.crmid=nectarcrm_notes.notesid and crm2.deleted=0
		LEFT JOIN nectarcrm_groups
		ON nectarcrm_groups.groupid = nectarcrm_crmentity.smownerid
		LEFT join nectarcrm_users on crm2.smownerid= nectarcrm_users.id
		where nectarcrm_crmentity.crmid=?";
	$res = $adb->pquery($query,array($id));
	$noofdata = $adb->num_rows($res);
	for( $j= 0;$j < $noofdata; $j++)
	{
...
function get_project_components($id,$module,$customerid,$sessionid) {
	...
	if ($module == 'ProjectTask') {
		$query ="SELECT nectarcrm_projecttask.*, nectarcrm_crmentity.smownerid
				FROM nectarcrm_projecttask
				INNER JOIN nectarcrm_project ON nectarcrm_project.projectid = nectarcrm_projecttask.projectid AND nectarcrm_project.projectid = ?
				INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_projecttask.projecttaskid AND nectarcrm_crmentity.deleted = 0";
	} elseif ($module == 'ProjectMilestone') {
		$query ="SELECT nectarcrm_projectmilestone.*, nectarcrm_crmentity.smownerid
				FROM nectarcrm_projectmilestone
				INNER JOIN nectarcrm_project ON nectarcrm_project.projectid = nectarcrm_projectmilestone.projectid AND nectarcrm_project.projectid = ?
				INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_projectmilestone.projectmilestoneid AND nectarcrm_crmentity.deleted = 0";
	}
...
function get_project_tickets($id,$module,$customerid,$sessionid) {
	...
	$query = "SELECT nectarcrm_troubletickets.*, nectarcrm_crmentity.smownerid FROM nectarcrm_troubletickets
		INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_troubletickets.ticketid
		INNER JOIN nectarcrm_crmentityrel ON (nectarcrm_crmentityrel.relcrmid = nectarcrm_crmentity.crmid OR nectarcrm_crmentityrel.crmid = nectarcrm_crmentity.crmid)
		WHERE nectarcrm_crmentity.deleted = 0 AND (nectarcrm_crmentityrel.crmid = ? OR nectarcrm_crmentityrel.relcrmid = ?)";
...
function get_service_list_values($id,$modulename,$sessionid,$only_mine='true')
{
	...
	$query[] = "select nectarcrm_service.*," .
		"case when nectarcrm_crmentityrel.crmid != nectarcrm_service.serviceid then nectarcrm_crmentityrel.crmid else nectarcrm_crmentityrel.relcrmid end as entityid, " .
		 "'' as setype from nectarcrm_service " .
		 "inner join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_service.serviceid " .
		 "left join nectarcrm_crmentityrel on (nectarcrm_crmentityrel.relcrmid=nectarcrm_service.serviceid or nectarcrm_crmentityrel.crmid=nectarcrm_service.serviceid) " .
		 "where nectarcrm_crmentity.deleted = 0 and " .
		 "( nectarcrm_crmentityrel.crmid in (".generateQuestionMarks($entity_ids_list).") OR " .
		 "(nectarcrm_crmentityrel.relcrmid in (".generateQuestionMarks($entity_ids_list).") AND nectarcrm_crmentityrel.module = 'Services')" .
		 ")";
	...
	$checkQuotes = checkModuleActive('Quotes');
	if($checkQuotes == true){
		$query[] = "select distinct nectarcrm_service.*,
			case when nectarcrm_quotes.contactid is not null then nectarcrm_quotes.contactid else nectarcrm_quotes.accountid end as entityid,
			case when nectarcrm_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype
			from nectarcrm_quotes INNER join nectarcrm_crmentity on nectarcrm_crmentity.crmid=nectarcrm_quotes.quoteid
			left join nectarcrm_inventoryproductrel on nectarcrm_inventoryproductrel.id=nectarcrm_quotes.quoteid
			left join nectarcrm_service on nectarcrm_service.serviceid = nectarcrm_inventoryproductrel.productid
			where nectarcrm_inventoryproductrel.productid = nectarcrm_service.serviceid AND nectarcrm_crmentity.deleted=0 and (accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
		$params[] = array($entity_ids_list,$entity_ids_list);
	}
	$checkInvoices = checkModuleActive('Invoice');
	if($checkInvoices == true){
		...
		for( $j= 0;$j < $noofdata[$k]; $j++)
		{
			...
			foreach($fields_list as $fieldlabel=> $fieldname) {
				...
				if($fieldname == 'entityid') {
					...
					$module = $adb->query_result($res[$k],$j,'setype');
					if($module == ''){
						$module = $adb->query_result($adb->pquery("SELECT setype FROM nectarcrm_crmentity WHERE crmid = ?", array($crmid)),0,'setype');
					}
					if ($crmid != '' && $module != '') {
						$fieldvalues = getEntityName($module, array($crmid));
						if($module == 'Contacts')
...
function get_modules()
{
	...
	$modules = nectarcrm_Soap_CustomerPortal::lookupAllowedModules();
	if($modules === false) {
		$modules = array();

		$query = $adb->pquery("SELECT nectarcrm_customerportal_tabs.* FROM nectarcrm_customerportal_tabs
			INNER JOIN nectarcrm_tab ON nectarcrm_tab.tabid = nectarcrm_customerportal_tabs.tabid
			WHERE nectarcrm_tab.presence = 0 AND nectarcrm_customerportal_tabs.visible = 1", array());
		...
		nectarcrm_Soap_CustomerPortal::updateAllowedModules($modules);
	}
...
function show_all($module){
	...
	$query = $adb->pquery("SELECT prefvalue from nectarcrm_customerportal_prefs where tabid = ?", array($tabid));
	$norows = $adb->num_rows($query);
	if($norows > 0){
...
function getRelatedServiceContracts($crmid){
	...
	$query = "SELECT * FROM nectarcrm_servicecontracts " .
	"INNER JOIN nectarcrm_crmentity ON nectarcrm_crmentity.crmid = nectarcrm_servicecontracts.servicecontractsid AND nectarcrm_crmentity.deleted = 0 " .
	"LEFT JOIN nectarcrm_crmentityrel ON nectarcrm_crmentityrel.crmid = nectarcrm_servicecontracts.servicecontractsid " .
	"WHERE (nectarcrm_crmentityrel.relcrmid = ? and nectarcrm_crmentityrel.module= 'ServiceContracts')";
...
function getPortalUserid() {
	...
	$userid = nectarcrm_Soap_CustomerPortal::lookupPrefValue('userid');
	if($userid === false) {
		$res = $adb->pquery("SELECT prefvalue FROM nectarcrm_customerportal_prefs WHERE prefkey = 'userid' AND tabid = 0", array());
		$norows = $adb->num_rows($res);
		if($norows > 0) {
			$userid = $adb->query_result($res,0,'prefvalue');
			// Update the cache information now.
			nectarcrm_Soap_CustomerPortal::updatePrefValue('userid', $userid);
		}
	}
	return $userid;
...
function getDefaultAssigneeId() {
	...
	// Look the value from cache first
	$defaultassignee = nectarcrm_Soap_CustomerPortal::lookupPrefValue('defaultassignee');
	if($defaultassignee === false) {
		$res = $adb->pquery("SELECT prefvalue FROM nectarcrm_customerportal_prefs WHERE prefkey = 'defaultassignee' AND tabid = 0", array());
		$norows = $adb->num_rows($res);
		if($norows > 0) {
			$defaultassignee = $adb->query_result($res,0,'prefvalue');
			// Update the cache information now.
			nectarcrm_Soap_CustomerPortal::updatePrefValue('defaultassignee', $defaultassignee);
		}
	}
